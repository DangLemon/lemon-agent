...
# node:26 image instead (Lemon AI pins its toolchain to Node 26 everywhere).
...
# Do not write .pyc files at runtime: /opt/lemon is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/lemon/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when lemon
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# lemon process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
db.execute(\"INSERT INTO docs VALUES ('lemon')\"); \
sys.exit('SQLite FTS5 trigram self-test failed') if db.execute(\"SELECT count(*) FROM docs WHERE docs MATCH 'lem'\").fetchone()[0] != 1 else None; \
db.close()"

# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main lemon process, the dashboard,
...
# 'Lemon AI WebUI' catalog, NAS compose projects that preserve an old
...
# Non-root user for runtime; UID can be overridden via LEMON_UID at runtime
RUN useradd -u 10000 -m -d /opt/data lemon
...
#
# ui-tui/packages/lemon-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/lemon-ink/ ui-tui/packages/lemon-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as lemon-ink above).
...
# lock on the @lemon-ai/ink entry, tripped the TUI launcher's
...
#
# The [otlp] extra contains the SDK/exporter imported by Lemon AI when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Lemon AI production image.
#
# The hindsight memory provider's client (hindsight-client) is baked in
# for the same reason: it lazy-installs into /opt/lemon/.venv at first
...
# gives the non-root lemon user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link lemon-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/lemon are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the lemon user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/lemon/bin && \
    cp /opt/lemon/docker/lemon-exec-shim.sh /opt/lemon/bin/lemon && \
    chmod 0755 /opt/lemon /opt/lemon/bin/lemon && \
    printf 'docker\n' > /opt/lemon/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $LEMON_HOME. $LEMON_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``lemon update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the lemon user via
# `s6-setuidgid lemon` in its run script. If LEMON_UID is unset, services
# run as the default lemon user (UID 10000).
...
# lives outside both /opt/lemon (which operators sometimes bind-mount as a
# checkout) and /opt/data (the mutable LEMON_HOME volume).
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `lemon dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the LEMON_GIT_SHA build-arg to
# /opt/lemon/.lemon-ai_build_sha at build time, and have
# lemon_cli/build_info.py read it at runtime.  Both `lemon dump` and
...
ARG LEMON_GIT_SHA=
RUN set -eu; \
    if [ -n "${LEMON_GIT_SHA}" ]; then \
        printf '%s\n' "${LEMON_GIT_SHA}" > /opt/lemon/.lemon-ai_build_sha; \
    fi; \
    mkdir -p /etc/lemon; \
    LEMON_GIT_SHA="${LEMON_GIT_SHA}" python3 -c 'import json, os, pathlib, tomllib; project = tomllib.loads(pathlib.Path("/opt/lemon/pyproject.toml").read_text(encoding="utf-8"))["project"]; marker = pathlib.Path("/etc/lemon/image-provenance.json"); marker.write_text(json.dumps({"schema": 1, "deployment_kind": "image", "manager": "docker", "image": "danglemon/lemon-agent", "version": project["version"], "revision": os.environ.get("LEMON_GIT_SHA") or None}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8"); marker.chmod(0o444)'

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-lemon + dashboard.
...
# `exec lemon`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $LEMON_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/lemon/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-lemon-setup && \
    chmod +x /etc/cont-init.d/01-lemon-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV LEMON_WEB_DIST=/opt/lemon/lemon_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/lemon/ui-tui/dist/entry.js`) and skip the
...
ENV LEMON_TUI_DIR=/opt/lemon/ui-tui
ENV LEMON_HOME=/opt/data
ENV LEMON_WRITE_SAFE_ROOT=/opt/data
ENV LEMON_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/lemon (root-owned, read-only) so a runtime
...
# is seeded + chowned to the lemon user by docker/stage2-hook.sh and lives
...
ENV LEMON_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> lemon ...` they default to root, and any file the
# command writes under $LEMON_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/lemon/bin/lemon, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid lemon`
# when invoked as root. Non-root callers (supervised processes,
# `--user lemon`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/lemon/.venv/bin/lemon). See the shim source for
# the opt-out env var (LEMON_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/lemon-exec-shim.sh /opt/lemon/bin/lemon
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/lemon/docker/entrypoint-dispatch.sh

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> lemon ...` and any
# subprocess that doesn't activate the venv first still find lemon.
#
# /opt/lemon/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/lemon/bin:/opt/lemon/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. lemon
# subcommand vs. no-args), drops to the lemon user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/lemon/docker/entrypoint-dispatch.sh" ]
CMD [ ]
