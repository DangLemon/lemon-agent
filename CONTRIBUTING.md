...
Thank you for contributing to Lemon AI! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Lemon AI to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo DangLemon/lemon-agent "<your terms>"
  gh search prs --repo DangLemon/lemon-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/DangLemon/lemon-agent/issues?q=) · [PRs (all states)](https://github.com/DangLemon/lemon-agent/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Lemon AI itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Lemon AI install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `lemon skills browse` (labeled "official") and install it with `lemon skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in [GitHub Issues](https://github.com/DangLemon/lemon-agent/issues). Users can install it with `lemon skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.lemon-ai/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(lemon_home, config)` for setup-wizard integration
...
- Integrate with `lemon memory setup` via `post_setup()` — no need to touch core code
...
The installer creates the Lemon AI venv, wires the `lemon` command, stamps the
install method for `lemon update`, and clones the full git project into
`$LEMON_HOME/lemon-agent` (usually `~/.lemon-ai/lemon-agent`). That keeps your
...
Use this only if you intentionally do not want Lemon AI' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `lemon` entrypoint from this venv; running the
system `python3 -m lemon_cli.main` can pick up unrelated system Python
packages.
...
uv venv ~/.lemon-ai/venvs/lemon-dev --python 3.11
...
```bash
mkdir -p ~/.lemon-ai/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.lemon-ai/config.yaml
touch ~/.lemon-ai/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.lemon-ai/.env
```

### Run

```bash
# The standard installer already put `lemon` on PATH.
lemon doctor
lemon chat -q "Hello"
```

If you used the manual clone fallback, run `./lemon` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/lemon" ~/.local/bin/lemon
```
...
├── cli.py                    # LemonCLI class — interactive CLI orchestrator (~4.6k LOC + lemon_cli/cli_*_mixin.py)
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (lemon-cli, lemon-telegram, etc.)
├── lemon_state.py           # SessionDB facade (~1.4k LOC); implementation in lemon_state_*.py (21 siblings) — FTS5 search, session titles
...
│
├── lemon_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── cli_*_mixin.py            # LemonCLI mixins (slash commands, display, session, ...)
...
│
├── skills/                   # Bundled skills (copied to ~/.lemon-ai/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (github.com/DangLemon/lemon-agent)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.lemon-ai/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.lemon-ai/`)

| Path | Purpose |
|------|---------|
| `~/.lemon-ai/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.lemon-ai/.env` | API keys and secrets |
| `~/.lemon-ai/auth.json` | OAuth credentials (Nous Portal) |
| `~/.lemon-ai/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.lemon-ai/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.lemon-ai/state.db` | SQLite session database |
| `~/.lemon-ai/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.lemon-ai/cron/` | Scheduled job data |
| `~/.lemon-ai/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`lemon_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.lemon-ai/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_LEMON_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.lemon`:
...
The user may skip setup and keep loading the skill. Lemon AI only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `lemon setup` or update `~/.lemon-ai/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Lemon AI tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Lemon AI" is the secondary collaborator. If the contributor's commit shows "Lemon AI" as author because they used Lemon AI to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Lemon AI tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `lemon --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Lemon AI uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.lemon-ai/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `lemon_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `lemon_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Lemon AI runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the lemon wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `lemon_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `lemon_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `lemon_cli/gateway_windows.py::
...
Lemon AI has terminal access. Security matters.
...
2. **Test manually**: Run `lemon` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/DangLemon/lemon-agent/issues)
- Include: OS, Python version, Lemon AI version (`lemon --version`), full error traceback
...
- **GitHub Issues**: [DangLemon/lemon-agent](https://github.com/DangLemon/lemon-agent/issues) — for questions, showcasing projects, and sharing skills
