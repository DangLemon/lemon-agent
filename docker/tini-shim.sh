...
# Lemon AI WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: LEMON_TINI_SHIM_TARGET overrides the `/init` path (and
# LEMON_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${LEMON_TINI_SHIM_TARGET:-/init}"
WRAPPER="${LEMON_TINI_SHIM_WRAPPER:-/opt/lemon/docker/main-wrapper.sh}"
