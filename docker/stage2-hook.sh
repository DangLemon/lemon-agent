...
#
# Wired into the image as /etc/cont-init.d/01-lemon-setup by the
...
LEMON_HOME="${LEMON_HOME:-/opt/data}"
INSTALL_DIR="/opt/lemon"

# Drop to lemon via s6-setuidgid, but skip it when already non-root.
as_lemon() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid lemon "$@"; }
...
# starts non-root. The baked install tree under /opt/lemon is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $LEMON_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass LEMON_UID/LEMON_GID — or the PUID/PGID aliases — which the
...
# `id -u lemon` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# lemon UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u lemon)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-lemon UID).
...
and the baked /opt/lemon install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e LEMON_UID=\$(id -u) -e LEMON_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the lemon user to that UID/GID at boot and chowns the data
...
# --- Bootstrap LEMON_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid lemon mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom LEMON_HOME paths whose parents only
# root can create (e.g. `LEMON_HOME=/home/lemon/.lemon-ai` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for LEMON_UID/LEMON_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  LEMON_UID/
# LEMON_GID still win when both are set.  See #15290, salvages #25872.
LEMON_UID="${LEMON_UID:-${PUID:-}}"
LEMON_GID="${LEMON_GID:-${PGID:-}}"

if [ -n "${LEMON_UID:-}" ] && validate_uid_gid "$LEMON_UID" && [ "$LEMON_UID" != "$(id -u lemon)" ]; then
    echo "[stage2] Changing lemon UID to $LEMON_UID"
    usermod -u "$LEMON_UID" lemon
fi
if [ -n "${LEMON_GID:-}" ] && validate_uid_gid "$LEMON_GID" && [ "$LEMON_GID" != "$(id -g lemon)" ]; then
    echo "[stage2] Changing lemon GID to $LEMON_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$LEMON_GID" lemon 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised lemon user (UID 10000)
...
# lemon process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes lemon. Idempotent across container
...
#   - lemon is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G lemon 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] lemon already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" lemon 2>/dev/null; then
        echo "[stage2] Added lemon to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group lemon failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When LEMON_UID is remapped or the top-level $LEMON_HOME isn't owned by
# the runtime lemon UID, restore ownership to lemon — but ONLY for the
# directories lemon actually writes to. The full $LEMON_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of lemon-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_lemon_uid=$(id -u lemon)
...
chown_lemon_tree() {
    ...
    chown -R lemon:lemon "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_lemon_owner() {
    target="$1"
    find "$target" \( ! -user lemon -o ! -group lemon \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$LEMON_HOME" 2>/dev/null)" != "$actual_lemon_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $LEMON_HOME (targeted) to lemon ($actual_lemon_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $LEMON_HOME: chown the directory itself (not its contents)
    # so lemon can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$LEMON_HOME"; then
        :
    else
        chown lemon:lemon "$LEMON_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $LEMON_HOME failed (rootless container?) — continuing"
    fi
    # Lemon AI-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by lemon (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$LEMON_HOME/$sub" ] && tree_has_non_lemon_owner "$LEMON_HOME/$sub"; then
            chown_lemon_tree "$LEMON_HOME/$sub"
        fi
    done
fi
...
# lemon user. Hosted/container instances keep mutable state under
# $LEMON_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# LEMON_DISABLE_LAZY_INSTALLS=1. Keeping /opt/lemon root-owned and
...
# $LEMON_HOME/lazy-packages dir on the data volume (Dockerfile sets
# LEMON_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to lemon
...
# Always reset ownership of $LEMON_HOME/profiles to lemon on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> lemon …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as lemon and walks
...
if [ -d "$LEMON_HOME/profiles" ] && tree_has_non_lemon_owner "$LEMON_HOME/profiles"; then
    chown_lemon_tree "$LEMON_HOME/profiles"
fi

# Always reset ownership of $LEMON_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged lemon runtime even
...
# gate as profiles/).
if [ -d "$LEMON_HOME/cron" ] && tree_has_non_lemon_owner "$LEMON_HOME/cron"; then
    chown_lemon_tree "$LEMON_HOME/cron"
fi

# Always ensure logs/gateways is lemon-owned (#45258). Formerly healed by
...
# top-level $LEMON_HOME is mis-owned, so a warm volume with lemon-owned
# LEMON_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid lemon mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as lemon.
if [ -d "$LEMON_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$LEMON_HOME/logs/gateways"; then
        :
    else
        chown lemon:lemon "$LEMON_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# lemon pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged lemon gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $LEMON_HOME is
...
# and cron/.
if [ -d "$LEMON_HOME/platforms/pairing" ] && tree_has_non_lemon_owner "$LEMON_HOME/platforms/pairing"; then
    chown_lemon_tree "$LEMON_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$LEMON_HOME/pairing" ] && tree_has_non_lemon_owner "$LEMON_HOME/pairing"; then
    chown_lemon_tree "$LEMON_HOME/pairing"
fi

# Reset ownership of lemon-owned top-level state files on every boot.
# The targeted data-volume chown above only covers lemon-owned
# *subdirectories*; loose state files living directly under $LEMON_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> lemon …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged lemon runtime then hits
...
# sweep so host-owned files in a bind-mounted $LEMON_HOME are never
...
# entries of lemon_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$LEMON_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$LEMON_HOME/$f"; then
            :
        else
            chown lemon:lemon "$LEMON_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the lemon runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$LEMON_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$LEMON_HOME/config.yaml"; then
        :
    else
        chown lemon:lemon "$LEMON_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$LEMON_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as lemon user ---
# Run as lemon via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $LEMON_HOME values
...
as_lemon mkdir -p \
    ...
    "$LEMON_HOME/sessions" \
    "$LEMON_HOME/logs" \
    "$LEMON_HOME/logs/gateways" \
    "$LEMON_HOME/hooks" \
    "$LEMON_HOME/memories" \
    "$LEMON_HOME/skills" \
    "$LEMON_HOME/skins" \
    "$LEMON_HOME/plans" \
    "$LEMON_HOME/workspace" \
    "$LEMON_HOME/home" \
    "$LEMON_HOME/pairing" \
    "$LEMON_HOME/platforms/pairing" \
    "$LEMON_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/lemon/.install_method (see Dockerfile), NOT written here into
# $LEMON_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $LEMON_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.lemon:/opt/data) and sometimes shared with a
...
# 'lemon update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $LEMON_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/lemon stamp, so deleting the data-dir copy is safe).
if [ -f "$LEMON_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$LEMON_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$LEMON_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$LEMON_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$LEMON_HOME/$dest"; then
            :
        else
            as_lemon cp "$INSTALL_DIR/$src" "$LEMON_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/lemon/.env.example being present
...
# not generate one. Lemon AI loads $LEMON_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$LEMON_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$LEMON_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $LEMON_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$LEMON_HOME/.env" ] && ! refuse_symlinked_path "clean" "$LEMON_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$LEMON_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$LEMON_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$LEMON_HOME/.env"; then
        :
    else
        if [ ! -f "$LEMON_HOME/.env" ]; then
            ...
            (umask 077 && as_lemon touch "$LEMON_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$LEMON_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$LEMON_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$LEMON_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $LEMON_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $LEMON_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$LEMON_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$LEMON_HOME/.env"; then
        :
    else
        chown lemon:lemon "$LEMON_HOME/.env" 2>/dev/null || true
        chmod 600 "$LEMON_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `lemon` user — without this it gets EACCES on every
...
# lemon group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp lemon /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted lemon group access to the Fly Machines API socket"
    else
...
# $LEMON_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `lemon update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set LEMON_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$LEMON_HOME/config.yaml" ]; then
    s6-setuidgid lemon "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$LEMON_HOME/auth.json" ] && [ -n "${LEMON_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$LEMON_HOME/auth.json"; then
        :
    else
        printf '%s' "$LEMON_AUTH_JSON_BOOTSTRAP" > "$LEMON_HOME/auth.json"
        chown lemon:lemon "$LEMON_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$LEMON_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via LEMON_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$LEMON_HOME/auth.json" ] && [ -n "${LEMON_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$LEMON_HOME/auth.json"; then
        :
    else
        s6-setuidgid lemon "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$LEMON_HOME/auth.json" \
...
# can set LEMON_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# LEMON_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$LEMON_HOME/gateway_state.json" ] && \
        [ "${LEMON_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$LEMON_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$LEMON_HOME/gateway_state.json"
        chown lemon:lemon "$LEMON_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$LEMON_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_lemon "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/lemon/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Lemon AI spawns for the
...
# main-wrapper.sh propagates it into the supervised ``lemon`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-lemon, dashboard, etc.).
