#!/bin/sh
# shellcheck shell=sh
# /opt/lemon/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching LEMON_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `lemon` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `lemon <args>` (subcommand passthrough)
#
# Drop to lemon via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${LEMON_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${LEMON_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export LEMON_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset LEMON_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid lemon "$@"; exec "$@"; }
...
# an arbitrary non-root, non-lemon UID, the bootstrap was skipped and the
# baked image dirs (owned by the lemon build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u lemon)" ]; then
    cat >&2 <<EOF
[lemon] ERROR: container started with --user $cur_uid (an arbitrary, non-lemon UID) — not supported.
...
# to the lemon user's home before dropping privileges so libraries that
...
_lemon_orig_cwd="${LEMON_ORIG_CWD:-$PWD}"
...
# the user's command so `lemon chat` starts in the Docker -w
...
# Lemon AI subcommand pass-through.
drop lemon "$@"
