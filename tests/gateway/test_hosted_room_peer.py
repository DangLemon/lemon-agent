...
def test_room_link_endpoint_reads_supported_config_with_env_override(
    ...
    (home / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://configured.example.test/lemon\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("LEMON_HOME", str(home))
    monkeypatch.delenv("LEMON_ROOM_LINK_URL", raising=False)
    assert local_room_link_endpoint() == {
        "available": True,
        "url": "https://configured.example.test/lemon",
        "transport_security": "tls",
    }

    monkeypatch.setenv(
        "LEMON_ROOM_LINK_URL", "https://override.example.test/lemon"
    )
    assert local_room_link_endpoint()["url"] == (
        "https://override.example.test/lemon"
    )
...
def test_named_profile_inherits_gateway_room_link_endpoint(tmp_path, monkeypatch):
    ...
    (root / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://gateway.example.test/lemon\n",
        encoding="utf-8",
    )
    ...
    try:
        assert local_room_link_endpoint() == {
            "available": True,
            "url": "https://gateway.example.test/lemon",
            "transport_security": "tls",
        }
    finally:
        reset_lemon_home_override(token)
...
def test_named_profile_room_link_override_wins_over_gateway_root(
    ...
    (profile / "config.yaml").write_text(
        "gateway:\n  room_link_url: https://profile.example.test/lemon\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("LEMON_HOME", str(root))
    monkeypatch.delenv("LEMON_ROOM_LINK_URL", raising=False)
    ...
    try:
        assert local_room_link_endpoint()["url"] == (
            "https://profile.example.test/lemon"
        )
    finally:
        reset_lemon_home_override(token)
...
def test_self_advertised_endpoint_is_explicit_and_validated(
    ...
    if configured is None:
        ...
        monkeypatch.setenv("LEMON_ROOM_LINK_URL", configured)
