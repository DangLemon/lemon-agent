...
(/opt/lemon/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable LEMON_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_lemon_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable LEMON_HOME."""
    ...
    monkeypatch.setattr(
        "lemon_constants.get_lemon_home", lambda: lemon_home
    )
    ...
    expected = lemon_home / "scripts" / "whatsapp-bridge"
