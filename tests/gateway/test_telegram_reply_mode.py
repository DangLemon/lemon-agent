...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/DangLemon/lemon-agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via Lemon AI-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/DangLemon/lemon-agent/issues/87051:
    ...
    response in a different chat lane than the Lemon AI topic the session runs
    in. The no-anchor branch must prefer the Lemon AI topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_lemon_topic_thread_id(self):
