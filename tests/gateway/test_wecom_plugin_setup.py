...
``lemon_cli.config`` (get_env_value / save_env_value / remove_env_value),
``lemon_cli.cli_output`` (prompt / prompt_yes_no / print_*), and
``lemon_cli.setup`` (prompt_choice); we patch each at its source module so
