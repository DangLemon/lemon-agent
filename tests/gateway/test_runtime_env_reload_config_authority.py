...
LEMON_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env LEMON_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``lemon config get terminal.backend`` still says local.
    """
    lemon_home = tmp_path / ".lemon-ai"
    lemon_home.mkdir()
    (lemon_home / "config.yaml").write_text(
    ...
    (lemon_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
