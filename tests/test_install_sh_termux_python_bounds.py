...
def _termux_env(tmp_path: Path, bin_dir: Path, *, brand: str) -> dict[str, str]:
...
def _run_install_prerequisites(
    tmp_path: Path, *, brand: str
) -> subprocess.CompletedProcess[str]:
    bin_dir = tmp_path / "bin"
    bin_dir.mkdir(exist_ok=True)
    _write_termux_command_stubs(bin_dir)
    env = _termux_env(tmp_path, bin_dir, brand=brand)
