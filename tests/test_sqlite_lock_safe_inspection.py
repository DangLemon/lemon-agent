"""POSIX advisory locks must survive Lemon AI' own database inspection.
...
Lemon AI used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `lemon sessions
...
out across Lemon AI' inspection calls.
"""
...
def test_failed_close_keeps_connection_tracked(tmp_path, clean_registry):
    ...
    class ControllableConnection(sqlite3.Connection):
        def close(self):
            if getattr(self, "_lemon_fail_close", False):
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import lemon_cli.sqlite_safe_read as ssr
