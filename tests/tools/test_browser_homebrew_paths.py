...
class TestRunBrowserCommandPathConstruction:
    ...
    def test_subprocess_preserves_executable_path_with_spaces(self, tmp_path):
        ...
        browser_path = "/Users/test/Library/Application Support/lemon/node_modules/.bin/agent-browser"
 ...
 patch("tools.browser_tool_install._chromium_installed", return_value=True), \
             ...
             patch("lemon_constants.Path.home", return_value=tmp_path), \
    ...
    def test_npx_sentinel_resolves_via_resolve_npx_bin_with_pinned_spec(self, tmp_path):
        ...
        could let a broken system npx shadow a healthy Lemon AI-managed one) and
        ...
        with patch("tools.browser_tool_install._find_agent_browser", return_value="npx agent-browser"), \
             patch("tools.browser_tool_install._resolve_npx_bin", return_value="/opt/lemon/node/bin/npx"), \
             patch("tools.browser_tool_install._chromium_installed", return_value=True), \
             patch("tools.browser_tool_session._get_session_info", return_value=fake_session), \
             patch("tools.browser_tool._socket_safe_tmpdir", return_value=str(tmp_path)), \
             patch("tools.browser_tool_install._discover_homebrew_node_dirs", return_value=[]), \
        ...
        assert captured_cmd[:5] == [
            "/opt/lemon/node/bin/npx", "--ignore-scripts", "--prefer-offline", "-y",
...
class TestResolveNpxBinPriority:
    ...
    Lemon AI-managed one — and each candidate must be validated (actually
    ...
    def test_prefers_managed_extended_path_over_bare_path(self, monkeypatch):
        ...
        monkeypatch.setattr("tools.browser_tool_install._merge_browser_path", lambda _p: "/lemon/node/bin")
        monkeypatch.setattr(
            shutil, "which",
            lambda cmd, path=None: (
                "/lemon/node/bin/npx" if path == "/lemon/node/bin"
                else "/usr/local/bin/npx"
            ),
        )
        monkeypatch.setattr("tools.browser_tool_install.node_tool_runnable", lambda p: True)

        assert bt_install._resolve_npx_bin() == "/lemon/node/bin/npx"
    ...
    def test_falls_back_to_bare_path_when_extended_dir_has_no_npx(self, monkeypatch):
        ...
        monkeypatch.setattr(
            shutil, "which",
            lambda cmd, path=None: None if path == "/lemon/node/bin" else "/usr/bin/npx",
        )
        monkeypatch.setattr("tools.browser_tool_install.node_tool_runnable", lambda p: True)
