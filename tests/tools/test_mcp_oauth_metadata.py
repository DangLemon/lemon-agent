...
Covers:
- :class:`LemonTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.LemonMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _LEMON_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# LemonTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = LemonTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("LEMON_HOME", str(tmp_path))
        storage = LemonTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (LemonMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: LemonTokenStorage, **context_attrs):
    ...
    provider = _LEMON_PROVIDER_CLS.__new__(_LEMON_PROVIDER_CLS)
    provider._lemon_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LEMON_HOME", str(tmp_path))
        storage = LemonTokenStorage("mgr-srv")
        ...
        with patch.object(
            _LEMON_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LEMON_HOME", str(tmp_path))
        storage = LemonTokenStorage("flow-srv")
        ...
        with patch.object(
            _LEMON_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
