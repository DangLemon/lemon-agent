...
was gated on ``[ -f "$LEMON_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'LEMON_HOME="{home}"\n'
        # In tests we run unprivileged; as_lemon is a passthrough then.
        'as_lemon() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Lemon AI loads $LEMON_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/lemon/.env.example -> $LEMON_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $LEMON_HOME/.env from it on first boot"
    )
