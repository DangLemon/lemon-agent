...
class TestWriteDenyExactPaths:
    ...
    def test_lemon_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.lemon-ai/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "lemon_root"
        ...
        # Sanity check: LEMON_HOME does point to the profile dir, not the root.
        from lemon_constants import get_lemon_home, get_default_lemon_root
...
class TestWriteAllowed:
    ...
    def test_lemon_control_files_requested_writable(self):
