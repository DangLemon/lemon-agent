...
Older images made /opt/lemon/ui-tui and /opt/lemon/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV LEMON_TUI_DIR=/opt/lemon/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R lemon:lemon /opt/lemon/ui-tui" not in text
    assert "chown -R lemon:lemon /opt/lemon/node_modules" not in text
