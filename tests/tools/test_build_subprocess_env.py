...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    token = set_lemon_home_override(str(tmp_path))
    try:
        env = build_subprocess_env(
    ...
    assert env["LEMON_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_lemon_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see LEMON_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('LEMON_HOME'), "
    ...
    assert result["home"] == str(lemon_home)
...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `lemon` on PATH
# ---------------------------------------------------------------------------


def test_e2e_scrubbed_env_resolves_bare_lemon_under_minimal_parent_path(monkeypatch):
    """Regression for #92998/#93082: a gateway launched by systemd/cron with a
    minimal PATH (no lemon console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``lemon``.
    ...
    bin_dir = local_mod._resolve_lemon_bin_dir()
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "lemon.exe" if os.name == "nt" else "lemon")
    ):
        pytest.skip("no real lemon console-script install available")

    # Simulate the service-manager minimal PATH: lemon dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("lemon", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("lemon", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'lemon' must resolve from the child PATH {env.get('PATH')!r}"
    )
