...
def test_output_path_rejects_lemon_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    lemon_home = tmp_path / "lemon-home"
    lemon_home.mkdir()
    monkeypatch.setattr(file_safety, "_lemon_home_path", lambda: lemon_home)
    monkeypatch.setattr(file_safety, "_lemon_root_path", lambda: lemon_home)

    target = lemon_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    lemon_home = tmp_path / "lemon-home"
    token_dir = lemon_home / "mcp-tokens"
