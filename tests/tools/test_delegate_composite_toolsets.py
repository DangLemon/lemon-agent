...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_lemon_cli_expands_web(self):
        """lemon-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"lemon-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("lemon-cli", expanded)


    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['lemon-cli'] yields ['web']."""
        parent_toolsets = {"lemon-cli"}
