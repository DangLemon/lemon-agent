"""Tests for tools.wake_word — the "Hey Lemon AI" hotword detector.
...
def test_config_defaults_and_clamping():
    ...
    assert ww.wake_phrase({"phrase": "hey lemon"}) == "hey lemon"
    assert ww.wake_phrase({}) == "hey lemon"
...
def test_looks_like_path():
    ...
    assert _looks_like_path("models/hey_lemon.onnx")
...
def test_load_wake_word_config_guards_non_dict(monkeypatch):
    monkeypatch.setattr(
        "lemon_cli.config.load_config", lambda: {"wake_word": "oops"}
    )
...
def test_requirements_openwakeword_available(monkeypatch):
    ...
    r = ww.check_wake_word_requirements(
        {"provider": "openwakeword", "phrase": "hey lemon"}
    )
    assert r["available"] is True
    assert r["provider"] == "openwakeword"
    assert r["phrase"] == "hey lemon"
...
def _install_fake_openwakeword(monkeypatch):
    ...
    class _FakeModel:
        def __init__(self, wakeword_models, inference_framework="onnx"):
            ...
            self.models = {"hey_lemon": object()}

        def predict(self, frame):
            return {"hey_lemon": 0.0}
...
def test_openwakeword_ensures_base_models_for_custom_path(monkeypatch):
    ...
    eng = ww._OpenWakeWordEngine(
        {"provider": "openwakeword", "openwakeword": {"model": "/models/hey_lemon.onnx"}}
    )
    assert calls["download"] == [["/models/hey_lemon.onnx"]]
    assert eng._labels == ["hey_lemon"]


def test_bundled_hey_lemon_model_ships_on_disk():
    # The "hey lemon" wake word works out of the box only if the model is
