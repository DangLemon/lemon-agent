...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects LEMON_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("LEMON_INTERACTIVE", "LEMON_GATEWAY_SESSION", "LEMON_EXEC_ASK", "LEMON_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["LEMON_INTERACTIVE"] = "1"
        session_key = os.getenv("LEMON_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No LEMON_INTERACTIVE or LEMON_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
