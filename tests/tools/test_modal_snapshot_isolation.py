...
def _restore_tool_modules():
    original_lemon_home = os.environ.get("LEMON_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_lemon_home is None:
            os.environ.pop("LEMON_HOME", None)
        else:
            os.environ["LEMON_HOME"] = original_lemon_home
        _reset_modules(("tools", "lemon_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    lemon_cli = types.ModuleType("lemon_cli")
    lemon_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["lemon_cli"] = lemon_cli
    lemon_home = tmp_path / "lemon-home"
    os.environ["LEMON_HOME"] = str(lemon_home)
    sys.modules["lemon_cli.config"] = types.SimpleNamespace(
        get_lemon_home=lambda: lemon_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="lemon-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": lemon_home / "modal_snapshots.json",
