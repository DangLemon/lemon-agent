...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad LEMON_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: LEMON_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_lemon_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "LEMON_HOME": "/h", "LEMON_PROFILE": "p",
        "LEMON_CONFIG": "/c.yaml", "LEMON_ENV": "/e",
        "LEMON_DELEGATED_CHILD_CONTEXT": "1",
        # other LEMON_* → dropped (broad prefix removed)
        "LEMON_BASE_URL": "https://x", "LEMON_INTERACTIVE": "1",
        "LEMON_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "LEMON_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "LEMON_BASE_URL", "LEMON_INTERACTIVE", "LEMON_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped LEMON_* vars."""
    import logging
