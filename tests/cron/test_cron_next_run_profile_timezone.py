...
``lemon_time`` under the process's own startup profile, then ticks OTHER
profiles' cron stores via ``set_lemon_home_override()`` + ``use_cron_store()``.
...
def test_foreign_process_tick_persists_owning_profile_offset(
    ...
    monkeypatch.setenv("LEMON_HOME", str(backend_home))

    # Backend process resolves its own timezone first (process startup).
    assert lemon_time.now().utcoffset() == datetime.now(
        ZoneInfo("UTC")
    ).utcoffset()
    ...
    token = set_lemon_home_override(str(profile_a))
    try:
