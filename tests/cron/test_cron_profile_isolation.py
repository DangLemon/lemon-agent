...
profile's LEMON_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.lemon-ai/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active LEMON_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever LEMON_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    LEMON_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import lemon_constants

    monkeypatch.setattr(
        lemon_constants, "_get_platform_default_lemon_home", lambda: root
    )
    monkeypatch.setenv("LEMON_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile LEMON_HOME (<root>/profiles/<name>), the cron store
    ...
    assert lemon_constants.get_lemon_home().resolve() == profile_home.resolve()
    assert lemon_constants.get_default_lemon_root().resolve() == root.resolve()

    # cron/jobs.py computes LEMON_DIR from get_lemon_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.LEMON_DIR.resolve() == profile_home.resolve()
        assert (
