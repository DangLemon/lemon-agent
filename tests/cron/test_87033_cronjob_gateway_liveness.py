...
(``lemon cron list`` / ``lemon cron status``); the agent path did not.
...
def lemon_env(tmp_path, monkeypatch):
    """Isolate LEMON_HOME for each test so jobs don't leak."""
...
class TestCreateSurfacesGatewayLiveness:
    def test_create_with_gateway_running_has_no_warning(self, lemon_env):
    ...
    def test_create_without_gateway_warns_not_scheduled(self, lemon_env):
        with (
    ...
    def test_non_builtin_provider_is_exempt(self, lemon_env):
    ...
    def test_failed_probe_stays_neutral(self, lemon_env):
...
class TestListSurfacesGatewayLiveness:
    ...
    def test_list_with_gateway_running_has_no_warning(self, lemon_env):
    ...
    def test_list_without_gateway_warns_jobs_inert(self, lemon_env):
        _create_job()
    ...
    def test_list_empty_without_gateway_stays_quiet(self, lemon_env):
    ...
    def test_list_non_builtin_provider_is_exempt(self, lemon_env):
        _create_job()
        with patch_liveness(provider="chronos", pids=[]):
            result = self._list_jobs()
...
class _LivenessPatches:
    ...
    def __enter__(self):
        ...
        self._stack.enter_context(
            patch(
                "lemon_cli.cron._active_cron_provider_name",
        ...
        self._stack.enter_context(
            patch(
                "lemon_cli.gateway.named_profile_served_by_running_multiplexer",
                return_value=False,
            )
        )
        self._stack.enter_context(
...
class TestRuntimeLockFirstLiveness:
    ...
    def test_lock_active_reports_alive_despite_empty_pid_scan(self, lemon_env):
    ...
    def test_lock_inactive_falls_back_to_pid_scan(self):
        ...
        import lemon_cli.cron as cron_cli

        with (
            patch("lemon_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("gateway.status.is_gateway_runtime_lock_active", return_value=False),
            patch("lemon_cli.gateway.find_gateway_pids", return_value=[424242]),
        ):
    ...
    def test_no_lock_no_pids_is_false(self):
        ...
        with (
            patch("lemon_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("gateway.status.is_gateway_runtime_lock_active", return_value=False),
            patch("lemon_cli.gateway.find_gateway_pids", return_value=[]),
            patch(
                "lemon_cli.gateway.named_profile_served_by_running_multiplexer",
                return_value=False,
            ),
...
class TestCronStatusLockFirst:
    """`lemon cron status` shares the lock-first false-alarm fix (#95947).
    ...
    def _run_status(self, *, pids, lock_active, lock_pid=None):
        ...
        with (
            patch("lemon_cli.cron._active_cron_provider_name", return_value="builtin"),
            patch("lemon_cli.gateway.find_gateway_pids", return_value=list(pids)),
            patch(
                "gateway.status.is_gateway_runtime_lock_active",
    ...
    def test_lock_active_suppresses_not_running_false_alarm(self, lemon_env):
    ...
    def test_no_lock_no_pids_still_warns(self, lemon_env):
