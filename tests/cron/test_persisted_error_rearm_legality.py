...
def cron_store(tmp_path, monkeypatch):
    ...
    (lemon_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("LEMON_HOME", str(lemon_home))
    monkeypatch.setattr(J, "LEMON_DIR", lemon_home)
    monkeypatch.setattr(J, "CRON_DIR", lemon_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", lemon_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", lemon_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_lemon_now", lambda: now):
            due = J.get_due_jobs()
