...
work on a file several Lemon AI processes share (gateway service, the Desktop
app's `lemon serve` backend, CLI sessions, the TUI slash worker):

* `lemon_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `lemon_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
