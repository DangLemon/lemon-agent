...
``[diff "x"] command=/textconv=`` driver. Lemon AI gathers workspace context by
...
context-gathering git path Lemon AI runs neutralizes every sink. They use a real
...
def test_goals_fingerprint_is_safe(malicious_repo):
    from lemon_cli.goals import workspace_fingerprint
    repo, marker = malicious_repo
