...
Under a gateway (Telegram, Discord, Slack, ...) Lemon AI passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LEMON_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Lemon AI via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Lemon AI profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `lemon -p partner` pinned to a different person's peer).
