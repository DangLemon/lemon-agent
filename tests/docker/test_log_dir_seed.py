"""Runtime smoke test for Docker $LEMON_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the lemon user after container boot.
...
registered later runs its log service as the dropped lemon user and
...
def test_logs_gateways_seeded_and_lemon_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by lemon after boot."""
    ...
    assert "logs=lemon" in r.stdout, (
        f"logs/ not owned by lemon: {r.stdout}"
    )
    assert "gateways=lemon" in r.stdout, (
        f"logs/gateways/ not owned by lemon: {r.stdout}"
    )
