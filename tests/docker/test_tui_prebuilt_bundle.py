...
package-lock.json. Without LEMON_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV LEMON_TUI_DIR=/opt/lemon/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the lemon user, return stdout."""
    inner = (
        "source /opt/lemon/.venv/bin/activate && "
        "cd /opt/lemon && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the lemon user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "lemon", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_lemon_tui_dir_env_is_set(built_image: str) -> None:
    """LEMON_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$LEMON_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/lemon/ui-tui", (
        f"LEMON_TUI_DIR={r.stdout.strip()!r} (expected /opt/lemon/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from lemon_cli.main_tui_launch import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/lemon/ui-tui')\n"
    ...
    # With LEMON_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
