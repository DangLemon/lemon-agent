...
Override the image with ``LEMON_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("LEMON_TEST_IMAGE", "lemon-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``LEMON_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("LEMON_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged lemon user
# ---------------------------------------------------------------------------
#
# Background: every Lemon AI runtime path inside the container drops to UID
# 10000 (the ``lemon`` user) via ``s6-setuidgid lemon``. ``docker exec``
...
# fails (PermissionError swallowed) for lemon, so a test that ran as root
...
# helpers below run every probe under ``-u lemon`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: lemon).
