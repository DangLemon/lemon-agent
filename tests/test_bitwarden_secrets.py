...
binary download are exercised manually by `lemon secrets bitwarden
...
def lemon_home(tmp_path, monkeypatch):
    """Point Lemon AI at an isolated home directory."""
    home = tmp_path / ".lemon-ai"
    home.mkdir()
    monkeypatch.setenv("LEMON_HOME", str(home))
    # Some modules cache get_lemon_home; clear if needed.
    import lemon_constants
    if hasattr(lemon_constants, "_LEMON_HOME_CACHE"):
        lemon_constants._LEMON_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(lemon_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from lemon_cli.env_loader import _apply_external_secret_sources
