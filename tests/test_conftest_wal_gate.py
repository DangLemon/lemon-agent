"""The conftest WAL gate must agree with lemon_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``lemon_state``. That is deliberate: importing
``lemon_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.lemon-ai`` before the per-test ``LEMON_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_lemon_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and lemon_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_lemon_state_at_collection():
    """The gate must stay import-free of lemon_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.lemon-ai, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``lemon_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "lemon_state" or name.startswith("lemon_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported lemon_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.lemon-ai during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("lemon_state", None)
