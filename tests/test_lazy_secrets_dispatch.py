...
1. `lemon secrets bitwarden setup --help` works (dispatch path)
2. `lemon update --check` works (update path)
3. `lemon secrets bitwarden disable` works (handler execution)
4. `lemon secrets onepassword status` works (lazy backend loads on demand)
...
def _run_lemon(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run lemon CLI as a subprocess from repo root.
    ...
    connectivity — but ``lemon update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-lemon-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-lemon-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_1": "git@",
        }
    )
    return subprocess.run(
        [sys.executable, "-m", "lemon_cli.main"] + args,
        capture_output=True,
        text=True,
        cwd=str(repo_root),
        env=env,
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`lemon secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_lemon(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`lemon secrets bitwarden disable` must exit 0."""
        result = _run_lemon(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`lemon secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_lemon(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`lemon secrets onepassword setup --help` must exit 0."""
        result = _run_lemon(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `lemon update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`lemon update --check` must not load cryptography._rust."""
        result = _run_lemon(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['lemon','update','--check']
...
sys.argv = ['lemon', 'update', '--check']

import lemon_cli.main as m
...
# resolves the lemon_cli.main module's lazily re-exported attribute —
# so the patch must land on update_cmd._cmd_update_check.
with patch('lemon_cli.update_cmd._cmd_update_check', capture_update_check):
    try:
        m.main()
