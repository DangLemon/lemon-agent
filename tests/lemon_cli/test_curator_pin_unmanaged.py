...
`lemon curator pin <name>` on an unmanaged skill (curation-eligible but no
...
becomes meaningful after `lemon curator adopt`) and makes the message say
...
def test_pin_unmanaged_records_flag_and_prints_adopt_hint(monkeypatch, capsys):
    import lemon_cli.curator as curator_cli
    ...
    assert "lemon curator adopt legacy-skill" in out
