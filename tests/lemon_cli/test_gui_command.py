"""Tests for ``lemon gui`` desktop launcher wiring."""
...
from lemon_cli import main as cli_main
from lemon_cli import main_desktop
from lemon_cli import main_install_repair
from lemon_cli import main_web_build
...
def _isolate_xdg_data_home(tmp_path, monkeypatch):
    ...
    call live, and on a Linux dev box it wrote a ``lemon.desktop`` pointing
...
def _stable_keychain_detection(monkeypatch):
    ...
    monkeypatch.delenv("LEMON_DESKTOP_PASSWORD_STORE", raising=False)
...
def _packaged_exe_rel() -> Path:
    ...
    if sys.platform == "darwin":
        return Path("mac-arm64") / "Lemon AI.app" / "Contents" / "MacOS" / "Lemon AI"
    if sys.platform == "win32":
        return Path("win-unpacked") / "Lemon AI.exe"
    return Path("linux-unpacked") / "Lemon AI"
...
def test_gui_without_selector_uses_checkout_lemon_packaging_contract(
    ...
    try:
        with (
            patch("lemon_cli.main_desktop._register_linux_desktop_entry"),
            patch(
                "lemon_cli.main.subprocess.run",
        ...
        selected = os.environ["LEMON_DESKTOP_HARNESS_CONFIG"]
        product_name = main_desktop._desktop_product_name()
    finally:
        os.environ.pop("LEMON_DESKTOP_HARNESS_CONFIG", None)
...
def test_invalid_checkout_lemon_config_preserves_ordinary_identity(
    ...
    assert "LEMON_DESKTOP_HARNESS_CONFIG" not in os.environ
    assert main_desktop._desktop_product_name() == "Lemon AI"
...
@pytest.mark.macos_only
def test_desktop_packaged_executable_prefers_lemon_ai_before_newer_legacy_mac_bundle(
    tmp_path, monkeypatch
):
    config = tmp_path / "internal.json"
    _write_internal_harness(config)
...
@pytest.mark.macos_only
def test_desktop_packaged_executable_invalid_internal_env_stays_ordinary(
    tmp_path, monkeypatch
):
...
@pytest.mark.macos_only
def test_desktop_packaged_executable_ordinary_prefers_lemon_ai_mac_bundle(
    tmp_path, monkeypatch
):
    monkeypatch.delenv("LEMON_DESKTOP_HARNESS_CONFIG", raising=False)
    release = tmp_path / "release"
    ...
    _write_executable(legacy)
    _write_executable(branded)
    os.utime(legacy.parents[2], (300, 300))
    os.utime(branded.parents[2], (100, 100))
...
@pytest.mark.macos_only
def test_desktop_packaged_executable_prefers_lemon_ai_when_bundle_mtimes_tie(
    tmp_path, monkeypatch
):
    config = tmp_path / "internal.json"
    _write_internal_harness(config)
...
@pytest.mark.macos_only
def test_desktop_packaged_executable_ignores_partial_lemon_ai_mac_bundle(
    tmp_path, monkeypatch
):
    config = tmp_path / "internal.json"
    _write_internal_harness(config)
...
@pytest.mark.macos_only
def test_desktop_packaged_executable_ignores_nonexecutable_lemon_ai_mac_binary(
    tmp_path, monkeypatch
):
    config = tmp_path / "internal.json"
    _write_internal_harness(config)
...
def _make_shell_mac_bundle(
    root: Path, name: str, *, app_mtime: int, executable_mtime: int = 50
) -> Path:
    app = root / name
    executable_name = "Lemon AI" if name == "Lemon AI.app" else "Hermes"
...
def test_shell_selector_prefers_lemon_bundle_before_newer_legacy(tmp_path, script):
    lemon = _make_shell_mac_bundle(tmp_path, "Lemon AI.app", app_mtime=100)
    legacy = _make_shell_mac_bundle(tmp_path, "Hermes.app", app_mtime=200)

    assert _select_mac_bundle_with_shell_script(script, lemon, legacy) == lemon

    (lemon / "Contents" / "MacOS" / "Lemon AI").chmod(0o644)
    assert _select_mac_bundle_with_shell_script(script, lemon, legacy) == legacy
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with (
        patch("lemon_cli.main.shutil.which", return_value="/usr/bin/npm"),
        patch(
            "lemon_cli.main_web_build._run_npm_install_deterministic",
            return_value=install_ok,
        ) as mock_install,
        patch("lemon_cli.main_desktop._desktop_build_needed", return_value=True),
        patch("lemon_cli.main_desktop._write_desktop_build_stamp"),
        patch("lemon_cli.main_desktop._desktop_macos_relaunchable_fixup"),
        patch(
            "lemon_cli.main_desktop._desktop_linux_sandbox_fixup", return_value=True
        ),
        patch("lemon_cli.main_desktop._register_linux_desktop_entry"),
        patch(
            "lemon_cli.main.subprocess.run", side_effect=_pack_into_staging(root)
        ) as mock_run,
        pytest.raises(SystemExit) as exc,
    ):
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the Lemon AI-managed
    Node ahead of that bare PATH or the install dies with ``node: not found``.
    """
    import os

    from lemon_constants import iter_lemon_node_dirs

    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)

    # A managed Node tree on disk so with_lemon_node_path() actually prepends it.
    home = tmp_path / "lemon-home"
    ...
    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    launch_ok = subprocess.CompletedProcess(["lemon"], 0)
    ...
    with (
        patch(
            "lemon_cli.main_install_repair._resolve_node_runtime_npm",
        ...
        patch("lemon_cli.main.subprocess.run", return_value=launch_ok),
        pytest.raises(SystemExit),
    ):
        cli_main.cmd_gui(_ns(skip_build=False))

    managed_dirs = [str(p) for p in iter_lemon_node_dirs() if p.is_dir()]
...
def test_gui_does_not_retry_after_packaged_executable_exists(
    ...
    Lemon AI.app/Contents/MacOS/Lemon AI in place. Re-downloading Electron can't
    ...
    with (
        ...
        patch("lemon_cli.main_desktop._desktop_macos_relaunchable_fixup"),
        patch(
            "lemon_cli.main_desktop._purge_electron_build_cache",
            return_value=[Path("/c/electron.zip")],
        ) as mock_purge,
        patch(
            "lemon_cli.main_desktop._redownload_electron_dist", return_value=True
        ) as mock_dl,
        patch("lemon_cli.main.subprocess.run", side_effect=pack_fail) as mock_run,
        pytest.raises(SystemExit) as exc,
    ):
        cli_main.cmd_gui(_ns())
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged Lemon AI.app with the pieces the signer must find."""
    ent_dir = desktop_dir / "electron"
    ent_dir.mkdir(parents=True, exist_ok=True)
    (ent_dir / "entitlements.mac.plist").write_text("<plist/>", encoding="utf-8")
    (ent_dir / "entitlements.mac.inherit.plist").write_text(
    ...
    (app / "Contents" / "Frameworks" / "chrome_crashpad_handler").write_text(
        "", encoding="utf-8"
    )
    return app
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    Lemon AI.app component, silently skipping every .node/.dylib/crashpad
    ...
    assert (
        main_desktop._desktop_macos_local_codesign(app, desktop_dir=desktop_dir) is True
    )

    signed = [
        c[-1] for c in calls if c[:3] == ["/usr/bin/codesign", "--force", "--sign"]
    ]
    assert (
        str(
            app
...
@pytest.mark.macos_only
def test_relaunchable_fixup_falls_back_to_legacy_adhoc_on_failure(
    tmp_path, monkeypatch, capsys
):
    ...
    monkeypatch.setattr(cli_main.subprocess, "run", fake_run)
    monkeypatch.setattr(
        main_desktop, "_desktop_macos_has_valid_real_signature", lambda a: False
    )
    monkeypatch.setattr(
        main_desktop, "_desktop_macos_local_signing_identity", lambda: None
    )
...
def test_setup_tcc_identity_creates_cert_imports_trusts_and_configures(
    tmp_path, monkeypatch, capsys
):
    ...
    identity = "Lemon AI Local Signing"
    calls = []
    state = {"trusted": False}
    ...
    monkeypatch.setattr(
        main_desktop, "_desktop_macos_relaunchable_fixup", lambda d: True
    )
    # Avoid writing the real user config.
    monkeypatch.setattr("lemon_cli.config.set_config_value", lambda key, value: None)
    ...
    assert any(
        c[0] == "/usr/bin/security" and c[1] == "add-trusted-cert" for c in calls
    )
    ...
    assert not list(tmp_path.glob("lemon-tcc-*"))


def test_setup_tcc_identity_retries_pkcs12_with_legacy_on_mac_verification_failure(
    tmp_path, monkeypatch, capsys
):
    ...
    assert (
        len([c for c in calls if c[0] == "/usr/bin/security" and c[1] == "import"]) == 2
    )
...
def test_setup_tcc_identity_fails_when_trust_step_fails(tmp_path, monkeypatch, capsys):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[0] == "/usr/bin/security" and cmd[1] == "add-trusted-cert":
            return _fake_proc(
                cmd,
                returncode=1,
                stderr="SecTrustSettingsSetTrustSettings: authorization denied",
            )
        return _fake_proc(cmd)
    ...
    assert (
        main_desktop._desktop_macos_setup_tcc_identity("Lemon AI Local Signing") is False
    )
    assert "could not trust the certificate" in capsys.readouterr().out


def test_setup_tcc_identity_fails_when_identity_never_becomes_valid(
    tmp_path, monkeypatch, capsys
):
...
def test_setup_tcc_identity_skips_generation_when_already_valid(
    tmp_path, monkeypatch, capsys
):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[:4] == ["/usr/bin/security", "find-identity", "-v", "-p"]:
            return _fake_proc(
                cmd,
                stdout='  1) ABCD "Lemon AI Local Signing"\n     1 valid identities found',
            )
        return _fake_proc(cmd)
    ...
    assert (
        main_desktop._desktop_macos_setup_tcc_identity("Lemon AI Local Signing") is True
    )
...
def test_setup_tcc_identity_untrusted_existing_cert_is_repaired(
    tmp_path, monkeypatch, capsys
):
...
def test_cmd_gui_setup_tcc_identity_exits_before_build(tmp_path, monkeypatch):
    """`lemon desktop --setup-tcc-identity` calls the setup and exits 0/1
    ...
    with (
        patch(
            "lemon_cli.main_desktop._desktop_macos_setup_tcc_identity",
            return_value=True,
        ) as mock_setup,
        patch(
            "lemon_cli.main_web_build._run_npm_install_deterministic"
        ...
        cli_main.cmd_gui(_ns(setup_tcc_identity=True, identity="Lemon AI Local Signing"))

    assert exc.value.code == 0
    mock_setup.assert_called_once_with("Lemon AI Local Signing")
    mock_install.assert_not_called()


@pytest.mark.macos_only
def test_relaunchable_fixup_stable_identity_never_touches_keychain(
    tmp_path, monkeypatch
):
    ...
    monkeypatch.setattr(
        main_desktop, "_desktop_macos_local_codesign", lambda app, **kw: True
    )
    monkeypatch.setattr(
        cli_main.subprocess,
        "run",
...
@pytest.mark.macos_only
def test_relaunchable_fixup_default_noconfig_success_never_touches_keychain(
    tmp_path, monkeypatch
):
...
@pytest.mark.macos_only
def test_relaunchable_fixup_legacy_adhoc_failure_never_touches_keychain(
    tmp_path, monkeypatch
):
...
@pytest.mark.macos_only
def test_relaunchable_fixup_legacy_adhoc_success_still_verifies_and_never_deletes(
    tmp_path, monkeypatch
):
...
def test_internal_desktop_user_facing_labels_use_lemon_identity(monkeypatch):
    monkeypatch.setattr(main_desktop, "_internal_desktop_build", lambda: True)
...
def test_ordinary_desktop_user_facing_labels_keep_lemon_identity(monkeypatch):
    monkeypatch.setattr(main_desktop, "_internal_desktop_build", lambda: False)
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    """`lemon desktop` gives the app a launcher presence on Linux."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    packaged_exe = _make_packaged_executable(root, monkeypatch)

    registered: list[Path] = []
    monkeypatch.setattr("lemon_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr(
        "lemon_cli.linux_desktop_entry.install_desktop_entry",
        lambda project_root: (
            registered.append(project_root) or (tmp_path / "lemon.desktop")
        ),
    )
    ...
    with (
        patch("lemon_cli.main_desktop._desktop_build_needed", return_value=False),
        patch(
            "lemon_cli.main_install_repair._resolve_node_runtime_npm",
            return_value="/usr/bin/npm",
        ),
...
def test_gui_launches_even_when_desktop_entry_install_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("lemon_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr("lemon_cli.linux_desktop_entry.install_desktop_entry", boom)
    ...
    with (
        ...
        patch("lemon_cli.main.subprocess.run", return_value=launch_ok) as mock_run,
        pytest.raises(SystemExit) as exc,
    ):
        cli_main.cmd_gui(_ns())
...
def test_gui_skips_desktop_entry_off_linux(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("lemon_cli.linux_desktop_entry.is_supported", lambda: False)

    def fail(_project_root):
        raise AssertionError("must not install a desktop entry off Linux")

    monkeypatch.setattr("lemon_cli.linux_desktop_entry.install_desktop_entry", fail)
...
def test_desktop_launch_options_ozone_hint_defaults_auto():
    with patch("lemon_cli.config.load_config", return_value={}):
...
def test_gui_bridges_ozone_hint_to_launch_env(tmp_path, monkeypatch):
    ...
    with (
        patch("lemon_cli.main.shutil.which", return_value="/usr/bin/npm"),
        patch(
            "lemon_cli.main_web_build._run_npm_install_deterministic", return_value=ok
        ),
        patch("lemon_cli.main_desktop._desktop_build_needed", return_value=True),
        patch("lemon_cli.main_desktop._write_desktop_build_stamp"),
        patch("lemon_cli.main_desktop._desktop_macos_relaunchable_fixup"),
        ...
        patch("lemon_cli.config.load_config", return_value=cfg),
        patch(
            "lemon_cli.linux_desktop_entry.install_desktop_entry", return_value=None
        ),
        patch(
            "lemon_cli.main.subprocess.run", side_effect=_pack_into_staging(root)
        ) as mock_run,
...
def test_detect_linux_password_store_prefers_kde_session(
    monkeypatch, kde_version, expected
):
...
def test_detect_linux_password_store_via_dbus_secret_service(monkeypatch):
    ...
    with patch("lemon_cli.main.subprocess.run", return_value=ping_ok) as mock_run:
        assert main_desktop._detect_linux_password_store() == "gnome-libsecret"
...
def test_detect_linux_password_store_none_when_no_keychain(monkeypatch):
    ...
    with patch("lemon_cli.main.subprocess.run", return_value=ping_fail):
        assert main_desktop._detect_linux_password_store() is None
    with patch("lemon_cli.main.subprocess.run", side_effect=FileNotFoundError):
        assert main_desktop._detect_linux_password_store() is None


@pytest.mark.linux_only
def test_gui_linux_packaged_launch_bridges_detected_password_store(
    tmp_path, monkeypatch
):
    _clear_keychain_env(monkeypatch)
    root = _make_desktop_tree(tmp_path)
    ...
    with (
        ...
        patch("lemon_cli.config.load_config", return_value={}),
        patch(
            "lemon_cli.linux_desktop_entry.install_desktop_entry", return_value=None
        ),
        patch(
            "lemon_cli.main_desktop._detect_linux_password_store",
            return_value="gnome-libsecret",
        ),
        patch(
            "lemon_cli.main.subprocess.run", side_effect=_pack_into_staging(root)
        ) as mock_run,
    ...
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["LEMON_DESKTOP_PASSWORD_STORE"] == "gnome-libsecret"
...
def test_gui_linux_source_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["LEMON_DESKTOP_PASSWORD_STORE"] == "kwallet6"
...
def test_gui_config_password_store_skips_detection(tmp_path, monkeypatch):
    ...
    with (
        ...
        patch("lemon_cli.main_desktop._detect_linux_password_store") as mock_detect,
        patch(
            "lemon_cli.main.subprocess.run", side_effect=_pack_into_staging(root)
        ) as mock_run,
        pytest.raises(SystemExit),
...
@pytest.mark.linux_only
def test_gui_explicit_password_store_env_wins_over_config_and_detection(
    tmp_path, monkeypatch
):
    _clear_keychain_env(monkeypatch)
    monkeypatch.setenv("LEMON_DESKTOP_PASSWORD_STORE", "basic")
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["LEMON_DESKTOP_PASSWORD_STORE"] == "basic"
...
def test_gui_password_store_bridge_is_linux_only(tmp_path, monkeypatch):
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert "LEMON_DESKTOP_PASSWORD_STORE" not in launch_env
...
def _gui_build_patches(root: Path, run_side_effect):
    return [
        ...
        patch("lemon_cli.main_desktop._register_linux_desktop_entry"),
        patch(
            "lemon_cli.main_desktop._stop_desktop_processes_locking_build",
            return_value=[],
        ),
        patch("lemon_cli.main_desktop._purge_electron_build_cache", return_value=[]),
        patch("lemon_cli.main_desktop._redownload_electron_dist", return_value=False),
        patch("lemon_cli.main.subprocess.run", side_effect=run_side_effect),
    ]
...
def test_swap_staged_desktop_app_promotes_staged_tree_and_drops_previous(tmp_path):
    ...
    assert sorted(p.name for p in (desktop_dir / "release").iterdir()) == [
        _packaged_exe_rel().parts[0]
    ]
...
def test_swap_staged_desktop_app_without_staged_exe_keeps_live_app(tmp_path):
    ...
    _write_executable(live_exe, "old")
    staging = main_desktop._desktop_staging_dir(desktop_dir)
    (staging / "linux-unpacked" / "resources").mkdir(
        parents=True
    )  # partial tree, no exe
...
def test_swap_staged_desktop_app_rolls_back_when_second_rename_fails(
    tmp_path, monkeypatch
):
    root = _make_desktop_tree(tmp_path)
    desktop_dir = root / "apps" / "desktop"
...
def test_gui_zero_exit_pack_without_artifact_keeps_previous_app(
    tmp_path, monkeypatch, capsys
):
    root = _make_desktop_tree(tmp_path)
    desktop_dir = root / "apps" / "desktop"
