...
Pre-#67193 lemon-setup binaries reuse ``bootstrap-cache/install-<branch>.ps1``
...
``Access denied``). Those binaries have no self-update path, so ``lemon
...
def _run(home, root, branch="main"):
    with patch.object(cli_main, "get_lemon_home", return_value=str(home)), patch.object(
...
def test_never_raises_on_io_error(tmp_path):
    home, root = _setup(tmp_path)
    with patch.object(cli_main, "get_lemon_home", side_effect=OSError("boom")):
...
def test_repository_specific_mutable_branch_cache_entries_are_removed(tmp_path, monkeypatch):
    ...
    ps1 = cache / "install-DangLemon__lemon-agent-main.ps1"
    sh = cache / "install-DangLemon__lemon-agent-main.sh"
    pinned = cache / ("install-DangLemon__lemon-agent-" + "a" * 40 + ".ps1")
