"""Regression tests: a failed quarantine restore must never strand `lemon`.

On Windows the updater renames the live ``lemon*.exe`` shims aside
(``lemon.exe.old.<unix-ms>``) so uv can write replacements. Gaps in the
recovery path ended with ``lemon`` gone from PATH — and, because the command
that repairs it IS ``lemon update``, unrecoverable without a manual reinstall
(#75584):
...
from lemon_cli import _early_recovery as er
from lemon_cli import _install_repair as ir
...
def test_cleanup_rescue_reports_when_it_cannot_recover(tmp_path, capsys):
    ...
    assert "FAILED to restore lemon.exe" in captured.err
...
def test_cleanup_rescues_newest_by_parsed_stamp_not_lexicographic(tmp_path):
    ...
    """
    scripts = _make_scripts_dir(tmp_path)
    (scripts / "lemon.exe.old.999").write_bytes(b"MZ-stray-short-stamp")
    (scripts / f"lemon.exe.old.{_stamp(60_000)}").write_bytes(b"MZ-genuine")

    with _run_cleanup(scripts):
        cli_main._cleanup_quarantined_exes(scripts)

    assert (scripts / "lemon.exe").read_bytes() == b"MZ-genuine"


def test_cleanup_ignores_names_it_did_not_create(tmp_path):
    """An unparseable suffix is not ours: never rescued, never deleted."""
    scripts = _make_scripts_dir(tmp_path)
    (scripts / "lemon.exe").write_bytes(b"MZ-live")
    foreign = scripts / "lemon.exe.old.backup"
    ...
    assert (scripts / "lemon.exe").read_bytes() == b"MZ-live"
...
def test_cleanup_does_not_rescue_from_a_foreign_name(tmp_path):
    ...
    assert not (scripts / "lemon.exe").exists()
...
def test_cleanup_leaves_fresh_quarantine_for_concurrent_update(tmp_path):
    ...
    scripts = _make_scripts_dir(tmp_path)
    (scripts / "lemon.exe").write_bytes(b"MZ-live")
    fresh = scripts / f"lemon.exe.old.{_stamp()}"
...
def test_cleanup_still_sweeps_genuinely_stale_quarantine(tmp_path):
    ...
    stale = scripts / f"lemon.exe.old.{_stamp(ancient_ms)}"
...
def test_quarantine_stamp_ms_parses_and_rejects():
    assert main_install_repair._quarantine_stamp_ms(Path("lemon.exe.old.1787020473885")) == 1787020473885
    assert main_install_repair._quarantine_stamp_ms(Path("lemon.exe.old.backup")) is None
    assert main_install_repair._quarantine_stamp_ms(Path("lemon.exe")) is None
...
def test_helper_retries_then_succeeds(tmp_path):
    scripts = _make_scripts_dir(tmp_path)
    quarantined = scripts / "lemon.exe.old.123"
    quarantined.write_bytes(b"MZ-old-lemon")
    original = scripts / "lemon.exe"
    ...
    assert failed == []
    assert original.read_bytes() == b"MZ-old-lemon"
    assert calls["n"] >= 2
...
def test_helper_reports_failure_and_returns_the_pair(tmp_path, capsys):
    ...
    assert "FAILED to restore lemon.exe" in captured.err
    assert "lemon.exe.old.123" in captured.err
