"""Tests for ``lemon doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from lemon_cli import doctor_live
from lemon_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `lemon doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("lemon_cli.doctor.LEMON_HOME", tmp_path / "home")
        monkeypatch.setattr("lemon_cli.doctor.PROJECT_ROOT", tmp_path / "root")
