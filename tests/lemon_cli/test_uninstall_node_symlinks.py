"""Tests for lemon_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $LEMON_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Lemon AI-managed node dir.
"""
...
def _make_lemon_node(lemon_home: Path) -> Path:
    """Create a fake $LEMON_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = lemon_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    lemon_home = fake_home / ".lemon-ai"
    _make_lemon_node(lemon_home)
    ...
    removed = uninstall.remove_node_symlinks(lemon_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    lemon_home = fake_home / ".lemon-ai"
    node_bin = _make_lemon_node(lemon_home)
