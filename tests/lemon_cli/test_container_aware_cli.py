...
def container_env(tmp_path, monkeypatch):
    """Set up a fake LEMON_HOME with .container-mode file."""
    lemon_home = tmp_path / ".lemon-ai"
    lemon_home.mkdir()
    monkeypatch.setenv("LEMON_HOME", str(lemon_home))
    monkeypatch.delenv("LEMON_DEV", raising=False)

    container_mode = lemon_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=lemon-agent\n"
        "exec_user=lemon\n"
        "lemon_bin=/data/current-package/bin/lemon\n"
    )
    return lemon_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("lemon_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "lemon-agent"
    assert info["exec_user"] == "lemon"
    assert info["lemon_bin"] == "/data/current-package/bin/lemon"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from lemon_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "lemon"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "lemon-agent" in cmd
    assert "/data/current-package/bin/lemon" in cmd
    assert "chat" in cmd
