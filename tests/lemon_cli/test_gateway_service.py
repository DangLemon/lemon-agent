...
import lemon_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`lemon gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `lemon gateway restart` in the background; when a
        ...
        assert "lemon gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``LEMON_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../lemon_test`` LEMON_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "lemon-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir LEMON_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="LEMON_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/lemon_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="LEMON_HOME=/tmp/lemon-e2e-41264"\n'
        assert (
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="LEMON_HOME={tmp_path}/lemon-home"\n'
...
class TestGetCronDrainTimeout:
    def test_missing_config_falls_back_to_default(self, monkeypatch):
        monkeypatch.delenv("LEMON_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_env_overrides_config(self, monkeypatch):
        monkeypatch.setenv("LEMON_CRON_DRAIN_TIMEOUT", "45")
        monkeypatch.setattr(
            gateway_cli,
            "read_raw_config",
...
class TestGeneratedSystemdUnits:
    ...
    def test_timeout_stop_sec_covers_default_cron_drain_floor(self, monkeypatch):
        ...
        monkeypatch.delenv("LEMON_RESTART_DRAIN_TIMEOUT", raising=False)
        monkeypatch.delenv("LEMON_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".lemon-ai" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `lemon gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import lemon_cli.resource_limits as resource_limits
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        plist_path = tmp_path / "ai.lemon.gateway.plist"
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                "<plist>--replace\n<key>LEMON_HOME</key>"
                "<string>/Users/alice/.lemon-ai</string></plist>"
            ),
        )
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_lemon_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_launchd_restart_uses_sigusr1_and_exit_wait_budget(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.lemon.gateway")
        ...
        assert ("observe", "ai.lemon.gateway", 654, "gui/501") in calls
        out = capsys.readouterr().out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.lemon.gateway"])
            ),
        )
...
class TestSystemUnitLemonHome:
    """LEMON_HOME in system units must reference the target user, not root."""

    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        self, monkeypatch, tmp_path
    ):
        managed_bin = tmp_path / ".lemon-ai" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".lemon-ai")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_lemon = target_home / ".lemon-ai"
        root_home = tmp_path / "root"
        root_lemon = root_home / ".lemon-ai"
        managed_bin = target_lemon / "node" / "bin"
        managed_bin.mkdir(parents=True)
        node = managed_bin / "node"
        node.write_text("#!/bin/sh\n")
        node.chmod(0o755)
        root_lemon.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", staticmethod(lambda: root_home))
        monkeypatch.setenv("LEMON_HOME", str(root_lemon))
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
        )
        monkeypatch.setattr(gateway_cli, "get_lemon_home", lambda: root_lemon)
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "lemon_constants.iter_lemon_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "lemon_constants.lemon_managed_node_tree_present",
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'LEMON_HOME=/home/alice/.lemon-ai' in unit
        assert '/root/.lemon-ai' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's LEMON_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        lemon_home = str(gateway_cli.get_lemon_home().resolve())
        assert f'LEMON_HOME={lemon_home}' in unit
    ...
    def test_user_unit_persists_update_repository(self, monkeypatch):
        ...
        assert 'Environment="LEMON_UPDATE_REPOSITORY=ExampleOrg/runtime-agent"' in unit
    ...
    def test_launchd_plist_persists_update_repository(self, monkeypatch):
        ...
        assert "<key>LEMON_UPDATE_REPOSITORY</key>" in plist
        assert "<string>ExampleOrg/runtime-agent</string>" in plist


class TestSystemUnitRefreshSyncsLemonHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.lemon-ai."""

    def test_refresh_adopts_unit_lemon_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_lemon = root_home / ".lemon-ai"
        alice_lemon = alice_home / ".lemon-ai"
        root_lemon.mkdir(parents=True)
        alice_lemon.mkdir(parents=True)
        (root_lemon / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_lemon / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's LEMON_HOME + drain timeout).
        monkeypatch.setenv("LEMON_HOME", str(alice_lemon))
        ...
        # Simulate sudo without inherited LEMON_HOME (falls back to root).
        monkeypatch.setenv("LEMON_HOME", str(root_lemon))
        ...
        assert os.environ["LEMON_HOME"] == str(alice_lemon)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned LEMON_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_lemon_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_lemon_home_from_systemd_unit",
...
class TestLemonHomeForTargetUser:
    """Unit tests for _lemon_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("LEMON_HOME", raising=False)

        result = gateway_cli._lemon_home_for_target_user("/home/alice")
        assert result == "/home/alice/.lemon-ai"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "lemon gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_home = tmp_path / "root"
        target_home = tmp_path / "home" / "alice"
        root_profile = root_home / ".lemon-ai" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("LEMON_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_lemon_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", str(target_home)),
        ...
        assert f'LEMON_HOME={target_home / ".lemon-ai" / "profiles" / "mybot"}' in unit

    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".lemon-ai" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LEMON_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_lemon_home", lambda: profile_dir)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".lemon-ai" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.lemon.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".lemon-ai" / "lemon-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".lemon-ai" / "lemon-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".lemon-ai" / "lemon-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("LEMON_HOME", str(root_home / ".lemon-ai"))
        monkeypatch.setattr(gateway_cli, "get_lemon_home", lambda: root_home / ".lemon-ai")
        ...
        # WorkingDirectory is anchored at the target user's LEMON_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.lemon-ai" in unit
        assert "WorkingDirectory=/home/alice/.lemon-ai/lemon-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "lemon-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "lemon-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'lemon gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyLemonUnitDetection:
    """Tests for _find_legacy_lemon_units / has_legacy_lemon_units.
    ...
    renamed to ``lemon-gateway.service``. After PR #5646 (signal recovery
    ...
    (``lemon-gateway-<profile>.service``) or unrelated third-party services.
    """
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        results = gateway_cli._find_legacy_lemon_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "lemon gateway migrate-legacy" in out



class TestRemoveLegacyLemonUnits:
    """Tests for remove_legacy_lemon_units (the migration action)."""
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_lemon_units(interactive=False)
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (lemon-gateway-coder.service)
        ...
        profile_unit = user_dir / "lemon-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "lemon-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `lemon gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import lemon_cli.main as cli_main
        ...
        # public, just confirm that `lemon gateway --help` shows it.
        import subprocess
        import sys
        ...
        result = subprocess.run(
            [sys.executable, "-m", "lemon_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_lemon_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "lemon_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_lemon_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_lemon_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_lemon_units", fake_remove)
    ...
    def test_install_skips_legacy_check_when_none_present(
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_lemon_units", lambda: False)
        monkeypatch.setattr(gateway_cli, "remove_legacy_lemon_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "lemon-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "lemon-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "lemon-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``lemon gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "lemon-gateway")
        ...
        assert "sudo systemctl start lemon-gateway" in out
        assert "sudo lemon gateway uninstall --system" in out
        assert "lemon gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``lemon gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (LEMON_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """



    def test_user_unit_workingdirectory_is_lemon_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".lemon-ai"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_lemon_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_lemon_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.lemon.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.lemon.gateway"
...
class TestTimeoutStopSecCoversCronFloor:
    ...
    def _unit_with_config(self, tmp_path, monkeypatch, config_yaml, env=None):
        lemon = tmp_path / "home" / ".lemon-ai"
        lemon.mkdir(parents=True)
        (lemon / "config.yaml").write_text(config_yaml, encoding="utf-8")
    ...
    def test_env_override_extends_the_leash(self, tmp_path, monkeypatch):
        unit = self._unit_with_config(
            ...
            monkeypatch,
            "agent:\n  restart_drain_timeout: 0\n",
            env={"LEMON_CRON_DRAIN_TIMEOUT": "200"},
        )
