...
def test_compare_behind_uses_configured_repository():
    with patch("urllib.request.urlopen", return_value=_FakeResponse(json.dumps({"ahead_by": 7}).encode())) as open_mock:
        assert banner._github_compare_behind(SHA_A, SHA_B, repository="DangLemon/lemon-agent") == 7

    request = open_mock.call_args.args[0]
    assert request.full_url == f"https://api.github.com/repos/DangLemon/lemon-agent/compare/{SHA_A}...{SHA_B}"
...
def test_check_via_rev_recovers_exact_count():
    with patch(
        "lemon_cli.banner.subprocess.run", return_value=_ls_remote_result(SHA_B)
    ), patch.object(banner, "_github_compare_behind", return_value=61) as compare:
        assert banner._check_via_rev(SHA_A) == 61
    compare.assert_called_once_with(SHA_A, SHA_B, repository="DangLemon/lemon-agent")
...
def test_check_via_rev_uses_internal_repository(monkeypatch):
    ...
    assert "https://github.com/DangLemon/lemon-agent.git" in ls_remote_cmd
    compare.assert_called_once_with(SHA_A, SHA_B, repository="DangLemon/lemon-agent")
...
def test_check_via_rev_up_to_date_short_circuits_compare():
    with patch(
        "lemon_cli.banner.subprocess.run", return_value=_ls_remote_result(SHA_A)
...
def test_check_via_local_git_internal_env_repoints_upstream_origin(monkeypatch, tmp_path):
    ...
    with patch(
        "lemon_cli.banner.subprocess.run",
...
def test_shallow_checkout_recovers_exact_count(tmp_path):
    ...
    with patch(
        "lemon_cli.banner.subprocess.run", side_effect=_shallow_git(SHA_A, SHA_B)
...
def test_shallow_checkout_equal_tips_up_to_date_without_compare(tmp_path):
    ...
    with patch(
        "lemon_cli.banner.subprocess.run", side_effect=_shallow_git(SHA_A, SHA_A)
    ), patch.object(banner, "_github_compare_behind") as compare:
