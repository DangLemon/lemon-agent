...
base deps) during ``lemon update``, leaving the venv in a broken state
...
def temp_pyproject(tmp_path, monkeypatch):
    """Point lemon_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("lemon_cli.main_install_repair._resolve_install_target_python", return_value=py), \
             patch("lemon_cli.main_install_repair.subprocess.run", side_effect=fake_subprocess_run), \
             patch("lemon_cli.main_install_repair._run_install_with_heartbeat"):

            from lemon_cli.main_install_repair import _verify_core_dependencies_installed
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("lemon_cli.main_install_repair._resolve_install_target_python") as mock_resolve, \
             patch("lemon_cli.main_install_repair._run_install_with_heartbeat") as mock_install:
            from lemon_cli.main_install_repair import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `lemon update`.
        ...
        with patch("lemon_cli.main_install_repair._is_windows", return_value=True):
            from lemon_cli.main_install_repair import _resolve_install_target_python
