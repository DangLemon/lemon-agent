...
The bug class: on Windows, when `lemon.exe`/sibling shims could not be
...
import lemon_cli._install_repair as ir
import lemon_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("lemon", "lemon-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # lemon.exe cannot be renamed; lemon-gateway.exe can
    real_rename = Path.rename

    def deny_lemon(self, target):
        if self.name == "lemon.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_lemon)
    monkeypatch.setattr(main_install_repair, "_lemon_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "lemon.exe" in exc_info.value.failed_shims
    # The successful rename (lemon-gateway.exe) was rolled back.
    assert (scripts / "lemon-gateway.exe").exists()
    assert not list(scripts.glob("lemon-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("lemon",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("lemon",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = main_install_repair.ShimQuarantineError(["lemon.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "lemon.exe" in out
