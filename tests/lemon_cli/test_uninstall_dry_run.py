...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_lemon_home", lambda: lemon_home)
    monkeypatch.setattr(uninstall, "_is_default_lemon_home", lambda home: False)
    ...
    assert str(lemon_home) in output
    assert project_root.exists()
    assert lemon_home.exists()
