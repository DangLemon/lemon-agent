"""Tests for lemon_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `lemon setup` and `lemon import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("lemon_constants.is_container", lambda: container)
