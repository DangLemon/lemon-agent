"""Regression tests for #34107 — Docker UID/GID handling in ensure_lemon_home.

When Lemon AI runs in Docker with ``LEMON_UID=1000`` / ``LEMON_GID=911``,
the entrypoint chowns the top-level ``LEMON_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_lemon_home()`` — especially
...
The fix is a ``_chown_to_lemon_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_lemon_uid_gid
# ---------------------------------------------------------------------------


class TestResolveLemonUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("LEMON_UID", "1000")
        monkeypatch.setenv("LEMON_GID", "911")
        from lemon_cli.config import _resolve_lemon_uid_gid
...
class TestChownToLemonUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_lemon_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("LEMON_UID", raising=False)
        monkeypatch.delenv("LEMON_GID", raising=False)
        from lemon_cli import config as cfg
