...
def lemon_home(tmp_path, monkeypatch):
    ...
    # /etc/lemon on the dev/CI box can't influence the test.
    monkeypatch.setenv("LEMON_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(lemon_home, monkeypatch):
...
def test_env_expansion_in_user_config(lemon_home, monkeypatch):
    from lemon_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_lemon_dotenv(lemon_home=str(home))
