...
from lemon_cli.subcommands.dashboard import build_dashboard_parser
...
from lemon_cli.subcommands.whatsapp import build_whatsapp_parser
...
def test_config_get_unset_subcommands_parse():
    """`lemon config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="lemon")
...
# ── deprecated `lemon login` fails gracefully, not with argparse error ────
#
# `lemon login` is a removed command; its handler (`login_command` in
# `lemon_cli/auth.py`) prints a deprecation notice pointing at `lemon auth` /
# `lemon model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `lemon --help`.
