...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "lemon_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # LEMON_HOME.  For a standard install (LEMON_HOME unset) that root is
        # the platform-native default (~/.lemon-ai), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from lemon_constants import get_default_lemon_root
        assert env.get("LEMON_HOME") == str(get_default_lemon_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (LEMON_DESKTOP=1) must NOT
