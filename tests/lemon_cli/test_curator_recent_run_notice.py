...
The notice prints the most recent curator run summary on `lemon update`,
...
session start) so users normally never see the rename map. `lemon update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from lemon_cli import update_cmd_maint
