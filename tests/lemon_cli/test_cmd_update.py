...
from lemon_cli.main import cmd_update, PROJECT_ROOT
from lemon_cli import main as lemon_main
...
def _patch_managed_uv(request):
    ...
    with patch("lemon_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("lemon_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("lemon_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv), \
         patch(
             "lemon_cli.update_cmd._post_update_sqlite_runtime_status",
...
def _patch_gateway_discovery():
    ...
    ``_purge_stale_lemon_modules`` must also be stubbed because the real
    updater evicts ``lemon_cli.gateway`` after replacing the checkout. A
    ...
    """
    with patch("lemon_cli.gateway.find_gateway_pids", return_value=[]), \
         patch("lemon_cli.gateway.supports_systemd_services", return_value=False), \
         patch("lemon_cli.gateway.find_profile_gateway_processes", return_value=[]), \
         patch.object(lemon_main, "_purge_stale_lemon_modules", lambda *a, **kw: None):
        yield
...
def test_prepare_git_command_reinstall_uses_configured_repository(
    ...
    from lemon_cli import main as hm
...
class TestCmdUpdateNpmLockfileCache:
    @staticmethod
    def _cache_file(lemon_root, project_root):
        cache_key = hashlib.sha256(str(project_root).encode()).hexdigest()[:12]
        return lemon_root / f".npm_lock_hash_{cache_key}"
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `lemon update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".lemon-ai"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(lemon_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            lemon_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("LEMON_HOME", str(shared_root))
            update_cmd._update_node_dependencies()

            monkeypatch.setenv("LEMON_HOME", str(named_profile))
            update_cmd._update_node_dependencies()
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $LEMON_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("lemon_cli.managed_uv.resolve_uv", lambda: None)
...
class TestUpdateManagedPythonEnvIsolation:
    ...
    forces UV_PYTHON_INSTALL_DIR to .lemon-ai-runtime/python), then re-points
    ...
    def test_managed_env_drops_third_party_uv_install_dir(self):
        from lemon_cli.managed_uv import managed_python_env
    ...
    def test_update_uv_env_points_venv_and_runtime_store(self):
        ...
        assert ".lemon-ai-runtime" in uv_env.get("UV_PYTHON_INSTALL_DIR", "")
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind DangLemon/lemon-agent silently misses updates.
        """
        from lemon_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/lemon-agent.git",
    ...
    def test_current_checkout_python_repair_failure_is_durable(
        ...
        with patch.object(
            ...
            "lemon_cli.update_receipt.finalize_update_receipt"
        ) as finalize_receipt, patch(
            "lemon_cli.update_receipt.finalize_pending_update_receipt"
        ):
    ...
    def test_fork_upstream_sync_that_moves_head_runs_post_update_steps(
        ...
        with patch.object(
            ...
            "lemon_cli.update_receipt.collect_fleet_versions",
            return_value=[],
        ), patch(
            ...
            "lemon_cli.gateway.find_gateway_pids",
            return_value=[],
        ), patch(
            "lemon_cli.gateway.find_profile_gateway_processes",
            return_value=[],
        ), patch(
            "lemon_cli.gateway._get_service_pids",
            return_value=set(),
        ), patch.object(
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "lemon_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "lemon_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch(
            "lemon_cli.update_cmd._reload_config_modules"
        ), patch(
            "lemon_cli.update_cmd._run_config_check_fresh", return_value=(1, 2)
        ), patch(
            "lemon_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ) as migrate_config, patch("lemon_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "lemon_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "lemon_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "lemon_cli.update_cmd._reload_config_modules"
        ), patch(
            "lemon_cli.update_cmd._run_config_check_fresh", return_value=(5, 24)
        ), patch(
            "lemon_cli.update_cmd._run_migrate_config_fresh",
    ...
    def test_version_bump_only_surfaces_migration_resets(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "lemon_cli.update_cmd._run_config_check_fresh", return_value=(33, 34)
        ), patch(
            "lemon_cli.update_cmd._run_migrate_config_fresh",
            return_value={
                "env_added": [],
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "lemon_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "lemon_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "lemon_cli.update_cmd._reload_config_modules"
        ), patch(
            "lemon_cli.update_cmd._run_config_check_fresh", return_value=(1, 24)
        ), patch(
            "lemon_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("lemon_cli.main.sys") as mock_sys:
...
class TestConfigVersionCheckUsesFreshModules:
    ...
    Before the fix, ``lemon update`` ran in the PRE-pull Python process.
    After ``git pull`` updated the source on disk, function-level imports
    returned the OLD cached ``lemon_cli.config`` module — so
    ...
    def test_run_config_check_fresh_reloads_modules(self):
        ...
        sys.modules held the OLD lemon_cli.config with the OLD
        DEFAULT_CONFIG["_config_version"] after git pull.
        """
        from unittest.mock import patch

        import lemon_cli.update_cmd as update_cmd
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.lemon-ai"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.lemon-ai/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.lemon-ai/profiles/work"))
        ...
        with (
            patch("lemon_cli.profiles.list_profiles", return_value=all_profiles),
            patch("lemon_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("lemon_cli.profiles.list_profiles", return_value=[default_p]),
            patch("lemon_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``lemon update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``lemon update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``lemon update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from lemon_cli.update_cmd import _update_via_zip
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_wsl_update_skips_windows_npm_build_paths(self, mock_args, monkeypatch):
        ...
        monkeypatch.setattr(lemon_constants, "is_wsl", lambda: True)
        monkeypatch.setattr(
            lemon_constants,
    ...
    def test_update_rebuilds_desktop_that_disappears_mid_update(self):
        ...
        packaged_exe = desktop_dir / "release" / "win-unpacked" / "Lemon AI.exe"
        ...
        desktop_build.assert_called_once_with(
            [hm.sys.executable, "-m", "lemon_cli.main", "desktop", "--build-only"],
            cwd=PROJECT_ROOT,
            env=ANY,
        )
    ...
    def test_git_failure_zip_fallback_rebuilds_missing_desktop(self, tmp_path, monkeypatch):
        ...
        desktop app (release/win-unpacked/Lemon AI.exe) is preserved THROUGH
        ...
        project_root = tmp_path / "lemon-agent"
        ...
        def write_source_zip(_url, destination):
            with zipfile.ZipFile(destination, "w") as archive:
                archive.writestr("lemon-agent-main/apps/desktop/package.json", "{}")
        ...
        monkeypatch.setattr(update_cmd, "get_lemon_home", lambda: tmp_path / "lemon-home")
        ...
        with (
            ...
            patch("lemon_cli.managed_uv.ensure_uv", return_value="uv"),
            patch("lemon_cli.managed_uv.update_managed_uv"),
            patch(
            ...
            patch("lemon_cli.model_catalog.seed_cache_from_checkout", return_value=False),
        ):
...
class TestUpdateNodeDependencies:
    ...
    def test_records_lockfile_hash_only_on_success(self, _which, mock_popen, tmp_path, monkeypatch):
        ...
        from lemon_cli import update_cmd_deps
...
class TestGitTrampolineSelfHeal:
    ...
    def test_healthy_git_command_unchanged(self):
        ...
        with (
            ...
            patch(
                "lemon_cli.update_cmd.subprocess.run",
                side_effect=self._fake_run_healthy,
            ),
            patch("lemon_cli.update_cmd._locate_real_git") as locate,
        ):
    ...
    def test_trampoline_swaps_to_real_git(self, capsys):
        ...
        with (
            ...
            patch(
                "lemon_cli.update_cmd._locate_real_git", return_value=real
            ),
        ):
            result = update_cmd._ensure_non_trampoline_git(git_cmd)
    ...
    def test_trampoline_no_real_git_keeps_command(self, capsys):
        ...
        with (
            ...
            patch("lemon_cli.update_cmd._locate_real_git", return_value=None),
        ):
            result = update_cmd._ensure_non_trampoline_git(git_cmd)
        assert result == git_cmd
        out = capsys.readouterr().out
    ...
    def test_off_windows_noop(self):
        ...
        with (
            ...
            patch("lemon_cli.update_cmd.subprocess.run") as run,
        ):
            result = update_cmd._ensure_non_trampoline_git(git_cmd)
        assert result == git_cmd
    ...
    def test_portable_git_candidates_check_shared_root_first(self, tmp_path, monkeypatch):
        # Profile-scoped layout: LEMON_HOME = <root>/profiles/foo, but the
        # PortableGit tree lives under the SHARED root (monerostar review on
        # #88136). The candidate list must check get_default_lemon_root()
        ...
        monkeypatch.setattr(update_cmd, "get_default_lemon_root", lambda: root)
        monkeypatch.setattr(update_cmd, "get_lemon_home", lambda: profile_home)
