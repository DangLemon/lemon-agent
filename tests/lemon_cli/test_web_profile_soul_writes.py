...
def client(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LEMON_DASHBOARD_SESSION_TOKEN", "soul-test-token")
...
def profile_dir(tmp_path, monkeypatch) -> Path:
    """Create a real profile directory under the test LEMON_HOME."""
    monkeypatch.setenv("LEMON_HOME", str(tmp_path))
...
class TestSoulWriteDurability:
    ...
    def test_existing_soul_survives_an_interrupted_save(
        ...
        # LEMON_HOME patch the client/profile_dir fixtures installed.
...
class TestSoulIoIsOffTheEventLoop:
    ...
    ``tests/lemon_cli/test_web_profiles_off_loop.py``.
    """
