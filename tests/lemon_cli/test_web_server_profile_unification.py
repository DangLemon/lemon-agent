...
profile switcher can target any profile's LEMON_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via LEMON_HOME.
"""
import json
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_lemon_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_lemon_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import lemon_cli.mcp_config as mcp_config
...
class TestProfileScopedModel:
    ...
    def test_model_options_uses_config_only_scope_for_selected_profile(
        ...
        monkeypatch.setattr(
            "lemon_cli.inventory.load_picker_context", lambda: object()
        )
        monkeypatch.setattr(
            "lemon_cli.inventory.build_model_options_payload",
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same LEMON_HOME the rest
        ...
        monkeypatch.setattr(
            "lemon_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_lemon_home()))
            return None
        ...
        assert resp.json()["lemon_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_lemon_home(self, isolated_profiles, monkeypatch):
        import lemon_cli.web_server as web_server

        monkeypatch.setattr(
            "lemon_cli.main_tui_launch._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = _web_server_chat._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["LEMON_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "LEMON_TUI_GATEWAY_URL" not in env
