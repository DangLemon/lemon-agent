"""Regression test: `lemon dashboard --tui` must not hard-crash.

Older Lemon AI desktop app shells (<= 0.15.x) spawn the backend as::

    lemon dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Lemon AI couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real lemon_cli.main parser in a subprocess.
