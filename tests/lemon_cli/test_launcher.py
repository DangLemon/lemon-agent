"""Tests for the top-level `./lemon` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./lemon` should use `lemon_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "lemon"
    called = []

    fake_main_module = types.ModuleType("lemon_cli.main")

    def fake_main():
        called.append("lemon_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "lemon_cli.main", fake_main_module)
