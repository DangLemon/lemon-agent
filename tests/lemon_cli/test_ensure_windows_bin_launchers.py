"""``lemon`` must survive git operations on the checkout (launcher layout).

The Windows ``lemon`` command is a wrapper around the venv console script.
Its canonical home is the managed binary dir ``LEMON_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``lemon-agent\\bin``) was swept by ``lemon update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``lemon`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``lemon update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``lemon_constants.venv_bin_dir``), so these
...
from lemon_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False, home_name: str = "lemon"):
    """Fake managed layout: LEMON_HOME/lemon-agent/venv/Scripts + launchers."""
    home = tmp_path / home_name
    root = home / "lemon-agent"
...
def test_windows_launcher_body_uses_unicode_safe_companion_across_drives():
    source = Path(r"D:\Lémon AI\lemon-agent\venv\Scripts\lemon.exe")
    target = Path(r"C:\Users\Dang\AppData\Local\Lemon AI\bin")
    body = _windows_launcher_body(source, "DangLemon/lemon-agent", target)
    ...
    assert "D:\\Lémon AI\\lemon-agent\\venv\\Scripts\\lemon.exe" in companion.decode("utf-8-sig")
...
def test_internal_desktop_marker_preserves_explicit_custom_repository(
    ...
    monkeypatch.setenv("LEMON_DESKTOP_INTERNAL", "1")
    monkeypatch.setenv("LEMON_INSTALL_REPOSITORY", "ExampleOrg/runtime-agent")
    ...
    for name in _WINDOWS_BIN_LAUNCHERS:
        body = (home / "bin" / f"{name}.cmd").read_text(encoding="ascii")
        assert 'set "LEMON_UPDATE_REPOSITORY=ExampleOrg/runtime-agent"' in body
        assert "DangLemon/lemon-agent" not in body
...
def test_matching_wrapper_removes_shadowing_exe(managed_install):
    ...
    for name in _WINDOWS_BIN_LAUNCHERS:
        ...
        (home / "bin" / f"{name}.cmd").write_text(
            "@echo off\r\n"
            "setlocal\r\n"
            'set "LEMON_UPDATE_REPOSITORY=DangLemon/lemon-agent"\r\n'
            f'{_relative_cmd_call(home / "bin", source)}\r\n'
            "exit /b %ERRORLEVEL%\r\n",
            encoding="ascii",
        )
    stale_exe = home / "bin" / "lemon.exe"
    stale_exe.write_bytes(b"legacy launcher")

    restored = ensure_windows_bin_launchers(root, windows=True, user_path_entries=[])

    assert str(home / "bin" / "lemon.cmd") in restored
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under LEMON_HOME gains nothing anywhere."""
    home = tmp_path / "lemon-home"
    home.mkdir()
    monkeypatch.setenv("LEMON_HOME", str(home))
    root = tmp_path / "src" / "lemon-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
        (scripts / f"{name}.exe").write_bytes(_fake_windows_executable())
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``lemon -p <name>`` LEMON_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("LEMON_HOME", str(home / "profiles" / "work"))
    monkeypatch.setenv("LEMON_UPDATE_REPOSITORY", "DangLemon/lemon-agent")
...
def test_truncated_console_scripts_are_not_treated_as_healthy(tmp_path, monkeypatch):
    ...
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    (scripts / "lemon.exe").write_bytes(b"MZ")
    (scripts / "lemon-acp.exe").write_bytes(b"")
    (root / "venv" / "pyvenv.cfg").write_text("home = X\n", encoding="utf-8")
    monkeypatch.setenv("LEMON_HOME", str(home))
    monkeypatch.setenv("LEMON_UPDATE_REPOSITORY", "DangLemon/lemon-agent")
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `lemon update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "lemon.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "lemon.exe").read_bytes() == b"legacy copy"
...
def test_migration_never_strips_path_when_one_console_script_is_missing(
    ...
    home, root = managed_install
    (root / "venv" / "Scripts" / "lemon-acp.exe").unlink()
    legacy_bin = str(root / "bin")
    state, read, write = _fake_registry([legacy_bin])
    ...
    assert state["entries"] == [legacy_bin]
    assert state["writes"] == 0
    assert (home / "bin" / "lemon.cmd").is_file()
    assert not (home / "bin" / "lemon-acp.cmd").exists()
...
def test_migration_never_strips_path_when_shadowing_exe_cannot_retire(
    ...
    from lemon_cli import _install_repair
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\lemon\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/LEMON/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``lemon update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/lemon.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/lemon.exe is not gitignored — lemon update's autostash "
