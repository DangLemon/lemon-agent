...
from lemon_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import lemon_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "lemon_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_logged_in_entitled_account_yields_a_state_for_every_feature(monkeypatch):
    ...
    crashing `lemon status`, `lemon tools`, and the dashboard toolsets API."""
...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    single Enter during `lemon model` overwrote it to `web.backend: nous`.
    """
...
def test_prompt_enable_tool_gateway_persists_decline(monkeypatch):
    ...
    captured = _capture_checklist(monkeypatch, selected_idx=[])
    monkeypatch.setattr(
        "lemon_cli.config.save_config", lambda cfg: saved.append(dict(cfg)), raising=False
    )
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `lemon tools`
# ---------------------------------------------------------------------------
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("lemon_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_path_check(monkeypatch):
    ...
    monkeypatch.setattr(
        "lemon_constants.agent_browser_runnable",
...
def test_has_agent_browser_import_failure_falls_back_to_lemon_managed_node_path(
    monkeypatch, tmp_path
):
    """If tools.browser_tool_install cannot be imported, the managed-Node rung must
    still find a runnable agent-browser under the Lemon AI Node dir even when
    ...
    monkeypatch.setattr(
        "lemon_constants.with_lemon_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "lemon_constants.agent_browser_runnable",
