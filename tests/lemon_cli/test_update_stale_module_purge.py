"""Tests for _purge_stale_lemon_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `lemon update`.

Field failure (2026-08-20, Teknium's Linux box): `lemon update` pulled a
checkout where lemon_cli/gateway.py newly imports `line_input` from
lemon_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from lemon_cli.gateway
...
the Lemon AI package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Lemon AI modules from the cache; later
...
def test_purge_evicts_lemon_prefixed_modules():
    victims = [
        "lemon_cli.cli_output",
        "lemon_cli.gateway",
        "gateway.status",
        "tools.ansi_strip",
        "tui_gateway.server",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_lemon_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_lemon_modules()
    assert sys.modules.get("lemon_cli.update_cmd") is update_cmd
    assert sys.modules.get("lemon_cli.main") is cli_main
    assert "lemon_cli" in sys.modules
...
def test_purge_preserves_active_update_receipt(tmp_path, monkeypatch):
    ...
    import lemon_cli.update_receipt as receipt
    ...
    try:
        ...
        cli_main._purge_stale_lemon_modules()
        post_purge_receipt = importlib.import_module("lemon_cli.update_receipt")
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["lemon_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_lemon_modules()
    finally:
        sys.modules.pop("lemon_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from lemon_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
...
def test_purge_keeps_plan_record_class_identity():
    ...
    from lemon_cli.update_inventory import RuntimeRecord as before

    cli_main._purge_stale_lemon_modules()
    from lemon_cli.update_inventory import RuntimeRecord as after
