...
board into `LEMON_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`lemon kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `lemon kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `LEMON_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("LEMON_KANBAN_BOARD")
    os.environ.pop("LEMON_KANBAN_BOARD", None)
    try:
        yield
    finally:
        if prev is None:
...
def test_pin_does_not_overwrite_existing_env(monkeypatch):
    ...
    assert main_mod.os.environ.get("LEMON_KANBAN_BOARD") == "preset"
