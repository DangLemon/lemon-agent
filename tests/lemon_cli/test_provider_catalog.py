"""Tests for the unified provider catalog (lemon_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``lemon model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from lemon_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
