...
calls ``os._exit(0)`` ~5ms after ``LEMON_BACKEND_READY``, which no in-process
...
def test_live_backend_survives_timezone_drifted_parent_marker(tmp_path):
    ...
    try:
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1",
            LEMON_PARENT_PID=str(parent.pid),
            LEMON_PARENT_START_MARKER=drifted_marker,
            LEMON_PARENT_NONCE="nonce-95693",
            LEMON_SERVE_WATCHDOG_POLL_S="0.5",
        )
        env.pop("LEMON_DESKTOP", None)
        code = (
        ...
        assert _read_until(serve, "LEMON_BACKEND_READY"), "backend never announced READY"
