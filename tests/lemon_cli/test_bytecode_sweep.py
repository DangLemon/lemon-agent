...
Bug class: the checkout's ``.py`` files change (``lemon update``, manual
...
def _make_pycache(repo: Path, subdir: str = "lemon_cli") -> Path:
...
def test_sweep_clears_pycache_when_checkout_changed(monkeypatch, tmp_path):
    ...
    lemon_main._sweep_stale_bytecode_if_checkout_changed()
...
# ---------------------------------------------------------------------------
# Plugin-update sibling site: __pycache__ under ~/.lemon-ai/plugins/<name>
# ---------------------------------------------------------------------------
