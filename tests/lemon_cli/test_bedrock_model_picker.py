...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (LEMON_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via LEMON_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in LEMON_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from lemon_cli.providers import LEMON_OVERLAYS
        assert "bedrock" in LEMON_OVERLAYS
