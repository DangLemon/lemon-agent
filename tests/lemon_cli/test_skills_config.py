"""Tests for lemon_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("lemon_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("lemon_cli.config.load_config")
    ...
    @patch("lemon_cli.config.load_config")
    @patch.dict("os.environ", {"LEMON_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """LEMON_SESSION_PLATFORM should be used when LEMON_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_lemon_platform_takes_precedence(self, tmp_path, monkeypatch):
        """LEMON_PLATFORM should win over LEMON_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from lemon_cli.skills_config import _get_categories
        skills = [
