...
install``. A locked ``lemon.exe`` makes the install exit 2, the handler
...
def test_windows_dep_failure_does_not_zip_fallback(monkeypatch):
    monkeypatch.setattr(lemon_main, "_is_windows", lambda: True)
...
def test_zip_overlay_blocked_on_modified_file(tmp_path, monkeypatch):
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(
        update_cmd.subprocess, "run", _porcelain_run(" M lemon_cli/update_cmd.py\n")
    )
...
def test_update_via_zip_aborts_before_download_when_dirty(
    ...
    monkeypatch.setattr(lemon_main, "PROJECT_ROOT", fake_root)
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
...
def test_staging_artifact_lines_are_recognized():
    ...
    assert is_artifact("?? agent.lemon-ai-update-staging/")
    assert is_artifact("?? cli.py.lemon-ai-update-staging")
    assert is_artifact("?? tools.lemon-ai-update-old/")
    ...
    assert not is_artifact(" M lemon_cli/update_cmd.py")
    assert not is_artifact("?? notes.lemon-ai-update-staging.txt")
...
def test_recheck_ignores_own_staging_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
        _porcelain_run("?? agent.lemon-ai-update-staging/\n?? cli.py.lemon-ai-update-old\n"),
    )
    assert (
