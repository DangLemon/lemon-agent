"""Tests for kanban board export / import (``lemon_cli.kanban_transfer``).
...
from lemon_cli import kanban_transfer as kt
from lemon_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("LEMON_KANBAN_HOME", str(root))
        for var in ("LEMON_KANBAN_DB", "LEMON_KANBAN_WORKSPACES_ROOT",
                    "LEMON_KANBAN_ATTACHMENTS_ROOT", "LEMON_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Lemon AI"):
