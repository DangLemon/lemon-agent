...
reports the state and prints the one-switch setup; `lemon setup` surfaces
...
import pathlib
from lemon_cli.setup_quick import _print_macos_fda_tip
...
class TestSetupFdaTip:
    ...
    def test_tip_printed_when_denied(self, monkeypatch, tmp_path):
        ...
        assert "Full Disk Access" in out
        assert "Privacy_AllFiles" in out
        assert "survives every Lemon AI update" in out
