...
from lemon_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    LEMON_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def lemon_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("lemon_cli.config") or mod == "lemon_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, lemon_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, lemon_home, capsys, monkeypatch):
