"""Tests for lemon_cli.update_receipt — Phase 1 of the fleet-update plan (#91277).
...
def receipt_home(tmp_path, monkeypatch):
    """Isolated LEMON_HOME for receipt writes."""
    home = tmp_path / ".lemon-ai"
    home.mkdir()
    monkeypatch.setattr(
        "lemon_cli.config.get_lemon_home", lambda: home, raising=False
    )
...
class TestReceiptLifecycle:
    def test_begin_record_finalize_roundtrip(self, receipt_home):
        ...
        ur.record_gateway_restart(
            restarted_services=["lemon-gateway"],
        ...
        assert gr["restarted_services"] == ["lemon-gateway"]
...
class TestCommandBoundaryFinalization:
    ...
    def test_pending_receipt_persisted_on_exit_2_refusal(self, receipt_home):
        ur.begin_update_receipt()
        ur.record_step("windows_preflight", False, "another lemon.exe running")
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        def _fake_impl(args, gateway_mode):
            ur.begin_update_receipt()
            ur.record_step("windows_preflight", False, "lemon.exe holds venv")
            sys.exit(2)

        monkeypatch.setattr(update_cmd, "_cmd_update_impl", _fake_impl)
        monkeypatch.setattr(
            lemon_main, "detect_install_method", lambda *a, **k: "git", raising=False
        )
        monkeypatch.setattr(
            lemon_main,
        ...
        monkeypatch.setattr(
            lemon_main, "_finalize_update_output", lambda state: None, raising=False
        )
        ...
        import lemon_cli.update_lock as update_lock_mod
...
class TestFleetClassification:
    def _fleet_with(self, monkeypatch, tmp_path, record, expected_sha="a" * 40):
        ...
        monkeypatch.setattr(
            "lemon_cli.profiles._get_default_lemon_home", lambda: home
        )
        monkeypatch.setattr(
            "lemon_cli.profiles._get_profiles_root",
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("lemon_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
