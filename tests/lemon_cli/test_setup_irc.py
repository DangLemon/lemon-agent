"""Tests for IRC gateway configuration via `lemon setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Lemon AI install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Lemon AI install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#lemon")
            monkeypatch.setenv("IRC_NICKNAME", "lemon-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `lemon setup gateway` experience with IRC present."""
