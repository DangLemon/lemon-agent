"""Verify `lemon -c` picks the session the user most recently used."""
...
def test_search_sessions_exposes_last_active_column(tmp_path, monkeypatch):
    ...
    db = lemon_state.SessionDB(db_path=Path(tmp_path / "state.db"))
    try:
...
def test_resolve_last_session_real_db_prefers_workspace(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("lemon_state.SessionDB", lambda: real_db(db_path=state_db))
