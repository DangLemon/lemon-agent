...
``lemon_cli.config`` while the older ``config.py`` does not export them.
Construction of ``LemonCLI`` then dies before the agent-setup mixin can
print ``partial_update_hint``. ``cmd_chat`` is the load-bearing catch:
bare ``lemon`` and ``lemon chat`` (including the fast-chat launch path)
all go through it.
"""
...
from lemon_constants import emit_partial_update_hint, partial_update_hint
...
def _missing_config_name_error(name: str = "resolve_turn_limit") -> ImportError:
    exc = ImportError(
        f"cannot import name '{name}' from 'lemon_cli.config'"
    )
    exc.name = "lemon_cli.config"
    return exc
...
def test_emit_hint_for_missing_split_model_config_default():
    ...
    buf = io.StringIO()
    assert emit_partial_update_hint(exc, file=buf) is True
    assert "lemon update" in buf.getvalue()
