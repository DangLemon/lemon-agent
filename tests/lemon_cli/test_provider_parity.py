...
the SAME provider universe as ``lemon model`` (the CLI/TUI picker).
...
def test_every_lemon_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `lemon model` universe."""
    ...
    assert not missing, (
        "providers shown in `lemon model` but not configurable in the desktop "
