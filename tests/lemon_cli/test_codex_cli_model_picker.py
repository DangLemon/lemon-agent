...
Covers:
 - Normal path (tokens already in Lemon AI auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Lemon AI
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `lemon auth openai-codex`. The old
...
@pytest.fixture()
def lemon_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Lemon AI auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(lemon_auth_only_env):
    """openai-codex appears when tokens are already in Lemon AI auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Lemon AI
    auth store."""
    lemon_home = tmp_path / ".lemon-ai"
    lemon_home.mkdir()
