...
lemon_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `lemon config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_lemon_home = get_lemon_home()`` at import time (module
    singleton), so monkeypatching LEMON_HOME after import doesn't move it.
    ...
    this only matters for tests that swap LEMON_HOME.)
    """
    import cli

    cli._lemon_home = home
