...
option absent from both sets, so every ``lemon --reasoning high <cmd> …``
invocation mis-classified ``high`` as the first positional and forced eager
plugin CLI discovery (~500-650ms startup tax).

Both sites now share ``lemon_cli._parser.top_level_value_flag_sets()``,
...
from lemon_cli.main import _first_positional_argv, _plugin_cli_discovery_needed
...
def test_first_positional_skips_reasoning_value(monkeypatch):
    monkeypatch.setattr("sys.argv", ["lemon", "--reasoning", "high", "chat", "hello"])
    assert _first_positional_argv() == "chat"
    assert _plugin_cli_discovery_needed() is False


@pytest.mark.parametrize("argv", [
    ["lemon", "--reasoning", "ultra", "--provider", "openai", "-z", "prompt here"],
    ["lemon", "-m", "x", "--reasoning=low", "chat"],
])
