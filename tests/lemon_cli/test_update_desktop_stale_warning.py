...
#88251: ``lemon update`` treated a failed desktop pack as non-fatal, printed
...
import lemon_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "lemon_constants.with_lemon_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "lemon_constants.display_lemon_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_lemon_home", lambda: tmp_path)
