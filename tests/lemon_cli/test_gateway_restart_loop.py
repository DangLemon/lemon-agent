...
Covers:
- Defense 1: gateway stop/restart refuse when _LEMON_GATEWAY=1
...
from lemon_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.lemon.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.lemon-ai/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l lemon-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.lemon.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.lemon.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.lemon.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.lemon-ai/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.lemon.gateway-apollo:/a.plist' "
            "'ai.lemon.gateway-cronus:/c.plist' 'ai.lemon.gateway-plutus:/p.plist' "
            "'ai.lemon.gateway:/Users/botuser/Library/LaunchAgents/ai.lemon.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated lemon note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.lemon.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.lemon.gateway',
        "launchctl kick'start' -k gui/501/ai.lemon.gateway",
        "launchctl kick\\start -k gui/501/ai.lemon.gateway",
        'launchctl "kickstart" -k gui/501/ai.lemon.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.lemon.gateway',
        "launchctl dis\\able gui/501/ai.lemon.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.lemon."gateway"',
        # Same class on the systemctl and lemon-CLI branches.
        'systemctl re"start" lemon-gateway',
        'lemon gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.lemon.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.lemon.update-checker',
        'systemctl re"start" lemon-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "lemon cron list",
        "lemon update",
        "lemon config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `lemon gateway start` is benign — starting a gateway from inside a
        ...
        "lemon gateway start",
        "lemon gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway lemon
        # services must not be falsely blocked (the old `.*lemon` matched any
        # lemon token).
        "launchctl unload ai.lemon.update-checker.plist",
        "launchctl restart ai.lemon.daemon",
        ...
        "systemctl restart lemon-meta.service",
        "systemctl restart lemon-cron-helper",
        ...
        "echo after the lemon gateway restarted cleanly",
        "the lemon gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "lemon skill view gateway-notes && echo lemon gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `lemon` is a path component
        # there, not a command.
        "cat '/docs/lemon gateway restart-notes.md'",
        "less /home/user/notes/lemon gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "lemon gateway restart",
        "lemon gateway restart; echo done",
        "lemon gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;lemon gateway restart",
        "true && lemon gateway stop",
        "echo $(lemon gateway restart)",
        "echo `lemon gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.lemon.gateway"])',
        'subprocess.run(["lemon", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.lemon.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["lemon", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: lemon gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `lemon -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry LEMON_HOME/LEMON_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("LEMON_PROFILE", "zeus")
        monkeypatch.delenv("LEMON_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "lemon -p zeus gateway stop",
        "lemon -p zeus gateway restart",
        "lemon --profile zeus gateway restart",
        "lemon --profile zeus gateway stop",
        "lemon --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "lemon -v -p zeus gateway restart",
        "lemon -p zeus -v gateway restart",
        "lemon --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "lemon -p venus gateway stop",
        "lemon -p venus gateway restart",
        "lemon --profile venus gateway restart",
        "lemon --profile=venus gateway stop",
        "lemon -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "lemon -p zeus gateway start",
        "lemon -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("lemon gateway restart")
        assert _contains_gateway_lifecycle_command("lemon gateway stop")

    def test_lemon_home_derived_profile(self, monkeypatch):
        # Without LEMON_PROFILE the guard falls back to the LEMON_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("LEMON_PROFILE", raising=False)
        monkeypatch.delenv("LEMON_PROFILE_NAME", raising=False)
        import lemon_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("lemon -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("lemon -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("lemon -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("lemon gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_lemon_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade lemon then run lemon gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_lemon_gateway_once.sh). The script must live under
        # LEMON_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("LEMON_HOME", str(tmp_path / ".lemon-ai"))
        scripts_dir = tmp_path / ".lemon-ai" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_LEMON_GATEWAY", raising=False)
        import lemon_cli.gateway as gw
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _LEMON_GATEWAY=1.

    Issue #37453: systemctl --user restart lemon-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart lemon-gateway",
        "systemctl --user restart lemon-gateway",
        "systemctl stop lemon-gateway.service",
        "lemon gateway restart",
        "lemon gateway uninstall",
        "launchctl kickstart gui/501/ai.lemon.gateway",
        "launchctl bootout gui/501/ai.lemon.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.lemon.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.lemon-ai/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.lemon.gateway.restart-once.plist",
        "pkill -f lemon.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart lemon-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.lemon.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-lemon label: label-independent detection is the point
        # (#62891 second reproduction used `ai.lemon.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.lemon.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _LEMON_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _LEMON_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_LEMON_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="lemon gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["lemon gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.lemon.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.lemon.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-lemon systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run lemon gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("lemon gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfelemon gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under LEMON_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("lemon gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/lemon\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 lemon gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_lemon_home()` falls back to Path.home(), which raises
        RuntimeError when neither LEMON_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart lemon-gateway' .",
        "grep -rn 'lemon gateway restart' ./logs",
        "rg 'lemon gateway restart' ../archive",
        "grep -c 'systemctl stop lemon-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM lemon_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell lemon gateway restart"',
        'sqlite3 ./db ".system systemctl restart lemon-gateway"',
        'psql ./x -c "\\! systemctl restart lemon-gateway"',
        "grep -r 'lemon gateway restart' . | sh",
        "grep -r 'lemon gateway restart' ./logs | bash",
        "grep -r 'lemon gateway restart' . | sudo sh",
        "grep -r 'x' . ; lemon gateway restart",
        "grep -r 'x' . && systemctl restart lemon-gateway",
        'grep -r "$(lemon gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart lemon-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell lemon gateway restart"',
        'sqlite3 db ".system systemctl restart lemon-gateway"',
        'psql -c "\\! systemctl restart lemon-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    lemon_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run lemon gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LEMON_HOME", str(tmp_path / ".lemon-ai"))
        (tmp_path / ".lemon-ai").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run lemon gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop lemon-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart lemon-gateway' /var/log/syslog",
        "rg 'lemon gateway restart' /home/user/.lemon-ai/logs/",
        "journalctl -u lemon-gateway --grep 'systemctl restart lemon-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'lemon_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell lemon gateway restart"',
        'psql -c "\\! systemctl restart lemon-gateway"',
        "grep 'systemctl restart lemon-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart lemon-gateway",
        'grep "$(systemctl restart lemon-gateway)" f',
        "grep 'restart' log; systemctl restart lemon-gateway",
        'sqlite3 db "SELECT 1"; lemon gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "lemon gateway restart",
        "sudo systemctl stop lemon-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart lemon-gateway%'\""
        )
