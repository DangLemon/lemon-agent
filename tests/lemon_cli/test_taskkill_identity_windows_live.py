...
- ``gateway.status.terminate_pid(force=True)`` requires a matching
  ...
  mismatched identity.
- ``lemon_cli._subprocess_compat.pid_is_lemon`` fails closed on foreign
  processes and identity mismatches.
- ``lemon_cli.update_cmd._refuse_gateway_ancestor_tree_kill`` refuses to
...
class TestPidIsLemonLive:
    def test_foreign_real_process_is_refused(self):
        """A live non-Lemon AI process (bare python sleeper in a temp-ish argv)
        must never be judged safe for taskkill."""
        from lemon_cli._subprocess_compat import pid_is_lemon

        proc = _spawn_sleeper()
        try:
            # sys.executable in CI lives under a uv/hostedtoolcache path with
            # no 'lemon' token; if the checkout path itself contains one this
            # assertion is environment-dependent, so guard for it.
            if "lemon" in sys.executable.lower():
                pytest.skip("interpreter path names lemon; probe would match")
            assert pid_is_lemon(proc.pid) is False
        finally:
            _cleanup(proc)

    def test_stale_fingerprint_is_refused_even_for_lemon_argv(self):
        ...
        try:
            ...
            assert (
                pid_is_lemon(proc.pid, expected_start_time=live + 12345) is False
            )
        finally:
            _cleanup(proc)
    ...
    def test_nonexistent_pid_is_refused(self):
        ...
        assert pid_is_lemon(2**24) is False
...
class TestAncestorRefusalLive:
    def test_real_parent_chain_is_refused(self, capsys):
        ...
        from lemon_cli.update_cmd import _refuse_gateway_ancestor_tree_kill
