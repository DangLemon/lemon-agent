"""Tests for the post-update *import* guard in ``lemon update``.
...
def test_syntax_guard_passes_but_import_guard_catches_skew(monkeypatch, tmp_path):
    ...
    # NOTE: patch update_cmd's global, not lemon_main's. Both modules expose
...
def test_import_guard_does_not_accept_forged_static_marker(monkeypatch, tmp_path):
    ...
    (tmp_path / "consumer.py").write_text(
        "import os, sys\n"
        "sys.stdout.write('__LEMON_IMPORT_HEALTH__[]')\n"
        "sys.stdout.flush()\n"
        "os._exit(7)\n"
    )
    monkeypatch.setattr(update_cmd, "_UPDATE_CRITICAL_MODULES", ("consumer",))
...
def test_hint_fires_for_first_party_import_error():
    ...
    assert any("lemon update" in line for line in hint)
...
def test_import_guard_prefers_the_project_venv_interpreter(monkeypatch, tmp_path):
    """``lemon update`` can run under a different Python than the install's.
...
def test_dependency_sync_import_failure_reinstall_guidance_uses_configured_source(
    ...
    monkeypatch.setattr(
        ...
        lambda _root: (False, "lemon_cli.main", "ImportError: boom"),
    )
    monkeypatch.setattr("lemon_cli.managed_uv.update_managed_uv", lambda: None)
    monkeypatch.setattr("lemon_cli.managed_uv.ensure_uv", lambda: "uv")
...
@pytest.mark.parametrize("modname", ["agents", "agentops", "toolsets_x", "lemonx"])
...
@pytest.mark.parametrize("modname", ["tools.todo_tool", "agent.context_compressor",
                                     "lemon_constants", "lemon_cli.config", "cli"])
...
def test_probe_and_hint_share_one_first_party_definition():
    ...
    `cli` was first-party to the hint but not the probe, and `lemonx`
    (third-party) matched the probe's loose `startswith("lemon")`. A user
    ...
    """
    from lemon_constants import FIRST_PARTY_MODULE_ROOTS, is_first_party_module
    ...
    for lookalike in ("agents", "agentops", "toolsets_x", "lemonx"):
