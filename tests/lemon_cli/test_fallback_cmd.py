"""Tests for `lemon fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate LEMON_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".lemon-ai" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from lemon_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from lemon_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("lemon_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("lemon_cli.main._require_tty"):
            from lemon_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        with patch("lemon_cli.setup._curses_prompt_choice", return_value=1):
            from lemon_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from lemon_cli.fallback_cmd import cmd_fallback_clear
...
class TestDispatcher:
    ...
    def test_unknown_subcommand_exits(self, isolated_home):
        _write_config(isolated_home, {})
        from lemon_cli.fallback_cmd import cmd_fallback
...
class TestArgparseWiring:
    """Verify `lemon fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "lemon_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
