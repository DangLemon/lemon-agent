"""Tests for the update check mechanism in lemon_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("lemon_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_rejects_cache_from_other_repository(tmp_path, monkeypatch):
    ...
    from lemon_cli import __version__, banner
    ...
    assert json.loads(cache_file.read_text(encoding="utf-8"))["repo"] == "DangLemon/lemon-agent"
...
def test_check_via_local_git_fetch_failure_returns_none(tmp_path, monkeypatch):
    ...
    def mock_git_stdout(args, *, cwd, timeout=5):
        if args[:2] == ["remote", "get-url"]:
            return "https://github.com/DangLemon/lemon-agent.git"
...
def test_check_for_updates_does_not_cache_none(tmp_path, monkeypatch):
    ...
    cache_file = tmp_path / ".update_check"
    monkeypatch.setenv("LEMON_HOME", str(tmp_path))
    monkeypatch.delenv("LEMON_REVISION", raising=False)
    ...
    monkeypatch.setattr(
        "lemon_cli.config.detect_install_method", lambda root: "git"
    )
    monkeypatch.setattr(
        "lemon_cli.config.get_project_root", lambda: repo_dir
    )
    ...
    # Simpler: just patch the get_lemon_home and the repo_dir resolution
    # by making check_for_updates find our fake repo via lemon_home fallback.
    # The code checks Path(__file__).parent.parent/.git first, then falls
    # back to lemon_home / "lemon-agent". We ensure the fallback hits.
    # To do this, we make Path(__file__).parent.parent.resolve() return
    # a path without .git, so it falls through to lemon_home / "lemon-agent".
    real_resolve = Path.resolve

    def fake_resolve(self, *args, **kwargs):
        s = str(self)
        if "banner.py" in s or s.endswith("lemon_cli"):
