"""Tests for the per-profile Projects store (lemon_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Lemon AI", folders=["/tmp/lemon"])
    ...
    assert [f.path for f in proj.folders] == ["/tmp/lemon"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' LEMON_HOME.
