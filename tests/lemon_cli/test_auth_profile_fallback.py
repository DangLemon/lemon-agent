...
When ``LEMON_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.lemon-ai/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.lemon-ai            (has its own auth.json fixture)
    * Profile     -> tmp_path/.lemon-ai/profiles/coder   (active, LEMON_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Lemon AI is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (LEMON_HOME != ~/.lemon-ai, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    lemon_home = tmp_path / "classic"
    lemon_home.mkdir()
