"""Tests for the Kanban DB layer (lemon_cli.kanban_db)."""
...
import lemon_state
import lemon_state_wal
from lemon_cli import kanban_db as kb
from lemon_cli import kanban_db_connect as kbc
from lemon_cli import kanban_db_dispatch as kbd
from lemon_cli import kanban_db_workspace as kbw


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated LEMON_HOME with an empty kanban DB."""
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import lemon_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    import lemon_cli.kanban_db as _kb
    from lemon_cli import kanban_db_dispatch as _kbd

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("LEMON_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("LEMON_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Lemon AI' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Lemon AI' DB/metadata/logs on task completion.
    """
...
# spawned with `lemon -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's LEMON_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's LEMON_HOME."""

    def _set_home(self, monkeypatch, tmp_path, lemon_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LEMON_HOME", str(lemon_home))
        monkeypatch.delenv("LEMON_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.lemon-ai/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.lemon-ai/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.lemon-ai/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # LEMON_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile LEMON_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # LEMON_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is LEMON_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["LEMON_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["LEMON_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["LEMON_KANBAN_TASK"] == "t_dispatch_env"
        assert env["LEMON_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "LEMON_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see lemon_state_wal.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import lemon_state_wal as _lemon_state_wal
    monkeypatch.setattr(
        _lemon_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _lemon_state_wal._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    lemon_state_wal._wal_fallback_warned_paths.clear()
    ...
    with _patch("lemon_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="lemon_state"):
            conn = kbc.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        lemon_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "lemon_cli.kanban_db.sqlite3.connect",
...
def test_sqlite_connect_closes_tracked_conn_on_setup_failure(tmp_path, monkeypatch):
    ...
    from lemon_cli import sqlite_safe_read
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'lemon kanban recompute' would promote it.
    """
    with kbc.connect() as conn:
        # A is done.
...
def test_add_column_if_missing_is_idempotent_on_race(kanban_home):
    ...
    from lemon_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_lemon_argv()
#
# Workers spawned by the dispatcher must use a `lemon` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["lemon", ...]`
...
def test_resolve_lemon_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m lemon_cli.main`.

    Pins the correct module name (NOT `lemon` — there is no top-level
    `lemon` package). Regression for #23198: the original PR shipped
    `python -m lemon` which fails with `No module named lemon` on every
    invocation.
    """
    import shutil
    import sys
    import lemon_cli.kanban_db as kb
    from lemon_cli import kanban_db_dispatch as kbd

    monkeypatch.delenv("LEMON_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kbd._resolve_lemon_argv()
    assert argv == [sys.executable, "-m", "lemon_cli.main"]


def test_resolve_lemon_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `lemon_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m lemon_cli.main --version`
    ...
    assert "Lemon AI" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="lemon_cli.kanban_db"):
        with kbc.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from lemon_cli.kanban_db_connect import connect
    from lemon_cli.sqlite_safe_read import file_length_matches_header
