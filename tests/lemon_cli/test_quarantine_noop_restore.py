...
``lemon*.exe`` shims aside (``lemon.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``lemon`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("lemon", "lemon-agent", "lemon-acp", "lemon-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "lemon.exe" in names, "lemon.exe must be restored after a no-op install"
    assert "lemon-acp.exe" in names
    assert "lemon-gateway.exe" in names
    assert (scripts / "lemon.exe").read_bytes() == b"MZ-old-lemon"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "lemon.exe").read_bytes() == b"MZ-new-lemon"
...
# ---------------------------------------------------------------------------
# lemon_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "lemon.exe" in names, "lemon.exe must be restored after a no-op recovery install"
    assert (scripts / "lemon.exe").read_bytes() == b"MZ-old-lemon"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "lemon.exe").write_bytes(b"MZ-new-lemon")
