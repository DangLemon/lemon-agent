"""ensure_lemon_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".lemon-ai"
    second = tmp_path / "b" / ".lemon-ai"

    monkeypatch.setenv("LEMON_HOME", str(first))
    cfg.ensure_lemon_home()

    # Profile switch: LEMON_HOME moves → the new path is ensured too.
