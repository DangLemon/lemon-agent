"""Tests for issue #26670 — concurrent lemon.exe detection and improved
quarantine retry / reboot-deferred fallback during `lemon update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_lemon_instances
# helper (and need the autouse stub in tests/lemon_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_lemon_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "lemon.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "lemon.exe"),
    ]
    ...
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_lemon_instances(scripts_dir)

    # No crash; helper completes. (Degenerate stub: launcher exe unreadable.)
    assert result == [(launcher_pid, "lemon.exe")]
...
# ---------------------------------------------------------------------------
# _quarantine_running_lemon_exe — retry, then report
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "lemon.exe"
    shim.write_bytes(b"old")

    pairs = main_install_repair._quarantine_running_lemon_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("lemon.exe.old.")
...
def test_quarantine_reports_a_lock_it_cannot_break(_winp, tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(main_install_repair, "_lemon_exe_shims", lambda d: [shim])
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "lemon_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "lemon_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_pause_and_resume_windows_gateway_service(
    ...
    import lemon_cli.update_cmd_windows as update_cmd_windows
    ...
    assert token == {
        ...
        "unmapped_pids": [],
        "unmapped": [],
        "services": ["LemonGateway"],
        "expected_services": ["LemonGateway"],
        "restarted_services": [],
        "service_profiles": {"LemonGateway": "default"},
    }
    assert stopped == ["LemonGateway"]

    cli_main._resume_windows_gateways_after_update(token)
    assert started == ["LemonGateway"]
...
def test_pause_windows_gateway_service_failure_restores_every_attempted_service(
    ...
    services = [
        SimpleNamespace(name="LemonGateway", service_pid=11, service_create_time=11.0, gateway_pid=101, gateway_create_time=101.0, descendant_identities=()),
        SimpleNamespace(name="LemonGatewayPicasso", service_pid=22, service_create_time=22.0, gateway_pid=202, gateway_create_time=202.0, descendant_identities=()),
    ]
    monkeypatch.setattr(gateway_mod, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(
    ...
    with pytest.raises(RuntimeError, match="LemonGatewayPicasso"):
        cli_main._pause_windows_gateways_for_update()

    assert restarted == ["LemonGatewayPicasso", "LemonGateway"]
...
def test_pause_windows_gateway_service_surfaces_rollback_start_failure(
    ...
    with pytest.raises(RuntimeError, match="rollback failures: LemonGateway"):
        cli_main._pause_windows_gateways_for_update()
...
def test_restore_windows_gateway_service_waits_out_stop_pending(monkeypatch):
    ...
    update_cmd._restore_windows_gateway_service("LemonGateway")

    assert restarted == ["LemonGateway"]
...
def test_resume_windows_gateway_service_failure_stays_retryable(
    ...
    with pytest.raises(RuntimeError, match="LemonGateway"):
        cli_main._resume_windows_gateways_after_update(token)

    assert token["resume_needed"] is True
    assert token["services"] == ["LemonGateway"]
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m lemon_cli.main gateway run"
...
# when every concurrent lemon.exe is a gateway runtime — the pause
...
def test_classify_concurrent_instance_recognises_gateway_runtimes(monkeypatch):
    ...
    launcher shape (python -m, lemon.exe shim, lemon-gateway.exe,
    gateway/run.py, bare `lemon gateway` which defaults to run)."""
    cases = [
        [r"C:\venv\Scripts\python.exe", "-m", "lemon_cli.main", "gateway", "run"],
        [r"C:\venv\Scripts\lemon.exe", "gateway", "run"],
        [r"C:\venv\Scripts\lemon-gateway.exe"],
        [r"C:\venv\Scripts\python.exe", "gateway/run.py"],
        ["lemon.exe", "GATEWAY", "RUN"],  # matcher is case-insensitive
        ["lemon.exe", "gateway"],  # bare `lemon gateway` defaults to run
        # profile selector before the subcommand — canonical matcher strips it
        ["lemon.exe", "--profile", "work", "gateway", "run"],
    ]
    for argv in cases:
...
def test_classify_concurrent_instance_recognises_non_gateways(monkeypatch):
    ...
    cases = [
        [r"C:\venv\Scripts\lemon.exe"],  # interactive REPL
        [r"C:\venv\Scripts\lemon.exe", "dashboard"],
        ["lemon.exe", "gateway", "status"],  # management, not runtime
        ["lemon.exe", "gateway", "stop"],
...
def test_filter_non_gateway_concurrent_instances_splits(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                100: ["lemon.exe", "gateway", "run"],
                200: ["lemon.exe"],  # REPL — keep
                300: ["lemon.exe", "dashboard"],  # keep
    ...
    assert kept == [(200, "lemon.exe"), (300, "lemon.exe"), (400, "lemon.exe")]
...
def test_filter_non_gateway_concurrent_instances_gateway_only(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                111: ["lemon.exe", "gateway", "run"],
                222: [r"C:\venv\Scripts\lemon-gateway.exe"],
            }
        ),
    )
    matches = [(111, "lemon.exe"), (222, "lemon-gateway.exe")]
...
def test_update_gate_skips_abort_when_only_concurrent_is_gateway(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_lemon_instances",
        return_value=[(1000, "lemon.exe"), (2000, "lemon-gateway.exe")],
    ), patch.object(
    ...
    captured = capsys.readouterr().out
    assert "Another lemon.exe is running" not in captured
...
def test_update_gate_still_aborts_on_non_gateway_concurrent(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_lemon_instances",
        return_value=[(1000, "lemon.exe"), (3000, "lemon.exe")],
    ), patch.object(
        cli_main,
        "_filter_non_gateway_concurrent_instances",
        return_value=[(3000, "lemon.exe")],
    ), patch.object(
        cli_main, "_run_pre_update_backup"
    ) as mock_backup:
...
def test_update_impl_refuses_before_terminating_gateway_ancestor(
    ...
    holder = (
        300,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m lemon_cli.main gateway run",
    )
    monkeypatch.setattr(
        gateway_cli,
        "_is_pid_ancestor_of_current_process",
...
def test_stop_service_refuses_pid_reuse_before_sc_stop(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="identity changed"):
        update_cmd._stop_windows_gateway_service(
            "LemonGateway", expected_service_identity=(11, 11.0)
        )
