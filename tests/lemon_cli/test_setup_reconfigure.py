...
- Bare `lemon setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `lemon setup --quick` runs the narrower "fill in missing items" flow.
- `lemon setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("lemon_cli.setup.ensure_lemon_home", {}),
        ("lemon_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("lemon_cli.config.is_managed", {"return_value": False}),
        ("lemon_cli.setup.load_config", {"return_value": {}}),
        ("lemon_cli.setup.save_config", {}),
        ("lemon_cli.setup.get_env_value", {"return_value": None}),
        ("lemon_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("lemon_cli.setup._print_setup_summary", {}),
        ("lemon_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("lemon_cli.setup.load_config", {"return_value": {}}),
        ("lemon_cli.setup.save_config", {}),
        ("lemon_cli.auth.get_active_provider", {"return_value": None}),
        ("lemon_cli.setup.get_env_value", {"return_value": None}),
        ("lemon_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `lemon setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="lemon_cli.setup.prompt_choice",
                quick="lemon_cli.setup._run_quick_setup",
                model="lemon_cli.setup.setup_model_provider",
                terminal="lemon_cli.setup.setup_terminal_backend",
                agent="lemon_cli.setup.setup_agent_settings",
                gateway="lemon_cli.setup.setup_gateway",
                tools="lemon_cli.setup.setup_tools",
            )
            from lemon_cli.setup import run_setup_wizard
...
class TestQuickFlag:
    ...
    def test_quick_flag_runs_quick_setup_only(self, existing_install):
        ...
        with ExitStack() as stack:
            ...
            from lemon_cli.setup import run_setup_wizard
            from lemon_cli import setup as setup_mod
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("lemon_cli.setup.prompt_choice", {"return_value": 0}),
                first="lemon_cli.setup_quick._run_first_time_quick_setup",
            )
            from lemon_cli.setup import run_setup_wizard
            from lemon_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("lemon_cli.setup.prompt_choice", {"return_value": 2}),
                blank="lemon_cli.setup_quick._run_blank_slate_setup",
            )
            from lemon_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "lemon_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["lemon", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
