...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            lemon_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(lemon_constants, "get_lemon_home", lambda: tmp_path)
    monkeypatch.setattr(lemon_state, "SessionDB", FakeSessionDB)
    ...
    assert "lemon sessions optimize-storage" in capsys.readouterr().out
    conn.close()
