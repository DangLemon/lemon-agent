...
import lemon_cli.doctor as doctor
import lemon_constants
from lemon_cli import config as config_mod
import lemon_cli.gateway as gateway_cli
from lemon_cli import doctor as doctor_mod
from lemon_cli.doctor_config import _has_provider_env_config
import shutil
from lemon_cli import doctor_tools
from lemon_cli import doctor_state
from lemon_cli import doctor_platform
from lemon_cli import doctor_config
...
class TestDoctorPlatformHints:
    def _mac_bundle(self, release_dir, relative, app_mtime, executable_mtime=50):
        ...
        executable_name = "Lemon AI" if app.name == "Lemon AI.app" else "Hermes"
    ...
    def test_desktop_app_bundle_prefers_newer_lemon_ai_bundle(
        self, tmp_path, monkeypatch
    ):
        fake_module = tmp_path / "lemon_cli" / "doctor_platform.py"
    ...
    @pytest.mark.macos_only
    def test_desktop_app_bundle_prefers_lemon_ai_before_newer_legacy_bundle(
        self, tmp_path, monkeypatch
    ):
        fake_module = tmp_path / "lemon_cli" / "doctor_platform.py"
        fake_module.parent.mkdir()
        ...
        monkeypatch.setattr(doctor_platform, "__file__", str(fake_module))
        release_dir = tmp_path / "apps" / "desktop" / "release"
        self._mac_bundle(release_dir, Path("mac-arm64") / "Hermes.app", 200)
        lemon = self._mac_bundle(release_dir, Path("mac-arm64") / "Lemon AI.app", 100)

        assert doctor_platform._desktop_app_bundle() == lemon

    @pytest.mark.macos_only
    def test_desktop_app_bundle_ignores_partial_bundle_without_lemon_executable(
        self, tmp_path, monkeypatch
    ):
        fake_module = tmp_path / "lemon_cli" / "doctor_platform.py"
        fake_module.parent.mkdir()
    ...
    @pytest.mark.macos_only
    def test_desktop_app_bundle_ignores_nonexecutable_lemon_binary(
        self, tmp_path, monkeypatch
    ):
        fake_module = tmp_path / "lemon_cli" / "doctor_platform.py"
        fake_module.parent.mkdir()
    ...
    def test_termux_package_hint(self, monkeypatch):
        ...
        assert lemon_constants.is_termux() is True
        assert doctor_platform._python_install_cmd() == "python -m pip install"
        assert (
            doctor_platform._system_package_install_cmd("ripgrep")
            == "pkg install ripgrep"
        )
...
class TestMacOSFullDiskAccessBranding:
    def test_internal_guidance_uses_lemon_ai_identity(self, monkeypatch, capsys):
        monkeypatch.setattr(doctor_mod.sys, "platform", "darwin")
        monkeypatch.setattr(doctor_platform, "internal_desktop_build", lambda: True)

        def denied(_path):
            raise PermissionError("denied")

        monkeypatch.setattr(doctor_platform.os, "listdir", denied)

        doctor_platform.check_macos_full_disk_access()
    ...
    def test_ordinary_guidance_keeps_lemon_identity(self, monkeypatch, capsys):
        monkeypatch.setattr(doctor_mod.sys, "platform", "darwin")
        monkeypatch.setattr(doctor_platform, "internal_desktop_build", lambda: False)
...
class TestDoctorToolAvailabilitySummary:
    def test_missing_api_key_summary_ignores_disabled_toolsets(self, monkeypatch):
        ...
        monkeypatch.setattr(
            doctor_tools, "_enabled_cli_toolsets_for_doctor", lambda: {"web"}
        )
    ...
    def test_web_capability_rows_warn_when_selected_provider_not_ready(
        self, monkeypatch
    ):
        ...
        assert any(
            "firecrawl selected; provider not configured" in detail
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `lemon doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        env_path = lemon_home / ".env"
        env_path.write_text(
            "OPENAI_API_KEY=sk-test  # em-dash here — should not crash\n",
            encoding="utf-8",
        )

        monkeypatch.setattr(doctor_mod, "LEMON_HOME", lemon_home)
...
class TestDoctorToolAvailabilityOverrides:
    def test_marks_kanban_available_only_when_missing_worker_env_gate(
        ...
        monkeypatch.setattr(
            doctor_state, "_honcho_is_configured_for_doctor", lambda: False
        )
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("LEMON_KANBAN_TASK", "probe")
...
def test_doctor_reports_vercel_backend_diagnostics(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        importlib.util, "find_spec", lambda name: object() if name == "vercel" else None
    )
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_lemon_home(self, tmp_path, provider="", memory_config=None):
        """Create a minimal LEMON_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(
        ...
        *,
        ...
        home = self._make_lemon_home(tmp_path, provider, memory_config)
        ...
        monkeypatch.setattr(doctor_mod, "LEMON_HOME", home)
        ...
        try:
            from lemon_cli import auth as _auth_mod
...
def test_run_doctor_accepts_lemon_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (lemon_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".lemon-ai"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
...
def test_run_doctor_reports_agent_browser_resolves_via_npx(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        bt_install, "_find_agent_browser", lambda **_kw: "npx agent-browser"
    )
    warm_calls = []
    monkeypatch.setattr(
        "tools.browser_tool_install.warm_agent_browser_npx_cache",
        lambda *a, **kw: warm_calls.append(1) or True,
    )
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`lemon doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp LEMON_HOME.

        ``run_doctor`` reads the module-level ``LEMON_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("LEMON_HOME")`` alone
        leaves doctor probing the REAL ~/.lemon-ai. On a dev machine with a
    ...
    def test_no_token_and_not_gh_authenticated_shows_warn(self, monkeypatch, tmp_path):
        ...
        from lemon_cli.doctor import run_doctor
        import io, contextlib
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(
        _auth_mod, "get_nous_auth_status_local", lambda: {"logged_in": True}
    )
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
    monkeypatch.setattr(
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from lemon_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("lemon_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "lemon_cli.doctor", raising=False)
        from lemon_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorXaiOAuthStatus:
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        monkeypatch.setattr(
            _auth_mod, "get_nous_auth_status_local", lambda: {"logged_in": False}
        )
        monkeypatch.setattr(
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale LEMON_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(
        ...
        if ghost is not None:
            env_lines.append(f"LEMON_MAX_ITERATIONS={ghost}\n")
        (lemon_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "LEMON_HOME", lemon_home)
        monkeypatch.setattr(doctor_mod, "get_lemon_home", lambda: lemon_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("LEMON_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("LEMON_MAX_ITERATIONS", raising=False)
        ...
        return buf.getvalue(), lemon_home

    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        out, lemon_home = self._run_config_section(
            monkeypatch,
            tmp_path,
            fix=False,
            ghost=90,
        ...
        assert "LEMON_MAX_ITERATIONS=90" in out
        assert "shadows" in out
        # Warn-only must NOT mutate .env.
        assert "LEMON_MAX_ITERATIONS=90" in (lemon_home / ".env").read_text(
            encoding="utf-8"
        )
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale LEMON_MAX_ITERATIONS" in out
        env_after = (lemon_home / ".env").read_text(encoding="utf-8")
        assert "LEMON_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_lemon_tool_progress_warning_says_unsupported_since_floor(self):
        """LEMON_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_config.collect_deprecated_env_vars({"LEMON_TOOL_PROGRESS": "true"})
        )
        assert (
            "ignored/unsupported since config floor v12"
            in findings["LEMON_TOOL_PROGRESS"]
        )
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_config.collect_deprecated_env_vars({
                "LEMON_TOOL_PROGRESS_MODE": "all"
            })
        )
        assert (
            mode["LEMON_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"
        )

    def _run_doctor_with_config(
        self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""
    ):
        lemon_home = tmp_path / ".lemon-ai"
        lemon_home.mkdir(parents=True)
        (lemon_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (lemon_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(
        ...
        findings = doctor_config.report_deprecated_config_and_env(
            ...
            {"LEMON_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: LEMON_TOOL_PROGRESS_MODE" in out
...
class TestMacOSTCCGrants:
    ...
    def test_ok_and_repair_info_uses_lemon_ai_bundle_identity(
        self, monkeypatch, capsys, tmp_path
    ):
        """Internal Lemon bundles must not tell users to reset Lemon AI TCC grants."""
        monkeypatch.setattr(doctor_mod.sys, "platform", "darwin")
        monkeypatch.setattr(
            doctor_platform,
            "_desktop_app_bundle",
        ...
        assert "tccutil reset ScreenCapture com.lemondigital.lemonai" in out
        assert "relaunch Lemon AI once" in out
        assert "com.nousresearch.lemon-ai" not in out

    def test_ok_on_certificate_anchored_dr(self, monkeypatch, capsys, tmp_path):
        """A cert-anchored DR (lemon desktop --setup-tcc-identity, or a
...
def test_run_doctor_reports_shadowed_lightpanda_engine(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "tools.browser_tool_lightpanda_fallback._using_lightpanda_engine", lambda: True
    )
    monkeypatch.setattr(
...
def test_run_doctor_reports_lightpanda_ok(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "tools.browser_tool_lightpanda_fallback.lightpanda_engine_status",
        lambda: (True, "Browser Use mode"),
    )
    monkeypatch.setattr(
        "tools.browser_lightpanda.find_lightpanda_binary", lambda: "/opt/lightpanda"
    )
    out = helper._run_doctor_and_capture(monkeypatch, tmp_path)
