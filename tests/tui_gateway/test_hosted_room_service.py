...
def test_same_thread_followup_migrates_and_delivers_committed_peer_reply(
    ...
    service.send(
        room_id="room-1",
        event_id="user-2",
        payload={"text": "@lemon continue", "thread_id": "thread-1"},
    )
    ...
    assert "User (user): @lemon continue" in prompt
...
def test_active_same_thread_followup_waits_for_current_task(tmp_path: Path):
    ...
    service.send(
        room_id="room-1",
        event_id="user-2",
        payload={"text": "@lemon follow up", "thread_id": "thread-1"},
    )
    ...
    assert service.stop(timeout=1.0)
    assert "User (user): @lemon follow up" in service.rpc.prompts[1][1]
...
def test_service_derives_room_deadline_from_agent_timeout(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("LEMON_AGENT_TIMEOUT", "90")
