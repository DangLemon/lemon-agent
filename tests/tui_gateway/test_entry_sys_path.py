...
Lemon AI's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
neutralize this before any non-stdlib import is resolved, by delegating to the
shared ``lemon_bootstrap.harden_import_path`` guard.
...
def test_entry_calls_shared_harden_guard_before_heavy_imports():
    """entry.py must call lemon_bootstrap.harden_import_path() before it
    ...
    assert harden_call_line is not None, (
        "entry.py must call lemon_bootstrap.harden_import_path()"
    )
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the Lemon AI root.  harden_import_path must relocate the
    Lemon AI root to the front so ``from utils import ...`` resolves to Lemon AI."""
    import sys

    original = sys.path[:]
    try:
        sys.path[:] = ["/home/user/tg-ws-proxy", "/opt/lemon", "/usr/lib"]
        lemon_bootstrap.harden_import_path(src_root="/opt/lemon")
        assert sys.path[0] == "/opt/lemon"
        assert sys.path.index("/opt/lemon") < sys.path.index(
