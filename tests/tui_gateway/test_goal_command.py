...
def lemon_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves LEMON_HOME.
...
@pytest.fixture()
def server(lemon_home, monkeypatch):
    ...
    # Pin config resolution to the isolated LEMON_HOME. Sibling test
    ...
    # module-level ``_lemon_home = get_lemon_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_lemon_home", lemon_home)
...
def test_moa_bare_returns_usage(server, session, lemon_home):
    _write_moa_config(lemon_home, """
moa:
