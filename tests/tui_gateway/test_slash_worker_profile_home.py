...
def test_slash_worker_accepts_profile_home():
    ...
    # lemon_state evaluates get_lemon_home() / "state.db" at import time, so
    # the mock must return a Path (a bare str raises TypeError under per-file
    # subprocess isolation).
    with patch.dict("sys.modules", {
        "lemon_constants": MagicMock(
            get_lemon_home=MagicMock(return_value=Path("/tmp/lemon_test")),
        ),
    }):
        with patch("subprocess.Popen") as mock_popen:
            ...
            worker = _SlashWorker(
                ...
                model="test-model",
                profile_home="/home/luke/.lemon-ai/profiles/work"
            )

            # Verify Popen was called
            assert mock_popen.called

            # Check that LEMON_HOME was set in the environment
            call_kwargs = mock_popen.call_args[1]
            assert "env" in call_kwargs
            assert call_kwargs["env"]["LEMON_HOME"] == "/home/luke/.lemon-ai/profiles/work"
