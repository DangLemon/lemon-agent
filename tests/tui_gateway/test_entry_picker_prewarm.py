...
guard is covered in ``tests/lemon_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``lemon_cli.model_switch_providers.prewarm_picker_cache_async``
...
import lemon_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from lemon_cli.model_switch,
