...
def server():
    ...
    # e.g. lemon_cli.active_sessions would bind the mocked get_lemon_home
    ...
    with patch.dict("sys.modules", {
        "lemon_constants": MagicMock(get_lemon_home=MagicMock(return_value="/tmp/lemon_test")),
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `LEMON_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("LEMON_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("LEMON_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/lemon-agent-dev": {"name": "lemon-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "lemon-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient LEMON_HOME
    ...
    # The gateway's own LEMON_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `lemon config set` does)
    ...
    import lemon_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_lemon_home", lambda: tmp_path)
    ...
    # Activate midnight, as `lemon config set display.skin midnight` would.
