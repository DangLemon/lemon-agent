...
def isolated_home(monkeypatch):
    """Temp LEMON_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="lemon_test_31179_")
    lemon_home = os.path.join(test_home, ".lemon-ai")
...
def _fresh_modules():
    """Drop cached lemon modules so each test reloads against current env."""
    _drop_reload_targets()
