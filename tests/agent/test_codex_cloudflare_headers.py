...
The official Codex endpoint must receive Lemon AI' own harness identity, rather
...
These tests pin:
- the required Lemon AI originator
- the versioned Lemon AI User-Agent
...
class TestCodexCloudflareHeaders:
    ...
    def test_user_agent_advertises_lemon_version(self):
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("agent.process_bootstrap.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "lemon-agent"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"LemonAgent/{__version__}"
