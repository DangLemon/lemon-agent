...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated LEMON_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="lemon_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".lemon-ai"))
    prev_home = os.environ.get("LEMON_HOME")
    os.environ["LEMON_HOME"] = os.path.join(test_home, ".lemon-ai")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("lemon_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into lemon_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["LEMON_HOME"] = prev_home
