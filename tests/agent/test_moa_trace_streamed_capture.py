...
path via ``lemon chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp LEMON_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point LEMON_HOME at a temp dir and turn moa.save_traces on."""
    lemon_home = tmp_path / ".lemon-ai"
    lemon_home.mkdir()
    monkeypatch.setenv("LEMON_HOME", str(lemon_home))

    # save_moa_turn reads config via lemon_cli.config.load_config; stub it to
