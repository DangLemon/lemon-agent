...
class TestKimiEffortVocabulary:
    def test_k3_maps_full_lemon_ladder(self):
        expected = {
            "minimal": "low",
            "low": "low",
            "medium": "high",
        ...
        for lemon_level, wire_level in expected.items():
            kw = _kimi_kwargs(
                "kimi-k3", {"enabled": True, "effort": lemon_level}
            )
            assert kw["reasoning_effort"] == wire_level, lemon_level
