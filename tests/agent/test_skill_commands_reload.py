...
def lemon_home(monkeypatch):
    """Isolate LEMON_HOME for ``reload_skills`` tests.

    Rather than popping cache-bearing modules from ``sys.modules``,
    we monkeypatch the module-level ``LEMON_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="lemon-reload-skills-")
    monkeypatch.setenv("LEMON_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "LEMON_HOME", home, raising=False)
...
class TestReloadSkillsHelper:
    ...
    def test_detects_removed_skill_carries_description(self, lemon_home):
        from agent.skill_commands import reload_skills

        skill_dir = _write_skill(lemon_home / "skills", "demo", "soon to be gone")
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, lemon_home):
