...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate LEMON_HOME under tmp_path."""
    monkeypatch.setenv("LEMON_HOME", str(tmp_path / ".lemon-ai"))
    (tmp_path / ".lemon-ai").mkdir()

    # Force the constants/image cache helpers to re-read LEMON_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("lemon_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_lemon_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under LEMON_HOME — gateway cleanup
