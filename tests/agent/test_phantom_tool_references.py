...
Covers:
  * LEMON_AGENT_HELP_GUIDANCE degrades to the docs-only variant when the
...
class TestLemonAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        from agent.prompt_builder import LEMON_AGENT_HELP_GUIDANCE
        assert "skill_view(name='lemon-agent')" in LEMON_AGENT_HELP_GUIDANCE
...
class TestEssentialSkillsUndisableable:
    def test_agent_side_reader_strips_essential(self, monkeypatch, tmp_path):
        ...
        assert "lemon-agent" not in disabled
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the lemon-agent skill."""
        ...
        for cat, name in [
            ("autonomous-ai-agents", "lemon-agent"),
            ("media", "gif-search"),
        ]:
            d = bundled / cat / name
        ...
        monkeypatch.setattr(ss, "_lemon_home", lambda: home)
        ...
        assert result["copied"] == ["lemon-agent"]
        assert (home / "skills" / "autonomous-ai-agents" / "lemon-agent" / "SKILL.md").exists()
