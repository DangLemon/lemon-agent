...
#
# Installs a real, earlier Lemon AI the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `lemon`.
#
...
# Keep sandbox state out of the default .lemon-ai-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".lemon-ai-sandbox-e2e-$ROUTE"
export LEMON_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/lemon/.lemon-ai/lemon-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/lemon-agent.git"
...
LOG_DIR="${LEMON_E2E_LOG_DIR:-$(mktemp -d -t lemon-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED lemon accept FLAG on `lemon update`?
#
...
# that silently rots. `lemon update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "lemon update --help 2>&1" | grep -qF -- "$flag"
}
...
require_lemon_works() {
  local when="$1" out
  out="$(in_sandbox "lemon --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "lemon --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "lemon runs $when"
}

# ── install the earlier Lemon AI ─────────────────────────────────────────────
...
require_lemon_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: lemon update'
    ...
    # lemon which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="lemon update --yes"
    else
      update_cmd="lemon update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "lemon update failed ($update_cmd)"
    fi
    require_landed_on_target 'lemon update'
    require_lemon_works 'after lemon update'
    ;;
  installer)
    ...
    require_lemon_works 'after installer re-run'
    ;;
esac
