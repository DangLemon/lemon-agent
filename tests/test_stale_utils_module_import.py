...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.lemon-ai/lemon-agent/utils.py)
...
2. ``lemon update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``lemon_cli/main.py`` (the ``lemon update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``lemon_constants`` after the
