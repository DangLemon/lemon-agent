"""Windows installer regression for Lemon AI children outside the venv.

The venv sweep deliberately selects process roots by executable path so it
does not kill unrelated Python processes. A selected Lemon AI process can spawn
...
def test_venv_sweep_stops_managed_runtime_children_but_not_unrelated_processes(
    ...
    install_dir = lemon_home / "lemon-agent"
    venv_scripts = install_dir / "venv" / "Scripts"
    runtime_dir = install_dir / ".lemon-ai-runtime" / "python" / "generation-test"
    ...
    # Keep the test away from real gateway tasks and real Lemon AI launchers
    ...
    wrapper.write_text(
        ...
        f"-InstallDir '{install_dir}' -LemonHome '{lemon_home}'\n"
        "exit $LASTEXITCODE\n",
        encoding="utf-8",
    )
