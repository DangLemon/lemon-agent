...
Uses a throwaway LEMON_HOME so it never touches ~/.lemon-ai. No real names.
"""
...
def _fresh_home():
    """Point LEMON_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_lemon_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["LEMON_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER LEMON_HOME is set.
...
# a different get_lemon_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"LEMON_HOME (throwaway): {HOME}")
