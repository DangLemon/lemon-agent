"""Shared fixtures for the lemon-agent test suite.
...
2. **Isolated LEMON_HOME.** LEMON_HOME points to a per-test tempdir so
   code reading ``~/.lemon-ai/*`` via ``get_lemon_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".lemon-ai"`` instead of the canonical
   ``get_lemon_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No LEMON_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox LEMON_HOME before ANY test module is imported ──────────────────
# `lemon_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_lemon_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.lemon-ai/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes LEMON_HOME, but fixtures run
...
# the REAL Lemon AI root — capture it BEFORE the sandbox rewires LEMON_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.lemon-ai (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("LEMON_KANBAN_HOME", "").strip()
_PRE_SANDBOX_LEMON_HOME = os.environ.get("LEMON_HOME", "")


def _lemon_home_points_at_production(value: str) -> bool:
    """True when a pre-set LEMON_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    LEMON_HOME=~/.lemon-ai``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``lemon_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.lemon-ai`` — the escape vector that landed pytest
    ...
    LEMON_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".lemon-ai").resolve()
    except Exception:
        return True
...
if _lemon_home_points_at_production(os.environ.get("LEMON_HOME", "")):
    _SESSION_LEMON_HOME = tempfile.mkdtemp(prefix="lemon-test-home-")
    os.environ["LEMON_HOME"] = _SESSION_LEMON_HOME
    atexit.register(shutil.rmtree, _SESSION_LEMON_HOME, True)
...
# like a real CLI") — which used to disarm lemon_state's live-DB guard in
# the child at the same moment the child lost the LEMON_HOME redirect.
# LEMON_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# lemon_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export LEMON_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["LEMON_TEST_ISOLATION"] = os.environ.get("LEMON_HOME", "") or "1"

#: LEMON_HOME as it stood when conftest was imported - i.e. before any test
...
LEMON_HOME_AT_CONFTEST_IMPORT = os.environ.get("LEMON_HOME", "")
...
# LEMON_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_LEMON_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``lemon_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Lemon AI agent session inherits them and
    # lemon_constants home-resolution helpers prefer them over monkeypatched
    ...
    "LEMON_MAX_ITERATIONS",
    "LEMON_SESSION_PLATFORM",
    "LEMON_SESSION_CHAT_ID",
    "LEMON_SESSION_CHAT_NAME",
    "LEMON_SESSION_CHAT_TYPE",
    "LEMON_SESSION_THREAD_ID",
    "LEMON_SESSION_SOURCE",
    "LEMON_SESSION_KEY",
    "LEMON_GATEWAY_SESSION",
    "LEMON_CRON_SESSION",
    "_LEMON_GATEWAY",
    "LEMON_PLATFORM",
    "LEMON_MODEL",
    "LEMON_INFERENCE_MODEL",
    "LEMON_INFERENCE_PROVIDER",
    "LEMON_TUI_PROVIDER",
    "LEMON_MANAGED",
    "LEMON_MANAGED_DIR",
    "LEMON_DEV",
    "LEMON_CONTAINER",
    "LEMON_EPHEMERAL_SYSTEM_PROMPT",
    "LEMON_TIMEZONE",
    "LEMON_REDACT_SECRETS",
    "LEMON_BACKGROUND_NOTIFICATIONS",
    "LEMON_EXEC_ASK",
    "LEMON_HOME_MODE",
    "LEMON_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.lemon-ai/kanban.db instead of the per-test LEMON_HOME.
    "LEMON_KANBAN_DB",
    "LEMON_KANBAN_BOARD",
    "LEMON_KANBAN_HOME",
    "LEMON_KANBAN_WORKSPACES_ROOT",
    "LEMON_KANBAN_LOGS_ROOT",
    "LEMON_KANBAN_TASK",
    "LEMON_KANBAN_WORKSPACE",
    "LEMON_KANBAN_RUN_ID",
    "LEMON_KANBAN_CLAIM_LOCK",
    "LEMON_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "lemon" host.
    "LEMON_HONCHO_HOST",
    ...
    "LEMON_DASHBOARD_OAUTH_CLIENT_ID",
    "LEMON_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and LEMON_HOME to per-test tempdirs so code that
    reads ``~/.lemon-ai/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral LEMON_* vars that could change test semantics.
    for name in _LEMON_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("LEMON_HONCHO_HOST", "lemon")

    # 3. Redirect LEMON_HOME to a per-test tempdir. Code that reads
    #    ``~/.lemon-ai/*`` via ``get_lemon_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.lemon-ai/*`` via
    #    ``Path.home() / ".lemon-ai"`` instead of ``get_lemon_home()``
    #    is a bug to fix at the callsite.
    fake_lemon_home = tmp_path / "lemon_test"
    fake_lemon_home.mkdir()
    (fake_lemon_home / "sessions").mkdir()
    (fake_lemon_home / "cron").mkdir()
    (fake_lemon_home / "memories").mkdir()
    (fake_lemon_home / "skills").mkdir()
    monkeypatch.setenv("LEMON_HOME", str(fake_lemon_home))
    ...
    # lemon_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("LEMON_TEST_ISOLATION", str(fake_lemon_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("LEMON_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. lemon_state computes ``DEFAULT_DB_PATH = get_lemon_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from lemon_state import ...``) that
    ...
    lemon_state_mod = sys.modules.get("lemon_state")
    if lemon_state_mod is not None and hasattr(lemon_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            lemon_state_mod, "DEFAULT_DB_PATH", fake_lemon_home / "state.db"
        )
    ...
    monkeypatch.setenv("LEMON_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.lemon-ai/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import lemon_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path LEMON_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_lemon_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from lemon_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.lemon-ai. This autouse
...
# allow-list because test-level fixtures legitimately move LEMON_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session LEMON_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``LEMON_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Lemon AI root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_LEMON_HOME and not _lemon_home_points_at_production(
        _PRE_SANDBOX_LEMON_HOME
    ):
        # LEMON_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_lemon_root().resolve()
    # No pre-existing LEMON_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".lemon-ai").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.lemon-ai`` captured at import time. Hermetic tests that legitimately
    move LEMON_HOME to sibling tempdirs are unaffected.

    Only patches when ``lemon_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("lemon_cli.kanban_db")
    _kdbc = sys.modules.get("lemon_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import lemon_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.lemon-ai. See #69283."
        )
...
# ``lemon_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Lemon AI root. This fixture wires the
# test-side knobs:
...
#     installs where LEMON_HOME is not ~/.lemon-ai) into the guard's
...
# so subprocess children that import lemon_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("lemon_state")
    ...
    if _PRE_SANDBOX_LEMON_HOME and not _lemon_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_LEMON_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Lemon AI home override redirects every later
    # ``get_lemon_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from lemon_constants import get_lemon_home_override, set_lemon_home_override

        if get_lemon_home_override() is not None:
            set_lemon_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal lemon config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``lemon-gateway`` process
...
# correlated with a ``tests/lemon_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> lemon-gateway``
...
def _wal_is_usable() -> bool:
    """True when Lemon AI will actually put a database into WAL mode here.

    Lemon AI refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Lemon AI can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Lemon AI managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``lemon_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_lemon_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_lemon_home`` fixture
    redirects ``LEMON_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.lemon-ai/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``lemon_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["LEMON_VOICE_TTS"] = "1"``. The
...
#      ``lemon_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``lemon_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``lemon_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``lemon_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Lemon AI runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.lemon-ai``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Lemon AI falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Lemon AI uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart lemon-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting lemon/python patterns are also blocked.
    """
    ...
    def _matches_lemon_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _LEMON_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_lemon_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch lemon-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m lemon_cli.main".
                if (
                    "lemon" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live lemon-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting lemon/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `lemon update` (or the
        # equivalent `python -m lemon_cli.main update`).  These commands
        ...
        if "update" in low and (
            # lemon update / lemon update --gateway / setsid bash -c ... lemon update
            ("lemon" in low and "update" in low.split())
            or
            # python -m lemon_cli.main update --gateway
            ("lemon_cli" in low and "update" in low.split())
            or
            # venv/bin/lemon update  (absolute path variant used in tests)
            (".venv/bin/lemon" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`lemon update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_LEMON_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import lemon_cli.voice as _voice
    except Exception:
