...
(SQLITE_PROTOCOL — typical on NFS/SMB), Lemon AI must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``LEMON_HOME`` silently lose
...
from lemon_state import SessionDB, format_session_db_unavailable, get_last_init_error
from lemon_state_wal import WalUnsupportedError, apply_wal_with_fallback
...
def _assume_fixed_sqlite(monkeypatch):
    ...
    lemon_state_wal._wal_reset_bug_warned_paths.clear()
    yield
    lemon_state_wal._wal_reset_bug_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_error_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see lemon_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("lemon_state.sqlite3.connect", side_effect=gated_connect):
            with pytest.raises(sqlite3.OperationalError):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_adds_nfs_hint_for_locking_protocol(self):
        ...
        lemon_state._set_last_init_error("OperationalError: locking protocol")
