...
from lemon_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(lemon_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <lemon_home>/plugins/<name> and opt it in."""
    plugin_dir = lemon_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LEMON_HOME", str(tmp_path / "lemon_empty"))
