"""Behavioral regression for Lemon AI-managed Python provenance on Windows."""
...
def test_venv_stage_rejects_third_party_python_and_uses_managed_path(
    ...
    install_dir = tmp_path / "install"
    managed_root = install_dir / ".lemon-ai-runtime" / "python"
    ...
    run = _run_venv_stage(powershell, tmp_path, lemon_home, install_dir, env)
...
def test_fallback_minor_is_reported_from_resolved_managed_interpreter(
    ...
    install_dir = tmp_path / "install"
    managed_python = (
        install_dir / ".lemon-ai-runtime" / "python" / "cpython-3.12" / "python.exe"
    )
    uv = lemon_home / "bin" / "uv.exe"
    uv.parent.mkdir(parents=True)
    managed_python.parent.mkdir(parents=True)
...
def test_python_find_drains_large_stderr_without_deadlock(tmp_path: Path) -> None:
    ...
    install_dir = tmp_path / "install"
    managed_python = (
        install_dir / ".lemon-ai-runtime" / "python" / "cpython-3.11" / "python.exe"
    )
    uv = lemon_home / "bin" / "uv.exe"
    uv.parent.mkdir(parents=True)
    managed_python.parent.mkdir(parents=True)
