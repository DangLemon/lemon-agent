...
def test_required_frontmatter_fields(p):
    ...
    lemon = (fm.get("metadata") or {}).get("lemon") or {}
    if not (lemon.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.lemon.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    lemon = (fm.get("metadata") or {}).get("lemon") or {}
    dangling = [
        rs for rs in (lemon.get("related_skills") or []) if rs not in _all_names()
    ]
