...
def test_frontmatter_required_fields():
    ...
    lemon = fm["metadata"]["lemon"]
    assert lemon["tags"]
    assert "related_skills" in lemon
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Lemon AI"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["lemon"]["related_skills"]:
        hits = (
