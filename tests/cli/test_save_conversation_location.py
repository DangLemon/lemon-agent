...
Regression: the old implementation wrote ``lemon_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``lemon sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.lemon-ai/sessions/saved/`` and prints
...
def lemon_home(tmp_path, monkeypatch):
    ...
    # Clear any cached lemon_home computation
    import lemon_constants
    if hasattr(lemon_constants, "_lemon_home_cache"):
        lemon_constants._lemon_home_cache = None
    return home
...
def test_save_conversation_writes_under_lemon_home(lemon_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.lemon-ai/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the LEMON_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "lemon_constants"]:
    ...
    cli.LemonCLI.save_conversation(stub, "/save json")

    # File must NOT be in CWD
    cwd_leak = list(work.glob("lemon_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.lemon-ai/sessions/saved/
    saved_dir = lemon_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("lemon_conversation_*.json"))
    ...
    assert "lemon --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(lemon_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "lemon_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(lemon_home, capsys):
    ...
    cli.LemonCLI.save_conversation(stub, "/save")
...
def test_save_conversation_bad_format_shows_usage(lemon_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "lemon_constants"]:
        sys.modules.pop(mod, None)
    import cli

    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.LemonCLI.save_conversation(stub, "/save pdf")
