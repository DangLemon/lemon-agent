...
Drives ``LemonCLI.process_command("/reset now")`` against a minimal stand-in
and verifies:
...
def _make_cli_stub():
    """Build a minimal LemonCLI-shaped object that can run ``process_command``
    ...
    self_._confirm_destructive_slash = LemonCLI._confirm_destructive_slash.__get__(
        self_, type(self_)
    )
    self_._slash_handler = LemonCLI._slash_handler
    self_._cmd_new = LemonCLI._cmd_new.__get__(self_, type(self_))
    self_.process_command = LemonCLI.process_command.__get__(self_, type(self_))
...
def test_new_without_skip_token_still_consults_modal():
    ...
    self_._split_destructive_skip = LemonCLI._split_destructive_skip
    self_._normalize_slash_confirm_choice = LemonCLI._normalize_slash_confirm_choice.__get__(
        self_, type(self_)
    )
    self_._confirm_destructive_slash = LemonCLI._confirm_destructive_slash.__get__(
        self_, type(self_)
