...
Run from the PR worktree:
    ...
    cd ~/.lemon-ai/lemon-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.lemon-ai/lemon-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.lemon-ai/lemon-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a lemon-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a lemon-agent checkout"
)
...
# Isolated LEMON_HOME for the config write.
