...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure LEMON_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", lemon_home=str(tmp_path / ".lemon-ai"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LEMON_HOME", str(lemon_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", lemon_home=str(lemon_home))
        return p
