"""Regression tests for #76414: `lemon honcho peers` showed "(not set)"
for every non-default profile.

_all_profile_host_configs() built the per-profile host key inline as
f"{HOST}.{profile}" ("lemon.work") while every other reader/writer —
profile_host_key(), resolve_active_host(), honcho status/enable/sync and
the runtime plugin — uses the underscore form ("lemon_work"). The lookup
...
a real honcho.json (temp LEMON_HOME, no network).
"""
import io
import json
import sys
...
def honcho_home(tmp_path, monkeypatch):
    cfg = {
        "peerName": "alice",
        "hosts": {
            "lemon": {"peerName": "alice", "aiPeer": "lemon"},
            "lemon_work": {"peerName": "alice", "aiPeer": "lemon"},
            "lemon_my_profile": {"peerName": "bob", "aiPeer": "lemon"},
        },
    }
...
class TestAllProfileHostConfigs:
    def test_profile_host_keys_match_writer_form(self, honcho_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "lemon_cli.profiles.list_profiles",
        ...
        assert host == "lemon_work"  # not "lemon.work"
    ...
    def test_sanitized_profile_names_resolve(self, honcho_home, monkeypatch):
        ...
        resolve — profile_host_key maps 'my.profile' -> 'lemon_my_profile';
    ...
    def test_legacy_dot_form_host_key_still_readable(self, honcho_home, monkeypatch):
        ...
        ("lemon.work") must keep working — the README promises those keys
        ...
        del cfg["hosts"]["lemon_work"]
        cfg["hosts"]["lemon.work"] = {"peerName": "carol", "aiPeer": "lemon"}
        path.write_text(json.dumps(cfg))
        monkeypatch.setattr(
            "lemon_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
...
class TestCmdPeers:
    def test_peers_shows_populated_identity_not_host_key_leak(
        ...
        raw malformed key 'lemon.work' (or '(not set)')."""
        monkeypatch.setattr(
            "lemon_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
        )
        out = _peers_output(SimpleNamespace())
        assert "lemon.work" not in out
        ...
        assert "alice" in work_line and "lemon" in work_line
    ...
    def test_peers_falls_back_cleanly_when_block_missing(
        ...
        out = _peers_output(SimpleNamespace())
        assert "lemon.new" not in out  # well-formed key, no dot-form leak
