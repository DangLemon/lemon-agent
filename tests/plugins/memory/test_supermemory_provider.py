...
def provider(monkeypatch, tmp_path):
    ...
    p.initialize("session-1", lemon_home=str(tmp_path), platform="cli")
    return p
...
def test_format_prefetch_context_deduplicates_overlap():
    result = _format_prefetch_context(
        ...
        dynamic_facts=["Jordan prefers short answers", "Uses Lemon AI"],
        search_results=[{"memory": "Uses Lemon AI", "similarity": 0.9}],
        max_results=10,
    )
    assert result.count("Jordan prefers short answers") == 1
    assert result.count("Uses Lemon AI") == 1
...
def test_prefetch_includes_profile_on_first_turn(provider):
    provider._client.profile_response = {
        ...
        "search_results": [{"memory": "Working on Lemon AI memory provider", "similarity": 0.88}],
    }
...
def test_merge_metadata_stamps_sm_source():
    # sm_source routes Lemon AI writes into the "Lemon AI" Space in the Supermemory
    ...
    assert merged2["sm_source"] == "lemon"
...
def test_identity_template_resolved_in_container_tag(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SUPERMEMORY_API_KEY", "test-key")
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    _save_supermemory_config({"container_tag": "lemon-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", lemon_home=str(tmp_path), platform="cli", agent_identity="coder")
    assert p._container_tag == "lemon_coder"
...
def test_container_tag_env_var_override(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    p = SupermemoryMemoryProvider()
    p.initialize("s1", lemon_home=str(tmp_path), platform="cli")
...
def test_probe_supermemory_connection_missing_key(tmp_path):
    ...
    assert status["container_tag"] == "lemon"
...
def test_post_setup_writes_config_and_prints_summary(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("lemon_cli.config.save_config", fake_save_config)
