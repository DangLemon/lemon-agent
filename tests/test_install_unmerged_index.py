...
The ``lemon update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m lemon_cli.main gateway run`` — image name
    ``pythonw``, not ``lemon.exe`` — so the ``taskkill /IM lemon.exe`` guard
    ...
    # The lemon.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM lemon.exe' in text
