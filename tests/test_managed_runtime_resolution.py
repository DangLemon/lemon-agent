"""Guard: Lemon AI-owned subprocesses must not resolve managed runtimes by bare PATH.

Lemon AI installs runtimes for itself — ``uv`` at ``$LEMON_HOME/bin/uv``, Node at
``$LEMON_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Lemon AI's own
...
* when a system copy also exists, the one Lemon AI does not own wins — which is
...
``iter_lemon_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Lemon AI provisions into LEMON_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Lemon AI-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("lemon_cli/update_cmd_deps.py", "uv"): (
    ...
    ("lemon_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (lemon_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Lemon AI-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$LEMON_HOME/bin (uv) and $LEMON_HOME/node are not on an "
        ...
        "  node/npm -> lemon_constants.find_node_executable()\n"
        "  PATH env -> lemon_constants.iter_lemon_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(lemon_constants, helper))


def test_managed_uv_helpers_exist():
    from lemon_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
