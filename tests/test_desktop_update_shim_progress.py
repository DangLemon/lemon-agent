...
def progress(tmp_path):
    ...
    status = tmp_path / "lemon-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Lemon AI to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `lemon update`, and reports the stage that was on screen
...
n="$(cat "$LEMON_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$LEMON_TEST_CALLS"
for f in "$TMPDIR"/lemon-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$LEMON_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$LEMON_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "lemon-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    lemon = install_root / "venv" / "bin" / "lemon"
    lemon.write_text(FAKE_LEMON)
    lemon.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "LEMON_TEST_CAPTURE": str(capture),
        "LEMON_TEST_CALLS": str(calls),
        "LEMON_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".lemon-ai-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`lemon update` is the longest wait in the hand-off and the one the
