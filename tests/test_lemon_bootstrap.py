"""Tests for lemon_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Lemon AI entry point
(lemon, lemon-agent, lemon-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Lemon AI entry point imports lemon_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported lemon_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("lemon_bootstrap", None)
    import lemon_bootstrap  # noqa: WPS433
    return lemon_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Lemon AI CLI
...
class TestEntryPointsImportBootstrap:
    """Every Lemon AI entry point must import lemon_bootstrap as its
    ...
    # Entry points that invoke Lemon AI as a process.  Each one must
    # import lemon_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "lemon_cli/main.py",   # lemon CLI (console_script)
        "run_agent.py",          # lemon-agent (console_script)
        "acp_adapter/entry.py",  # lemon-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import lemon_bootstrap' and that
        ...
        half-finished ``lemon update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``lemon_bootstrap`` as a top-level module) leaves lemon
        ...
        """
        # Resolve relative to the lemon-agent repo root.  Tests live
        # at tests/test_lemon_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # lemon start even when lemon_bootstrap hasn't been
        ...
        assert first_import_name == "lemon_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'lemon_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import lemon_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Lemon AI's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("LEMON_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["LEMON_PYTHON_SRC_ROOT"] = env
            elif "LEMON_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["LEMON_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/lemon")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("LEMON_PYTHON_SRC_ROOT", None)
            else:
                os.environ["LEMON_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/lemon", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Lemon AI root.  The guard must relocate Lemon AI to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/lemon"])
        assert result[0] == "/opt/lemon"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Lemon AI root.
        assert result.index("/opt/lemon") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["LEMON_PYTHON_SRC_ROOT"] = "/env/lemon"
            hb.harden_import_path()
            assert sys.path[0] == "/env/lemon"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("LEMON_PYTHON_SRC_ROOT", None)
