...
The defect: lemon_state already knows macOS `fsync()` does not guarantee
...
import lemon_state
from lemon_state import repair_state_db_schema
from lemon_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in lemon_state_repair; lemon_state only
    # re-imports them.
    import lemon_state_repair

    source = Path(lemon_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(lemon_state_repair.__file__))
