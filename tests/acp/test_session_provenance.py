...
def test_root_session_no_compression(db):
    ...
    assert prov["currentLemonSessionId"] == "root1"
    assert prov["rootLemonSessionId"] == "root1"
    assert prov["parentLemonSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_lemon_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentLemonSessionId"] == "old"
    assert prov["rootLemonSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousLemonSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootLemonSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"lemon"}
    assert "sessionProvenance" in meta["lemon"]
    assert meta["lemon"]["sessionProvenance"]["currentLemonSessionId"] == "root1"
