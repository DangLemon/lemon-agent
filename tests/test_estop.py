"""Global emergency stop (`lemon pause` / `lemon resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`lemon resume`)
...
def lemon_home(tmp_path, monkeypatch):
    """Point LEMON_HOME at a temp dir and reset estop module log state."""
...
def test_engage_creates_sentinel_and_is_engaged(lemon_home):
    assert estop.is_engaged() is False
    estop.engage()
    assert (lemon_home / "ESTOP").exists()
    assert estop.is_engaged() is True


def test_disengage_removes_sentinel(lemon_home):
    estop.engage()
    assert estop.disengage() is True
    assert not (lemon_home / "ESTOP").exists()
    assert estop.is_engaged() is False
    # Disengaging when not engaged is a no-op that reports False.
    assert estop.disengage() is False


def test_reason_and_timestamp_stored(lemon_home):
    ...
    raw = json.loads((lemon_home / "ESTOP").read_text(encoding="utf-8"))
    assert raw["reason"] == "runaway cron fan-out"


def test_get_state_none_when_disengaged(lemon_home):
    assert estop.get_state() is None


def test_corrupt_sentinel_still_engages(lemon_home):
    """A hand-touched/corrupt ESTOP file must still pause (fail safe)."""
    (lemon_home / "ESTOP").write_text("not json", encoding="utf-8")
    assert estop.is_engaged() is True
    state = estop.get_state()
    assert state is not None
...
def test_paused_reply_none_when_disengaged(lemon_home):
    assert estop.paused_reply() is None


def test_paused_reply_surfaces_reason_and_resume_hint(lemon_home):
    ...
    assert "lemon resume" in notice


def test_paused_reply_without_reason(lemon_home):
    estop.engage()
    notice = estop.paused_reply()
    assert notice is not None
    assert "paused" in notice.lower()
...
def test_check_paused_logs_once_per_engagement(lemon_home, caplog):
...
def test_cron_tick_skips_dispatch_when_engaged(lemon_home, monkeypatch):
...
def test_cron_tick_resumes_after_disengage(lemon_home, monkeypatch):
    from cron import scheduler
...
def test_kanban_dispatch_blocked_when_engaged(lemon_home):
...
@pytest.mark.asyncio
async def test_gateway_new_turn_gets_paused_reply(lemon_home):
...
@pytest.mark.asyncio
async def test_gateway_internal_events_bypass_estop(lemon_home):
...
# ── CLI: lemon pause / lemon resume ───────────────────────────────────────


def test_cli_pause_engages_with_reason(lemon_home, capsys):
    from lemon_cli.subcommands.pause import cmd_pause
...
def test_cli_pause_idempotent(lemon_home, capsys):
    from lemon_cli.subcommands.pause import cmd_pause
...
def test_cli_resume_disengages(lemon_home, capsys):
    from lemon_cli.subcommands.pause import cmd_pause, cmd_resume
...
def test_cli_resume_when_not_paused(lemon_home, capsys):
    from lemon_cli.subcommands.pause import cmd_resume
...
def test_builtin_subcommands_include_pause_resume():
    from lemon_cli.main import _BUILTIN_SUBCOMMANDS

    assert "pause" in _BUILTIN_SUBCOMMANDS
    assert "resume" in _BUILTIN_SUBCOMMANDS


# ── lemon status surfacing ─────────────────────────────────────────────────


def test_status_line_when_paused(lemon_home):
    from lemon_cli.status import _estop_status_line
...
def test_is_engaged_fails_safe_on_stat_error(lemon_home, monkeypatch):
    """A stat failure must report ENGAGED (fail safe) — the pause has to
    hold even when LEMON_HOME is misbehaving, matching the module's
...
@pytest.mark.asyncio
async def test_gateway_slash_commands_bypass_estop(lemon_home):
    ...
    try:
        ...
        return
    assert reply is None or "lemon is paused" not in (reply or "").lower()
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes(lemon_home):
    from gateway.run import GatewayRunner
...
def test_pause_command_registered_for_gateway():
    from lemon_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command
...
def test_profile_gateway_honors_canonical_root_estop(tmp_path, monkeypatch):
    """fleet-analyst-class: LEMON_HOME is a profile dir; pause lives at root.

    A process launched with LEMON_HOME=~/.lemon-ai/profiles/fleet-analyst must
    still treat ~/.lemon-ai/ESTOP as engaged. Otherwise `lemon pause` is not
