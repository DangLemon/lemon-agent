...
an unlinked SQLite DB/WAL/SHM generation.  ``lemon_state`` supplies only the
...
_LEMON_EXECUTABLES = frozenset({"lemon", "lemon-agent", "lemon-acp"})
_LEMON_PYTHON_MODULES = frozenset({"acp_adapter", "lemon_cli.main"})
_LEMON_PYTHON_SCRIPTS = frozenset({"lemon_cli/main.py", "run_agent.py"})
...
def _looks_like_lemon(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Lemon AI execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _LEMON_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _LEMON_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _LEMON_PYTHON_SCRIPTS
    )
        ...
        try:
            ...
            for pid_str in os.listdir("/proc"):
                ...
                try:
                    ...
                    if argv is not None and _looks_like_lemon(argv):
                        cmdline = " ".join(argv)
