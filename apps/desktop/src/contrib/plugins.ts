 ...
 *    a `LemonPlugin` registers automatically (vite glob — drop a folder in).
 *    `lemon-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `lemon-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<lemon home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<lemon home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: LemonPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default LemonPlugin export — skipped`)
