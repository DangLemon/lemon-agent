...
import type { LemonReviewFile, LemonReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Lemon AI' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'lemon.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'lemon.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'lemon.desktop.reviewTreeMode'
const SELECTED_KEY = 'lemon.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<LemonReviewFile[]>([])
...
export const $reviewShipInfo = atom<LemonReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['lemonDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: LemonReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly LemonReviewFile[], path: string): LemonReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.lemonDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.lemonDesktop?.openExternal?.(url)
    }
