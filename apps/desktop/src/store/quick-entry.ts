...
export function canUseQuickEntry(): boolean {
  return typeof window !== 'undefined' && typeof window.lemonDesktop?.quickEntry?.getSettings === 'function'
}
...
export async function loadQuickEntrySettings(): Promise<void> {
  ...
  try {
    applyStatus(await window.lemonDesktop.quickEntry.getSettings())
  } catch {
...
export async function saveQuickEntrySettings(patch: { enabled?: boolean; shortcut?: string }): Promise<void> {
  ...
  try {
    applyStatus(await window.lemonDesktop.quickEntry.setSettings(patch))
  } catch {
...
export function initQuickEntryBridge(): () => void {
  const api = typeof window === 'undefined' ? undefined : window.lemonDesktop?.quickEntry
