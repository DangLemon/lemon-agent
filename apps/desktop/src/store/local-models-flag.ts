 ...
 *
 * Local models ship on main behind `--local` (either `lemon desktop --local`
 * or the flag on Lemon AI.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.lemonDesktop?.localModelsEnabled === true
)
