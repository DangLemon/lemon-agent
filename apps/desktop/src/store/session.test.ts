...
describe('composer model persistence scope', () => {
  ...
  it('keeps inferred local-primary connections on the historical bare keys', () => {
    ...
    window.localStorage.setItem('lemon.desktop.composer.model', 'legacy-model')
    window.localStorage.setItem('lemon.desktop.composer.provider', 'legacy-provider')
    ...
    expect(window.localStorage.getItem('lemon.desktop.composer.model')).toBe('next-model')
    expect(window.localStorage.getItem('lemon.desktop.composer.model.registry.local.default')).toBeNull()
  })
...
describe('session owner hints', () => {
  ...
  it('ignores malformed persisted entries and never throws on hydrate', () => {
    window.localStorage.setItem(
      'lemon.desktop.sessionOwnerHints.v1',
      JSON.stringify([
        'junk',
        ['no-route', null],
...
describe('workspaceCwdForNewSession', () => {
  afterEach(() => {
    ...
    window.localStorage.removeItem('lemon.desktop.workspace-cwd')
    window.localStorage.removeItem('lemon.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-a.default')
    window.localStorage.removeItem('lemon.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-b.default')
  ...
  it('does not publish a delayed configured default after ownership is lost', async () => {
    ...
    ;(window as { lemonDesktop?: unknown }).lemonDesktop = {
  ...
  it('prefers the configured default over the sticky remembered workspace', () => {
    window.localStorage.setItem('lemon.desktop.workspace-cwd', '/home/user/sticky')
  ...
  it('keeps remote workspace memory separate from local and other remotes', () => {
    window.localStorage.setItem('lemon.desktop.workspace-cwd', '/local/project')
    $currentCwd.set('/live/session/path')
...
describe('remembered session id (per profile)', () => {
  ...
  it('discards legacy unsuffixed keys on first read (zero-migration, refuse-to-guess)', () => {
    ...
    localStorage.setItem('lemon.desktop.lastSessionId', 'legacy-session')
    ...
    expect(localStorage.getItem('lemon.desktop.lastSessionId')).toBeNull()
  })
  ...
  it('uses encodeURIComponent so profile names with reserved chars are isolated', () => {
    ...
    expect(localStorage.getItem('lemon.desktop.lastSessionId.profile.research%2Fops')).toBe('ops-session')
...
describe('remembered route (per profile)', () => {
  ...
  it('discards legacy unsuffixed keys on first read (zero-migration, refuse-to-guess)', () => {
    localStorage.setItem('lemon.desktop.lastRoute', '/skills')
    ...
    expect(localStorage.getItem('lemon.desktop.lastRoute')).toBeNull()
  })
  ...
  it('uses encodeURIComponent so profile names with reserved chars are isolated', () => {
    ...
    expect(localStorage.getItem('lemon.desktop.lastRoute.profile.research%2Fops')).toBe('/cron')
    expect(getRememberedRoute('research')).toBeNull()
  })
