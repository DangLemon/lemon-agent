 ...
 *
 * The archive is the CLI's own `lemon profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/lemon'
...
import type { ProfileDesktopOverlay } from '@/types/lemon'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Lemon AI profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.lemonDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.lemonDesktop?.selectPaths?.({
