import type { ConnectionState } from '@lemon-ai/shared'
...
import type { SessionInfo, UsageStats } from '@/types/lemon'
...
const COMPOSER_MODEL_KEY = 'lemon.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'lemon.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'lemon.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'lemon.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'lemon.desktop.composer.fast'
...
function composerScopeForConnection(connection: LemonConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'lemon.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'lemon.desktop.lastRoute'
...
function workspaceCwdKey(connection: LemonConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.lemonDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.lemonDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<LemonConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'lemon.desktop.sessionOwnerHints.v1'
 ...
 * LemonConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<LemonConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useLemonConfig.
