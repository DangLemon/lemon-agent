 ...
 * app's bounds and stays visible while Lemon AI is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'lemon.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'lemon.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.lemonDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.lemonDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.lemonDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.lemonDesktop?.petOverlay?.close()
}
