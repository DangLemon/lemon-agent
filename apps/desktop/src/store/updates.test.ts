...
const checkLemonUpdateSpy = vi.fn()
const updateLemonSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/lemon', () => ({
  checkLemonUpdate: (...args: unknown[]) => checkLemonUpdateSpy(...args),
  updateLemon: (...args: unknown[]) => updateLemonSpy(...args),
...
describe('checkBackendUpdates', () => {
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkLemonUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'lemon update',
      message: null,
    ...
    expect(checkLemonUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkLemonUpdateSpy).not.toHaveBeenCalled()
  })
})

// The ⌘K "Update Lemon AI" row. It used to call applyBackendUpdate() flat, which
...
describe('requestActiveUpdate', () => {
  ...
  beforeEach(() => {
    ...
    updateLemonSpy.mockReset().mockResolvedValue({ ok: true, name: 'update' })
    checkLemonUpdateSpy.mockReset().mockResolvedValue({
      install_method: 'git',
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      lemonDesktop: { updates: { apply: applyClientMock, check: checkClientMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('applies the CLIENT update in local mode, never the backend', async () => {
    ...
    expect(updateLemonSpy).not.toHaveBeenCalled()
  ...
  it('applies the BACKEND update in remote mode', async () => {
    ...
    requestActiveUpdate()
    await vi.waitFor(() => expect(updateLemonSpy).toHaveBeenCalled())
...
describe('explicit update targets', () => {
  ...
  // for updating THIS app — routes here via `lemon:open-updates`.
  ...
  it('still defaults to the connected machine when no target is named', async () => {
    ...
    expect($updateOverlayTarget.get()).toBe('backend')
    await vi.waitFor(() => expect(checkLemonUpdateSpy).toHaveBeenCalled())
  ...
  it('keeps the everything-flow for the generic, target-less apply', async () => {
    ...
    await vi.waitFor(() => expect(updateLemonSpy).toHaveBeenCalled(), { timeout: 5000 })
  })
...
describe('applyEverythingUpdate', () => {
  ...
  it('remote mode: updates the backend, then the still-behind client — the stale-GUI gap', async () => {
    ...
    expect(updateLemonSpy).toHaveBeenCalledTimes(1)
  ...
  it('a failed backend leg does not strand the fan-out or the client', async () => {
    setRemote(true)
    $mockConnectionsRegistry.set(registryOf(['local', 'vps']))
    updateLemonSpy.mockRejectedValue(new Error('backend gone'))
...
describe('checkUpdates branding', () => {
  ...
  beforeEach(() => {
    ...
    $updateStatus.set(null)
    ;(globalThis as unknown as { window: unknown }).window = {
      lemonDesktop: { updates: { check: checkClientMock } }
    }
  })
...
describe('applyUpdates terminal state', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as unknown as { window: unknown }).window = {
      lemonDesktop: { updates: { apply: applyMock } }
    }
    vi.useRealTimers()
  })
  ...
  it('preserves dynamic paths when apply resolves not-ok', async () => {
    const message = 'Missing app at /Users/test/Lemon AI Runtime/Lemon AI.app'
  ...
  it('keeps the manual command state for CLI installs with no staged updater', async () => {
    applyMock.mockResolvedValue({ ok: true, manual: true, command: 'lemon update' })

    await applyUpdates()

    expect($updateApply.get().stage).toBe('manual')
    expect($updateApply.get().command).toBe('lemon update')
    expect($updateOverlayOpen.get()).toBe(true)
    expect(notifySpy).not.toHaveBeenCalled()
  })
  ...
  it('lands on a closeable manual-restart state when the rebuilt sandbox blocks auto-relaunch', async () => {
    ...
    applyMock.mockResolvedValue({
      ...
      manualRestart: true,
      sandboxBlocked: true,
      message: 'Backend updated. Quit and reopen Lemon AI to finish.'
    })
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    ...
    updateLemonSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: [`=== lemon-update completed ${actionId} ===`],
      name: 'update',
      pid: null,
      running: false
    })
  ...
  it('keeps waiting past the old 45-second cutoff while the update action is running', async () => {
    ...
    updateLemonSpy.mockResolvedValue({ action_id: actionId, ok: true, name: 'lemon-update', pid: 1 })

    for (let attempt = 0; attempt < 31; attempt += 1) {
      getActionStatusSpy.mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== lemon-update started now ===', `step ${attempt}`],
        name: 'lemon-update',
        pid: 1,
        running: true
      })
  ...
  it('treats a successful no-op as complete without waiting for a restart', async () => {
    updateLemonSpy.mockResolvedValue({ ok: true, name: 'lemon-update', pid: 1 })
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['stale output from another run', '=== lemon-update started now ===', '✓ Already up to date!'],
      name: 'lemon-update',
      pid: 1,
      running: false
    })
  ...
  it('treats a successful dependency repair as complete without waiting for a restart', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== lemon-update started now ===', '✓ Dependencies repaired!', '✓ Update complete!'],
      name: 'lemon-update',
      pid: 1,
      running: false
    })
  ...
  it('waits for current-action completion proof after the backend restarts', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== lemon-update completed ${'c'.repeat(32)} ===`],
        name: 'lemon-update',
        pid: null,
        running: false
      })
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['Update complete!', `=== lemon-update completed ${actionId} ===`],
        name: 'lemon-update',
        pid: null,
        running: false
      })
  ...
  it('accepts its terminal receipt when a verbose update pushes the start marker out of the log tail', async () => {
    ...
    getActionStatusSpy.mockRejectedValueOnce(new Error('ECONNREFUSED')).mockResolvedValueOnce({
      exit_code: null,
      lines: ['final build output', 'Update complete!', `=== lemon-update completed ${actionId} ===`],
      name: 'lemon-update',
      pid: null,
      running: false
    })
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(getActionStatusSpy).toHaveBeenCalledWith('lemon-update', 2000)
  })
  ...
  it('proves a pre-action-ID backend reached its requested commit after restart', async () => {
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkLemonUpdateSpy).toHaveBeenCalledTimes(2)
  })
  ...
  it('proves a fast pre-action-ID packaged update by its changed version', async () => {
    ...
    await expect(promise).resolves.toMatchObject({ ok: true })
    expect(checkLemonUpdateSpy).toHaveBeenCalledWith(true)
  })
  ...
  it('resumes action polling after a transient status failure', async () => {
    ...
    getActionStatusSpy
      ...
      .mockResolvedValueOnce({
        exit_code: null,
        lines: ['=== lemon-update started now ===', 'still running'],
        name: 'lemon-update',
        pid: 1,
        running: true
      })
      .mockResolvedValueOnce({
        exit_code: 0,
        lines: ['=== lemon-update started now ===', 'Update complete!'],
        name: 'lemon-update',
        pid: 1,
        running: false
      })
  ...
  it('restores the fixed action deadline after reconnecting', async () => {
    updateLemonSpy.mockResolvedValue({ action_id: 'a'.repeat(32), ok: true, name: 'lemon-update', pid: 1 })
  ...
  it('shares one in-flight update between concurrent apply requests', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 0,
      lines: ['=== lemon-update started now ===', '✓ Already up to date!'],
      name: 'lemon-update',
      pid: 1,
      running: false
    })
  ...
  it('fails immediately when the update action exits nonzero', async () => {
    ...
    getActionStatusSpy.mockResolvedValue({
      exit_code: 1,
      lines: ['=== lemon-update started now ===', 'update failed'],
      name: 'lemon-update',
      pid: 1,
      running: false
    })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateLemonSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkLemonUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
...
describe('startUpdatePoller', () => {
  ...
  beforeEach(() => {
    ...
    $updateStatus.set(null)
    ;(globalThis as unknown as { window: unknown }).window = {
      lemonDesktop: { updates: { check: checkMock, onProgress: onProgressMock } },
  ...
  it('preserves dynamic paths from updater progress events', async () => {
    ...
    progress?.({
      stage: 'error',
      message: 'Missing app at /Users/test/Lemon AI Runtime/Lemon AI.app',
      error: 'ENOENT /Users/test/Lemon AI Runtime/Lemon AI.app',
      percent: null,
      at: 1
    })

    expect($updateApply.get().message).toBe('Missing app at /Users/test/Lemon AI Runtime/Lemon AI.app')
    expect($updateApply.get().error).toBe('ENOENT /Users/test/Lemon AI Runtime/Lemon AI.app')
  })
})
