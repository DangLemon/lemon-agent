...
vi.mock('@/lemon', () => ({
  getLemonConfigRecord: vi.fn(async () => ({})),
  saveLemonConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye lemon', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye lemon')
  })
