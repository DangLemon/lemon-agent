...
export function setZoomPercent(percent: number): void {
  window.lemonDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.lemonDesktop?.zoom) {
  void window.lemonDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.lemonDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
