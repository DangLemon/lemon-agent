...
import { getLemonConfigRecord, saveLemonConfig } from '@/lemon'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getLemonConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveLemonConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
