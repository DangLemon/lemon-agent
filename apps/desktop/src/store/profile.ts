import { LOCAL_CONNECTION_ID } from '@lemon-ai/shared'
import { atom, batch, computed } from 'nanostores'

import type { LemonConnection } from '@/global'
import { getProfiles, lemonApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/lemon'
...
import type { ProfileInfo } from '@/types/lemon'
...
// /api/profiles/active `current`). "default" is the root ~/.lemon-ai. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'lemon.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'lemon.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await lemonApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new LEMON_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.lemonDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<LemonConnection | null> {
  const getConnection = window.lemonDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<LemonConnection | null> {
  const getConnectionFor = window.lemonDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'lemon.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.lemonDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.lemonDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.lemon-ai) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.lemonDesktop?.touchBackend?.(target).catch(() => undefined)
}
