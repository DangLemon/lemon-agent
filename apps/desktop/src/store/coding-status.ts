...
import type { LemonGitWorktree, LemonRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: LemonGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, LemonRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, LemonGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<LemonRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<LemonGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<LemonRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<LemonGitWorktree[]>>()
const $noRepoStatus = atom<LemonRepoStatus | null>(null)
const $noWorktrees = atom<LemonGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<LemonRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<LemonGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: LemonRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: LemonGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<LemonRepoStatus | null>
  seq: number
  target: string
}
