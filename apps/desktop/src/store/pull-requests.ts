...
import type { LemonBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/lemon'
...
export const $pullRequestsByBranch = atom<Record<string, LemonBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'lemon.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('lemon.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: LemonBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
