 ...
 * `@lemon-ai/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-lemon-glass]`
 * block in styles.css).
 */
...
} from '@lemon-ai/shared/translucency'
 ...
 * sets `lemonDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.lemonDesktop?.glassSupported === 'boolean'
    ? window.lemonDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.lemonDesktop?.translucencySupported === 'boolean'
    ? window.lemonDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-lemon-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-lemon-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-lemon-glass-on', glassLive)
  root.toggleAttribute('data-lemon-glass', glassOn)
  root.toggleAttribute('data-lemon-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-lemon-glass-scope', scope)
  } else {
    root.removeAttribute('data-lemon-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.lemonDesktop?.setTranslucency?.(state)
  })
