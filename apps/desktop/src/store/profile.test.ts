...
const { getProfiles } = await import('@/lemon')
...
const remoteConn = (over: Partial<LemonConnection> = {}): LemonConnection =>
  ({ baseUrl: 'https://lemon-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as LemonConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { lemonDesktop: { getConnection } })
