 ...
 * composer with the reply scrolling above it, so Lemon AI can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.lemonDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.lemonDesktop?.hud
...
export function resetHudLayout(): void {
  void window.lemonDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.lemonDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.lemonDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
