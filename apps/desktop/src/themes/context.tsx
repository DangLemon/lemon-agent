...
const SKIN_KEY = 'lemon-desktop-theme-v2'
const MODE_KEY = 'lemon-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'lemon-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'lemon-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'lemon-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.lemonTheme = skinName
  root.dataset.lemonMode = rendered
  ...
  window.lemonDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('lemon-boot-background', chromeBg)
    window.localStorage.setItem('lemon-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.lemonThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.lemonDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Lemon AI authoring/activating a skin from a
