...
describe('ingestBackendSkin', () => {
  ...
  it('seed does not paint, but a later same-name skin.changed applies (missed-activation recovery)', () => {
    ...
    // restarted). Lemon AI re-affirms it — `lemon config set display.skin neon`
    // or a `lemon skin set` recolor. That explicit event must repaint even
    ...
    ingestBackendSkin(skin('forest'), { apply: true }) // Lemon AI authored a new skin
    expect($pendingSkinApply.get()).toBe('forest')
  })
  ...
  it('applies a runtime switch back to default (repaints the desktop to its own default)', () => {
    ...
    ingestBackendSkin(skin('default'), { apply: true }) // Lemon AI switched back to default
  ...
  it('hydrates the registry from storage on load, so a persisted pick resolves before the gateway connects', async () => {
    ingestBackendSkin(skin('neon'), { apply: false })
    expect(JSON.parse(window.localStorage.getItem('lemon-desktop-backend-themes-v1') ?? '{}').neon?.name).toBe('neon')
  ...
  it('drops junk and built-in names from the cached registry', async () => {
    ...
    window.localStorage.setItem('lemon-desktop-backend-themes-v1', JSON.stringify({ mono, bad: { name: 'bad' } }))
