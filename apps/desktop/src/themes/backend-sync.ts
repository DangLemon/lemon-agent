/**
 * Live skin sync from the Lemon AI backend.
 *
 * The backend resolves the active skin (built-in or `$LEMON_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Lemon AI
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { LemonSkin } from '@lemon-ai/shared/skin'
...
const BACKEND_THEMES_KEY = 'lemon-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: LemonSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as LemonSkin)
