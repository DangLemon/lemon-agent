...
//
// When someone is in a hands-free "Hey Lemon AI" voice chat, the natural way to
...
// phrase (optionally addressed to Lemon AI), so a real turn that merely contains
...
// Optional address prefixes so "lemon stop" / "ok stop" / "hey lemon, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey lemon', 'hey lemon,', 'lemon', 'lemon,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("lemon") is not a stop command on its own.
      continue
    }
 ...
 * to Lemon AI). Returns false for anything that merely contains "stop" as part of
