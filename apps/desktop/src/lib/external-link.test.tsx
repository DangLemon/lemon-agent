...
function installDesktopBridge(partial: Partial<Window['lemonDesktop']> = {}) {
...
function installTitleBridge(title: string) {
  ...
  installDesktopBridge({ fetchLinkTitle: bridge as unknown as Window['lemonDesktop']['fetchLinkTitle'] })
...
describe('external link helpers', () => {
  ...
  it('opens a web link in the in-app browser', async () => {
    ...
    installDesktopBridge({ openExternal: openExternal as unknown as Window['lemonDesktop']['openExternal'] })
  ...
  it('prefixes a pretty link to a known host with its brand glyph', () => {
    ...
    const url = 'https://github.com/DangLemon/lemon-agent/pull/123'
