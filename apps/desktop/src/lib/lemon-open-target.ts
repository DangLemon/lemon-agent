 ...
 * `lemon://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `lemon://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `lemon://open/my-page?item=x` → `/my-page?item=x` (generic open)
...
export type LemonOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const LEMON_PROTOCOL = 'lemon:'
...
export function normalizeLemonOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('lemon://') || trimmed.startsWith(`${LEMON_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // lemon://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // lemon://index-network/intent/1 → /index-network/intent/1
...
export function resolveLemonOpenPath(target: LemonOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeLemonOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeLemonOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeLemonOpenString(target.path)
...
export function resolveInternalCompanyOpenPath(
  target: LemonOpenTarget | null | undefined,
  state: Pick<InternalCompanyCapabilityState, 'allowedRoutes' | 'mode'>
): string | null {
  const path = resolveLemonOpenPath(target)
 ...
 * (`lemon://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromLemonDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveLemonOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveLemonOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `lemon://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromLemonDeepLink('open', name, params)
}
