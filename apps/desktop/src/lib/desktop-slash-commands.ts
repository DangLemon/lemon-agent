...
/** Why a known Lemon AI command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Lemon AI profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownLemonSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Lemon AI' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownLemonSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Lemon AI and were never asked for. It is
