...
// Lemon AI is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.lemonMode
 ...
 * the only carriers of Lemon AI' palette — are dropped so the paste target's
