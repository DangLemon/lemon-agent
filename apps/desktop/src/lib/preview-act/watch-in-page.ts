...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __lemonWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__lemonWatch || (win.__lemonWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('lemon-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('lemon-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __lemonWatchTag?: number }).__lemonWatchTag
