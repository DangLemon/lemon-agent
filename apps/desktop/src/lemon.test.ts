...
describe('Lemon AI REST helpers', () => {
    ...
    api.mockImplementation(({ path }: { path: string }) => {
      if (path.startsWith('/api/profiles/sessions/sidebar')) {
        ...
        return Promise.reject(
          new Error(
            'Error invoking remote method \'lemon:api\': Error: 404: {"detail":"No such API endpoint: /api/profiles/sessions/sidebar"}'
          )
        )
      }
  ...
  it('gives the whole startup data burst the long timeout, not just profiles', async () => {
    ...
    const bootCalls: [() => Promise<unknown>, string][] = [
      [getLemonConfig, '/api/config'],
      [getLemonConfigDefaults, '/api/config/defaults'],
  ...
  it('reads raw config metadata through the capability-scoped route', async () => {
    ...
    await expect(getLemonRawConfig()).resolves.toEqual({
