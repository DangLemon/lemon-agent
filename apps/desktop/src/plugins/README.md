...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `LemonPlugin` and
...
`$LEMON_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`lemon-desktop-plugins` skill.
