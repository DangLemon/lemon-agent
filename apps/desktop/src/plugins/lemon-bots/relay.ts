...
import { host, LruCache } from '@lemon-ai/plugin-sdk'
...
// to (local, remote URL, SSH, Lemon AI Cloud, docker) must be able to find
...
// lemon_cli/config_defaults.py and tui_gateway/methods_bot_relay.py and fails
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['lemon-bots']?.title || profile?.display_name || ''),
