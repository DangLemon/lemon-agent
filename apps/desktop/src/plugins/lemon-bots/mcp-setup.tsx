...
import { Button, host, Input, useI18n } from '@lemon-ai/plugin-sdk'
...
// and the row falls back to the "run lemon mcp / Settings" hint. profile is
...
export function McpSetupButton({ profile, entry, onDone, ensureProfile }: McpSetupButtonProps) {
  ...
  const beginOAuth = async () => {
    ...
    const mcpOauthBridge =
      typeof window !== 'undefined' && window.lemonDesktop && window.lemonDesktop.mcpOauth
        ? window.lemonDesktop.mcpOauth
        : null
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.lemonDesktop && window.lemonDesktop.openExternal) {
        window.lemonDesktop.openExternal(authUrl)
      } else {
