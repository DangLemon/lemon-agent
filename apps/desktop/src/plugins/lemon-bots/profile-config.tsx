...
export function AdvancedProfileConfig({ bot, state, setState }: AdvancedProfileConfigProps) {
  ...
  if (unsupported) {
    return (
      <div className="px-2 py-3 text-center text-xs text-(--ui-text-tertiary)">
        {brandDisplayString('Full configuration needs a newer gateway (restart it after updating Lemon AI).')}
      </div>
    )
  }
  ...
  // (lemon-agent#87317): Skills (installed list + one-click hub installs +
