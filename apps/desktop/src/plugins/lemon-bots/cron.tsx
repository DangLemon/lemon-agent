/**
 * Routines: the Lemon AI cron jobs scoped to the bot you're chatting with — the
...
// lemon -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `lemon -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Lemon AI-native schedule string; Advanced exposes it raw.
...
/** Compose the Lemon AI schedule string from picker state. */
