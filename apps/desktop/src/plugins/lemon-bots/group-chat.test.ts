...
vi.mock('@lemon-ai/plugin-sdk', async () => {
...
function published(room: Room) {
  ...
  return (configure?.params.ui_meta as Record<string, Record<string, unknown>>)?.['lemon-bots-groups']
}
...
describe('speaker labels', () => {
  it('the default profile speaks as Lemon AI in transcripts, not @default', async () => {
    ...
    expect(line).toBe('Lemon AI: hello room')
    ...
    ).toBe('Lemon AI (you): hi')
    expect(
      rounds.formatGroupChatLine({ from: { kind: 'member', name: 'builder' }, text: 'yo' } as GroupMessage, 'research')
    ).toBe('builder: yo')
  })

  it('honor friendly identity: Bot Mode title, then display_name, never a stale Lemon AI', async () => {
    const { chat, rounds } = await loadRoom()
    const data = await import('./data')

    // A renamed default (core display_name via `lemon profile rename`) must
    // read as its new name — the community report was "Lucy" still showing
    // "Lemon AI is thinking…" in group rooms.
    ...
    // Untitled rows keep today's behavior: default → Lemon AI, others verbatim.
    data.$botMeta.set({})

    expect(chat.groupSpeakerLabel('default')).toBe('Lemon AI')
...
describe('sync worker', () => {
  it('retries a gateway CAS conflict and publishes the merged room', async () => {
    ...
    await drain(() => (room.gateway.uiMetaRevisions['lemon-bots-groups'] || 0) < 2, 60)

    const stored = room.gateway.uiMeta['lemon-bots-groups'] as {
      rooms: Record<string, { log: GroupMessage[] }>
    }

    expect(room.gateway.uiMetaRevisions['lemon-bots-groups']).toBe(2)
  ...
  it('fans a room write out to every reachable default-profile gateway', async () => {
    ...
    host.requestProfile = async (route: { connectionId: string }, method: string) => {
      ...
      if (method === 'profiles.configure') {
        return { applied: { ui_meta: true, ui_meta_revisions: { 'lemon-bots-groups': 1 } } }
      }
