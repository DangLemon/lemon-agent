...
import { atom, host } from '@lemon-ai/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'lemon-bots-groups'
...
/** The v3 envelope stored under the default profile's `lemon-bots-groups`
 ...
 *  clients (Lemon AI Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Lemon AI"
 *  (community report, Aug 21 2026: renamed default still read "Lemon AI is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Lemon AI (matching displayName and the
 *  @lemon handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`lemon profile rename …` / dashboard) from
