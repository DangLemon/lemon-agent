...
import { forgetSessionUnread, host, queryClient } from '@lemon-ai/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['lemon-bots']
...
/** Permanently delete a bot's Lemon AI profile, then remove plugin-local state
 ...
 * directory (lemon-agent#52279). That is the "can't delete a bot" error. */
