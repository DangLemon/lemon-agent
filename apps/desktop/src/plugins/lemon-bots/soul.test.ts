...
vi.mock('./data', () => ({
  botHandle: (name: string) => (String(name).toLowerCase() === 'default' ? 'lemon' : name),
...
describe('appending the protocol to an existing SOUL', () => {
  ...
  it('points at the real CLI verb, not the plural that does not exist', async () => {
    ...
    expect(soul).toMatch(/run `lemon profile list` for the LIVE/)
    expect(soul).not.toMatch(/lemon profiles list/)
  })
