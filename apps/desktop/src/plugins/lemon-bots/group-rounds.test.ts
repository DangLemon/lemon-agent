...
describe('routing', () => {
  ...
  it('resolves @lemon to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@lemon take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @lemon', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @lemon,/)
