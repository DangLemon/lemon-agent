 ...
 * `{type:'lemon-skill-pick'}` back and the plugin installs via skills.manage.
 *
...
import type * as LemonSdk from '@lemon-ai/plugin-sdk'
import { cleanup, fireEvent, render, screen } from '@testing-library/react'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'

const HUB_ORIGIN = 'https://github.com/DangLemon/lemon-agent'
...
vi.mock('@lemon-ai/plugin-sdk', async importOriginal => {
  const original = await importOriginal<typeof LemonSdk>()
...
describe('hub pick messages', () => {
  it('installs the picked skill when it comes from our own frame', () => {
    ...
    postPick(
      { identifier: 'nous/web-research', name: 'Web Research', type: 'lemon-skill-pick' },
      {
  ...
  it('regression: refuses identifiers outside the slug charset', () => {
    ...
    for (const identifier of ['../../etc/passwd', 'skill; rm -rf /', '-flag', 'name with spaces', '']) {
      postPick({ identifier, name: 'Web Research', type: 'lemon-skill-pick' }, { source: frame.contentWindow })
    }
  ...
  it('ignores messages that are not a skill pick', () => {
    ...
    postPick({ identifier: 'nous/web-research', type: 'lemon-skill-pick' }, { source: frame.contentWindow })
