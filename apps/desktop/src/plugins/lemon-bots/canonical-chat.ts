...
import * as sdk from '@lemon-ai/plugin-sdk'
import { host } from '@lemon-ai/plugin-sdk'
...
// grandfather, or recover. Stored-id pins (ui_meta['lemon-bots'].chat) were
 ...
 *  click path's tile-staleness probe (lemon-agent#90102), which must
...
async function openStoredBotChat(
  ...
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error(brandDisplayString('This Lemon AI version cannot open stored sessions'))
  }
  ...
  // even though the session is fine (lemon-agent#89617) — clicking Retry
  ...
  // until an app restart (lemon-agent#93604). A resume is cheap and
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        brandDisplayString('Update Lemon AI to chat with bots on other connections.')
    )
  }
...
export async function ensureBotMetadata(bot: RosterRow): Promise<BotMeta> {
  ...
  const server = row?.ui_meta?.['lemon-bots']
