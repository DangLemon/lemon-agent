 ...
 *    desktop shell fires only while the user is AWAY from Lemon AI. This is the
...
import { host, type PluginOs, type PluginRestOptions, type PluginTranslate } from '@lemon-ai/plugin-sdk'
...
function notifyOne(kind: string, spec: { titleKey: string; toast: ToastKind }, ev: CompletionEvent): void {
  ...
  // is away from Lemon AI (the toast above covers the foreground case). Isolated:
