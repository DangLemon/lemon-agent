...
import { setLemonConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `lemon://mcp/install` deep links. The payload is
 ...
 * confirming lets Lemon AI spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getLemonConfigRecord())
      ...
      setLemonConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
