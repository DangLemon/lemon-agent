 ...
 * the default browser's profile (cookies/logins) into a Lemon AI-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useLemonConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        await saveLemonConfigRecord(next, profile)
        notify({
          kind: 'info',
