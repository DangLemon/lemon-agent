...
import { disconnectOAuthProvider, listOAuthProviders } from '@/lemon'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/lemon'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.lemonDesktop?.terminal)
...
//      in lemon_cli/provider_catalog.py) — the SAME provider identity
//      `lemon model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Lemon AI can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Lemon AI can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Lemon AI never deletes creds another tool owns behind a silent API call.
