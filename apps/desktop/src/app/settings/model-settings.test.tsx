...
const getLemonConfigRecord = vi.fn()
const saveLemonConfig = vi.fn()
...
vi.mock('@/lemon', () => ({
  ...
  getLemonConfigRecord: () => getLemonConfigRecord(),
  saveLemonConfig: (config: unknown) => saveLemonConfig(config),
...
beforeEach(() => {
  ...
  getLemonConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveLemonConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getLemonConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveLemonConfig).toHaveBeenCalledWith(
...
describe('ModelSettings code-skew 503', () => {
  const skewError = new Error(
    'Error invoking remote method \'lemon:api\': Error: 503: {"detail":"Restart required: This process is running code from 08b4875f4a but the checkout on disk is now 48d2528066. The model picker would risk a stale-module crash — restart the Desktop-owned backend to load the new code (use Restart backend in Lemon AI, or quit and reopen the app)"}'
  )
  ...
  it('unwraps the stale-backend 503 instead of dumping IPC JSON', async () => {
    ...
    expect(screen.queryByText(/lemon:api/)).toBeNull()
  ...
  it('recycles the Desktop-owned backend and reloads the catalog', async () => {
    ...
    ;(window as unknown as { lemonDesktop: { recycleBackend: typeof recycleBackend } }).lemonDesktop = {
      recycleBackend
    }
