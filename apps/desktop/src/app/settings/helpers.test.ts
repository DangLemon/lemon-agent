...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: LemonConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: LemonConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // LEMON_QWEN_ shares the LEMON_ stem with other integrations.
      expect(providerGroup('LEMON_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
  ...
  describe('enumOptionsFor — backend selector dropdowns', () => {
    ...
    it('surfaces user-defined command-type TTS providers (canonical providers nesting + legacy)', () => {
      const withCustom: LemonConfigRecord = {
        tts: {
          provider: 'neutts',
    ...
    it('never offers a built-in name as a command provider, even one absent from the dropdown list', () => {
      const shadowing: LemonConfigRecord = {
        tts: {
          provider: 'edge',
          providers: {
  ...
  describe('sectionFieldEntries', () => {
    it('renders memory.provider from config even when the backend schema omits it', () => {
      ...
      const config: LemonConfigRecord = { memory: { memory_enabled: true, provider: '' } }
    ...
    it('infers the field type from the config value when the schema omits the key', () => {
      const config: LemonConfigRecord = { memory: { provider: '', memory_enabled: true, memory_char_limit: 2200 } }
    ...
    it('prefers the backend schema entry over inference when both exist', () => {
      ...
      const config: LemonConfigRecord = { memory: { provider: 'honcho' } }
  ...
  describe('clearsEnabledToolsets', () => {
    it('flags a non-empty → empty transition', () => {
      const prev: LemonConfigRecord = { toolsets: ['memory', 'terminal', 'web_search'] }
      const next: LemonConfigRecord = { toolsets: [] }
    ...
    it('does not flag a non-empty → missing transition (deep-merge preserves the key)', () => {
      ...
      const prev: LemonConfigRecord = { toolsets: ['memory'] }
      const next: LemonConfigRecord = {}

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when at least one toolset remains', () => {
      const prev: LemonConfigRecord = { toolsets: ['memory', 'terminal'] }
      const next: LemonConfigRecord = { toolsets: ['memory'] }

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when the list was already empty', () => {
      const prev: LemonConfigRecord = { toolsets: [] }
      const next: LemonConfigRecord = { toolsets: [] }
    ...
    it('does not flag an unrelated edit that never touched toolsets', () => {
      const prev: LemonConfigRecord = { model: 'a', toolsets: ['memory'] }
      const next: LemonConfigRecord = { model: 'b', toolsets: ['memory'] }
  ...
  describe('diffConfig', () => {
    it('omits a top-level key the draft never touched', () => {
      ...
      // an agent set via `lemon config set` while the page sat open must not
      // come back in the patch just because it's still present in the draft.
      const baseline: LemonConfigRecord = { fallback_providers: ['nara1'], timezone: 'UTC' }
      const draft: LemonConfigRecord = { fallback_providers: ['nara1'], timezone: 'America/New_York' }
    ...
    it('includes a nested key only when it actually changed, leaving siblings out', () => {
      const baseline: LemonConfigRecord = { display: { personality: 'default', show_reasoning: true } }
      const draft: LemonConfigRecord = { display: { personality: 'default', show_reasoning: false } }
    ...
    it('sends a new key that was absent from the baseline', () => {
      const baseline: LemonConfigRecord = {}
      const draft: LemonConfigRecord = { timezone: 'UTC' }
    ...
    it('returns an empty object when the draft matches the baseline exactly', () => {
      const baseline: LemonConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }
      const draft: LemonConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }

      expect(diffConfig(baseline, draft)).toEqual({})
    })

    it('treats an array as a whole value, not diffed element by element', () => {
      const baseline: LemonConfigRecord = { toolsets: ['memory', 'terminal'] }
      const draft: LemonConfigRecord = { toolsets: ['memory'] }
