...
// User-installed plugins first — mirrors `lemon plugins list --user`.
...
function reveal(file: string) {
  void window.lemonDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // lemon_home breaks against a remote backend (#66899).
    const dir = await window.lemonDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.lemonDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's lemon home (profile-aware), so the
// path comes from the gateway — not from the renderer's local LEMON_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.lemonDesktop?.openDir?.(`${home}/plugins`)
