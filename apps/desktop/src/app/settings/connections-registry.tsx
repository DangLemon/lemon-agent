 ...
 * agent sources (local runtime + any number of remote gateways / Lemon AI Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.lemonDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.lemonDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `lemon update` to every eligible source; per-connection results
