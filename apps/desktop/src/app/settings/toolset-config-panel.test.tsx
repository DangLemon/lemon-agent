...
import type { ToolsetConfig } from '@/types/lemon'
...
beforeEach(() => {
  ...
  getLemonConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getLemonConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveLemonConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveLemonConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
