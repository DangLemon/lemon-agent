...
import { getActionStatus, getLogs, getStatus, getUsageAnalytics, restartGateway, updateLemon } from '@/lemon'
import type { ActionStatusResponse, AnalyticsResponse, SessionInfo, StatusResponse } from '@/lemon'
...
export function CommandCenterView({ initialSection, onClose, onDeleteSession, onOpenSession }: CommandCenterViewProps) {
  ...
  // Client-side substring filter over the fetched tail (matches `lemon logs --search`).
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updateLemon()
                    ...
                    <div className="flex items-start justify-between gap-3 max-[47.5rem]:flex-col max-[47.5rem]:gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                          {cc.lemonActiveSessions(status.version, status.active_sessions)}
                        </div>
                      </div>
