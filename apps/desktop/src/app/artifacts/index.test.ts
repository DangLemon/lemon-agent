...
import type { SessionInfo, SessionMessage } from '@/types/lemon'
...
describe('collectArtifactsForSession', () => {
  ...
  it('keeps an explicit browser screenshot but ignores page assets', () => {
    const payload = JSON.stringify({
      ...
      screenshot_path: '/tmp/lemon-browser/screenshot.png'
    })
...
${payload}
    ...
    expect(artifacts[0]).toMatchObject({
      kind: 'image',
      value: '/tmp/lemon-browser/screenshot.png'
    })
  })
  ...
  it('keeps native browser screenshots without indexing embedded image data', () => {
    const artifacts = collectArtifactsForSession(makeSession({ id: 'native-browser-session' }), [
      {
        content: {
          ...
          meta: { screenshot_path: '/tmp/lemon-browser/native-screenshot.png' },
      ...
      {
        content: 'Image attached. Screenshot path: /tmp/lemon browser/summary screenshot.png',
        role: 'tool',
        timestamp: 1_781_774_002,
        tool_name: 'browser_vision'
      },
      {
        content: 'Image attached. Screenshot path: C:\\Users\\Example User\\.lemon-ai\\screenshot.png',
        role: 'tool',
        timestamp: 1_781_774_003,
        tool_name: 'browser_vision'
      }
    ...
    expect(artifacts.map(artifact => artifact.value)).toEqual([
      '/tmp/lemon-browser/native-screenshot.png',
      '/tmp/lemon browser/summary screenshot.png',
      'C:\\Users\\Example User\\.lemon-ai\\screenshot.png'
    ])
  })
