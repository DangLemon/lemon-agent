...
function setWorkspaceTransfer(transferring: boolean): void {
  window.lemonDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.lemonDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.lemonDesktop?.hud?.beginMove?.()
}
 ...
 * (see lemon:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.lemonDesktop?.hud?.endMove?.()
      }
