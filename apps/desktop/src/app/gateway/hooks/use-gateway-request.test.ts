...
vi.mock('@/lemon', async importOriginal => {
  const actual = await importOriginal<typeof LemonModule>()

  class FakeLemonGateway {
  ...
  return { ...actual, LemonGateway: FakeLemonGateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as LemonGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as LemonGateway, 'default')
  $gateway.set(primary as unknown as LemonGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.lemonDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as LemonGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { lemonDesktop: unknown }).lemonDesktop = { getConnection }
    $gateway.set(dropped)
