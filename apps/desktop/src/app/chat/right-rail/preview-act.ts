...
const preamble = () => `  var w = window;
  ...
  w.__lemonActHolder = w.__lemonActHolder || {};
  w.__lemonAct = ${actEngineSource()};
  w.__lemonWatch_fn = (${watchInPage.toString()});
  w.__lemonWatchTag = ${WATCH_TAG};
  var holder = w.__lemonActHolder;
  var act = function (a) { return w.__lemonAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__lemonWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__lemonHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'LEMON-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
