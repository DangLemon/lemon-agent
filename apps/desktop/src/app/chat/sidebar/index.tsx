...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/lemon'
...
// highlight markers (sqlite snippet() delimiters — see lemon_state_search.py).
...
export function ChatSidebar({
  ...
  const contributedNav = useMemo<SidebarNavItem[]>(
    () =>
      navContributions.flatMap(c => {
        ...
        if (!data?.path?.startsWith('/') || !data.label || !internalCompanyRouteAllowed(data.path, internalCompany)) {
          return []
        }
    ...
    [internalCompany, navContributions]
  )
  ...
  useEffect(() => {
    ...
    window.addEventListener('lemon:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('lemon:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // A turn can `git worktree add/remove` in the terminal (e.g. you ask Lemon AI to
