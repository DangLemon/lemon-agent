...
import type { ProfileScope } from '@/lemon'
...
export const EmbeddedHubPicker = memo(function EmbeddedHubPicker({
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      if (event.origin !== HUB_ORIGIN || event.source !== iframeRef.current?.contentWindow) {
        return
      }

      const data = event.data as SkillPickMessage | null

      if (!data || data.type !== 'lemon-skill-pick' || !data.name) {
        return
      }
