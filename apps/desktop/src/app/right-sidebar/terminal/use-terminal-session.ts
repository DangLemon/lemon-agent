...
const LEMON_PATHS_MIME = 'application/x-lemon-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(LEMON_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(LEMON_PATHS_MIME)
  ...
  const getPath = window.lemonDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.lemonDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (lemon --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.lemonDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[lemon-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.lemonDesktop?.terminal?.write(sessionId, `${command}\r`)
