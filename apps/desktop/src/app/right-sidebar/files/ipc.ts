...
import type { LemonReadDirEntry, LemonReadDirResult } from '@/global'
...
export type ProjectTreeEntry = LemonReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: LemonReadDirEntry) {
...
async function filterIgnored(entries: LemonReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<LemonReadDirResult> {
  if (!window.lemonDesktop) {
