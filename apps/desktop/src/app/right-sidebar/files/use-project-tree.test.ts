...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { lemonDesktop: { readDir: typeof readDir } }).lemonDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): LemonReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { lemonDesktop: unknown }).lemonDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: LemonReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<LemonReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: LemonReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: LemonReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: LemonReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<LemonReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.lemonDesktop is missing', async () => {
    delete (window as unknown as { lemonDesktop?: unknown }).lemonDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: LemonReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: LemonReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: LemonReadDirResult) => void) | undefined
