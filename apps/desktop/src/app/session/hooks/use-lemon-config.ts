...
import { getLemonConfig, getLemonConfigDefaults } from '@/lemon'
...
interface LemonConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useLemonConfig({ activeSessionIdRef }: LemonConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshLemonConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getLemonConfig(), getLemonConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Lemon AI' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshLemonConfig, sttEnabled, voiceMaxRecordingSeconds }
}
