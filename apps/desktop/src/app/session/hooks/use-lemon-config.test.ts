...
import { getLemonConfig } from '@/lemon'
...
import { useLemonConfig } from './use-lemon-config'

vi.mock('@/lemon', () => ({
  getLemonConfig: vi.fn(),
  getLemonConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'lemon.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getLemonConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getLemonConfig>>)

describe('useLemonConfig refreshLemonConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Lemon AI' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useLemonConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshLemonConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useLemonConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getLemonConfig>>>()
    vi.mocked(getLemonConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useLemonConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshLemonConfig(true)
    })
    expect(getLemonConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getLemonConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getLemonConfig>>>()
    vi.mocked(getLemonConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useLemonConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshLemonConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getLemonConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getLemonConfig>>>()
    vi.mocked(getLemonConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshLemonConfig(true)
      refreshC = result.current.refreshLemonConfig(true)
    })
