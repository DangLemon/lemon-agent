...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Lemon AI doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in lemon_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `lemon model` uses — instead of
...
export type LemonConfigRecord = Record<string, unknown>

export interface LemonRawConfigResponse {
...
export interface ModelOptionProvider {
   ...
   *  the `lemon model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Lemon AI simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (lemon.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors lemon_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Lemon AI, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Lemon AI but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `lemon tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Lemon AI); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** lemon-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** lemon-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/lemon/update/check` — the backend's own update state.
