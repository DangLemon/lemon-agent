import type { LocalCatalogModel, LocalHardware, LocalModelsStatus, LocalRuntimeJob } from '@/types/lemon'

import { lemonApi, profileScoped } from './client'
...
export function getLocalModelsStatus(): Promise<LocalModelsStatus> {
  return lemonApi<LocalModelsStatus>({
    ...profileScoped(),
...
export function getLocalCatalog(): Promise<{ models: LocalCatalogModel[] }> {
  return lemonApi<{ models: LocalCatalogModel[] }>({
    ...profileScoped(),
...
export function installLocalRuntime(backend?: string): Promise<{ backend: string; job_id: string; tag: string }> {
  return lemonApi<{ backend: string; job_id: string; tag: string }>({
    ...profileScoped(),
...
export function quickstartLocalModels(modelId?: string): Promise<QuickstartResponse> {
  return lemonApi<QuickstartResponse>({
    ...profileScoped(),
...
export function downloadLocalModel(modelId: string): Promise<{ already_downloaded?: boolean; job_id: null | string }> {
  return lemonApi<{ already_downloaded?: boolean; job_id: null | string }>({
    ...profileScoped(),
...
export function getLocalRuntimeJob(jobId: string): Promise<LocalRuntimeJob> {
  return lemonApi<LocalRuntimeJob>({
    ...profileScoped(),
...
export function getLocalModelsJobs(): Promise<{ jobs: LocalRuntimeJob[] }> {
  return lemonApi<{ jobs: LocalRuntimeJob[] }>({
    ...profileScoped(),
...
export function searchHFModels(q: string, limit = 20): Promise<{ hits: HFSearchHit[] }> {
  return lemonApi<{ hits: HFSearchHit[] }>({
    ...profileScoped(),
...
export function listHFRepoFiles(repo: string): Promise<{ files: HFFileGroup[] }> {
  return lemonApi<{ files: HFFileGroup[] }>({
    ...profileScoped(),
...
export function downloadBrowsedModel(
  ...
  return lemonApi<{ already_downloaded?: boolean; job_id: null | string; model_id: string }>({
    ...profileScoped(),
    body: { paths, repo },
    method: 'POST',
...
export function sideloadLocalModel(
  ...
  return lemonApi<{ already_present?: boolean; model_id: string; ok: boolean }>({
    ...profileScoped(),
    body: { path },
    method: 'POST',
