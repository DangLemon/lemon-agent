...
vi.mock('@/lemon', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), lemonApi: vi.fn() }))
...
const { deleteProfile, lemonApi } = await import('@/lemon')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its LEMON_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { lemonDesktop: unknown }).lemonDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(lemonApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(vi.mocked(lemonApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
