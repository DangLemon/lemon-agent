...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Lemon AI gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.lemonDesktop
  Object.defineProperty(window, 'lemonDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'lemonDesktop', { configurable: true, value: original })
}
