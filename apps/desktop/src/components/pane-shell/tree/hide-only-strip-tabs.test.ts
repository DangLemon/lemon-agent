...
function sessionsBotsTree() {
  ...
  registerPane('lemon-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'lemon-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('lemon-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('lemon-bots:pane')
    expect($hiddenStripTabs.get()).toContain('lemon-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('lemon-bots:pane')

    expect(setStripTabHidden('lemon-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('lemon-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('lemon-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('lemon-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('lemon.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('lemon-bots:pane')
    ...
    revealTreePane('lemon-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('lemon-bots:pane')
    expect(window.localStorage.getItem('lemon.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'lemon-bots:pane', title: 'lemon-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'lemon-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
