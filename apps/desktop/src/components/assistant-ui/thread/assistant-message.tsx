...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's LEMON_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.lemonDesktop?.logsRoot?.()
      ...
      const result = await window.lemonDesktop?.openDir?.(root)
