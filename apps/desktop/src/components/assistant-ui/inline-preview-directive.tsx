 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.lemonAI.send(prompt)` (or declarative
 * `data-lemon-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'lemon-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'lemon-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `lemon.send(prompt)`.
 ...
 *  wires `data-lemon-send` so declarative HTML works with zero script:
 *  `<button data-lemon-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.lemonAI={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-lemon-send]"):null;' +
    'if(el)send(el.getAttribute("data-lemon-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `lemon serve` — renders live.)
