...
const LEMON_REF_TYPES = WIRE_REFERENCE_KINDS
type LemonRefType = (typeof LEMON_REF_TYPES)[number]
...
const LEMON_DIRECTIVE_RE = referenceRe()
...
export const lemonDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(LEMON_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...lemonDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Lemon AI directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.lemonDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.lemonDesktop?.readFileDataUrl(id)
