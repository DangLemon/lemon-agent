...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.lemonHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.lemonHome)
  const builder = await RealSessionBuilder.start(sandbox.lemonHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.lemonHome, mock.url)
  fs.appendFileSync(path.join(sandbox.lemonHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
