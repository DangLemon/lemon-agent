 ...
 * AIAgent loop, using the E2E mock provider configured in `lemonHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(lemonHome: string) {
  ...
  static async start(lemonHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(lemonHome)
