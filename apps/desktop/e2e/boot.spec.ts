 ...
 * packaged binary) with a real `lemon serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → lemon serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Lemon AI title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Lemon AI')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { lemonDesktop?: Record<string, unknown> }).lemonDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → lemon serve is spawned → WS connects → config
