/**
 * Shared E2E fixtures for the Lemon AI desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → lemon serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `lemon desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `lemon-e2e-${prefix}-${Math.random()}`))
  const lemonHome = path.join(root, 'lemon-home')
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(lemonHome, 'config.yaml')
 ...
 */
export function writeEnvFile(lemonHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(lemonHome, '.env')
 ...
 */
function writeEmptyConfig(lemonHome: string): void {
  const configPath = path.join(lemonHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - LEMON_HOME → sandbox lemon-home (isolated config/sessions)
 *  - LEMON_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - LEMON_DESKTOP_IGNORE_EXISTING=1 → don't pick up `lemon` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - LEMON_DESKTOP_LEMON_ROOT → REPO_ROOT (dev checkout resolution)
 *  - LEMON_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    LEMON_HOME: sandbox.lemonHome,
    LEMON_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    LEMON_DESKTOP_IGNORE_EXISTING: '1',
    LEMON_DESKTOP_LEMON_ROOT: REPO_ROOT,
    LEMON_DESKTOP_APP_NAME: `LemonE2E-${Date.now()}`,
    ...
    LEMON_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated LEMON_HOME + userData
 * @param env      - the process environment (already has LEMON_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.lemonHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.lemonHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via LEMON_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`lemon serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.lemonHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { LEMON_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Lemon AI backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
 ...
 * progress without spawning a real Lemon AI backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * LEMON_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    LEMON_DESKTOP_BOOT_FAKE: '1',
    LEMON_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + lemon-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).LEMON_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).LEMON_DESKTOP_LEMON
  delete (env as Record<string, string | undefined>).LEMON_DESKTOP_LEMON_ROOT
