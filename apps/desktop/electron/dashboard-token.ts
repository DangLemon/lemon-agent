 ...
 *
 * The desktop main process can pass LEMON_DASHBOARD_SESSION_TOKEN when it
...
async function fetchPublicText(url, options: any = {}) {
  ...
  if (protocol !== 'http:' && protocol !== 'https:') {
    throw new Error(`Unsupported Lemon AI backend URL protocol: ${protocol}`)
  }
  ...
  const res = await fetch(url, { signal: AbortSignal.timeout(timeoutMs) }).catch(error => {
    if (error.name === 'TimeoutError') {
      throw new Error(`Timed out connecting to Lemon AI backend after ${timeoutMs}ms`)
    }
...
function extractInjectedDashboardToken(html) {
  const match = /window\.__LEMON_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")/.exec(String(html || ''))
 ...
 */
async function adoptServedDashboardToken(baseUrl, spawnToken, { childAlive, label = 'Lemon AI backend', ...options }) {
