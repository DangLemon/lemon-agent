...
test('stagedUpdaterSupportsPrewrittenMarker rejects installers predating the self-adopt fix', () => {
  ...
  // refreshed because copy_self_to_lemon_home no-ops during --update.
  assert.equal(
    stagedUpdaterSupportsPrewrittenMarker('C:\\Lemon AI\\lemon-setup.exe', {
...
test('resolveStagedUpdaterBinary still returns a stale staged updater on Windows', () => {
  ...
  assert.equal(
    resolveStagedUpdaterBinary('C:\\Lemon AI', {
      fileExists: () => true,
      isWindows: true,
      stagedMtimeMs: () => MARKER_SELF_ADOPT_EPOCH_MS - 60 * DAY_MS
    }),
    path.join('C:\\Lemon AI', 'lemon-setup.exe')
  )
})
...
test('spawnUpdaterProcess hides the updater console and detaches the child on Windows', () => {
  ...
  const result = spawnUpdaterProcess(
    'lemon-setup.exe',
    ['--update', '--branch', 'main'],
    { cwd: 'C:\\Lemon AI', detached: true, stdio: 'ignore' },
    {
      isWindows: true,
  ...
  assert.deepEqual(calls, [
    {
      ...
      command: 'lemon-setup.exe',
      options: { cwd: 'C:\\Lemon AI', detached: true, stdio: 'ignore', windowsHide: true }
    }
  ])
})
...
test('resolveStagedUpdaterBinary hands Windows the staged installer it finds', () => {
  const home = 'C:\\Users\\lemon\\AppData\\Local\\lemon'
  const staged = path.join(home, 'lemon-setup.exe')
  const probed: string[] = []
...
test('resolveStagedUpdaterBinary prefers Lemon AI setup and falls back to legacy setup on Windows', () => {
  const home = 'C:\\Users\\lemon\\AppData\\Local\\Lemon AI'
  const lemon = path.join(home, 'lemon-ai-setup.exe')
  const legacy = path.join(home, 'hermes-setup.exe')
...
test('resolveStagedUpdaterBinary returns null off Windows even when lemon-setup is staged (#74836)', () => {
  const home = '/Users/lemon/.lemon-ai'
  let probes = 0

  const resolved = resolveStagedUpdaterBinary(home, {
    // The installer stages lemon-setup on macOS/Linux too, so "it exists" is
...
test('resolveStagedUpdaterBinary returns null on Windows when nothing is staged', () => {
  const resolved = resolveStagedUpdaterBinary('C:\\Users\\lemon\\AppData\\Local\\lemon', {
    fileExists: () => false,
    isWindows: true
  })
...
test('resolveUpdateScriptHandoff prefers the repo script on Windows when present', () => {
  const root = String.raw`C:\Users\lemon\AppData\Local\lemon\lemon-agent`
...
test('resolveUpdateScriptHandoff returns null when the checkout predates the script', () => {
  const handoff = resolveUpdateScriptHandoff(String.raw`C:\Users\lemon\AppData\Local\lemon\lemon-agent`, {
    isWindows: true,
    fileExists: () => false
  })
...
test('resolveUpdateScriptHandoff is Windows-only (POSIX updates in place)', () => {
  const handoff = resolveUpdateScriptHandoff('/home/lemon/.lemon-ai/lemon-agent', {
    isWindows: false,
    fileExists: () => true
  })
...
test('resolvePosixScriptHandoff returns the bash recipe when the script exists', () => {
  const root = '/home/lemon/.lemon-ai/lemon-agent'
...
test('resolvePosixScriptHandoff is null when the checkout predates the script', () => {
  const handoff = resolvePosixScriptHandoff('/home/lemon/.lemon-ai/lemon-agent', {
    isWindows: false,
    fileExists: () => false
  })
...
test('resolvePosixScriptHandoff is null on Windows', () => {
  const handoff = resolvePosixScriptHandoff(String.raw`C:\Users\lemon\AppData\Local\lemon\lemon-agent`, {
    isWindows: true,
    fileExists: () => true
  })
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--no-sandbox',
    'lemon://open/session/abc',
    '--profile=work'
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'lemon://open/session/abc', '--profile=work'])
