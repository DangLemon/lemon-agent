...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'lemon serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/lemon serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m lemon_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/lemon --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Lemon AI Runtime\\lemon.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('lemon chat --query serve'), false)
