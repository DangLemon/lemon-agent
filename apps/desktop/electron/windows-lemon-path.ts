/**
 * windows-lemon-path.ts
 *
 * Pure, dependency-injected pieces of Windows `lemon` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvLemonCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `lemon` shim shadowed the real
 *      lemon.cmd/lemon.exe; the shim then failed the --version probe and
 ...
 *      and Lemon AI launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvLemonCommand() — unwrapWindowsVenvLemonCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `lemon`) must
 * not shadow `lemon.cmd`/`lemon.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Lemon AI launcher that drives `lemon update`.
...
export interface ResolveVenvLemonCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportLemonCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `lemon`/`lemon.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/lemon(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m lemon_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportLemonCli(). A
 * venv whose update died mid-`pip install` still has python.exe + lemon.exe
 ...
 *
 * Returns null when `command` is not a venv lemon shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvLemonCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvLemonCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^lemon(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Lemon AI at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Lemon AI Python at ${python}`,
    command: python,
