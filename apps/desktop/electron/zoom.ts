...
export const ZOOM_STORAGE_KEY = 'lemon:desktop:zoomLevel'
 ...
 * but forgot to emit 'lemon:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('lemon:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `lemon:zoom:changed`, which never fired —
