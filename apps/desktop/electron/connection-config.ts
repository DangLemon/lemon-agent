 ...
 *     `X-Lemon-Session-Token` header; WS uses `?token=`.
...
// lemon_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - lemon_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - lemon_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / lemon #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-lemon_session_at', '__Secure-lemon_session_at', 'lemon_session_at']
const RT_COOKIE_VARIANTS = ['__Host-lemon_session_rt', '__Secure-lemon_session_rt', 'lemon_session_rt']

// The Nous portal (NAS) does NOT use Lemon AI gateway session cookies — it is a
...
// Keep this aligned with lemon_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['lemon', 'test', 'tmp', 'root', 'sudo'])
...
/** Which Lemon AI profile the remote SSH dashboard should actually run as.
...
// Lemon AI Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteLemonPath = String(entry.remoteLemonPath || '').trim()

  if (remoteLemonPath) {
    out.remoteLemonPath = remoteLemonPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Lemon AI installation. Preserve an explicit mapping
  // when it is a valid Lemon AI profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_lemon_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `LEMON_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process LEMON_HOME can belong to a different
 ...
 * `lemon:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://lemon.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `lemon:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a lemon session ACCESS-token cookie
 ...
 * the Lemon AI gateway session cookies, so cloud sign-in / discovery liveness
