 ...
 *
 * So we delegate to `lemon gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `lemon update`'s
...
export function stopGatewayBeforeUpdate(
  ...
  return runGatewayLifecycleCommand(lemonCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(lemonCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(lemonCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(lemonCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(lemonCliPath, args)
  }

  if (!existsSync(lemonCliPath)) {
    return false
  }

  try {
    exec(lemonCliPath, args, {
