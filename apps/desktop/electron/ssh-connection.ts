...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(LEMON_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Lemon-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.lemon-ai/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'lemon-desktop-ssh')
  }

  return path.join(os.homedir(), '.lemon-ai', 'desktop-ssh')
}
