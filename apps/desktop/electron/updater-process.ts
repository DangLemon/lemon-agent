 ...
 *
 * The staged Tauri `lemon-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `lemon update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `lemon update`, swaps/relaunches
 * the app, and writes .lemon-ai-update-result.json. With the app gone before
 * the update starts, the LEMON_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `lemon update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into LEMON_HOME on *every* platform
 * (`lemon-setup.exe` on Windows, `lemon-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_lemon_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `lemon update`,
...
export function resolveStagedUpdaterBinary(
  ...
  for (const name of stagedUpdaterNames) {
    const candidate = path.join(lemonHome, name)
 ...
 *
 * `copy_self_to_lemon_home` deliberately no-ops during `--update`
 ...
 * "Another Lemon AI update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `lemon update`, which
 ...
 * the installer is written to LEMON_HOME at install/repair time, making mtime
