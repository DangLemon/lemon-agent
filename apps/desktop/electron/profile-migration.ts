...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.lemon-ai`). Default's state.db and gateway.pid live here. */
  lemonHome: string
  /** Named-profile root (`~/.lemon-ai/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isLemonProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `lemonHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, lemonHome: string, profilesRoot: string): string {
  return name === 'default' ? `${lemonHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, lemonHome: string, profilesRoot: string): string {
  return name === 'default' ? `${lemonHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveLemonHome(profilesRoot: string, lemonHome?: string): string {
  if (lemonHome) {
    return lemonHome
  }

  // Tests that predate lemonHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live lemon process.
 ...
 *
 * `lemonHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isLemonProcess'> & { lemonHome?: string }
): string[] {
  const lemonHome = resolveLemonHome(profilesRoot, deps.lemonHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, lemonHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.lemonHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$LEMON_HOME`, not `$LEMON_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `lemon --profile default` and is worse than writing nothing
