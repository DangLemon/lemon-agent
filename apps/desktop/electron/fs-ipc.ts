...
// hardening, LEMON_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('lemon:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('lemon:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('lemon:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('lemon:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<LEMON_HOME>/desktop-plugins`,
  // resolved from the main-process LEMON_HOME (see resolveLemonHome) — NOT from
  // the connected backend. A remote backend reports its own `lemon_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped lemon_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(lemonHome, 'profiles', profile) : lemonHome
  ...
  ipcMain.handle('lemon:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<LEMON_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where LEMON_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('lemon:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<LEMON_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('lemon:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('lemon:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('lemon:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('lemon:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('lemon:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('lemon:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
