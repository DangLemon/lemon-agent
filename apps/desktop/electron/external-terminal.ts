// Launching the Lemon AI TUI in the user's OWN terminal emulator.
//
...
// `lemon --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Lemon AI runtime is often a venv Python invoked
//     as `python -m lemon_cli.main`, not a `lemon` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Lemon AI
