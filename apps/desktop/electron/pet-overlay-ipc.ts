...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('lemon:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('lemon:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('lemon:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('lemon:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('lemon:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('lemon:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('lemon:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('lemon:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('lemon:pet-overlay:control', payload)
  })
}
