 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Lemon AI dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Lemon AI install on the remote (login-shell probe),
 ...
 *     `LEMON_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForLemon in.
 *
 * The minted LEMON_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^LEMON_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.lemon-ai/desktop-ssh'
...
// Resolve the remote lemon executable. An EXPLICIT path is honored strictly
...
async function locateLemon(
  ...
  const resolveLauncher = async (candidate: string) => {
    ...
    //     the Lemon AI version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/DangLemon/lemon-agent/issues/74411
    return candidate
  }
  ...
  if (remoteLemonPath) {
    if (await isExecutable(remoteLemonPath)) {
      return resolveLauncher(remoteLemonPath)
    }

    const err: any = new Error(
      `The ${appName} path you set is not an executable on the remote host: "${remoteLemonPath}". ` +
        'Check the path (it must be the full path to the `lemon` binary on the remote, e.g. ' +
        `~/lemon-agent/.venv/bin/lemon), or clear it to auto-detect.`
    )

    err.kind = 'lemon-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v lemon')}`)).trim()
  ...
  candidates.push('~/.local/bin/lemon')
  candidates.push('/usr/local/bin/lemon')
  candidates.push('~/.lemon-ai/lemon-agent/venv/bin/lemon')
  ...
  const err: any = new Error(
    `${appName} is not installed on the remote host (could not find a \`lemon\` executable). ` +
...
// Probe the resolved binary's version string (first line of `<lemon> --version`,
// e.g. "Lemon AI v0.18.2 ..."), or '' on failure. Surfaces WHICH lemon a
// connection uses, so a stale/unexpected install is visible.
async function probeLemonVersion(ssh, lemonPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(lemonPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh, hostAppName = 'Lemon AI') {
...
// The LEMON_HOME the remote dashboard will use (explicit env wins, else
// ~/.lemon-ai). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteLemonHome(ssh, hostAppName = 'Lemon AI') {
  const appName = displayAppName(hostAppName)

  try {
    const out = (await ssh.exec('echo "${LEMON_HOME:-$HOME/.lemon-ai}"')).trim().split('\n').pop()
...
marker=home/'.lemon-ai-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Lemon AI checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, lemonHome, hostAppName = 'Lemon AI') {
  const appName = displayAppName(hostAppName)
  const home = assertSafeRemoteHome(lemonHome, hostAppName)
  let observation = ''
...
async function listRemoteLemonProfiles(ssh, hostAppName = 'Lemon AI') {
  const appName = displayAppName(hostAppName)
  const home = assertSafeRemoteHome(await probeRemoteLemonHome(ssh, hostAppName), hostAppName)
...
function assertSafeRemoteHome(home, hostAppName = 'Lemon AI') {
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'lemonPath', 'lemonHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !lemonPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(lemonPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/lemon`, leaving neither
      // launcher nor LEMON_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `lemon_home=os.path.expanduser(${shq(lemonHome)}) if ${shq(lemonHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if lemon_home:\n' +
      ' expected_entries.add(os.path.join(lemon_home,"lemon-agent","venv","bin","lemon"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.lemonPath)
  const expectedHome = lock.lemonHome ? expandRemotePath(lock.lemonHome) : "''"
  ...
  const executableMatch = lock.lemonHome
...
expected_path=os.path.expanduser(${py(lock.lemonPath)})
lemon_home=os.path.expanduser(${py(lock.lemonHome)})
expected_entries={expected_path,os.path.join(lemon_home,"lemon-agent","venv","bin","lemon")}
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"lemon-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(lemonPath, profile, opts: any = {}) {
  const lemon = expandRemotePath(lemonPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.lemonHome || '~/.lemon-ai')}/.lemon-ai-update-in-progress`)

  const updateMutex = `${remoteInstallRoot(opts.lemonHome || '~/.lemon-ai')}/.lemon-ai-update-in-progress.mutex`
  ...
  const dashCmd =
    ...
    `exec env LEMON_DESKTOP=1 ${lemon} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} lemon-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, lemonPath) {
  const lemon = expandRemotePath(lemonPath)

  const out = await ssh.exec(
    `help="$(${lemon} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, lemonPath))) {
    const err: any = new Error(
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(lemonPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        lemonHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForLemon, a token-gated
...
async function connect(deps) {
  ...
  const lemonHome = await probeRemoteLemonHome(ssh, hostAppName)
  await assertRemoteInstallUpdateClear(ssh, lemonHome, hostAppName)
  const lemonPath = await locateLemon(ssh, remoteLemonPath, deps.sourceRepository, hostAppName)
  log(`located lemon at ${lemonPath}`)
  const lemonVersion = await probeLemonVersion(ssh, lemonPath)

  if (lemonVersion) {
    log(`remote lemon version: ${lemonVersion}`)
  }
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.lemonPath === lemonPath &&
      lock.lemonHome === lemonHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    lemonHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, lemonHome, hostAppName),
    hostAppName
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForLemon(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
