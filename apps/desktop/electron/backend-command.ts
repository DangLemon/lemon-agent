// Backend subcommand routing for the desktop-managed Lemon AI process.
//
// The desktop app launches its own headless backend via `lemon serve` — it
...
// app hasn't updated yet, or an older `lemon` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Lemon AI profile to pin via `--profile`.
 */
 ...
 * `-m lemon_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `lemon_cli/subcommands/dashboard.py` source registers
