...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'lemon-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'lemon-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Lemon AI Test'], { cwd: dir })
