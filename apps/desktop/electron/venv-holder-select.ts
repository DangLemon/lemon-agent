 ...
 *
 * The pre-update handoff kills Lemon AI-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `lemon`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Lemon AI-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Lemon AI and holds venv shims mapped).
 */
export function isLemonOwnedVenvDaemon(
