...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/lemon', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/lemon/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/lemon'), 'https://gw.example.com/lemon/auth/native/refresh')
})
