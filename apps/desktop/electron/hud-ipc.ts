...
export function registerHudIpc({
  ...
  ipcMain.on('lemon:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('lemon:hud:windowing', event => {
  ...
  ipcMain.on('lemon:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('lemon:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('lemon:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('lemon:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('lemon:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('lemon:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('lemon:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('lemon:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('lemon:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('lemon:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('lemon:hud:close', async () => {
    closeHudWindow()
