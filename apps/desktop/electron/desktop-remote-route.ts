...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'LEMON_DESKTOP_REMOTE_URL is set but LEMON_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Lemon AI backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `lemon serve` the desktop never
