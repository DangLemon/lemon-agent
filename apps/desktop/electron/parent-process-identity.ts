export type ParentWatchdogEnv = {
  LEMON_PARENT_PID: string
  LEMON_PARENT_START_MARKER?: string
  LEMON_PARENT_NONCE?: string
  /** Spawn tag consumed by lemon_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { LEMON_PARENT_PID: String(pid) }
  env.LEMON_SPAWN = spawnTag(pid, startMarker)
  ...
  env.LEMON_PARENT_START_MARKER = startMarker
  env.LEMON_PARENT_NONCE = nonce

  return env
}

/** Build the `LEMON_SPAWN` tag mirrored by lemon_cli/process_identity.py.
