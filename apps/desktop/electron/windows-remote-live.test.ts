...
//   LEMON_WIN_SSH_HOST   ssh alias/host of the Windows box
//   LEMON_WIN_SSH_USER   remote user
//   LEMON_WIN_SSH_LEMON absolute path to the remote lemon.exe under test
const liveHost = process.env.LEMON_WIN_SSH_HOST || ''
const liveUser = process.env.LEMON_WIN_SSH_USER || ''
const configuredLemon = process.env.LEMON_WIN_SSH_LEMON || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Lemon-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredLemon)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredLemon.replace('lemon.exe', 'python.exe')}' -m lemon_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredLemon.replace('lemon.exe', 'python.exe')
        const terminate = `& '${python}' -m lemon_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.lemonPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m lemon_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m lemon_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
