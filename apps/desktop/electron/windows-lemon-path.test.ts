// Unit tests for the pure Windows `lemon` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvLemonCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `lemon` shim shadows
//      the real lemon.cmd/lemon.exe.
...
//   3. resolveVenvLemonCommand() — must probe the venv python via
//      canImportLemonCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvLemon: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvLemon: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvLemon: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvLemon: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvLemon: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvLemon: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvLemonCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportLemonCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    lemonHome: '/fake/lemon-home',
...
test('resolveVenvLemonCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvLemonCommand('/root/venv/Scripts/lemon.exe', [], deps), null)
})

test('resolveVenvLemonCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvLemonCommand('/root/venv/Scripts/lemon.cmd', [], deps), null)
})

test('resolveVenvLemonCommand: returns null when the basename is not lemon/lemon.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvLemonCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvLemonCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvLemonCommand('/root/venv/bin/lemon.exe', [], deps), null)
})

test('resolveVenvLemonCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvLemonCommand('/root/venv/Scripts/lemon.exe', [], deps), null)
})

test('resolveVenvLemonCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportLemonCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvLemonCommand('/root/venv/Scripts/lemon.exe', ['serve'], deps)
...
test('resolveVenvLemonCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvLemonCommand('/root/venv/Scripts/lemon.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'lemon_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvLemonCommand: is case-insensitive on lemon.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvLemonCommand('/root/venv/Scripts/LEMON.EXE', [], deps))
  assert.ok(resolveVenvLemonCommand('/root/venv/SCRIPTS/lemon.exe', [], deps))
})
