 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `lemon update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.lemonHome, 'Lemon AI home')
  const lemonPath = validateRemoteValue(target.lemonPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(lemonPath)

  const updateCommand =
    `env LEMON_HOME=${homeWord} ` +
    `LEMON_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'LEMON_UPDATE_ORIGIN_PROFILE=default ' +
    `LEMON_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `LEMON_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:LEMON_HOME=${psLiteral(home)}`,
    `$env:LEMON_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:LEMON_UPDATE_ORIGIN_PROFILE="default"',
    `$env:LEMON_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:LEMON_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:LEMON_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:LEMON_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:LEMON_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(lemonPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.lemon-ai-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Lemon AI updated and every managed SSH profile is ready.'
        : restoreOk
