...
test('decodingFileProbe rejects a missing file', () => {
  const missing = path.join(os.tmpdir(), `lemon-icon-missing-${process.pid}.png`)
  assert.equal(decodingFileProbe(missing), false)
})

test('decodingFileProbe rejects an existing but empty (0-byte) file', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'lemon-icon-'))
...
test('decodingFileProbe rejects a directory', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'lemon-icon-dir-'))
...
test('appIconCandidates keeps the documented precedence ladder', () => {
  const mac = appIconCandidates({
    isWindows: false,
    appRoot: '/Applications/Lemon AI.app/Contents/Resources',
    unpackedPathFor: p => `${p}.unpacked`
  })

  assert.deepEqual(mac, [
    path.join('/Applications/Lemon AI.app/Contents/Resources', 'public', 'apple-touch-icon.png'),
    path.join('/Applications/Lemon AI.app/Contents/Resources', 'dist', 'apple-touch-icon.png'),
    path.join('/Applications/Lemon AI.app/Contents/Resources.unpacked', 'dist', 'apple-touch-icon.png')
  ])
...
test('appIconCandidates selects Lemon runtime icons for the internal harness with Lemon AI fallbacks', () => {
  const mac = appIconCandidates({
  ...
  assert.deepEqual(mac, [
    path.join('/Applications/Lemon AI.app/Contents/Resources', 'public', 'lemon-apple-touch-icon.png'),
    path.join('/Applications/Lemon AI.app/Contents/Resources', 'dist', 'lemon-apple-touch-icon.png'),
    path.join('/Applications/Lemon AI.app/Contents/Resources.unpacked', 'dist', 'lemon-apple-touch-icon.png'),
    path.join('/Applications/Lemon AI.app/Contents/Resources', 'public', 'apple-touch-icon.png'),
    path.join('/Applications/Lemon AI.app/Contents/Resources', 'dist', 'apple-touch-icon.png'),
    path.join('/Applications/Lemon AI.app/Contents/Resources.unpacked', 'dist', 'apple-touch-icon.png')
  ])
