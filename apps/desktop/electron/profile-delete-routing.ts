// Profile-delete routing logic for the `lemon:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_lemon_home() and
...
/** Parse a `lemon:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its LEMON_HOME.
 *
...
/**
 * Route the next `lemon:api` request away from the primary/window backend
 ...
 * ensure_lemon_home() recreates the directory the delete just removed.
 */
