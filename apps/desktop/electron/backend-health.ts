...
export interface LemonReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForLemonReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Lemon AI gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForLemonReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed Lemon AI Cloud instance
 ...
 * `isReauthRequired` so startLemon latches; `needsOauthLogin` alone only
...
export async function waitForLemonReady(baseUrl: string, options: LemonReadyOptions): Promise<void> {
  ...
  throw new Error(`Lemon AI backend did not become ready: ${detail}`)
}
