...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `lemon-dev-mock-${Date.now()}`))
  ...
  return { root, lemonHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(lemonHome, mockUrl) {
  fs.writeFileSync(
    path.join(lemonHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(lemonHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.lemonHome, mock.url)
  console.log(`  LEMON_HOME: ${sandbox.lemonHome}`)
  ...
  const env = {
    ...
    LEMON_DESKTOP_APP_NAME: `LemonDevMock-${Date.now()}`,
  }
