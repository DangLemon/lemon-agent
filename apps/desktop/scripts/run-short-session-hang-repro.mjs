...
const UPSTREAM_URL = 'https://github.com/DangLemon/lemon-agent.git'
...
const LEGACY_STORAGE_KEY = 'lemon.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'lemon.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.lemonDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `lemon-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const lemonHome = join(sandbox, 'lemon-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(lemonHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(lemonHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        LEMON_DESKTOP_APP_NAME: `LemonShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        LEMON_DESKTOP_LEMON_ROOT: target.targetRoot,
        LEMON_DESKTOP_IGNORE_EXISTING: '1',
        LEMON_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'lemon-short-session-ab-'))
  const prepared = []
