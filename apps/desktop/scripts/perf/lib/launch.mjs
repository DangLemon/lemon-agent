...
//   · its own LEMON_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · LEMON_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated LEMON_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate LEMON_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = lemonHome ?? mkTemp('lemon-perf-home-')
  const userData = userDataDir ?? mkTemp('lemon-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !lemonHome) {
    seedConfigFrom(join(homedir(), '.lemon-ai'), home)
  }
  ...
  try {
    ...
    // LEMON_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set LEMON_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...
      LEMON_DESKTOP_CDP_PORT: String(port),
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.LEMON_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'lemon-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'lemon-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.lemon-ai'), home)
