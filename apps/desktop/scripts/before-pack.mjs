 ...
 * product name (`Lemon AI`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Lemon AI'
 *
 * This is a hard failure with no obvious cause for the user — `lemon desktop`
 ...
 * (the mac-unpacked Lemon AI.app bundle) and Windows (win-unpacked), so we
 ...
 * Lemon AI.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `lemon desktop --build-only` (lemon_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Lemon AI.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Lemon AI'}.exe`
