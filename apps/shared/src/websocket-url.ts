...
export async function resolveGatewayWsUrl(deps: ResolveGatewayWsUrlDeps, conn: GatewayWsConnection): Promise<string> {
  ...
  if (conn.authMode === 'oauth') {
    if (!mint) {
      throw new Error('This Desktop build cannot refresh OAuth WebSocket tickets. Update Lemon AI and try again.')
    }
...
export interface LemonWebSocketUrlOptions {
...
export function buildLemonWebSocketUrl(options: LemonWebSocketUrlOptions): string {
