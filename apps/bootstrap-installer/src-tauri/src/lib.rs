//! Lemon AI Setup — Tauri entrypoint.
//!
...
pub fn run() {
    // Tracing → bootstrap-installer.log under LEMON_HOME/logs/ so install
    ...
    // Lemon AI is already installed, so users can re-run setup to repair a broken
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Lemon AI is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Lemon AI" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Lemon AI" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_lemon_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                ...
                if bootstrap::lemon_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_lemon_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "lemon already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_lemon_desktop,
            // Diagnostics
            paths::get_log_path,
            paths::get_lemon_home,
