...
# ladders, parked-server self-probes and repeated `lemon mcp test` runs re-run the preflight
...
# The cache is also persisted under the Lemon AI home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``lemon_constants.get_lemon_home()`` so the cache follows the
    active profile and is isolated across Lemon AI homes. The cache directory
    is created on demand. Returns ``None`` when Lemon AI home cannot be
...
def _query_osv(package: str, ecosystem: str, version: Optional[str] = None) -> list:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "lemon-agent-osv-check/1.0"},
        method="POST")
