...
class MCPServerHealthMixin:
    ...
    def _make_logging_callback(self):
        """``logging_callback`` forwarding server ``notifications/message`` into Lemon AI logging (SDK default drops them).

        Routes MCP ``notifications/message`` log notifications from the server into Lemon AI' logging
        (agent.log via lemon_logging), tagged with the server name. Without this, the SDK's default
