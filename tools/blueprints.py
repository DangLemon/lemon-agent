...
``metadata.lemon.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.lemon.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.lemon.blueprint`` is a mapping with a
    ...
    lemon = meta.get("lemon") if isinstance(meta, dict) else None
    blueprint = lemon.get("blueprint") if isinstance(lemon, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.lemon.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.lemon.blueprint`` block) ready for ``lemon skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"lemon": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
