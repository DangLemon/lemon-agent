...
def _home_candidates() -> list:
    ...
    try:
        ...
        candidates.append(Path(get_lemon_home()) / "bin" / "lightpanda")
    except Exception as e:  # pragma: no cover - defensive
        logger.debug("lemon home unavailable for lightpanda lookup: %s", e)
    return candidates
...
def find_lightpanda_binary() -> Optional[str]:
    ...
    fallbacks), then installer/agent-browser locations, then ``$LEMON_HOME/bin``. No Windows build."""
    if os.name == "nt":
...
def _state_dir() -> Path:
    from lemon_constants import get_lemon_home
    path = Path(get_lemon_home()) / "cache" / "browser-use" / "lightpanda"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _http_cache_dir() -> Path:
    """Filesystem HTTP cache shared by every Lightpanda this Lemon AI spawns.
    ...
    bounds it without Lemon AI managing eviction.
    """
...
def _cdp_ready(url: str, timeout: float = 0.2) -> bool:
    try:
        from lemon_cli.browser_connect import is_browser_debug_ready
    ...
    if not binary:
        if os.name == "nt":
            return None, ("browser.engine is 'lightpanda' but Lightpanda has no Windows "
                          "build. Set browser.engine to auto (or run Lemon AI under WSL2).")
...
def reap_orphaned_lightpanda() -> int:
    """Kill ``lightpanda serve`` processes whose owning Lemon AI is gone; return the count. A live owner is
