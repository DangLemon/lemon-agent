...
_LEMON_CONFIG_FILES = r'\.lemon-ai/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.lemon-ai/\.env|\~/\.lemon-ai/\.env',
     "lemon_env_access", "critical", "exfiltration", "directly references Lemon AI secrets file"),
    ...
    # quarantine only acts on "dangerous") but high for Lemon AI/other config (setup docs routinely say
    ...
    (_prose_modify_re(_LEMON_CONFIG_FILES),
     "lemon_config_mod", "high", "persistence", "modification language aimed at Lemon AI configuration files (verify intent)"),
    (_shell_write_re(_LEMON_CONFIG_FILES),
     "lemon_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Lemon AI configuration files"),
    (r'\.lemon-ai/config\.yaml|\.lemon-ai/SOUL\.md',
     "lemon_config_ref", "low", "persistence", "references Lemon AI configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Lemon AI-native; `.clawhubignore` is honored for skills published through ClawHub.
