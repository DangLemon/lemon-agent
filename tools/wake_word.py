"""Wake-word ("Hey Lemon AI") detection — hands-free session trigger.
...
# Minimum gap between two wake fires, so one "hey lemon" can't retrigger across
...
_DEFAULTS: Dict[str, Any] = {
    ...
    "provider": "openwakeword", "phrase": "hey lemon", "sensitivity": 0.6,
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey lemon"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    with suppress(Exception):
        ...
        from lemon_cli.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return ("Microphone delivers only silence. Grant the Lemon AI backend "
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    ladder = (
        ...
        (bool(missing), lambda: (f"Wake word needs {missing} configured — run `lemon tools` "
...
# ── Process-wide singleton (mirrors lemon_cli.voice's continuous API) ──
...
def _lock_path() -> Path:
    ...
    return get_default_lemon_root() / "runtime" / "wake-word.lock"
