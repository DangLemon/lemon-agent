"""Skills Hub discovery: the centralized Lemon AI index fetch (cached, stale-
...
from tools.skills_hub_official import LemonIndexSource, OptionalSkillSource
...
def _lemon_index_cache_file() -> Path:
    from tools.skills_hub import _index_cache_dir
    return _index_cache_dir() / "lemon-index.json"


def _load_lemon_index() -> Optional[dict]:
    """Fetch the centralized skills index (docs site, rebuilt daily), cached
    locally for LEMON_INDEX_TTL; on any failure serve the stale cache.
    ...
    cache_file = _lemon_index_cache_file()
    cached = _read_json_if_fresh(cache_file, LEMON_INDEX_TTL)
    if cached is not None:
        return cached
    data = None
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            resp = httpx.get(LEMON_INDEX_URL, timeout=15, follow_redirects=True,
                             headers={"Accept-Encoding": accept_encoding})
            if resp.status_code != 200:
                logger.debug("Lemon AI index fetch returned %d", resp.status_code)
                return _load_stale_index_cache()
            data = resp.json()
            break
        except httpx.DecodingError as e:
            logger.debug("Lemon AI index decode failed (Accept-Encoding=%s): %s", accept_encoding, e)
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Lemon AI index fetch failed: %s", e)
            return _load_stale_index_cache()
...
def _load_stale_index_cache() -> Optional[dict]:
    ...
    from tools.skills_hub import _read_json_if_fresh
    return _read_json_if_fresh(_lemon_index_cache_file(), float("inf"))
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    return [
        ...
        LemonIndexSource(auth=auth),     # centralized index (search + resolved install paths)
...
def _select_active_sources(sources: List[SkillSource], source_filter: str) -> List[SkillSource]:
    ...
    index_available = effective == "all" and any(
        src.source_id() == "lemon-index" and getattr(src, "is_available", False) for src in sources
    )
