...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    lemon_meta = meta.get("lemon") if isinstance(meta, dict) else None
    if not isinstance(lemon_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.lemon.{tags, related_skills}.")
    elif "tags" not in lemon_meta:
        yield _warn("missing-metadata", "metadata.lemon.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("lemon", "agent", "lemon agent") and (
        author != "Lemon AI"):
        yield _warn("author-caps", f"author '{author}' should be 'Lemon AI' (proper caps) "
