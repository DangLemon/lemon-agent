...
refresh; this module supplies ``LemonTokenStorage`` (on-disk persistence), the localhost callback
listener and ``build_oauth_auth()`` (legacy entry point). client_id is Lemon AI' Client ID Metadata
...
from lemon_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__lemon_user_skipped__"


def _get_token_dir(lemon_home: str | Path | None = None) -> Path:
    """``LEMON_HOME/mcp-tokens/`` — per-profile token directory."""
    from lemon_constants import get_lemon_home

    return Path(lemon_home if lemon_home is not None else get_lemon_home()) / "mcp-tokens"
...
def _cached_client_info(storage: "LemonTokenStorage | None") -> dict | None:
...
def _cached_redirect(storage: "LemonTokenStorage | None") -> "tuple[str | None, int | None]":
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    ``lead`` is the boundary-specific first sentence; this helper appends the shared, actionable ``lemon
    mcp login`` next-step so the guidance wording lives in one place across every non-interactive OAuth
    boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `lemon mcp login <server>` interactively to (re)authorize, then restart or reload the gateway."
        )
...
def _write_json(path: Path, data: dict) -> None:
    ...
    # secure_parent_dir refuses to chmod /, top-level dirs, or the lemon-agent install tree (#25821,
    # #93050).
    ...
    # lemon-agent install tree (#25821, #93050).
...
class LemonTokenStorage:
    """Persist OAuth state as ``LEMON_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ``.client.json`` (client info), ``.meta.json`` (server metadata), ``.cimd-off`` (CIMD refused)."""

    def __init__(self, server_name: str, *, lemon_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._lemon_home = Path(lemon_home) if lemon_home is not None else None

    def _path(self, suffix: str) -> Path:
        return _get_token_dir(self._lemon_home) / f"{self._server_name}{suffix}"
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._lemon_home)
...
def _make_callback_handler() -> tuple[type, dict]:
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = ("<h2>Authorization Successful</h2><p>You can close this tab and return to Lemon AI.</p>" if parsed["code"]
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `lemon mcp login <server>` later to authenticate, "
...
_SSH_HINT_LOOPBACK = (
    ...
    "\n"
    "  See: https://github.com/DangLemon/lemon-agent/docs/guides/oauth-over-ssh\n")
...
def _callback_outcome(result: dict, cimd_url: str | None):
    ...
    if result["auth_code"] is None:
        hint = (
            ...
            f"server rejected Lemon AI' Client ID Metadata Document ({cimd_url}); set ``cimd: false`` under that "
...
LemonOAuthClientProvider: Any = None


def remove_oauth_tokens(server_name: str, *, lemon_home: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    LemonTokenStorage(server_name, lemon_home=lemon_home).remove()
    logger.info("OAuth tokens removed for '%s'", server_name)


# CIMD (OAuth Client ID Metadata Documents): the client_id IS an HTTPS URL the server fetches for our
# name/logo/redirect URIs, replacing per-install DCR. The SDK does the protocol; Lemon AI only decides
# eligibility. Published from ``website/static/oauth/client-metadata.json``; the github.io origin is
# deliberate — servers MUST NOT follow redirects when fetching it, and github.com/DangLemon/lemon-agent/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = "https://nousresearch.github.io/lemon-agent/docs/oauth/client-metadata.json"
...
def _server_declined_cimd(storage: "LemonTokenStorage | None") -> bool:
    """True when cached metadata shows this server doesn't advertise CIMD. The SDK decides CIMD vs DCR
    in its 401 branch — after Lemon AI must fix the redirect URI — so cached metadata closes the gap;
...
def _maybe_use_cimd(cfg: dict, storage: "LemonTokenStorage | None" = None) -> "tuple[str, int] | None":
...
def _configure_callback_port(cfg: dict, storage: "LemonTokenStorage | None" = None) -> int:
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    metadata_kwargs: dict[str, Any] = {
        "client_name": cfg.get("client_name", "Lemon AI"),
...
def _invalidate_tokens_on_client_change(
    storage: "LemonTokenStorage", new_client_id: str, new_client_secret: str | None) -> None:
    ...
    if removed:
        logger.warning(
            ...
            "the previous client. Re-authorize with: lemon mcp login %s",
            storage._server_name, old_client_id, new_client_id, storage._server_name)


def _maybe_preregister_client(storage: "LemonTokenStorage", cfg: dict, client_metadata: "OAuthClientMetadata") -> None:
...
def humanize_oauth_registration_error(
    ...
    when the user overrode it or an older Lemon AI is running."""
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None = None) -> "OAuthClientProvider | None":
    ...
    global LemonOAuthClientProvider
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"requires browser authorization. Run `lemon mcp login {server_name}` interactively first to complete "
            "initial authorization, then cached tokens will be reused.")
    kwargs = build_provider_kwargs(cfg, storage, ssh_proxy_hint=True)
    if LemonOAuthClientProvider is None:
        from tools.mcp_oauth_provider import LemonProviderMixin

        LemonOAuthClientProvider = type("LemonOAuthClientProvider", (LemonProviderMixin, _sdk_class("OAuthClientProvider")), {
            "__doc__": "SDK provider plus Lemon AI' token-endpoint fixes (see ``LemonProviderMixin``).",
            "__module__": __name__, "_lemon_logger": logger})
    return LemonOAuthClientProvider(server_url=server_url, **kwargs)
