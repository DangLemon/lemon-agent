...
GitHub releases to $LEMON_HOME/bin/tirith in a background thread -- SHA-256 always verified,
...
def _failure_marker_path() -> str:
    return os.path.join(str(get_lemon_home()), ".tirith-install-failed")
...
# --- Auto-install ---
def _lemon_bin_dir() -> str:
    """$LEMON_HOME/bin, created if needed."""
    os.makedirs(d := os.path.join(str(get_lemon_home()), "bin"), exist_ok=True)
    return d
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $LEMON_HOME/bin/tirith -> ``(installed_path,
    ...
    try:
        ...
        if src is None:
            return None, reason
        dest = os.path.join(_lemon_bin_dir(), "tirith")
        try:
...
def _find_local_tirith() -> str | None:
    """Cheap local lookup for the default "tirith": PATH, then $LEMON_HOME/bin."""
    lemon_bin = os.path.join(_lemon_bin_dir(), "tirith")
    return shutil.which("tirith") or (lemon_bin if _is_executable(lemon_bin) else None)
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    $LEMON_HOME/bin/tirith → install; failures cached in-process and on disk for 24h). On a miss
