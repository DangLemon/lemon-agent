...
is ALWAYS host-side ``$LEMON_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
def get_spillover_dir():
    """Return $LEMON_HOME/cache/spillover as a Path (not created)."""
    from lemon_constants import get_lemon_home
    return get_lemon_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $LEMON_HOME/cache/spillover; returns path str or None."""
    try:
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/lemon-results" if temp_dir else STORAGE_DIR
...
HEREDOC_MARKER = "LEMON_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
