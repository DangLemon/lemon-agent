# Lemon AI Security Policy

This document describes Lemon AI's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/DangLemon/lemon-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Lemon AI does not operate a bug
bounty program.**
...
- Environment details (`lemon --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Lemon AI access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Lemon AI's documentation
...
Lemon AI supports two OS-level isolation postures. They address
...
Lemon AI supports this in two ways:

- **Lemon AI's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Lemon AI's in-process heuristics
...
Lemon AI filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Lemon AI
itself. Bugs in Lemon AI's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Lemon AI does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Lemon AI's own documentation, or reasonable
  operator expectations would predict — including cases where
  Lemon AI has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  lemon-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Lemon AI's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Lemon AI. Bugs in Lemon AI
...
- Lemon AI includes supply-chain guards for MCP server
