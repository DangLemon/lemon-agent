...
A cron job that restarts/stops the gateway from inside the gateway (``lemon gateway restart``,
``launchctl kickstart ai.lemon.gateway``, ``systemctl restart lemon-gateway``) kills the process,
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `lemon gateway` ops. `start` is excluded: starting from inside a
    # gateway is benign and a job may legitimately start a sibling profile. The lookbehind keeps
    # `lemon` from being a path component or word tail (`/docs/lemon gateway restart-notes.md`)
    # while every real command position (text start, whitespace, `;`/`&`/`|`, `$(`, backtick,
    # U+FFFD) still matches.
    # See #77173.
    r"(?:(?<![/\w.\-])(?:lemon|hermes)\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops anchored on a lemon-gateway label so unrelated lemon services stay
    ...
    # -l ai.lemon.gateway-<suffix> -- <helper-script>` (or `launchctl bootstrap gui/<uid> <plist>`) creates
    ...
    # Branch C: systemctl ops on a lemon-gateway unit.
...
# split across continuation lines (e.g. `launchctl submit \` / `  -l ai.lemon.gateway-... \` / `  -- ...`,
...
# Branch A2: `lemon -p <profile> gateway restart|stop` (also `--profile <name>` /
...
# `lemon -p venus gateway restart` operates on a sibling profile's gateway and is a legitimate fleet
...
# `$label` next to the verb. These verbs act on an EXISTING job, so the lemon-gateway label anchor
...
_LEMON_GATEWAY_LABEL_RE = re.compile(r"(?i)\blemon[.\-]?gateway\b")
...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard: ``LEMON_PROFILE_NAME``/``LEMON_PROFILE`` env first, then
    ``lemon_cli.profiles.get_active_profile_name`` (from ``LEMON_HOME``); ``None`` if neither."""
    for env_name in ("LEMON_PROFILE_NAME", "LEMON_PROFILE"):
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _LEMON_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    ``lemon``/``gateway`` token) — this function is the single choke point
    ...
    # <<'EOF'`) are masked before scanning (#88336): a runbook line "a human can run: lemon gateway
    ...
    # Profile-flag form (#78028): `lemon -p <profile> gateway restart|stop` bypasses Branch A because the
    # selector sits between `lemon` and `gateway`. It is only the same foot-gun when the named profile IS
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<LEMON_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_lemon_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_lemon_home() falls back to Path.home(), which raises when neither LEMON_HOME nor
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str] = None) -> None:
    ...
    if script:
        ...
        if resolved_script is not None and _on_cloud_path(resolved_script):
            raise GatewayLifecycleBlocked(
                ...
                "(e.g. ~/.lemon-ai/scripts/) and recreate the job."
            )
    ...
    if python_script:
        ...
        # The direct command regex below still scans the full text, so a literal `lemon gateway restart`
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `lemon gateway restart` from a shell outside "
