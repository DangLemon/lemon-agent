...
from lemon_state_common import _RECOVERABLE_END_REASONS_SQL, _RESET_END_REASONS_SQL, _sql_session_last_active
...
class SessionGatewayMixin:
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        from lemon_state import psutil
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from lemon_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
        now = time.time()
        candidates = []
