...
def _is_repo_junk(root: str) -> bool:
    ...
    LEMON_HOME. User-created projects pointing there are still honored."""
    if not root:
        return True
    from lemon_constants import get_lemon_home
    real = os.path.realpath(root)
    lemon_home = os.path.realpath(str(get_lemon_home()))
    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == lemon_home
        or real.startswith(lemon_home + os.sep))


def _is_session_cwd_junk(cwd: str) -> bool:
    """A non-git cwd that stays in flat Recents. A DESCENDANT of LEMON_HOME may be an
    intentional prose/data workspace, so only LEMON_HOME itself is excluded here."""
    if not cwd:
        return True
    from lemon_constants import get_lemon_home
    real = os.path.normcase(os.path.realpath(cwd))
    lemon_home = os.path.normcase(os.path.realpath(str(get_lemon_home())))
    return real in _non_workspace_dirs() or real == lemon_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    host must scan its own disk so repos with zero Lemon AI sessions still appear in the sidebar (#81723).
