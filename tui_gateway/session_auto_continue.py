...
def _session_home(session: dict) -> Path:
    """The LEMON_HOME the session's durable state lives in (profile-aware)."""
    return Path(session.get("profile_home") or _lemon_home)
...
def _maybe_schedule_auto_continue(sid: str, session: dict, session_key: str) -> dict | None:
    ...
    def kickoff() -> None:
        ...
        # Ownership admission BEFORE message.start: a sibling backend sharing this LEMON_HOME may have written the
