"""
Lemon AI MCP Server — expose messaging conversations as MCP tools (`lemon mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Lemon AI-specific
channels_list. Client config: {"mcpServers": {"lemon": {"command": "lemon", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("lemon.mcp_serve")
...
def _lemon_home() -> Path:
    try:
        ...
        return Path(os.environ.get("LEMON_HOME", Path.home() / ".lemon-ai"))


def _get_sessions_dir() -> Path:
    return _lemon_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_lemon_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_lemon_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Lemon AI analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Lemon AI MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("lemon", instructions=(
        "Lemon AI messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Lemon AI MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
