...
> **Heads up:** Native Windows runs Lemon AI without WSL — CLI, gateway, TUI, and tools all work natively. If you'd rather use WSL2, the Linux/macOS one-liner above works there too. Found a bug? Please [file issues](https://github.com/DangLemon/lemon-agent/issues).
...
If your antivirus (Bitdefender, Windows Defender, etc.) quarantines `uv.exe` from the Lemon AI `bin` folder (`%LOCALAPPDATA%\Lemon AI\bin\uv.exe`), this is a **false positive**. The file is Astral's `uv` — the Rust Python package manager Lemon AI bundles to manage its Python environment. ML-based antivirus engines commonly flag unsigned Rust binaries that download and install packages.
...
# Run verification
$uv = "$env:LOCALAPPDATA\lemon\bin\uv.exe"
...
**To whitelist Lemon AI:**
- **Windows Defender:** Run PowerShell as Admin → `Add-MpPreference -ExclusionPath "$env:LOCALAPPDATA\lemon\bin"`
- **Bitdefender:** Add an exception in the Bitdefender console (Protection > Antivirus > Settings > Manage Exceptions)
- Whitelist the **folder**, not the file hash — Lemon AI updates `uv` and the hash changes every version
...
```bash
lemon              # Interactive CLI — start a conversation
...
lemon config get   # Print individual config values
lemon gateway      # Start the messaging gateway (Telegram, Discord, etc.)
lemon setup        # Run the full setup wizard (configures everything at once)
lemon claw migrate # Migrate from OpenClaw (if coming from OpenClaw)
lemon update       # Update to the latest version
...
Lemon AI works with whatever provider you want — that's not changing. But if you'd rather not collect five separate API keys for the model, web search, image generation, TTS, and a cloud browser, **[Nous Portal](https://portal.nousresearch.com)** covers all of them under one subscription:
...
Lemon AI has two entry points: start the terminal UI with `lemon`, or run the gateway and talk to it from Telegram, Discord, Slack, WhatsApp, Signal, or Email. Once you're in a conversation, many slash commands are shared across both interfaces.

| Action                         | CLI                                           | Messaging platforms                                                              |
| ------------------------------ | --------------------------------------------- | -------------------------------------------------------------------------------- |
| Start chatting                 | `lemon`                                      | Run `lemon gateway setup` + `lemon gateway start`, then send the bot a message |
...
If you're coming from OpenClaw, Lemon AI can automatically import your settings, memories, skills, and API keys.

**During first-time setup:** The setup wizard (`lemon setup`) automatically detects `~/.openclaw` and offers to migrate before configuration begins.

**Anytime after install:**

```bash
lemon claw migrate              # Interactive migration (full preset)
...
- **Skills** — user-created skills → `~/.lemon-ai/skills/openclaw-imports/`
...
See `lemon claw migrate --help` for all options, or use the `openclaw-migration` skill for an interactive agent-guided migration with dry-run previews.
...
`~/.lemon-ai/lemon-agent`). This matches the layout used by `lemon update`, the
...
source ~/.lemon-ai/venvs/lemon-dev/bin/activate
uv pip install -e ".[all,dev]"
scripts/run_tests.sh
```
...
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — Linux desktop-control MCP server for Lemon AI and other MCP hosts, with AT-SPI accessibility trees, Wayland/X11 input, screenshots, and compositor window targeting.
- 🔌 [LemonClaw](https://github.com/AaronWong1999/lemonclaw) — Community WeChat bridge: Run Lemon AI and OpenClaw on the same WeChat account.
