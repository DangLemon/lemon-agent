"""Timezone-aware clock for Lemon AI.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``LEMON_TIMEZONE`` env var, then ``timezone`` in ``~/.lemon-ai/config.yaml``, else server-local
...
# LEMON_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _timezone_cache_identity() -> Tuple[str, str]:
    tz_env = os.getenv("LEMON_TIMEZONE", "").strip()
