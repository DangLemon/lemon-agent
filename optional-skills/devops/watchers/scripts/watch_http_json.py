...
Usage (via cron with --no-agent):
    ...
    lemon cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$LEMON_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Lemon AI-Watcher/1.0"})
