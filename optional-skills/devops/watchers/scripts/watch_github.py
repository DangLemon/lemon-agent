...
Usage (via cron with --no-agent):
    ...
    lemon cron create lemon-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$LEMON_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name lemon-issues --repo DangLemon/lemon-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Lemon AI .env file
(``${LEMON_HOME:-~/.lemon-ai}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Lemon AI-Watcher/1.0",
    }
