...
is typically only in `~/.lemon-ai/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Lemon AI,
...
`lemon-custom-providers` skill, pitfall 2). Lemon AI hits this because it forwards
