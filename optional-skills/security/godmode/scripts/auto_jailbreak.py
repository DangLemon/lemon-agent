...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("LEMON_HOME", os.path.expanduser("~/.lemon-ai")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("LEMON_HOME", Path.home() / ".lemon-ai")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Lemon AI config paths
# ═══════════════════════════════════════════════════════════════════

LEMON_HOME = Path(os.getenv("LEMON_HOME", Path.home() / ".lemon-ai"))
CONFIG_PATH = LEMON_HOME / "config.yaml"
PREFILL_PATH = LEMON_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Lemon AI models — already uncensored, just needs clean prompt
    "lemon": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "lemon" in model_lower or "nous" in model_lower:
        return "lemon"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Lemon AI config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.lemon-ai/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Lemon AI for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Lemon AI for changes to take effect.")
