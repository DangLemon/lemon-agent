...
All per-subject data lives under PDD_DATA_DIR (default: $LEMON_HOME/unbroker),
which is the same trust boundary Lemon AI uses for .env and OAuth tokens.
"""
...
def lemon_home() -> Path:
    return Path(os.environ.get("LEMON_HOME") or (Path.home() / ".lemon-ai"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else lemon_home() / "unbroker"
