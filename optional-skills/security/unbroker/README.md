...
The design is **Lemon AI-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
lemon skills install official/security/unbroker
```

Then start a new Lemon AI session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Lemon AI session:
...
- **PII handling.** Dossiers live under the Lemon AI home directory (`0600`, optionally
