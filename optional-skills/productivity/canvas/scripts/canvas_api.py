#!/usr/bin/env python3
"""Canvas LMS API CLI for Lemon AI.
...
def _check_config():
    ...
    if missing:
        lemon_env = os.path.join(
            os.environ.get("LEMON_HOME", os.path.expanduser("~/.lemon-ai")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {lemon_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Lemon AI"
    )
