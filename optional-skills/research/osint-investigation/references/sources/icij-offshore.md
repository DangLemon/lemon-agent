...
- **Note:** The previous Open Refine reconciliation endpoint at
  ...
  the ZIP locally (default `~/.cache/lemon-osint/icij/`, refreshes after
...
First call downloads the ~70 MB ZIP under `~/.cache/lemon-osint/icij/`
(or `$LEMON_OSINT_CACHE/icij/`). Subsequent calls reuse the cache for 30 days.
