...
Usage:
  ...
  <server_name> matches the files in $LEMON_HOME/mcp-tokens/<server>.json etc.
...
def _lemon_home():
    # Prefer Lemon AI' own resolver (profile-safe); fall back to env then ~/.lemon-ai.
    try:
        from lemon_constants import get_lemon_home
        return str(get_lemon_home())
    except Exception:
        return os.environ.get("LEMON_HOME") or os.path.expanduser("~/.lemon-ai")


def _tokens_dir():
    return os.path.join(_lemon_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "lemon-diag", "version": "1.0"}},
        },
        headers={
