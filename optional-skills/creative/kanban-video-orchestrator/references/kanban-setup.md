...
the actual `setup.sh` that creates the project workspace, configures Lemon AI
...
2. **Create profiles** — `lemon profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.lemon-ai/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `lemon kanban create` assigned to the director
...
```bash
lemon profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.lemon-ai/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.lemon-ai/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Lemon AI
...
Each profile gets a `SOUL.md` at `~/.lemon-ai/profiles/<name>/SOUL.md` that
...
```bash
lemon kanban create "Direct production of <video title>" \
    --assignee director \
...
the Lemon AI `.env` (`${LEMON_HOME:-$HOME/.lemon-ai}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _lemon_env="${LEMON_HOME:-$HOME/.lemon-ai}/.env"
    if grep -q "^${var}=" "$_lemon_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_lemon_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_lemon_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY lemon ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY lemon OPENROUTER_API_KEY || exit 1
# ...
```
