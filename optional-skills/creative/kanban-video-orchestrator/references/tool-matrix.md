...
Maps each role archetype to the Lemon AI skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public lemon-agent
...
## Lemon AI skills relevant to video production

### Visual / rendering skills (`lemon-agent/skills/creative/`)
...
### Design / pre-production skills (`lemon-agent/skills/creative/`)
...
### Audio / media skills (`lemon-agent/skills/creative/` + `skills/media/`)
...
These are **not** Lemon AI skills but external CLIs / APIs that profiles invoke.
...
## Built-in Lemon AI tools for media review

These are native Lemon AI tools — not invoked via terminal but through their own
...
key is present in `${LEMON_HOME:-~/.lemon-ai}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Lemon AI profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${LEMON_HOME:-~/.lemon-ai}/.env` → environment variable.
...
When a new Lemon AI-public video skill ships:
