...
### 1. Start order: editor first, Lemon AI second

Lemon AI probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Lemon AI session.
...
restart the editor, restart the Lemon AI session.
...
server fails to bind (Output Log) or Lemon AI' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.lemon-ai/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Lemon AI

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Lemon AI' connection
lives in `~/.lemon-ai/config.yaml` via `lemon mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Lemon AI — just don't
mistake it for the Lemon AI setup step.
...
explicitly warns against overlapping calls. Lemon AI executes same-turn tool
...
### 9. Timeouts: Lemon AI gives up before Unreal does

Lemon AI' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Lemon AI runs elsewhere (SSH backend,
