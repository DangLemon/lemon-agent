...
## 5. Lemon AI Configuration

### Config location

`$LEMON_HOME/config.yaml` (defaults to `~/.lemon-ai/config.yaml` when `LEMON_HOME` is unset)
...
Restart the Lemon AI session for changes to take effect. The MCP connection is
