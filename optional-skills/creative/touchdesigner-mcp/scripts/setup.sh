...
LEMON_HOME_DIR="${LEMON_HOME:-$HOME/.lemon-ai}"
LEMON_CFG="${LEMON_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Lemon AI config has twozero_td MCP entry ──
if [[ ! -f "$LEMON_CFG" ]]; then
    echo -e " ${FAIL} Lemon AI config not found at ${LEMON_CFG}"
    manual_steps+=("Create ${LEMON_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$LEMON_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Lemon AI config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Lemon AI config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${LEMON_CFG} manually"); }
    manual_steps+=("Restart Lemon AI session to pick up config change")
fi
