...
# tree generates llms.txt — the index every LLM (Lemon AI included, via the
# lemon-agent skill) reads to learn what Lemon AI can do — and
...
_MCP_CATALOG_FILES = {"lemon_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Lemon AI-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``lemon serve`` backend, the Docker image)
