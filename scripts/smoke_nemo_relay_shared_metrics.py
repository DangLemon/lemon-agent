"""Run a real Lemon AI CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_lemon_executable(lemon_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "lemon",
        Path(".venv") / "Scripts" / "lemon.exe",
    ):
        candidate = lemon_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("lemon")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Lemon AI executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Lemon AI source checkout containing .venv/bin/lemon",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated LEMON_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["lemon.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['lemon.client.active']}"
        )
    [model] = by_name["lemon.model_route.count"]
    expected_model = {
        "name": "lemon.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['lemon.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["lemon.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['lemon.task_run.started']}"
        )
    [terminal] = by_name["lemon.task_run.finished"]
    ...
    [tool] = by_name["lemon.tool_call.count"]
    ...
    lifecycle = by_name["lemon.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["lemon.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Lemon AI development environment requires jsonschema"
        ) from exc
    ...
    if metrics["lemon.client.active"] != [
        {
            "name": "lemon.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['lemon.client.active']}"
        )
    [model] = metrics["lemon.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['lemon.model_route.count']}"
        )
    [terminal] = metrics["lemon.task_run.finished"]
    ...
    [tool] = metrics["lemon.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["lemon.skill.lifecycle.count"]
    ...
    loads = metrics["lemon.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    lemon_repo = args.lemon_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    lemon = _resolve_lemon_executable(lemon_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="lemon-relay-shared-metrics-"))
    home = root / "lemon-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["LEMON_HOME"] = str(home)
        python_paths = [str(lemon_repo)]
    ...
    (root / "lemon.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "lemon.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Lemon AI exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Lemon AI model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Lemon AI did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Lemon AI did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "lemon.shared_metrics.v2.schema.json",
    )

    print("Lemon AI -> NeMo Relay shared-metrics smoke test passed")
