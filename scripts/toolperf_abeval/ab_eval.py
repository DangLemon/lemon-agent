...
measured in the production session DB) through `lemon chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    LEMON_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Lemon AI home with credentials for the
...
(tracker: DangLemon/lemon-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "LEMON_HOME": str(HOME),
                "LEMON_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "lemon_cli.main", "chat", "--query", q,
