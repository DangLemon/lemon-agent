...
When Lemon AI runs in TUI mode, the gateway child process communicates with
...
# ``get_lemon_home() / "plugins"`` (user) and ``./.lemon-ai/plugins/`` (project,
# gated behind ``LEMON_ENABLE_PROJECT_PLUGINS``) — see
# ``lemon_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``lemon_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import lemon_constants (this script
    ...
    # plugins in get_lemon_home()/plugins/ and project plugins in
    # ./.lemon-ai/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_lemon_home() / "plugins"]
    if os.environ.get("LEMON_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".lemon-ai" / "plugins")
