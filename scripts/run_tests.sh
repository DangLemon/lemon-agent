#!/usr/bin/env bash
# Canonical test runner for lemon-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.lemon-ai/...)
#
# Usage:
...
# (LEMON_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.lemon-ai/lemon-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.lemon-ai/lemon-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${LEMON_PYTHON:-}" ] && [ -x "$LEMON_PYTHON" ] \
    && "$LEMON_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: LEMON_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `lemon` binary rather
  # than the devShell hook.
  PYTHON="$LEMON_PYTHON"
  echo "▶ no local venv — using Nix dev venv via LEMON_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and LEMON_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.lemon-ai/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.lemon-ai"
...
#
#   * LEMON_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * LEMON_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     LEMON_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# LEMON_RUN_SLOW_PET_TESTS / LEMON_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no LEMON_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in LEMON_TEST_IMAGE LEMON_TEST_WORKERS LEMON_TEST_PATHS \
  LEMON_TEST_FILE_TIMEOUT LEMON_TEST_FILE_RETRIES LEMON_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${LEMON_RUN_SLOW_PET_TESTS:+LEMON_RUN_SLOW_PET_TESTS="$LEMON_RUN_SLOW_PET_TESTS"} \
  ${LEMON_E2E_BROWSER:+LEMON_E2E_BROWSER="$LEMON_E2E_BROWSER"} \
