#!/usr/bin/env python3
"""Live test harness for Lemon AI's Tool Search feature.
...
# Force-isolate the test environment BEFORE any lemon imports.
ORIGINAL_HOME = os.environ.get("LEMON_HOME")
ORIGINAL_AUTH = Path.home() / ".lemon-ai" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.lemon-ai/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.lemon-ai/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="lemon_ts_live_"))
    lemon_home = home_dir / ".lemon-ai"
    lemon_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, lemon_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".lemon-ai" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, lemon_home / ".env")
        ...
        load_lemon_dotenv(lemon_home=str(Path.home() / ".lemon-ai"))
    ...
    (lemon_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return lemon_home
...
def reset_module_state():
    """Drop cached modules so the new LEMON_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "lemon_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["LEMON_HOME"] = str(home)
...
def main():
    ...
    # Restore original LEMON_HOME
    if ORIGINAL_HOME is not None:
        os.environ["LEMON_HOME"] = ORIGINAL_HOME
    else:
