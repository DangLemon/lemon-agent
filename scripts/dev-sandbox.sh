...
unprivileged `lemon` user, giving the layout most people have —
$LEMON_HOME/lemon-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  LEMON_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .lemon-ai-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `lemon setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${LEMON_DEV_SANDBOX_UPSTREAM:-https://github.com/DangLemon/lemon-agent.git}"
...
GIT_ROOT="${LEMON_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${LEMON_DEV_SANDBOX_DIR:-.lemon-ai-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t lemon-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t lemon-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/github.com/DangLemon/lemon-agent"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/github.com/DangLemon/lemon-agent/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/github.com/DangLemon/lemon-agent/install.sh"
  fi
  set -- bash -c '
    set +e
    ...
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      ...
      if git --git-dir=/work/repos/lemon-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `lemon` launcher to ~/.local/bin and the
# checkout to $LEMON_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/lemon-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q lemon-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t lemon-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Lemon AI sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/lemon-sandbox/next"
...
if [ ! -f "$SANDBOX_ROOT/root/certs/ca.pem" ]; then
  if ! ca_error="$(OPENSSL_CONF="$SANDBOX_ROOT/root/certs/openssl.cnf" \
    ...
    -subj '/CN=Lemon AI dev sandbox CA' \
