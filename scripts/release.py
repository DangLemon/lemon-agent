...
VERSION_FILE = REPO_ROOT / "lemon_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@lemon.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.lemon-ai": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level LEMON_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (lemon journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "lemon.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.lemon-ai/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows lemon-shim resolution + prefer --update on recovery; #52378)
    ...
    "274096618+lemon-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "lemon@nousresearch.com": "NousResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on lemon mcp add)
    ...
    "lemon@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyalemon.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (LEMON_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (LEMON_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: lemon-eval community link)
    ...
    "lemonagent26@gmail.com": "lemonagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+lemonagent26@users.noreply.github.com": "lemonagent26",
    ...
    "vigo@lemon": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.lemon-ai@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Lemon AI runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level LEMON_HOME state files)
    ...
    "zhchl@lemon-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Lemon AI
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "lemon@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/DangLemon/lemon-agent",
    ...
    lines.append(f"# Lemon AI v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Lemon AI. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Lemon AI Release Tool")
    ...
    print(f"{'='*60}")
    print("  Lemon AI Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Lemon AI v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Lemon AI v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Lemon AI v{new_version} ({calver_date})' "
