...
# of truth for gateway/run.py and lemon_cli/web_server.py validation).
...
class GatewayConfig:
    ...
    sessions_dir: Path = field(default_factory=lambda: get_lemon_home() / "sessions")
    ...
    # route); LEMON_ROOM_LINK_URL overrides.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            sessions_dir=Path(data["sessions_dir"]) if "sessions_dir" in data else get_lemon_home() / "sessions",
...
def load_gateway_config() -> GatewayConfig:
    """Load gateway configuration. Priority: env > ~/.lemon-ai/config.yaml > legacy gateway.json > defaults."""
    from gateway import config_loader

    _home = get_lemon_home()
