...
Defenses 1-2 (``_LEMON_GATEWAY`` guard on ``lemon gateway stop|restart`` /
...
persisted to ``<LEMON_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_lemon_home() / "gateway" / "restart_loop.json"
