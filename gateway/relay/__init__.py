"""Relay/connector support package for the Lemon AI gateway.
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from lemon_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # prefix every reply "**Lemon AI:**" and shadow the connector's
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``lemon gateway enroll``.
...
def self_provision_relay() -> bool:
    ...
    ``~/.lemon-ai/.env``), so a hosted container re-provisions every boot; the
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'lemon'}"
