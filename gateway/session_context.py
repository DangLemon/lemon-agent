"""Session-scoped context variables for the Lemon AI gateway.

Replaces the old ``os.environ``-based ``LEMON_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "LEMON_SESSION_PLATFORM", "LEMON_SESSION_SOURCE", "LEMON_SESSION_CHAT_ID",
    "LEMON_SESSION_CHAT_TYPE", "LEMON_SESSION_CHAT_NAME", "LEMON_SESSION_THREAD_ID",
    "LEMON_SESSION_USER_ID", "LEMON_SESSION_USER_ID_ALT", "LEMON_SESSION_USER_NAME",
    "LEMON_SESSION_SCOPE_ID", "LEMON_SESSION_KEY", "LEMON_SESSION_ID",
    "LEMON_UI_SESSION_ID", "LEMON_SESSION_MESSAGE_ID", "LEMON_SESSION_PROFILE",
    "LEMON_BROWSER_CONTROL_PRINCIPAL", "LEMON_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "LEMON_CRON_SESSION",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("LEMON_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("LEMON_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("LEMON_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("LEMON_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``LEMON_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``LEMON_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds LEMON_SESSION_PLATFORM, CLI/TUI/
# desktop bind LEMON_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``LEMON_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("LEMON_PLATFORM") or get_session_env("LEMON_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("LEMON_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See DangLemon/lemon-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``LEMON_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
