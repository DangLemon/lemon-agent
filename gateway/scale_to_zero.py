...
SCALE_TO_ZERO_ENV = "LEMON_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(lemon_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under LEMON_HOME."""
    if lemon_home is None:
        from lemon_constants import get_lemon_home
        lemon_home = get_lemon_home()
    return Path(lemon_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
