...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.lemon-ai to the active profile home."""
    ...
    if raw == "~/.lemon-ai" or raw.startswith("~/.lemon-ai/"):
        return lemon_home / raw[len("~/.lemon-ai/"):]
    path = Path(raw).expanduser()
    return path if path.is_absolute() else lemon_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script; must live under LEMON_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from lemon_constants import get_lemon_home
    scripts_root = (get_lemon_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.lemon-ai" or raw_text.startswith("~/.lemon-ai/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        Non-zero exit, empty/``[SILENT]`` stdout, or a ``[SILENT]``/``__lemon_ignore__`` flag drops the
        ...
        if not isinstance(transformed, dict):
            ...
            return False, None
        silenced = transformed.get("[SILENT]") is True or transformed.get("__lemon_ignore__") is True
