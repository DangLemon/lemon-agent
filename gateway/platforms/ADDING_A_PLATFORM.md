...
There are two ways to add a platform to the Lemon AI gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.lemon-ai/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Lemon AI code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `lemon gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `lemon_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Lemon AI core.
...
"lemon-your-platform": {
    ...
    "tools": _LEMON_CORE_TOOLS,
    "includes": []
},
```

And add it to the `lemon-gateway` composite:

```python
"lemon-gateway": {
    "includes": [..., "lemon-your-platform"]
}
```
...
## 12. Status Display (`lemon_cli/status.py`)
...
## 13. Gateway Setup Wizard (`lemon_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ lemon_cli/ toolsets.py \
  --include="*.py" -l | sort -u
