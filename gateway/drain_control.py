...
(or removes) ``{LEMON_HOME}/.drain_request.json`` and a gateway watcher reacts;
an ACTIVE marker means ``gateway_state -> "draining"``.  Two lenient staleness
signals (either suffices): epoch mismatch (LEMON_HOME is a durable volume on
Lemon AI Cloud, so a marker survives the restart a drain-gated action ends in and
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting LEMON_HOME."""
    return Path(home if home is not None else get_lemon_home()) / _DRAIN_REQUEST_FILENAME
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    survived a container/VM restart (LEMON_HOME is a durable Fly volume on Lemon AI Cloud) and the lifecycle
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    machine restart on the durable LEMON_HOME volume, or an expired same-epoch orphan (#85433), is ignored
