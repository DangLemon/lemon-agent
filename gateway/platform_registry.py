...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``lemon setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``lemon gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved LEMON_HOME and overlay the
        ...
        # `lemon` invocation, so the import happens only when a lookup asks for it.
