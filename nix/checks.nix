...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "lemon-check";
                homeDirectory = "/home/lemon-check";
                stateVersion = "24.11";
              };
            }
            { services.lemon-ai-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.lemon-ai-agent = programs;
              services.lemon-ai-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.lemon-ai-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.lemon-ai-agent);
      ...
      configKeys = pkgs.runCommand "lemon-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${lemonVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "lemon-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "lemon-build-package" { } ''
          echo "PASS: package built at ${lemon-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "lemon-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.LEMON_TEST = "1";
                environmentFiles = [ "/run/secrets/lemon-env" ];
                lemonHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "lemon" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "lemon" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.lemon-aiAgentSetup.data;
            ...
            failures =
              ...
              ++ lib.optional (
                cfg.home.sessionVariables.LEMON_HOME or null != "/home/lemon-check/.lemon-ai"
              ) "programs.lemon-ai-agent.enable must export LEMON_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "lemon-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/lemon-check/.lemon-ai/SOUL.md" activation
              ) "lemonHomeFiles must install into LEMON_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads LEMON_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "LEMON_MANAGED=home-manager" (envOf n)) names
              ) "processes must report LEMON_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "lemon-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "lemon-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/lemon-check";
            ...
            cases = [
              ...
              {
                name = "lemonHomeFiles needs no directory";
                ok = accepts { lemonHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "lemon-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one LEMON_HOME ───────────────
        # `programs.enable` exports LEMON_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.lemon-ai
        # while the services use the LEMON_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/lemon-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                lemonHome = "/home/lemon-check/.lemon-ai-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "lemon-desktop") cfg.home.packages;
            ...
            # second Lemon AI runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "lemon-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.lemon-ai-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.lemon-ai-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one lemon-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "LEMON_HOME" != "/home/lemon-check/.lemon-ai-work"
              ) "the launcher must carry LEMON_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "LEMON_HOME")})"
              ++ lib.optional (
                setValue "LEMON_MANAGED" != "home-manager"
              ) "the launcher must report LEMON_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "LEMON_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "LEMON_DESKTOP_LEMON" != "${lib.head agentPackages}/bin/lemon"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "LEMON_DESKTOP_LEMON")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "LEMON_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "LEMON_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "LEMON_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "LEMON_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "LEMON_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "lemon-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares LEMON_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set LEMON_HOME" wrapper
              ) "the launcher must carry LEMON_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "LEMON_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "LEMON_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? lemon-backend || cfg.launchd.agents ? lemon-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "lemon-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `lemon`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.lemon-ai-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.lemon-ai-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "lemon-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "lemon-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "lemon" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."lemon-agent-setup".text;
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "/var/lib/lemon/.lemon-ai/SOUL.md" activation
              ) "lemonHomeFiles must install into LEMON_HOME";
          ...
          in
          pkgs.runCommand "lemon-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs lemon, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.lemon-ai-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/lemon serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec lemon directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "lemon-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "lemon-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec lemon, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/lemon dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "lemon-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $LEMON_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                LEMON_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "lemon-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'LEMON_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--lemon-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "lemon-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "lemon-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${lemon-agent}/bin/lemon || (echo "FAIL: lemon binary missing"; exit 1)
          test -x ${lemon-agent}/bin/lemon-agent || (echo "FAIL: lemon-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${lemon-agent}/bin/lemon --version 2>&1 | grep -qi "lemon" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "lemon-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in lemon lemon-agent lemon-acp; do
            test -x ${lemon-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "lemon-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking lemon --help ==="
          ${lemon-agent}/bin/lemon --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${lemon-agent}/bin/lemon --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "lemon-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${lemon-agent}/share/lemon-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${lemon-agent}/share/lemon-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "LEMON_BUNDLED_SKILLS" ${lemon-agent}/bin/lemon || \
            (echo "FAIL: LEMON_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: LEMON_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${lemon-agent}/share/lemon-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${lemon-agent}/share/lemon-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "LEMON_OPTIONAL_SKILLS" ${lemon-agent}/bin/lemon || \
            (echo "FAIL: LEMON_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, LEMON_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "lemon-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${lemon-agent}/share/lemon-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${lemon-agent}/share/lemon-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "LEMON_BUNDLED_PLUGINS" ${lemon-agent}/bin/lemon || \
            (echo "FAIL: LEMON_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: LEMON_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "lemon-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${lemon-agent}/share/lemon-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${lemon-agent}/share/lemon-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${lemon-agent}/share/lemon-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "LEMON_BUNDLED_LOCALES" ${lemon-agent}/bin/lemon || \
            (echo "FAIL: LEMON_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: LEMON_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + LEMON_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && LEMON_BUNDLED_LOCALES=${lemon-agent}/share/lemon-agent/locales \
            ${lemonVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with LEMON_BUNDLED_LOCALES set"; exit 1)
        ...
        # LEMON_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "lemon-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${lemon-agent}/share/lemon-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${lemon-agent}/share/lemon-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "LEMON_OPTIONAL_MCPS" ${lemon-agent}/bin/lemon || \
            (echo "FAIL: LEMON_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: LEMON_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${lemon-agent}/bin/lemon mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: lemon mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "lemon-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${lemon-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${lemon-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "LEMON_TUI_DIR" ${lemon-agent}/bin/lemon || \
            (echo "FAIL: LEMON_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: LEMON_TUI_DIR set in wrapper"
        ...
        # Verify LEMON_NODE is set in wrapper and points to Node 26+
        # (Lemon AI pins its toolchain to Node 26 everywhere)
        lemon-node = pkgs.runCommand "lemon-node-version" { } ''
          set -e
          echo "=== Checking LEMON_NODE in wrapper ==="
          grep -q "LEMON_NODE" ${lemon-agent}/bin/lemon || \
            (echo "FAIL: LEMON_NODE not set in wrapper"; exit 1)
          echo "PASS: LEMON_NODE present in wrapper"

          LEMON_NODE=$(sed -n "s/^export LEMON_NODE='\(.*\)'/\1/p" ${lemon-agent}/bin/lemon)
          test -x "$LEMON_NODE" || (echo "FAIL: LEMON_NODE=$LEMON_NODE not executable"; exit 1)
          echo "PASS: LEMON_NODE executable at $LEMON_NODE"

          NODE_MAJOR=$("$LEMON_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Lemon AI requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All LEMON_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify LEMON_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "lemon-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(LEMON_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking LEMON_MANAGED guards ==="
          check_blocked "config set" ${lemon-agent}/bin/lemon config set model foo
          check_blocked "config edit" ${lemon-agent}/bin/lemon config edit
        ...
        extra-python-packages = let
          ...
          lemonWithExtra = lemon-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "lemon-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${lemonWithExtra}/bin/lemon || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${lemonWithExtra}/bin/lemon || \
          ...
          if grep -q "PYTHONPATH" ${lemon-agent}/bin/lemon; then
        ...
        extra-dependency-groups = let
          lemonWithGroups = lemon-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "lemon-extra-dependency-groups" { } ''
          set -e
        ...
        in pkgs.runCommand "lemon-config-roundtrip" {
          ...
          merge_and_load() {
            local lemon_home="$1"
            export LEMON_HOME="$lemon_home"
            ${configMergeScript} ${nixSettings} "$lemon_home/config.yaml"
            ${lemonVenv}/bin/python3 -c '
import json, sys
