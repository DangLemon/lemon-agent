# nix/nixosModules.nix — the NixOS module for lemon-agent
#
...
#
# Container mode: lemon runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $LEMON_HOME/.env
# and read by lemon at startup — no container recreation needed for env changes.
#
# Tool resolution: the lemon wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      lemonHome = "${cfg.stateDir}/.lemon-ai";
      ...
      # LEMON_HOME via addToSystemPackages, so they can save settings through the
      ...
      # lemon user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "lemon-container-entrypoint" ''
        set -eu

        LEMON_UID="''${LEMON_UID:?LEMON_UID must be set}"
        LEMON_GID="''${LEMON_GID:?LEMON_GID must be set}"

        # ── Group: ensure a group with GID=$LEMON_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$LEMON_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="lemon"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$LEMON_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$LEMON_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$LEMON_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$LEMON_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="lemon"
          TARGET_HOME="/home/lemon"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$LEMON_UID" -g "$LEMON_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$LEMON_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$LEMON_UID:$LEMON_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure LEMON_HOME is owned by the target user.
        ...
        if [ -n "''${LEMON_HOME:-}" ] && [ -d "$LEMON_HOME" ]; then
          find "$LEMON_HOME" \! -user "$LEMON_UID" -exec chown "$LEMON_UID:$LEMON_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/lemon-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/lemon-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/lemon ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/lemon
          chmod 0440 /etc/sudoers.d/lemon
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$LEMON_UID" --regid="$LEMON_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $LEMON_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "lemon-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        lemon_bin=${containerDataDir}/current-package/bin/lemon
      '';

      # Default: /var/lib/lemon/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the lemon group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit lemonHome; };
    ...
    in
    {
      options.services.lemon-ai-agent =
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/lemon";
              description = "State directory. Contains .lemon-ai/ subdir (LEMON_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the lemon CLI to environment.systemPackages and export
                LEMON_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.lemon-ai symlink to the service
                  stateDir. These users are automatically added to the lemon group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the lemon CLI to system PATH and export LEMON_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.lemon-ai/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.LEMON_HOME = lemonHome;
          })
              ...
              ++ [
                {
                  ...
                  message = "services.lemon-ai-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the lemon user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.lemon-ai-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed lemon
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure lemon is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${lemonHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${lemonHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."lemon-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${lemonHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${lemonHome} ${cfg.workingDirectory}
                  ...
                  find ${lemonHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${lemonHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${lemonHome}/$_subdir"
                    chmod 2770 "${lemonHome}/$_subdir"
                    find "${lemonHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg lemonHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${lemonHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${lemonHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${lemonHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.lemon-ai";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${lemonHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "lemon-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.lemon-ai -> stateDir/.lemon-ai for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.lemon-ai-agent = {
              ...
              # $LEMON_HOME/.env by the activation script. load_lemon_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.lemon-ai-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.lemon-ai-agent = {
              description = "Lemon AI Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  LEMON_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  LEMON_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env LEMON_UID="$LEMON_UID" \
                    --env LEMON_GID="$LEMON_GID" \
                    --env LEMON_HOME=${containerDataDir}/.lemon-ai \
                    --env LEMON_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/lemon gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
