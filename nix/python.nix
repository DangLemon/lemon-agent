...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Lemon AI derivation. This is deliberately a derivation environment
  ...
  # $LEMON_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$LEMON_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
...
in
{
  venv = pythonSet.mkVirtualEnv "lemon-agent-env" {
    lemon-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "lemon-agent-editable-env" {
    lemon-agent = dependency-groups;
  };
}
