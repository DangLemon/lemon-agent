...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      lemonAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "lemon" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../lemon} $out/bin/lemon
          '')
        ...
        shellHook = ''
          ${lemonAgentDevShellHook}
          ${lemonNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export LEMON_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Lemon AI dev shell in $LEMON_PYTHON_SRC_ROOT"
          echo "Ready. Run 'lemon' or 'sandbox lemon' to start."
        '';
      };
    };
}
