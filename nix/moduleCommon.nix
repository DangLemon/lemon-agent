...
#
# `services.lemon-ai-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  lemonHome, programs.lemon-ai-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "lemon-config-attrs";
    description = "Lemon AI YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $LEMON_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Lemon AI";
      ...
      package = mkOption {
        ...
        description = "The lemon-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Lemon AI configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `lemon config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $LEMON_HOME/.env. Lemon AI reads that file at each start, with
          load_lemon_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."lemon/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Lemon AI refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Lemon AI reads SOUL.md and memories/ from LEMON_HOME, so put those
          files in `lemonHomeFiles`.
      ...
      lemonHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into LEMON_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Lemon AI reads SOUL.md and the memory files from LEMON_HOME and not
          from the working directory. Declare those files here, or Lemon AI
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the lemon plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Lemon AI discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          lemon_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as lemon (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in lemon-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `lemon gateway`.";
      };
      ...
      # ── The backend: `lemon serve` or `lemon dashboard` ──────────────
      # `lemon serve` and `lemon dashboard` are the same entry point,
      # lemon_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Lemon AI
      ...
      # controls an external gateway, with `lemon gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Lemon AI
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            LEMON_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Lemon AI presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."lemon/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `lemon` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.lemon-ai-agent.installPackage was removed. Lemon AI now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.lemon-ai-agent.enable = ${lib.boolToString (value != false)};  # the lemon CLI, and LEMON_HOME for your shells
        programs.lemon-ai-agent.desktop.enable = true;  # the desktop application

      `services.lemon-ai-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "lemon-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "lemon-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "lemon-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "lemon-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "lemon-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # LEMON_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Lemon AI make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${lemonHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Lemon AI
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${lemonHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${lemonHome}/config.yaml
            ${run}chmod ${modes.config} ${lemonHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "lemon-managed" managedSystem} ${lemonHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${lemonHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${lemonHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${lemonHome}/auth.json"
        else
          ''
            if [ ! -e ${lemonHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${lemonHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree lemonHome cfg.lemonHomeFiles}
      ...
      ${run}find ${lemonHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "lemon-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${lemonHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/lemon"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps lemon as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "lemon-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "lemon-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "lemon-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          LEMON_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export LEMON_DASHBOARD_SESSION_TOKEN

          if [ -z "$LEMON_DASHBOARD_SESSION_TOKEN" ]; then
            echo "lemon-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "lemon-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "lemon-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "lemon-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "lemon-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "lemon-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "lemon-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Lemon AI web dashboard and desktop backend"
    else
      "Lemon AI backend for Lemon AI";

  # The environment that each Lemon AI process needs, from either module.
  #
  # managedSystem gives the value of LEMON_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Lemon AI an identity and a memory, use
          ${optionPath}.lemonHomeFiles instead. Those files go to
          LEMON_HOME. Lemon AI reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of LEMON_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
