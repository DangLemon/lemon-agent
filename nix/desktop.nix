# nix/desktop.nix — Lemon AI (Electron) app build + wrapper
#
# `lemonAgent` is the fully-built `.#default` package — it ships the
# `lemon` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `LEMON_DESKTOP_LEMON` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Lemon AI CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # LEMON_HOME and LEMON_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "lemon-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "lemon-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = lemonNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "lemon-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe lemonNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/lemon-desktop $out/bin
    cp -r ${renderer}/* $out/share/lemon-desktop/
    ...
    substituteInPlace $out/share/lemon-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/lemon-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # LEMON_DESKTOP_LEMON to the absolute path of the nix-built `lemon`
    # binary so the desktop's resolver step 4 ("existing Lemon AI CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/lemon-desktop \
      --add-flags "$out/share/lemon-desktop" \
      --set LEMON_DESKTOP_LEMON "${lib.getExe lemonAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/lemon.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../lemon_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/lemon-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/lemon.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/lemon.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Lemon AI";
    homepage = "https://github.com/DangLemon/lemon-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "lemon-desktop";
  };
}
