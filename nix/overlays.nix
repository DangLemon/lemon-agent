# nix/overlays.nix — Expose pkgs.lemon-ai-agent for external NixOS configs
#
...
# `pkgs.lemon-ai-agent`, `nix build .#default`, and the NixOS module's
...
{ inputs, ... }:
{
  flake.overlays.default = final: _: {
    lemon-agent = inputs.self.packages.${final.stdenv.hostPlatform.system}.default;
  };
}
