# nix/web.nix — Lemon AI Web Dashboard (Vite/React) frontend build
{ lemonNpmLib, ... }:
lemonNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @lemon-ai/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../lemon_cli/web_dist for the
