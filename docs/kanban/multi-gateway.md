...
Lemon AI supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.lemon-ai/config.yaml`:
...
Or set the env var: `LEMON_KANBAN_DISPATCH_IN_GATEWAY=false`
