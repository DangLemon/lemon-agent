...
When running Lemon AI inside Docker, the default `network_mode: host` gives the
...
The Lemon AI [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   LEMON_UID=$(id -u) LEMON_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=lemon,lemon-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://lemon-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Lemon AI trust model and vulnerability reporting
