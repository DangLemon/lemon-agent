...
`lemon doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `lemon doctor --fix` while the gateway
is still up. Next steps:

```bash
lemon gateway stop
LEMON_HOME="$HOME/.lemon-ai" lemon sessions recover --source "$HOME/.lemon-ai/state.db" --inspect-only
# if recoverable:
LEMON_HOME="$HOME/.lemon-ai" lemon sessions recover --source "$HOME/.lemon-ai/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
lemon gateway stop
LEMON_HOME="$HOME/.lemon-ai" lemon sessions repair --check-only
LEMON_HOME="$HOME/.lemon-ai" lemon sessions repair
```
...
```bash
LEMON_HOME="$HOME/.lemon-ai" lemon sessions repair --check-only
sqlite3 "$HOME/.lemon-ai/state.db" \
