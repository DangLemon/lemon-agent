...
or change arbitrary Lemon AI configuration.
...
small host fact requested by the RFC. Settings writes use Lemon AI'
...
`LEMON_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<LEMON_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  Lemon AI home on every operation.
...
The implementation is covered with real temporary-Lemon AI-home tests for:
...
- [Issue #64227](https://github.com/DangLemon/lemon-agent/issues/64227)
- [RFC PR #58542](https://github.com/DangLemon/lemon-agent/pull/58542) by Topher Ross
