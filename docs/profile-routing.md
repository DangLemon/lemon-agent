...
`lemon config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.lemon-ai/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `LEMON_HOME`, secret scope, and
