...
Lemon AI middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Lemon AI'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Lemon AI calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Lemon AI retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Lemon AI guardrails,
...
- `telemetry_schema_version`: currently `lemon.observer.v1`
- `middleware_schema_version`: currently `lemon.middleware.v1`
...
Lemon AI stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Lemon AI runs
them as a nested chain in registration order. Middleware failures are fail-open:
Lemon AI logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Lemon AI applies middleware in this order:
...
For each tool call, Lemon AI applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Lemon AI pre-execution path against the effective arguments:
...
For isolated local testing, use one `LEMON_HOME` for plugin enablement and the
agent run:

```bash
export LEMON_HOME=/tmp/lemon-middleware-test
mkdir -p "$LEMON_HOME"
lemon plugins enable <plugin-name>
lemon chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run lemon plugins enable <plugin-name>
uv run lemon chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["lemon_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/lemon-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Lemon AI expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Lemon AI treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Lemon AI preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Lemon AI does not convert downstream
