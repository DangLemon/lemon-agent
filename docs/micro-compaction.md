...
has to be thrown away or summarized. Lemon AI has always done this in one batch:
...
Lemon AI folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Lemon AI's normal session flush is
...
With `micro_compact` unset or `false` Lemon AI behaves exactly as it always has:
...
Defaults to `$LEMON_HOME/logs/agent.log`. It reports passes, outcome mix, net
