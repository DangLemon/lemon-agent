# Lemon AI Observer Hooks

Lemon AI observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Lemon AI' planner, model providers, memory, tool registry,
...
Lemon AI also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "lemon.observer.v1"
```

Hook callbacks are fail-open. Lemon AI catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Lemon AI only builds
...
The native NeMo Relay SDK integration maps Lemon AI session, turn, LLM, and tool
