...
Lemon AI includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Lemon AI and does not require a Lemon AI observability plugin. Lemon AI remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Lemon AI execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `lemon-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Lemon AI `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `LEMON_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `LEMON_NEMO_RELAY_ATOF_*` and `LEMON_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Lemon AI does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Lemon AI requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Lemon AI
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Lemon AI installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Lemon AI process so configured interceptors can
...
Collection remains off unless Lemon AI policy enables it:
...
opt-in. Set `LEMON_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Lemon AI does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Lemon AI reports the error and does not invoke
...
retain the same `session_id` and add `lemon.session.segment` plus
`lemon.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Lemon AI
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Lemon AI process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Lemon AI logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Lemon AI
process and share the same policy boundary.

Run profiles in separate Lemon AI processes when they require different trust
...
Lemon AI core owns one Relay host and one isolated Relay session scope per Lemon AI
...
marks do not require Lemon AI plugin registration. Shared-metrics marks must
...
```text
Lemon AI turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Lemon AI shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Lemon AI sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Lemon AI used for the logical call, such as
...
New calls use `lemon.model_route.count`. The previous
`lemon.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `lemon.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Lemon AI
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Lemon AI processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `lemon.task_run`, parented to
the owning Lemon AI session. The start counter contains only bounded execution
...
provider attempts for the same Lemon AI API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Lemon AI tool-call
...
cancellations. Active task ownership follows the task ID if Lemon AI rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`lemon.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Lemon AI
...
tool or plugin names. Lemon AI does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `lemon.tool_approval` marks and
...
Successful skill mutations emit `lemon.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `lemon.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Lemon AI derives reuse and patch-generation
...
```text
$LEMON_HOME/telemetry/shared_metrics/metrics.sqlite3
$LEMON_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Lemon AI version,
...
schema field name, its current scope is one `LEMON_HOME`, so it is more
...
Deleting `$LEMON_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `LEMON_HOME`. To reset it, stop Lemon AI
processes and remove `$LEMON_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Lemon AI CLI turn against the deterministic local model server:
...
`$LEMON_HOME/telemetry/shared_metrics/outbox/` to the Lemon AI telemetry ingest
...
Removing `$LEMON_HOME/telemetry/shared_metrics` still resets local identity,
