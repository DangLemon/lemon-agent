...
Lemon AI gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Lemon AI's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `lemon.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `lemon.platform.up/degraded` with bounded `error_code` attributes |
...
`lemon.gateway.active_agents`, `lemon.gateway.background_work`, and
`lemon.gateway.background_delegations` are complementary. `active_agents`
...
```bash
lemon monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'lemon-agent[otlp]'`),
...
a plain `lemon gateway run`: the exporter lives in the gateway process, so
...
`lemon.gateway.up`, `lemon.platform.up`, and `lemon.platform.degraded`.
...
absent_over_time(lemon_gateway_up[5m])

# Locally owned bridge is explicitly down.
lemon_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
lemon_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
lemon_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
lemon_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(lemon_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `lemon.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
lemon.status = failed|unknown
lemon.delivery_outcome = failed|not_configured
lemon.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `lemon.job_key`;
...
Lemon AI-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/lemon_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/lemon_otel_capture.jsonl --wait 8
...
`lemon.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
lemon monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `lemon monitoring` CLI intentionally exposes `status` only. This first
release covers only Lemon AI-owned service-health and operational-diagnostic
signals, including Lemon AI-owned Relay transport health. Team Gateway's
