...
The canonical transcript store is SQLite via `SessionDB` (from `lemon_state`). The
...
Counts consecutive restarts via a JSON file (`{LEMON_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `lemon update`, `lemon gateway restart`,
or `/restart`.
