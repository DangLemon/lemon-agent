...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_lemon_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`LEMON_HOME`, `LEMON_PROFILE`, proxy settings,
...
reload helper: `lemon_cli.env_loader.load_lemon_dotenv` skips the
...
## The LEMON_HOME override

`lemon_constants.py` holds a context-local override consulted by
`get_lemon_home()` before the `LEMON_HOME` env var. Everything that resolves
...
automatically. `get_process_lemon_home()` exists for the few machine-level
assets that must not follow the override. `lemon_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `lemon_cli/profiles.py`
...
DB handles are resolved at call time through the active LEMON_HOME override —
...
LEMON_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `lemon_home_key()`. |
...
- `agent/secret_scope.py`, `lemon_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `lemon_cli/profiles.py`
