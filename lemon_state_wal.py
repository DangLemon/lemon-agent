"""SQLite journal-mode and PRAGMA policy for state.db (split from lemon_state).

Patchable helpers are looked up as module globals at call time, so tests patch ``lemon_state_wal.<name>``.
"""
...
from lemon_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable
...
# line would repeat per connection). Tests clear these via ``lemon_state_wal.<name>``.
...
def apply_wal_with_fallback(conn: sqlite3.Connection, *, db_label: str = "state.db", require_wal: bool = False) -> str:
    ...
    Returns the mode actually set. Shared by :class:`SessionDB` and ``lemon_cli.kanban_db_connect.connect``.
...
def _wal_reset_repair_hint() -> str:
    """Repair hint matching what ``lemon update`` can actually do for this install type.

    See #75153.
    """
    try:
        from lemon_cli.config import detect_install_method, get_project_root, recommended_update_command_for_method
        method = detect_install_method(get_project_root())
        cmd = recommended_update_command_for_method(method)
        if method in {"git", "unknown"}:
            return f"Lemon AI-managed installs can repair the embedded runtime with `{cmd}`"
        return f"update the container image with `{cmd}`" if method == "docker" else cmd  # else nix/nixos
    except Exception:
        return "install a Python build bundled with SQLite 3.51.3+ (or backports 3.50.7 / 3.44.6) and restart Lemon AI"
...
_ONCE_LOGS = {
    "wal_reset_bug": (_wal_reset_bug_warned_lock, "_wal_reset_bug_warned_paths", logging.WARNING,
        ...
        "%s. See `lemon doctor`. This warning fires once per process per database."),
...
def apply_database_pragmas(conn: sqlite3.Connection, *, db_label: str = "state.db") -> None:
    ...
    try:
        from lemon_cli.config import cfg_get, load_config_readonly  # local: avoids a circular import
