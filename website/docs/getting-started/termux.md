...
description: "Run Lemon AI directly on an Android phone with Termux"
---

# Lemon AI on Android with Termux
...
Lemon AI can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Lemon AI from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Lemon AI installs are marked with install method `apt`. Lemon AI therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade lemon-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Lemon AI runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

Lemon AI now ships a Termux-aware installer path:
...
- links `lemon` into `$PREFIX/bin` so it stays on your Termux PATH
...
Lemon AI requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `lemon` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/lemon" "$PREFIX/bin/lemon"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `lemon` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.lemon-ai/.env`.
...
### `lemon doctor` says ripgrep or Node is missing
