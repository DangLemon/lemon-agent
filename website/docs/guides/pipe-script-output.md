...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `lemon send`."
---

# Pipe Script Output to Messaging Platforms

`lemon send` is a small, scriptable CLI that pushes a message to any
messaging platform Lemon AI is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | lemon send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `lemon
...
lemon send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
lemon send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
lemon send --list

# Filter by platform
lemon send --list telegram
```
...
Any platform Lemon AI ships adapters for works as a target:
...
`lemon send` resolves the message body in this order:

1. **Positional argument** — `lemon send --to telegram "hi"`
2. **`--file PATH`** — `lemon send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | lemon send --to telegram`

When stdin is a TTY (no pipe), Lemon AI does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  lemon send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `lemon send` reuses your Lemon AI config, the same script works on
any host where Lemon AI is installed — no need to export bot tokens into
...
`lemon send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  lemon send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | lemon send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.lemon-ai/bin/lemon send \
...
```bash
./train.py --epochs 200 && \
  lemon send --to telegram "training done" || \
  lemon send --to telegram "training failed (exit $?)"
```
...
lemon send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(lemon send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `lemon send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `lemon send` calls
the platform's REST endpoint directly using credentials from
`~/.lemon-ai/.env` and `~/.lemon-ai/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `lemon gateway start` and retry.
...
# Machine-readable
lemon send --list --json
```

The listing is built from `~/.lemon-ai/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`lemon gateway
...
| Approach | Multi-platform | Reuses Lemon AI creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `lemon send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`lemon send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `lemon send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `lemon send` shares with cron delivery.
