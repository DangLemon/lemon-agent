...
```bash
lemon teams-pipeline token-health
lemon teams-pipeline token-health --force-refresh
```
...
```bash
lemon teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
lemon teams-pipeline maintain-subscriptions
lemon teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Lemon AI cron (recommended if you already run the Lemon AI gateway)

Lemon AI ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.lemon-ai/scripts/`. First create the script:

```bash
mkdir -p ~/.lemon-ai/scripts
cat > ~/.lemon-ai/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec lemon teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.lemon-ai/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
lemon cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/lemon-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Lemon AI Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=lemon
EnvironmentFile=/etc/lemon/env
ExecStart=/usr/local/bin/lemon teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/lemon-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Lemon AI Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now lemon-teams-pipeline-maintain.timer
systemctl list-timers lemon-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/lemon teams-pipeline maintain-subscriptions >> /var/log/lemon/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.lemon-ai/.env` at the top of a wrapper script that crontab calls.
...
```bash
lemon teams-pipeline subscriptions   # should show expirationDateTime advanced
lemon teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
lemon teams-pipeline list
lemon teams-pipeline list --status failed
lemon teams-pipeline show <job-id>
```

### Replay a stored job

```bash
lemon teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
lemon teams-pipeline fetch --meeting-id <meeting-id>
lemon teams-pipeline fetch --join-web-url "<join-url>"
lemon teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `lemon teams-pipeline maintain-subscriptions --dry-run`
- inspect `lemon teams-pipeline list --status failed`
...
- run `lemon teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `lemon teams-pipeline run`
...
- [ ] `lemon teams-pipeline validate` returns an OK snapshot
- [ ] `lemon teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Lemon AI cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
