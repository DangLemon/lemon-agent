...
description: "Diagnose and fix common Lemon AI cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`lemon gateway` for foreground, or `lemon gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `lemon cron tick`.
...
```bash
date
lemon cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.lemon-ai/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.lemon-ai/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.lemon-ai/.env` |
...
| `local` | Write access to `~/.lemon-ai/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `lemon cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Lemon AI Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `lemon skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `lemon skills list`.
...
2. `~/.lemon-ai/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `lemon cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Lemon AI config directory). Verify:
```bash
ls ~/.lemon-ai/scripts/your-script.py   # Must exist
lemon cron edit <job_id> --script ~/.lemon-ai/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `lemon skills install <skill-name>`.
...
Jobs are stored in `~/.lemon-ai/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.lemon-ai/cron/jobs.json
chmod 600 ~/.lemon-ai/cron/jobs.json   # Your user should own it
```
...
```bash
lemon cron list                    # Show all jobs, states, next_run times
lemon cron run <job_id>            # Schedule for next tick (for testing)
lemon cron edit <job_id>           # Fix configuration issues
lemon logs                         # View recent Lemon AI logs
lemon skills list                  # Verify installed skills
```
...
1. Run the job with `lemon cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.lemon-ai/logs/agent.log` for scheduler messages and `~/.lemon-ai/logs/errors.log` for warnings
3. Open an issue at [github.com/DangLemon/lemon-agent](https://github.com/DangLemon/lemon-agent) with:
