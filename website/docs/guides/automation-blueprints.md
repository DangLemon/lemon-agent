...
Copy-paste blueprints for common automation patterns. Each blueprint uses Lemon AI's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`lemon webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `lemon webhook subscribe`) |
...
```bash
lemon cron create "0 2 * * *" \
  "You are a project manager triaging the DangLemon/lemon-agent GitHub repo.

1. Run: gh issue list --repo DangLemon/lemon-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
lemon webhook subscribe github-pr-review \
  --events "pull_request" \
...
lemon cron create "0 9 * * 1" \
  "Scan the DangLemon/lemon-agent repo for documentation drift.

1. Run: gh pr list --repo DangLemon/lemon-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (lemon_cli/commands.py, lemon_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (lemon_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
lemon cron create "0 6 * * *" \
  "Run a dependency security audit on the lemon-agent project.

1. cd ~/.lemon-ai/lemon-agent && source .venv/bin/activate
...
```bash
lemon webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
lemon webhook subscribe alert-triage \
...
```python title="~/.lemon-ai/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Lemon AI-Monitor/1.0"})
...
lemon cron create "every 30m" \
  ...
  --script ~/.lemon-ai/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
lemon cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Lemon AI development." \
...
```bash
lemon webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
lemon webhook subscribe stripe-payments \
...
```bash
lemon cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the lemon-agent codebase.
