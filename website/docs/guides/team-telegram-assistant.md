...
This tutorial walks you through setting up a Telegram bot powered by Lemon AI that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Lemon AI installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.lemon-ai/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Lemon AI itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Lemon AI Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_lemon_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Lemon AI. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.lemon-ai/.env`:
...
```
[Gateway] Starting Lemon AI Gateway...
...
# View live logs
journalctl --user -u lemon-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `lemon gateway install` to update the plist.
:::
...
```bash
lemon gateway stop && lemon gateway start
```
...
lemon pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.lemon-ai/.env`:
...
Control how much detail the bot shows when using tools. In `~/.lemon-ai/config.yaml`:
...
Customize how the bot communicates by editing `~/.lemon-ai/SOUL.md`:

For a full guide, see [Use SOUL.md with Lemon AI](/guides/use-soul-with-lemon).
...
```bash
# From the CLI
lemon cron list          # View all scheduled jobs
lemon cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u lemon-gateway` (Linux) or `~/.lemon-ai/logs/gateway.log` (macOS) |
| Cron job output | `~/.lemon-ai/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.lemon-ai/cron/jobs.json` |
| Pairing data | `~/.lemon-ai/pairing/` |
| Session history | `~/.lemon-ai/sessions/` |
