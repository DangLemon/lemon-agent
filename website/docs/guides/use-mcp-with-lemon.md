---
sidebar_position: 6
title: "Use MCP with Lemon AI"
description: "A practical guide to connecting MCP servers to Lemon AI, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Lemon AI

This guide shows how to actually use MCP with Lemon AI in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Lemon AI tool
- you want Lemon AI to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Lemon AI to internal APIs, databases, or company systems without modifying Lemon AI core

Do not use MCP when:
- a built-in Lemon AI tool already solves the job well
...
- Lemon AI remains the agent
- MCP servers contribute tools
- Lemon AI discovers those tools at startup or reload time
...
If you installed Lemon AI with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Lemon AI banner/status should show MCP integration when configured
- ask Lemon AI what tools it has available
...
## WSL2: bridge Lemon AI in WSL to Windows Chrome

This is the practical setup when:

- Lemon AI runs inside WSL2
...
In this setup, Lemon AI does **not** connect to Chrome directly. Instead:

- Lemon AI runs in WSL
- Lemon AI starts a local stdio MCP server
...
```text
Lemon AI (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Lemon AI stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Lemon AI core browser transport
...
```bash
lemon mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
lemon mcp test chrome-devtools-win
```

Then start a fresh Lemon AI session or run:
...
Once loaded, Lemon AI can use the MCP-prefixed browser tools directly. For example:
...
If Lemon AI runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Lemon AI from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Lemon AI from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Lemon AI:
...
2. Lemon AI-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Lemon AI will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Lemon AI to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Lemon AI to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Lemon AI remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
lemon mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
lemon mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `lemon mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Lemon AI to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Lemon AI to merge, publish, deploy, or spawn runtimes.
...
Now Lemon AI can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Lemon AI core.
...
Because Lemon AI now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
