...
Connect Lemon AI to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Lemon AI supports two WeCom integration modes:
...
> Run `lemon gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Lemon AI decrypts the message, queues it for the agent
...
(Use `lemon gateway start` only after `lemon gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Lemon AI and the token the
...
in `~/.lemon-ai/.env` values around `=` will also break signature checks. After
fixing, restart `lemon gateway run`.
...
Check `lemon gateway run` logs for the bound host/port. If the adapter bound to
