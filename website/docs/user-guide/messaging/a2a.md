...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The Lemon AI A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your Lemon AI over HTTP.

It interoperates with any A2A-compliant peer — another Lemon AI, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **Lemon AI ↔ Lemon AI across machines** — let your desktop agent hand tasks to a Lemon AI on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your Lemon AI so other frameworks' agents can send it tasks.
...
```bash
lemon tools enable a2a --platform cli        # CLI/TUI sessions
lemon tools enable a2a --platform telegram   # or any messaging platform
lemon tools enable a2a --platform a2a        # let inbound A2A tasks call peers (agent chaining)
```
...
With the platform enabled, Lemon AI serves:
...
- **Audit log** — every exchange appends to `~/.lemon-ai/a2a_audit.jsonl`.
