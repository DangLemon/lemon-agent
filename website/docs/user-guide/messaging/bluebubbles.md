...
Connect Lemon AI to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Lemon AI and the BlueBubbles server
...
Or set environment variables directly in `~/.lemon-ai/.env`:
...
By default, Lemon AI responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Lemon AI uses conservative defaults for `Lemon AI` and `@Lemon AI agent` variants.
...
When someone messages your iMessage, Lemon AI automatically sends them a pairing code. Approve it with:
```bash
lemon pairing approve bluebubbles <CODE>
```
Use `lemon pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.lemon-ai/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.lemon-ai/.env`):
```bash
...
Lemon AI will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Lemon AI
Lemon AI → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Lemon AI sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Lemon AI wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.lemon-ai/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Lemon AI resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `lemon logs gateway` for webhook errors (or `lemon logs -f` to follow in real-time)
