...
Connect Lemon AI to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Lemon AI — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Lemon AI runs it in a
...
lemon photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Lemon AI` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.lemon-ai/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.lemon-ai/.env`
...
(device token, dashboard project id) lives in `~/.lemon-ai/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Lemon AI
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Lemon AI replies with a pairing code. Approve it with:

```bash
lemon pairing approve photon <CODE>
```

Use `lemon pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.lemon-ai/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.lemon-ai/.env`):
...
By default Lemon AI responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Lemon AI` and
`@Lemon AI agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Lemon AI will reply.
...
assigned iMessage line Lemon AI uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run lemon photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `lemon photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `lemon photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `lemon photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Lemon AI sends images, voice
  ...
  media.
- **Native polls are supported.** Lemon AI sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Lemon AI, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for Lemon AI-sent messages
...
- **Message effects are supported.** Lemon AI sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `lemon send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<lemon-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Lemon AI is set
...
| `PHOTON_MENTION_PATTERNS` | Lemon AI wake words  | JSON list / comma / newline regex patterns for group mentions |
