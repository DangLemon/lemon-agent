...
sidebar_position: 12
title: "Google Chat"
description: "Set up Lemon AI as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Lemon AI to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Lemon AI
...
> Run `lemon gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Lemon AI can read (e.g.,
`~/.lemon-ai/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a lemon restart)
...
- Principal: `lemon-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Lemon AI calls
...
- **App name**: whatever you want users to see ("Lemon AI" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/lemon-chat-events`.
...
`ADDED_TO_SPACE` event that Lemon AI uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Lemon AI

Add the Google Chat section to `~/.lemon-ai/.env`:
...
Send "hola" in the test DM. The bot posts a "Lemon AI is thinking…" marker, then
...
`~/.lemon-ai/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Lemon AI is thinking…").
...
Thread support: when a user replies inside a thread, Lemon AI detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Lemon AI session.
...
3. Download the JSON. Move it onto the host that runs Lemon AI.
4. Register the client with Lemon AI (run under the profile you want it scoped to):
...
lemon -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Lemon AI home (e.g.
`~/.lemon-ai/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.lemon-ai/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.lemon-ai/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Lemon AI isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `lemon gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Lemon AI will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.lemon-ai/google_chat_user_tokens/<sanitized_email>.json` (filesystem
