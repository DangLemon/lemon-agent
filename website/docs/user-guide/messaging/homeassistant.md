...
description: Control your smart home with Lemon AI via Home Assistant integration.
...
Lemon AI integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Lemon AI"
5. Copy the token
...
Lemon AI registers four tools for smart home control:
...
Configure which events the agent sees in `~/.lemon-ai/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Lemon AI".
...
The adapter reads credentials from `~/.lemon-ai/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Lemon AI profile
...
running Lemon AI — `curl -H "Authorization: Bearer <token>" <url>/api/` should
