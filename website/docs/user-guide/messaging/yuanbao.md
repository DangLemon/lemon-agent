...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Lemon AI to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Lemon AI to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.lemon-ai/.env`:
...
All standard Lemon AI commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.lemon-ai/logs/gateway.log`
...
4. Consider enabling verbose logging: `lemon gateway run -vv`
...
Yuanbao has a maximum message size. Lemon AI automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
lemon chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
