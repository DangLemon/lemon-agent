...
description: "Chat with Lemon AI from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Lemon AI from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Lemon AI](/guides/use-voice-mode-with-lemon).
...
```mermaid
flowchart TB
    subgraph Gateway["Lemon AI Gateway"]
...
For group chats, hooks, and automation flows, Lemon AI supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Lemon AI keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Lemon AI does not hide failures just because the text resembles a silence token.
...
```bash
lemon gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
lemon gateway              # Run in foreground
lemon gateway setup        # Configure messaging platforms interactively
lemon gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo lemon gateway install --system   # Linux only: install a boot-time system service
lemon gateway start        # Start the default service
lemon gateway stop         # Stop the default service
lemon gateway status       # Check default service status
lemon gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.lemon-ai/config.yaml"
gateway:
...
```bash
lemon gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Lemon AI sends heartbeats
...
| `/update` | Update Lemon AI to the latest version |
...
with the `session_reset` section in `~/.lemon-ai/config.yaml`:
...
Configure per-platform overrides in `~/.lemon-ai/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.lemon-ai/gateway-config.yaml`:
...
lemon pairing approve telegram XKGH5N7P

# Other pairing commands:
lemon pairing list          # View pending + approved users
lemon pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Lemon AI appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.lemon-ai/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.lemon-ai/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `LEMON_HOME`:

- `~/.lemon-ai/status_phrases.yaml` or any `*.yaml` in `~/.lemon-ai/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to LEMON_HOME
...
Off by default. When enabled, Lemon AI prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.lemon-ai/config.yaml`:
...
```bash
LEMON_BACKGROUND_NOTIFICATIONS=result
```
...
lemon gateway start                 # Start the service
lemon gateway stop                  # Stop the service
lemon gateway status                # Check status
journalctl --user -u lemon-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo lemon gateway install --system
sudo lemon gateway start --system
sudo lemon gateway status --system
journalctl -u lemon-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Lemon AI installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit lemon-gateway` (or `sudo systemctl edit lemon-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `lemon update`. When `lemon update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart lemon-gateway` command (it never blocks on an interactive password prompt).
...
```bash
lemon gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `lemon update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo lemon update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/lemon-gateway`:

```
lemon ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed lemon-gateway*, /usr/bin/systemctl --no-ask-password start lemon-gateway*, /usr/bin/systemctl --no-ask-password restart lemon-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Lemon AI will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Lemon AI installations on the same machine (with different `LEMON_HOME` directories), each gets its own systemd service name. The default `~/.lemon-ai` uses `lemon-gateway`; other installations use `lemon-gateway-<hash>`. The `lemon gateway` commands automatically target the correct service for your current `LEMON_HOME`.
:::

### macOS (launchd)

```bash
lemon gateway install               # Install as launchd agent
lemon gateway start                 # Start the service
lemon gateway stop                  # Stop the service
lemon gateway status                # Check status
tail -f ~/.lemon-ai/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.lemon.gateway.plist`. It includes three environment variables:
...
- **LEMON_HOME** — scopes the gateway to your Lemon AI installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `lemon gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `LEMON_HOME` directory gets its own launchd label. The default `~/.lemon-ai` uses `ai.lemon.gateway`; other installations use `ai.lemon.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `lemon-cli` | Full access |
| Telegram | `lemon-telegram` | Full tools including terminal |
| Discord | `lemon-discord` | Full tools including terminal |
| WhatsApp | `lemon-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `lemon-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `lemon-slack` | Full tools including terminal |
| Google Chat | `lemon-google_chat` | Full tools including terminal |
| Signal | `lemon-signal` | Full tools including terminal |
| SMS | `lemon-sms` | Full tools including terminal |
| Email | `lemon-email` | Full tools including terminal |
| Home Assistant | `lemon-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `lemon-mattermost` | Full tools including terminal |
| Matrix | `lemon-matrix` | Full tools including terminal |
| DingTalk | `lemon-dingtalk` | Full tools including terminal |
| Feishu/Lark | `lemon-feishu` | Full tools including terminal |
| WeCom | `lemon-wecom` | Full tools including terminal |
| WeCom Callback | `lemon-wecom-callback` | Full tools including terminal |
| Weixin | `lemon-weixin` | Full tools including terminal |
| BlueBubbles | `lemon-bluebubbles` | Full tools including terminal |
| QQBot | `lemon-qqbot` | Full tools including terminal |
| Yuanbao | `lemon-yuanbao` | Full tools including terminal |
| Microsoft Teams | `lemon-teams` | Full tools including terminal |
| API Server | `lemon-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `lemon-webhook` | Full tools including terminal |
| Raft | `lemon-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
`platforms.<name>.enabled: false` in `~/.lemon-ai/config.yaml` is authoritative.
...
1. **Gateway log** (`~/.lemon-ai/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
