...
ntfy makes a great lightweight push channel for Lemon AI: subscribe to a topic from the [ntfy mobile app](https://ntfy.sh/docs/subscribe/phone/), send messages to the topic to talk to the agent, get the response back on your phone.

> Run `lemon gateway setup` and pick **ntfy** for a guided walk-through.

## Prerequisites

- A topic name (any unique string — `lemon-myname-2026` works fine)
...
That's it. No SDK, no daemon, no Node.js. The adapter uses `httpx` which is already a Lemon AI dependency.
...
Add these to `~/.lemon-ai/.env`:

```
NTFY_TOPIC=lemon-myname-2026
NTFY_ALLOWED_USERS=lemon-myname-2026
NTFY_HOME_CHANNEL=lemon-myname-2026
```
...
ntfy has no native authenticated user identity. The `title` field on a published message is **publisher-controlled** and can be anything the sender wants. The Lemon AI adapter does NOT use `title` for authorization — it would let any publisher who knows the topic spoof an allowed user.
...
1. Pick a topic name: `lemon-myname-2026`
2. On your phone: install the [ntfy app](https://ntfy.sh/docs/subscribe/phone/), tap **+**, enter `lemon-myname-2026`
3. On the host:
   ```bash
   echo 'NTFY_TOPIC=lemon-myname-2026' >> ~/.lemon-ai/.env
   echo 'NTFY_ALLOWED_USERS=lemon-myname-2026' >> ~/.lemon-ai/.env
...
Or target a specific topic explicitly via the cron job's `deliver:` field, or from a shell script with the [`lemon send` CLI](/guides/pipe-script-output):

```bash
lemon send ntfy:alerts-channel "Done!"
```
...
ntfy clients render markdown when the publisher sets the `X-Markdown: true` header. To enable for outgoing Lemon AI replies:
...
If you only want Lemon AI to *push* notifications to ntfy (cron summaries, alerts) and never accept messages back, set both `NTFY_TOPIC` and `NTFY_PUBLISH_TOPIC` to the same value and skip `NTFY_ALLOWED_USERS` entirely. With no allowlist, the agent never responds to inbound messages — your phone gets the pushes, but the conversation is one-way.

## Limits

- **Message size**: ntfy caps message bodies at 4096 chars. Lemon AI truncates with a warning when this is exceeded.
