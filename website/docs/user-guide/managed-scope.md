...
`~/.lemon-ai/config.yaml`, `~/.lemon-ai/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/lemon`:

```text
/etc/lemon/
├── config.yaml     # managed config layer (wins over ~/.lemon-ai/config.yaml)
└── .env            # managed env layer (wins over ~/.lemon-ai/.env + shell)
```
...
The location can be relocated with the `LEMON_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `LEMON_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Lemon AI.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export LEMON_MANAGED_DIR=/opt/org/lemon-policy
```

:::warning
A user who can set `LEMON_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `lemon doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/lemon/config.yaml` (managed) | `/etc/lemon/.env` (managed) |
| 2 | `~/.lemon-ai/config.yaml` (user) | `~/.lemon-ai/.env` (user) |
...
```bash
lemon config        # shows a header naming the managed source + the pinned keys
lemon doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Lemon AI refuses and names the source:

```bash
$ lemon config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/lemon/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `lemon config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/lemon

# Pin some config values for every user on this machine
sudo tee /etc/lemon/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/lemon/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/lemon
sudo chmod 0644 /etc/lemon/config.yaml /etc/lemon/.env
```

Changes take effect on the next Lemon AI start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`lemon doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Lemon AI as `root`), managed scope is advisory.
