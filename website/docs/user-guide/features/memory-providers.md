...
Lemon AI ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
lemon memory setup      # interactive picker + configuration
lemon memory status     # check what's active
lemon memory off        # disable external provider
```

You can also select the active memory provider via `lemon plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.lemon-ai/config.yaml`:
...
When a memory provider is active, Lemon AI automatically:
...
**Setup Wizard:**
```bash
lemon memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `lemon honcho setup` command still works (it now redirects to `lemon memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$LEMON_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$LEMON_HOME/honcho.json` > `~/.lemon-ai/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/DangLemon/lemon-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/lemon).
...
:::tip Migrating from `lemon honcho`
If you previously used `lemon honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Lemon AI profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Lemon AI profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Lemon AI profile. Host key `lemon` → default; `lemon.<profile>` for others. |
...
```bash
lemon profile create coder --clone
```

`--clone` creates a `lemon.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Lemon AI profile, creates host blocks for any profile without one, inherits settings from the default `lemon` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `lemon memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/DangLemon/lemon-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/lemon).
...
lemon memory setup    # select "openviking"
# Or manually:
lemon config set memory.provider openviking
```

`lemon memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.lemon-ai/.env`, and for named profiles use
`~/.lemon-ai/profiles/<profile>/.env`.
...
Peer identity is optional. By default, Lemon AI sends no peer ID and writes
...
Set `memory.openviking.agent: lemon` to restore the old peer-scoped writes.
...
Lemon AI sends `User-Agent: openviking-memory-lemon/<version>` on OpenViking
...
**Setup (Platform):**
```bash
lemon memory setup    # select "mem0" → "Platform"
# Or manually:
lemon config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.lemon-ai/.env
```

**Setup (OSS):**
```bash
lemon memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
lemon memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
lemon memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
lemon memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
lemon memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.lemon-ai/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.lemon-ai/.env
```
...
**Config:** `$LEMON_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.lemon-ai/.env`.
...
| `user_id` | `lemon-user` | User identifier |
| `agent_id` | `lemon` | Agent identifier |
...
**Switching modes:** Re-run `lemon memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
lemon memory setup    # select "hindsight"
# Or manually:
lemon config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.lemon-ai/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p lemon ui start`

**Config:** `$LEMON_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `lemon` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Lemon AI and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/DangLemon/lemon-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
lemon memory setup    # select "holographic"
# Or manually:
lemon config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.lemon-ai-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$LEMON_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
lemon memory setup    # select "retaindb"
# Or manually:
lemon config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.lemon-ai/.env
```
...
# Then configure Lemon AI
lemon memory setup    # select "byterover"
# Or manually:
lemon config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$LEMON_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=lemon), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
lemon memory setup    # select "supermemory"
# Or manually:
lemon config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.lemon-ai/.env
```
...
Before running `lemon memory setup`, set `base_url` in
`$LEMON_HOME/supermemory.json`:
...
Then run `lemon memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$LEMON_HOME/supermemory.json`
...
| `container_tag` | `lemon` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `lemon-{identity}` → `lemon-coder`) to isolate memories per Lemon AI profile
...
| **Requires** | `pip install lemon-memori` + `lemon-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install lemon-memori
lemon-memori install
lemon config set memory.provider memori
lemon memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `lemon-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$LEMON_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$LEMON_HOME/` so each profile has its own credentials
