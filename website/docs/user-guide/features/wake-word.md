...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Lemon AI' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Lemon AI")

The wake word turns Lemon AI into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Lemon AI listens in the background for a spoken
trigger phrase. Say it, and Lemon AI starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Lemon AI backend (for example a
...
Lemon AI supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey lemon"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey lemon"** — a model for it ships with Lemon AI, so
...
```bash
cd ~/.lemon-ai/lemon-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `lemon` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.lemon-ai/config.yaml`,
...
wake_word:
  ...
  phrase: "hey lemon"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_lemon         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey lemon" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey lemon" utterances are missed.
...
default) to let Lemon AI choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Lemon AI installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Lemon AI surfaces, and `surface` picks which one
...
| `cli` | Only the classic `lemon` CLI. |
| `tui` | Only `lemon --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Lemon AI surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Lemon AI does not silently fail over to another open surface.
...
"Hey Lemon AI" works out of the box — the bundled openWakeWord model
(`model: hey_lemon`) is the default. To wake on something else, the easiest
...
command (`lemon -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.lemon-ai/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("lemon" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Lemon AI" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey lemon"
  porcupine:
    keyword: ~/.lemon-ai/wakewords/hey_lemon.ppn
```

Set your access key in `~/.lemon-ai/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `lemon tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `lemon-agent[wake]`).
...
but the phrase never fires. Lemon AI detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Lemon AI
backend (it may appear as your terminal, `python`, or Lemon AI), then toggle the
...
```bash
lemon config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
lemon config set wake_word.input_device null
```
