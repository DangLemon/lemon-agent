...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Lemon AI CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Lemon AI CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.lemon-ai/skins/mytheme.yaml
```

Or set the default skin in `~/.lemon-ai/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Lemon AI — gold and kawaii | `Lemon AI` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Lemon AI` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Lemon AI` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Lemon AI` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Lemon AI` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Lemon AI` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Lemon AI! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` ⚕ Lemon AI ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default LEMON_AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.lemon-ai/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.lemon-ai/skins/mytheme.yaml
...
## Lemon AI Mod — Visual Skin Editor

[Lemon AI Mod](https://github.com/cocktailpeanut/lemon-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Lemon AI Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/lemon-mod/master/nous.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Lemon AI skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.lemon-ai/skins/`
- Activates a skin by updating `~/.lemon-ai/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/lemon-mod.git
cd lemon-mod/app
npm install
npm start
```
...
6. Click **Save** to write the skin YAML to `~/.lemon-ai/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Lemon AI Mod respects the `LEMON_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `lemon_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.lemon-ai/skins/` take precedence over built-in skins with the same name.
