...
Lemon AI can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<LEMON_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Lemon AI renders the sprite at full fidelity when your
...
lemon pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `lemon pets list [query] [--limit N]` |
| List installed pets | `lemon pets list --installed` |
| Install a pet | `lemon pets install <slug> [--select] [--force]` |
| Set the active pet | `lemon pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `lemon pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `lemon pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `lemon pets off` |
| Remove an installed pet | `lemon pets remove <slug>` |
| Diagnose setup | `lemon pets doctor` |

`lemon pets show` flags:
...
Beyond installing pre-made pets from the gallery, Lemon AI can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `lemon pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `lemon tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `LEMON_PET_IMAGE_PROVIDER` env var (e.g. `LEMON_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Lemon AI is
...
Run `lemon pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Lemon AI uses its own
  profile-scoped `<LEMON_HOME>/pets/` instead — install through `lemon pets`.

## See also

- The [`lemon-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-lemon-agent.md)
