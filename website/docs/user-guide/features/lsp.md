...
Lemon AI runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Lemon AI
...
1. Lemon AI captures a baseline of current diagnostics for the file.
...
…). Lemon AI auto-detects the binary on PATH or in
`<LEMON_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Lemon AI at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<LEMON_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`lemon lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Lemon AI installs both packages together when you
run `lemon lsp install typescript` or auto-install fires on first
use.

## CLI

```
lemon lsp status          # service state + per-server install status
lemon lsp list            # registry, optionally --installed-only
lemon lsp install <id>    # eagerly install one server
lemon lsp install-all     # try every server with a known recipe
lemon lsp restart         # tear down running clients
lemon lsp which <id>      # print resolved binary path
```

`lemon lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <LEMON_HOME>/lsp/bin
...
When `install_strategy: auto`, Lemon AI installs binaries into
`<LEMON_HOME>/lsp/bin/`. NPM packages land in
`<LEMON_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Lemon AI-owned and is
...
**single process** per Lemon AI process: the first Python project spawns
...
**`lemon lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<LEMON_HOME>/lsp/bin/`. Run
`lemon lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `lemon lsp status`**
...
When `lemon lsp status` shows a `Backend warnings` section, install
...
`~/.lemon-ai/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.lemon-ai/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `lemon lsp restart` to clear the set;
