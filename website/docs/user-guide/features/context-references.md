...
Type `@` followed by a reference to inject content directly into your message. Lemon AI expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Lemon AI env: `$LEMON_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$LEMON_HOME/skills/.hub/`
