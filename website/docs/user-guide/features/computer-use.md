...
Lemon AI can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Lemon AI integration. It
...
**Fresh installs already have the driver.** The Lemon AI installer
...
- **`lemon tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
lemon computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `lemon computer-use
status` to verify the install.

Already have cua-driver? Lemon AI reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `lemon update`, and the first
`computer_use` call of a session, Lemon AI checks the local version and
...
selected with `LEMON_CUA_DRIVER_CMD` stays
under your control, so Lemon AI reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Lemon AI autodetection is a
planned cua-driver follow-up, so currently point Lemon AI at that directory or
...
interface. The built-in toolset provides Lemon AI actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `lemon computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Lemon AI host identity. |
...
```
lemon -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.lemon-ai/config.yaml`.

## Permission modes and logged-in browser profiles

Lemon AI maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Lemon AI session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Lemon AI approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Lemon AI risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.lemon-ai/cua-manifest.yaml
```
...
for the format). Lemon AI launches a private runtime with
...
identity instead of resetting with every Lemon AI build), and Lemon AI verifies
...
resetting or closing the Lemon AI session, cancellation cleanup, or process exit
closes that transport session. Lemon AI also stops private runtimes that it
launched for bounded or unrestricted access. One Lemon AI
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Lemon AI host identity.
...
## `lemon computer-use doctor` — your first triage stop

`lemon computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ lemon computer-use doctor
...
Lemon AI run declares a public cua-driver **session name** (something like
...
without it. Lemon AI disables it automatically where it is a known failure
...
Lemon AI keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Lemon AI-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Lemon AI
autodetection is a planned cua-driver follow-up, so currently point Lemon AI at
...
These are **platform deep dives, not duplicates of the Lemon AI skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Lemon AI
...
image capture also saves a bounded, shareable copy under Lemon AI' image cache
...
Lemon AI applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.lemon-ai/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Lemon AI applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Lemon AI agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Lemon AI agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Lemon AI warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Lemon AI from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. Lemon AI passes the
...
```
LEMON_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
LEMON_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Lemon AI disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Lemon AI sets
...
When it's on, `lemon computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Lemon AI at a binary you built from source instead
of the published release. Lemon AI resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`LEMON_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Lemon AI testing (see notes).
...
### Option B — point Lemon AI straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`LEMON_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
LEMON_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
LEMON_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Lemon AI is using your build

- `lemon computer-use status` prints the resolved binary path and
  version.
- `lemon computer-use doctor` confirms the binary is reachable and
...
- **Lemon AI spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Lemon AI YOLO, Lemon AI instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Lemon AI
  change needed either way.
- **Local builds skip the version check.** Lemon AI warns when the
...
**First action when anything's off: run `lemon computer-use doctor`.**
...
Run `lemon computer-use install` to fetch the cua-driver binary, or
run `lemon tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `lemon computer-use doctor` will
...
- **Lemon AI-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Lemon AI `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Lemon AI autodetection is a
  planned follow-up; currently point Lemon AI at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Lemon AI among them)
