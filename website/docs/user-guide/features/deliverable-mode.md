...
When Lemon AI runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.lemon-ai/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.lemon-ai/config.yaml` (switchable per session
via `/personality`).
...
If you use Lemon AI' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.lemon-ai/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Lemon AI's deliverable
