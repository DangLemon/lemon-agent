...
chat completions — use your Lemon AI-managed provider subscription as their
...
| Use case | "Use Lemon AI as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Lemon AI stores
the refresh token in `~/.lemon-ai/auth.json` — the same place all Lemon AI
...
interface in `lemon_cli/proxy/adapters/`.
...
If you see `not logged in`, run `lemon portal`. If you see
...
`lemon_cli/proxy/adapters/<provider>.py` and registering it in
