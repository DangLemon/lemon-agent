...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Lemon AI automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Lemon AI auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
lemon auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
lemon auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
lemon auth add anthropic --type oauth
...
Run `lemon auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `lemon auth` | Interactive pool management wizard |
| `lemon auth list` | Show all pools and credentials |
| `lemon auth list <provider>` | Show a specific provider's pool |
| `lemon auth add <provider>` | Add a credential (prompts for type and key) |
| `lemon auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `lemon auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `lemon auth remove <provider> <index>` | Remove credential by 1-based index |
| `lemon auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `lemon auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `lemon model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via lemon model:
lemon auth list
# Shows:
...
lemon auth add Together.ai --api-key sk-together-second-key
```
...
Lemon AI automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Lemon AI PKCE OAuth | `~/.lemon-ai/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `lemon auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `lemon auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Lemon AI can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Lemon AI-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`lemon_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`lemon_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`agent/turn_api_error.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.lemon-ai/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Lemon AI' credential store, so its token remains persistable.
