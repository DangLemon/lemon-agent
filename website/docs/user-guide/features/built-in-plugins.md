...
description: "Plugins shipped with Lemon AI that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Lemon AI ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.lemon-ai/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Lemon AI Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.lemon-ai/plugins/<name>/`
3. **Project** — `./.lemon-ai/plugins/<name>/` (requires `LEMON_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `lemon_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `lemon memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `lemon plugins list` and the interactive `lemon plugins` UI), but none load until you explicitly enable them:

```bash
lemon plugins enable disk-cleanup
```

Or via `~/.lemon-ai/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Lemon AI. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
lemon plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `lemon plugins enable <name>`.
...
| `lemon-achievements` | dashboard tab | Steam-style collectible badges generated from your real Lemon AI session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `lemon memory` and `lemon plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `LEMON_HOME` or `/tmp/lemon-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under LEMON_HOME | always | Never |
...
**State** — everything lives at `$LEMON_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `LEMON_HOME` or `/tmp/lemon-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `lemon plugins enable disk-cleanup` (or check the box in `lemon plugins`).

**Disabling again:** `lemon plugins disable disk-cleanup`.
...
**Enabling:** `lemon plugins enable security-guidance` (or check the box in `lemon plugins`).

**Disabling again:** `lemon plugins disable security-guidance`.
...
Traces Lemon AI turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Lemon AI' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `lemon logs`.
...
```bash
lemon tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Lemon AI and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
lemon plugins enable observability/langfuse
```

Then put the credentials in `~/.lemon-ai/.env`:

```bash
LEMON_LANGFUSE_PUBLIC_KEY=pk-lf-...
LEMON_LANGFUSE_SECRET_KEY=sk-lf-...
LEMON_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Lemon AI turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `LEMON_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Lemon AI session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `lemon chat` session lives under one Langfuse session.

**Verify:**

```bash
lemon plugins list                 # observability/langfuse should show "enabled"
lemon chat -q "hello"              # check the Langfuse UI for a "Lemon AI turn" trace
```
...
| `LEMON_LANGFUSE_RELEASE` | — | Release/version tag |
| `LEMON_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `LEMON_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `LEMON_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Lemon AI-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Lemon AI-prefixed wins when both are set.
...
**Disabling:** `lemon plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### NeMo Relay native integration (migration note)

NeMo Relay is no longer a bundled Lemon AI plugin. Do not run `lemon plugins enable observability/nemo_relay`; Lemon AI core now owns the Relay session, turn, LLM, and tool lifecycles.

To opt into Relay middleware or exporters, create a standard Relay `plugins.toml`, then set `LEMON_NEMO_RELAY_PLUGINS_TOML` to that file before starting Lemon AI. The policy is process-wide for every profile hosted by that Lemon AI process. See the [NeMo Relay observability configuration](https://docs.nvidia.com/nemo/relay/configure-plugins/observability/about) for ATOF, ATIF, and OpenTelemetry options.

The old `LEMON_NEMO_RELAY_ATOF_*` and `LEMON_NEMO_RELAY_ATIF_*` settings no longer activate exporters. `lemon doctor` reports these stale settings when no replacement `plugins.toml` is selected.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `lemon.session.segment` plus `lemon.session.segment_reason` (`compaction` or `max_turns`).
...
- Post-meeting artifacts (transcript, status) saved under `~/.lemon-ai/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
lemon plugins enable google_meet
lemon meet setup   # preflight: playwright, chromium, auth file
lemon meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `lemon plugins disable google_meet`. Any saved transcripts stay in `~/.lemon-ai/workspace/meetings/` until you remove them.

### lemon-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Lemon AI session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Lemon AI feature changes.

**How it works:**

- Scans your entire `~/.lemon-ai/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$LEMON_HOME/plugins/lemon-achievements/state.json`
...
| Secret | Hidden until Lemon AI detects the first related signal in your history |

**API** — routes mount under `/api/plugins/lemon-achievements/`:
...
**State files** — live under `$LEMON_HOME/plugins/lemon-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Lemon AI updates. |
...
**Enabling:** Nothing to enable — `lemon-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `lemon dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/lemon-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.lemon-ai/plugins/lemon-achievements/` that ships no dashboard. The plugin's state files under `$LEMON_HOME/plugins/lemon-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Lemon AI plugin — see [Build a Lemon AI Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.lemon-ai/plugins/<name>/`
- Manifest source is reported as `bundled` in `lemon plugins list`
