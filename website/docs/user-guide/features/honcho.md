...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Lemon AI's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Lemon AI instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
lemon memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `lemon` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Lemon AI syncs them back at session init.
...
The `lemon honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `lemon memory setup honcho` (or run `lemon memory setup` and pick it from the list); the `lemon honcho` subcommand then appears on the next invocation.

```bash
lemon memory setup honcho    # Configure Honcho directly (works before activation)
lemon honcho status          # Connection status, config, and key settings
lemon honcho setup           # Redirects to `lemon memory setup` (post-activation alias)
lemon honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
lemon honcho peer            # Show or update peer names + dialectic reasoning level
lemon honcho mode            # Show or set recall mode (hybrid/context/tools)
lemon honcho tokens          # Show or set token budget for context and dialectic
lemon honcho identity        # Seed or show the AI peer's Honcho identity
lemon honcho sync            # Sync Honcho config to all existing profiles
lemon honcho peers           # Show peer identities across all profiles
lemon honcho sessions        # List known Honcho session mappings
lemon honcho map             # Map current directory to a Honcho session name
lemon honcho enable          # Enable Honcho for the active profile
lemon honcho disable         # Disable Honcho for the active profile
lemon honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `lemon honcho`

If you previously used the standalone `lemon honcho setup`:
...
No re-login or re-setup needed. Run `lemon memory setup` and select "honcho" — the wizard detects your existing config.
