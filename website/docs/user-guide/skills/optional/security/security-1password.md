...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Lemon AI |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Lemon AI terminal calls (desktop app flow only)
...
### Service Account (recommended for Lemon AI)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${LEMON_HOME:-~/.lemon-ai}/.env` (the skill will prompt for this on first load).
...
## Lemon AI Execution Pattern (desktop app flow)

Lemon AI terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/lemon-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/lemon-op.sock"
