...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/devops/watchers` |
...
All three live in `$LEMON_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$LEMON_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $LEMON_HOME/skills/devops/watchers/scripts/watch_rss.py \
  --name hn --url https://news.ycombinator.com/rss --max 5
```

Watch a GitHub repo (set `GITHUB_TOKEN` in `${LEMON_HOME:-~/.lemon-ai}/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $LEMON_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name lemon-issues --repo DangLemon/lemon-agent --scope issues
```

Poll an arbitrary JSON API:

```bash
python $LEMON_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$LEMON_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $LEMON_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $LEMON_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$LEMON_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
