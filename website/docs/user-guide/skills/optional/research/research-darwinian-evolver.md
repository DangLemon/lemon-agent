...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research\darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Lemon AI |
| License | MIT |
...
import upstream classes into Lemon AI itself.
...
```bash
mkdir -p ~/.lemon-ai/cache/darwinian-evolver && cd ~/.lemon-ai/cache/darwinian-evolver
...
```bash
cd ~/.lemon-ai/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.lemon-ai/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.lemon-ai/skills/research/darwinian-evolver
DE_DIR=~/.lemon-ai/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Lemon AI core.
   Custom driver scripts under `~/.lemon-ai/skills/...` are user-side and fine.
