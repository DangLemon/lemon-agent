...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Lemon AI core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Lemon AI native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Lemon AI native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Lemon AI native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Lemon AI usage patterns
...
- Do not silently choose Parallel when Lemon AI native tools are already sufficient.
