...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${LEMON_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${LEMON_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${LEMON_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client lemon
```
...
```bash
PUBLISH="${LEMON_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client lemon
```
...
```bash
DRIVE="${LEMON_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `lemon`)    |
