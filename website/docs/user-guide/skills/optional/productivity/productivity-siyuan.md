...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/productivity/siyuan` |
...
3. Store it in `${LEMON_HOME:-~/.lemon-ai}/.env`:
   ```
...
```yaml
# In ~/.lemon-ai/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
