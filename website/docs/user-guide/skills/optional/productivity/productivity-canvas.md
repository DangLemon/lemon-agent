...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Lemon AI"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${LEMON_HOME:-~/.lemon-ai}/.env`:
...
```bash
CANVAS="python $LEMON_HOME/skills/productivity/canvas/scripts/canvas_api.py"
