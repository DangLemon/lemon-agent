...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/productivity/telephony` |
...
This optional skill gives Lemon AI practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${LEMON_HOME:-~/.lemon-ai}/.env`
...
It does **not** turn Lemon AI into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Lemon AI memory
...
### 1) "I want Lemon AI to own a real phone number"
Use **Twilio**.
...
- pairs well with Lemon AI `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${LEMON_HOME:-~/.lemon-ai}/.env`
...
```bash
SCRIPT="$(find ~/.lemon-ai/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
lemon skills search telephony
lemon skills install official/productivity/telephony
```
...
Then save credentials into Lemon AI:
...
3. Buy it and save it into `${LEMON_HOME:-~/.lemon-ai}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Lemon AI calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Lemon AI's existing `text_to_speech` support.

Use this when:
- you want the call to use Lemon AI's configured TTS voice rather than Twilio `<Say>`
...
Recommended Lemon AI TTS -> Twilio Play workflow:

1. Generate the audio with Lemon AI `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Lemon AI to create the message audio with `text_to_speech`
...
Important note:
- Lemon AI TTS is great for prerecorded outbound messages
...
- Lemon AI STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Lemon AI memory.
...
- Do not store arbitrary third-party phone numbers in Lemon AI memory.
...
3. persist that number to `${LEMON_HOME:-~/.lemon-ai}/.env`
