...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/mcp/fastmcp` |
...
| Related skills | [`lemon-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-lemon-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Lemon AI or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Lemon AI. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.lemon-ai/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.lemon-ai/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Lemon AI integration, either:

- configure the server in `~/.lemon-ai/config.yaml` using the `native-mcp` skill, or
...
### Lemon AI cannot see the deployed server

The server-building part may be correct while the Lemon AI config is not. Load the `native-mcp` skill and configure the server in `~/.lemon-ai/config.yaml`, then restart Lemon AI.
