---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Lemon AI"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Lemon AI"
---
...
Import an OpenClaw setup (memories, skills) into Lemon AI.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `lemon skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | Lemon AI (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Lemon AI`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Lemon AI Migration

Use this skill when a user wants to move their OpenClaw setup into Lemon AI with minimal manual cleanup.
...
```bash
lemon claw migrate              # Full interactive migration
lemon claw migrate --dry-run    # Preview what would be migrated
lemon claw migrate --preset user-data   # Migrate without secrets
lemon claw migrate --overwrite  # Overwrite existing conflicts
lemon claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `lemon setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_lemon.py` to:

- import `SOUL.md` into the Lemon AI home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Lemon AI memory entries
- merge OpenClaw command approval patterns into Lemon AI `command_allowlist`
- migrate Lemon AI-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Lemon AI working-directory configuration
- copy OpenClaw skills into `~/.lemon-ai/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Lemon AI workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.lemon-ai/tts/`
- archive non-secret docs that do not have a direct Lemon AI destination
...
- `scripts/openclaw_to_lemon.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.lemon-ai/skills/migration/openclaw-migration/scripts/openclaw_to_lemon.py`

Do not guess a shorter path like `~/.lemon-ai/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.lemon-ai/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Lemon AI-compatible secrets, currently:
...
Lemon AI CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Lemon AI. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Lemon AI workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Lemon AI skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.lemon-ai/skills/migration/openclaw-migration/scripts/openclaw_to_lemon.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.lemon-ai/skills/migration/openclaw-migration/scripts/openclaw_to_lemon.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.lemon-ai/skills/migration/openclaw-migration/scripts/openclaw_to_lemon.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.lemon-ai/skills/migration/openclaw-migration/scripts/openclaw_to_lemon.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.lemon-ai/skills/migration/openclaw-migration/scripts/openclaw_to_lemon.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.lemon-ai/skills/migration/openclaw-migration/scripts/openclaw_to_lemon.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Lemon AI unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Lemon AI targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Lemon AI destination. Unsupported auth blobs must still be reported as skipped.
...
- Lemon AI persona state imported
- Lemon AI memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.lemon-ai/skills/openclaw-imports/`
