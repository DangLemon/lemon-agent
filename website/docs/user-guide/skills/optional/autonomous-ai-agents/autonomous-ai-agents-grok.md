...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/autonomous-ai-agents/grok` |
| Path | `optional-skills/autonomous-ai-agents\grok` |
| Version | `0.1.1` |
| Author | Matt Maximo (MattMaximo), Lemon AI |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Coding-Agent`, `Grok`, `xAI`, `Code-Review`, `Refactoring`, `Automation` |
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`lemon-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-lemon-agent) |
...
# Grok Build CLI — Lemon AI Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Lemon AI terminal. Grok
...
1. Prepare stable input files first with Lemon AI tools (`read_file`,
...
2. **Don't conflate Lemon AI' xAI auth with the `grok` CLI's auth.** Lemon AI'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Lemon AI' xAI auth carries
   over.
