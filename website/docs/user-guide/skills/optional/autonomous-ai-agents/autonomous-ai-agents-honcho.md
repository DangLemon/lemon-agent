---
title: "Honcho — Configure and troubleshoot Honcho memory for Lemon AI"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Lemon AI"
---
...
Configure and troubleshoot Honcho memory for Lemon AI.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `lemon skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Lemon AI

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Lemon AI profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/lemon#running-honcho-locally-with-lemon

### Verify

```bash
lemon honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Lemon AI profile's AI peer
...
Honcho models conversations as interactions between **peers**. Lemon AI creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Lemon AI instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Lemon AI run |
| `global` | Single session across all directories |

Manual override: `lemon honcho map my-project-name`
...
Each Lemon AI profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block lemon.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `lemon.coder` host block in `honcho.json`
...
```bash
lemon honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Lemon AI when Honcho memory is active.
...
Config file: `$LEMON_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `lemon honcho setup`. Ensure `memory.provider: honcho` is in `~/.lemon-ai/config.yaml`.

### Memory not persisting across sessions
Check `lemon honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `lemon profile create <name> --clone`. For existing profiles: `lemon honcho sync`.
...
| Command | Description |
|---------|-------------|
| `lemon honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `lemon honcho status` | Show resolved config, connection test, peer info for active profile |
| `lemon honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `lemon honcho disable` | Disable Honcho for the active profile |
| `lemon honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `lemon honcho peers` | Show peer identities across all profiles |
| `lemon honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `lemon honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `lemon honcho sessions` | List known directory-to-session-name mappings |
| `lemon honcho map <name>` | Map current working directory to a Honcho session name |
| `lemon honcho identity` | Seed AI peer identity or show both peer representations |
| `lemon honcho sync` | Create host blocks for all Lemon AI profiles that don't have one yet |
| `lemon honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Lemon AI + Honcho |
| `lemon memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `lemon memory status` | Show active memory provider and config |
| `lemon memory off` | Disable external memory provider |
