...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/creative/touchdesigner-mcp` |
...
```
Lemon AI -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${LEMON_HOME:-$HOME/.lemon-ai}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Lemon AI config (if missing)
...
3. **Restart Lemon AI session** to pick up the new MCP server
