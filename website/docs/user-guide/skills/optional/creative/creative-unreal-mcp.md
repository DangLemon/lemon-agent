...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Lemon AI MCP catalog. The
...
Two halves, in this order: the editor side must be up before Lemon AI connects.
...
### One-time, Lemon AI side

    lemon mcp install unreal-engine
...
`~/.lemon-ai/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Lemon AI — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Lemon AI connects
...
2. Start the Lemon AI session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Lemon AI
they appear as:

| Lemon AI tool | Purpose |
|---|---|
...
then happens at `lemon mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Lemon AI runs batched calls
...
- **Start order matters.** Editor + server up first, then the Lemon AI
...
- **Timeouts on long operations.** Lemon AI' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.lemon-ai/config.yaml` for
