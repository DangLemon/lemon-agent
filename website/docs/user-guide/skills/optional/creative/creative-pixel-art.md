...
| | |
|---|---|
| Source | Optional — install with `lemon skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.lemon-ai/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.lemon-ai/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Lemon AI installs package this)
