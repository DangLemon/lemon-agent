...
| List/filter by metadata (age, source, cost, tokens, workspace) | `lemon sessions list` / `stats` via terminal |
| Rename | `lemon sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `lemon sessions archive <filters>` |
| Delete (destructive) | `lemon sessions delete` / `lemon sessions prune <filters>` |
| Export before deleting anything valuable | `lemon sessions export --session-id <id> --format md` |
...
`lemon sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `lemon sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `lemon sessions list`
