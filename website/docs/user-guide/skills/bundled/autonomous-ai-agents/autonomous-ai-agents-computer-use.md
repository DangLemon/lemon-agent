...
| Version | `2.0.0` |
| Author | Francesco Bonacci (f-trycua), Lemon AI |
| License | MIT |
...
Lemon AI drives [cua-driver](https://github.com/trycua/cua) under the hood.
This wrapper skill teaches the Lemon AI `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. Lemon AI autodetection is a
planned cua-driver follow-up, so currently point Lemon AI at the resulting
...
|---|---|
| `cua-driver not installed` | Run `lemon computer-use install`, or `lemon tools` and enable Computer Use |
| Captures consistently return empty / "no on-screen window" | On Linux: DISPLAY may not be set (X11) or you're on pure Wayland — ask the user to run `lemon computer-use doctor`. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |
...
| Type text disappears into a terminal emulator | cua-driver detects terminals (Ghostty, iTerm2, Terminal.app, Windows Terminal, mintty, etc.) and routes through key-event synthesis — should "just work" on a recent cua-driver. If it doesn't, ask the user to run `lemon computer-use doctor` |
...
| Anything else weird | **First action: ask the user to run `lemon computer-use doctor`.** It runs the cua-driver `health_report` MCP tool and prints a structured per-check matrix. Their output tells you (and them) exactly what's wrong |
...
Lemon AI intentionally keeps THIS skill focused on the Lemon AI-side
...
Lemon AI autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point Lemon AI at that
