...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`lemon-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-lemon-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Lemon AI terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Lemon AI itself, `model.provider: openai-codex` uses Lemon AI-managed Codex
OAuth from `~/.lemon-ai/auth.json` after `lemon auth add openai-codex`. For the
...
## Lemon AI Gateway Caveat

When invoking the Codex CLI from a Lemon AI gateway/service context (for example,
