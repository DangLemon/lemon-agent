...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Lemon AI terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Lemon AI's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `lemon setup --portal` — and you're ready to `lemon chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Lemon AI also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `lemon --tui` — see the [TUI](tui.md) guide.
:::
...
lemon chat --query-file prompt.txt
lemon chat --query-file - < prompt.txt

# With a specific model
lemon chat --model "anthropic/claude-sonnet-4"

# With a specific provider
lemon chat --provider nous        # Use Nous Portal
lemon chat --provider openrouter  # Force OpenRouter

# With specific toolsets
lemon chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
lemon -s lemon-agent-dev,github-auth
lemon chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
lemon --continue             # Resume the most recent CLI session (-c)
lemon --resume <session_id>  # Resume a specific session by ID (-r)
lemon --resume latest        # Resume the most recent session (same as -c)
lemon --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
lemon chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
lemon -w                         # Interactive mode in worktree
lemon -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`lemon -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
lemon worktree list              # audit: age, size, verdict, reason per tree
lemon worktree prune             # remove safe trees + delete merged branches
lemon worktree prune --dry-run   # show the plan without changing anything
lemon worktree prune --trees-only     # leave local branches alone
lemon worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running lemon session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.lemon-ai/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `lemon -w` for days — no longer accumulate merged scratch trees
...
The `lemon plugins` commands manage native Lemon AI plugins and portable Agent
...
lemon plugins disable <plugin-name>
lemon plugins update <plugin-name>
lemon plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Lemon AI currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Lemon AI CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Lemon AI CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `LEMON_YOLO_MODE` is on (either `lemon --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Lemon AI to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.lemon-ai/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Lemon AI supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Lemon AI replies |
...
```bash
lemon -s lemon-agent-dev,github-auth
lemon chat -s github-pr-workflow -s github-auth
```

Lemon AI loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.lemon-ai/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.lemon-ai/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Lemon AI..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Lemon AI also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Lemon AI recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Lemon AI — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Lemon AI is working, Lemon AI prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Lemon AI to the background — just like any terminal process. The shell prints a confirmation:

```
Lemon AI has been suspended. Run `fg` to bring Lemon AI back.
```
...
Resume this session with:
  lemon --resume 20260225_143052_a1b2c3
...
```bash
lemon --continue                          # Resume the most recent CLI session
lemon -c                                  # Short form
lemon -c "my project"                     # Resume a named session (latest in lineage)
lemon --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
lemon --resume "refactoring auth"         # Resume by title
lemon --resume latest                     # Resume the most recent session (same as -c)
lemon --resume latest --in ./my-project   # Latest session for ./my-project's workspace
lemon -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `lemon sessions rename <id> <title>` from the command line. Use `lemon sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Lemon AI's SQLite state database under `~/.lemon-ai/state.db`. The database keeps:
...
Lemon AI immediately confirms the task and gives you back the prompt:
