...
If you only run one Lemon AI agent, you don't need this page — see
...
[Connecting Desktop to Many Lemon AI Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Lemon AI agents that should all
...
(`ai.lemon.gateway-<name>.plist`) or systemd user service
(`lemon-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
lemon profile create coder
lemon profile create personal-bot
...
```bash
lemon config set gateway.multiplex_profiles true
lemon gateway restart
```

Equivalently, in the default profile's `~/.lemon-ai/config.yaml`:
...
You do **not** run `lemon gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `lemon gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.lemon-ai/profiles/coder/.env`; the default listener key is rejected.
...
home). `lemon status` reports the multiplexer and the profiles it serves;
`lemon status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/lemon-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: lemon-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    lemon gateway "$action"
  else
    lemon -p "$profile" gateway "$action"
  fi
}
...
```bash
lemon-gateways start      # start every configured profile
lemon-gateways stop       # stop every configured profile
lemon-gateways restart    # restart all
lemon-gateways status     # status across all
lemon-gateways list       # delegates to `lemon gateway list`
```

:::tip
The `default` profile is targeted with `lemon gateway <action>` (no `-p`),
not `lemon -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `lemon -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.lemon.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/lemon-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.lemon.gateway.plist` /
`lemon-gateway.service`.
...
```bash
# Default profile
tail -f ~/.lemon-ai/logs/gateway.log
tail -f ~/.lemon-ai/logs/gateway.error.log

# Named profile
tail -f ~/.lemon-ai/profiles/<name>/logs/gateway.log
tail -f ~/.lemon-ai/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.lemon-ai/logs/gateway.log ~/.lemon-ai/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
lemon logs -f                  # follow default profile
lemon -p coder logs -f         # follow one profile
lemon logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
lemon profile list             # profiles + model + gateway state
lemon-gateways status          # full status across every profile
launchctl list | grep lemon    # macOS — PIDs and labels
systemctl --user list-units 'lemon-gateway-*'   # Linux — units
```
...
```
~/.lemon-ai/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.lemon-ai/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
lemon config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.lemon-ai/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=lemon --why="gateways running" \
  sleep infinity &
...
`lemon-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.lemon-ai/.env ~/.lemon-ai/profiles/*/.env
```

## Updating the code

`lemon update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `lemon gateway start` after a previous `lemon gateway stop`. The
...
```bash
ps -ef | grep "lemon_cli.*-p <profile>"
cat ~/.lemon-ai/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.lemon.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.lemon.gateway-<profile>.plist

# Linux
systemctl --user restart lemon-gateway-<profile>.service
```

### Health check

```bash
lemon doctor                  # default profile
lemon -p <profile> doctor     # one profile
```
