...
# Connecting Desktop to Many Lemon AI Instances

Register every Lemon AI backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Lemon AI Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Lemon AI gateway…"**) deep-links straight to
...
list of Lemon AI gateways. Its intro says it plainly: *"Manage this device and
every Lemon AI gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Lemon AI runtime managed by this app." | automatic |
| **Remote gateway** | "A Lemon AI gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Lemon AI install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Lemon AI Cloud** | "A hosted instance discovered through your Lemon AI Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Lemon AI Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Lemon AI Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **Lemon AI Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Lemon AI Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `lemon serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated Lemon AI agents
...
connection is registered) dispatches `lemon update` to every eligible
...
- **Lemon AI Cloud** instances are skipped with a *"Managed by Lemon AI Cloud"*
...
panel, ⌘K **Update Lemon AI**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `lemon serve` is running on the remote host, the
...
- **"Update Lemon AI to chat with agents on other connections"** — the
