...
Lemon AI can run open models entirely on your own machine. It downloads and
...
sizes, GPU layers, or quantization. You pick a model; Lemon AI does the rest.
...
2. Click **Install runtime**. Lemon AI downloads the official llama.cpp
...
That's the whole flow. The server starts and stops with Lemon AI, restarts
survive app restarts, and switching back to a cloud provider is one click
in the model picker.

## How Lemon AI picks what to download
...
Models ship in several quality grades (quantizations). Lemon AI picks the
...
guarantees. Below 4-bit the quality loss is too severe, so Lemon AI never
...
Local models live or die by memory placement, so Lemon AI manages it
...
- **Every recommended model gets at least a 64K context window.** When a
  model is larger than your GPU's memory, Lemon AI deliberately places the
...
- Results show download counts and a per-file fit check sized to your
  ...
  your GPU.
- Anything you download behaves exactly like a catalog model — Lemon AI
...
If a llama-server is already running on your machine, Lemon AI detects it
...
[Run Lemon AI Locally with Ollama](/guides/local-ollama-setup) and
...
```yaml
local_runtime:
  enabled: false     # true = start the managed server with Lemon AI.
                     # The desktop "Use" button sets this automatically.
  backend: auto      # auto | cuda | metal | vulkan | hip | cpu
  tag: b10362        # pinned llama.cpp release; Lemon AI updates it with
                     # each release after re-validation
```

Models and runtime builds live under the Lemon AI home directory
