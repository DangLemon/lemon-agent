...
Lemon AI automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.lemon-ai/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Lemon AI stores session history so it can resume conversations, but it does not
...
Lemon AI explicitly injects for that turn.
...
For example, if a user sends an image and asks Lemon AI to make a meme from it,
Lemon AI may inspect that image once with vision and run an image-processing
...
`lemon sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `lemon sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`lemon` or `lemon chat`) |
...
A bare `-c` is terminal-aware: each CLI session drops a small breadcrumb file under `~/.lemon-ai/terminal-sessions/` keyed by the terminal it runs in (tty device, tmux pane, kitty window, wezterm pane, Zellij pane, Windows Terminal session, ...). When you run `lemon -c` again in the *same* terminal, Lemon AI resumes that terminal's own session — so two panes side by side each continue their own conversation instead of both grabbing the globally most-recent one. If there's no breadcrumb for the terminal (first use, deleted session, or a stale breadcrumb older than 30 days), `-c` falls back to the most-recent-session behavior. `-c "name"` and `--resume` are unaffected. Disable with `session.terminal_continue: false` in `config.yaml`.
...
lemon -c "my project"   # → resumes "my project #3"
```
...
lemon --resume 20250305_091523_a1b2c3d4
lemon -r 20250305_091523_a1b2c3d4

# Resume by title
lemon --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
lemon --resume latest

# Or with the chat subcommand
lemon chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `lemon sessions list`.
...
lemon --resume latest --in ./my-project

# Works with the TUI too
lemon --tui --resume latest --in ./my-project
```
...
```bash
lemon --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`lemon sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
lemon sessions list --workspace my-project
lemon sessions list --workspace ~/code/lemon-agent
```

### Conversation Recap on Resume

When you resume a session, Lemon AI displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Lemon AI session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.lemon-ai/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `lemon -r "<title>"` from the shell) and pick up where the platform left off.
...
Lemon AI automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `lemon sessions list` or `lemon sessions browse`.
...
```bash
lemon sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Lemon AI creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`lemon -c "my project"`), it automatically picks the most recent session in the lineage.
...
Lemon AI provides a full set of session management commands via `lemon sessions`:
...
# Filter by platform
lemon sessions list --source telegram

# Show more sessions
lemon sessions list --limit 50
```
...
`lemon sessions export` is one surface for every export format, selected with `--format`:
...
lemon sessions export backup.jsonl

# Export sessions from a specific platform
lemon sessions export telegram-history.jsonl --source telegram

# Export a single session
lemon sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
lemon sessions export backup.jsonl --redact
```
...
lemon sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
lemon sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
lemon sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
lemon sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `lemon-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
lemon sessions export --format trace

# One session to a local trace file
lemon sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
lemon sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.lemon-ai/session-exports`).

```bash
# Export one session to Markdown
lemon sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
lemon sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
lemon sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
lemon sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
lemon sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
lemon sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
lemon sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
lemon sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
lemon sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
lemon sessions pin 20250305_091523_a1b2c3d4
lemon sessions pin 20250305 20250306

# Remove the pin
lemon sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
lemon sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
lemon sessions pinned --json > pinned-sessions.json
```
...
lemon sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
lemon sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
lemon sessions prune --newer-than 5h

# Explicit window with absolute timestamps
lemon sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
lemon sessions prune --source telegram
lemon sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
lemon sessions prune --newer-than 5h --title "smoke test"   # title substring
lemon sessions prune --older-than 30 --max-messages 3        # tiny sessions
lemon sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
lemon sessions prune --model gpt-5 --older-than 1w           # by model (substring)
lemon sessions prune --provider openrouter --older-than 60   # by billing provider
lemon sessions prune --branch feature/old-experiment         # by git branch
lemon sessions prune --user 12345678 --chat-type group       # by messaging origin
lemon sessions prune --max-tokens 500 --older-than 7         # by token usage
lemon sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
lemon sessions prune --newer-than 5h --dry-run

# Skip confirmation
lemon sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `lemon sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `lemon sessions prune` keeps the 90-day cutoff. Every
...
`lemon sessions archive` takes the same filters as `prune` but soft-hides
...
lemon sessions archive --newer-than 5h

# Archive by title substring, preview first
lemon sessions archive --title "dry run" --dry-run
lemon sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `lemon sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`lemon sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
lemon sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`lemon insights`](/reference/cli-commands#lemon-insights).
...
`lemon sessions repair-routing` finds message-bearing session rows with no
...
lemon sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
lemon sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.lemon-ai/state.db ~/.lemon-ai/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into Lemon AI and
continue it here. Lemon AI reads Claude Code's session logs
...
lemon sessions import

# Limit to one tool, or point at a specific file
lemon sessions import --from claude
lemon sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl
...
`lemon sessions import` creates a new Lemon AI session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `lemon --resume <id>` command.
...
When Lemon AI cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Lemon AI uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Lemon AI in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.lemon-ai/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.lemon-ai/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.lemon-ai/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.lemon-ai/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.lemon-ai/sessions/sessions.json` is a **legacy mirror** of it,
...
`lemon sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.lemon-ai/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `lemon sessions list`, the cause is
`state.db` not receiving them — run `lemon sessions repair` and watch for a
...
`*.jsonl` files in `~/.lemon-ai/sessions/`. They are no longer written or
read by Lemon AI. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Lemon AI process in the same `LEMON_HOME`

Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever (the pre-#54189 behavior), turn it off in `~/.lemon-ai/config.yaml`:
...
`in its tip segment`). `lemon sessions export` still works for such sessions.
...
lemon sessions delete <session_id>
