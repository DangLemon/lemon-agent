...
title: "Git Worktrees"
description: "Run multiple Lemon AI agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Lemon AI is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Lemon AI so each session has a clean, isolated working directory.

## Why Use Worktrees with Lemon AI?

Lemon AI treats the **current working directory** as the project root:

- CLI: the directory where you run `lemon` or `lemon chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.lemon-ai/config.yaml`
...
Lemon AI creates `.worktrees/my-experiment/` inside the repo (branch
`lemon/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `lemon-<id>`
...
git worktree add ../repo-feature feature/lemon-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/lemon-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Lemon AI there:

```bash
cd ../repo-feature

# Start Lemon AI in the worktree
lemon
```
...
git worktree add ../repo-experiment-a feature/lemon-a
git worktree add ../repo-experiment-b feature/lemon-b
```
...
Each Lemon AI process:

- Works on its own branch (`feature/lemon-a` vs `feature/lemon-b`).
...
- Lemon AI checkpoint data under `~/.lemon-ai/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Lemon AI experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/lemon-checkpoints-docs`, `feature/lemon-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Lemon AI from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `lemon -w` (Automatic Worktree Mode)

Lemon AI has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `lemon/lemon-<hash>`).
...
```bash
lemon -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `lemon -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Lemon AI session its own clean checkout.
