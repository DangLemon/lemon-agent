...
description: "Build wrapper CLIs that extend the Lemon AI TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Lemon AI exposes protected extension hooks on `LemonCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the `run()` method or the TUI construction in `lemon_cli/cli_tui_mixin.py` (where these hooks are defined; `LemonCLI` in `cli.py` mixes it in). This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends Lemon AI."""
...
Called after Lemon AI registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
