...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Lemon AI — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Lemon AI tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Lemon AI core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Lemon AI Plugin](/developer-guide/plugins)
...
2. **`toolsets.py`** — add tool name to `_LEMON_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `lemon_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `lemon_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `lemon chat -q "Use the weather tool for London"`
