...
title: "Desktop Plugin SDK (@lemon-ai/plugin-sdk)"
description: "Extend the native Lemon AI app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Lemon AI](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `LemonPlugin`.
It imports one module — `@lemon-ai/plugin-sdk` — and gets everything: the app's
...
`$LEMON_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Lemon AI. This page is the **native
desktop app** (`lemon desktop`) SDK — the `@lemon-ai/plugin-sdk` module and
`$LEMON_HOME/desktop-plugins/`. The **web dashboard** (`lemon dashboard`) has
its own, unrelated plugin system on `window.__LEMON_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Lemon AI Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$LEMON_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$LEMON_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `LemonPlugin` contract, appear in **Settings → Plugins**,
...
[`lemon-example-plugins`](https://github.com/NousResearch/lemon-example-plugins)
repo.

## Quick start — your first plugin

Create `$LEMON_HOME/desktop-plugins/hello/plugin.js` (that's `~/.lemon-ai/...`
by default, or `~/.lemon-ai/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.lemon-ai/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@lemon-ai/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Lemon AI' }),
      jsx('div', {
...
The only importable specifiers are `@lemon-ai/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `LemonPlugin`:

```ts
interface LemonPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `lemon.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@lemon-ai/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@lemon-ai/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@lemon-ai/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@lemon-ai/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@lemon-ai/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@lemon-ai/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@lemon-ai/plugin-sdk'
...
`window.lemonAI.send('get-price eth')` (or a declarative
`<button data-lemon-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@lemon-ai/plugin-sdk'
...
for requests; `targetProfile` is the backend Lemon AI profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Lemon AI
...
ctx.os.notify({
  ...
  // Body click → focus Lemon AI + navigate. Same vocabulary as OS deep links:
  activate: 'lemon://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'lemon://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `lemon://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `lemon://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@lemon-ai/plugin-sdk'
...
```javascript
import { queryClient } from '@lemon-ai/plugin-sdk'
...
a package into `~/.lemon-ai/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.lemon-ai/plugins` is not reachable as a filesystem —
...
the `lemon://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular Lemon AI plugin and declare it in a
`manifest.json`:
...
lemon-agent codebase directly (`lemon_state`, `lemon_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.lemon-ai/`)
...
(`lemon.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `LemonPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@lemon-ai/plugin-sdk` alias.
2. It's still lint-fenced to `@lemon-ai/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@lemon-ai/plugin-sdk`, `react`,
...
| Plugin contract | `LemonPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `LemonOpenTarget`, `Contribution` |
...
### Agents: the `lemon-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`lemon-desktop-plugins`** skill — it carries the same contract as this page in
...
`$LEMON_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `lemon logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@lemon-ai/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.lemon-ai/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.lemon-ai/logs/errors.log` for
