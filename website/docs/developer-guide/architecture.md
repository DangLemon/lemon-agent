...
sidebar_position: 1
title: "Architecture"
description: "Lemon AI internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Lemon AI internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ lemon_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # LemonCLI facade — mixins in lemon_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── lemon_state.py           # SQLite session/state database facade (+ lemon_state_*.py siblings)
├── lemon_constants.py       # LEMON_HOME, profile-aware paths
...
│
├── lemon_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `lemon` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # lemon skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # lemon tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # lemon gateway start/stop
│
...
```text
User input → LemonCLI.process_input()
...
Three discovery sources: `~/.lemon-ai/plugins/` (user), `.lemon-ai/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `lemon plugins` or `config.yaml`.
...
Exposes Lemon AI as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`lemon -p <name>`) gets its own LEMON_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
