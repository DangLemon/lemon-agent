...
The ACP adapter wraps Lemon AI' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
lemon acp / lemon-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.lemon-ai/.env
  -> configure stderr logging
  -> construct LemonACPAgent
...
- `allow_once` -> Lemon AI `once`
- `allow_always` -> Lemon AI `always`
- reject options -> Lemon AI `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Lemon AI tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["lemon-acp"])
...
Instead it reuses Lemon AI' runtime resolver:

- `acp_adapter/auth.py`
- `lemon_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Lemon AI provider/credentials. It also always advertises a terminal setup auth method (`lemon-setup`, args `--setup`) so first-run ACP clients can open Lemon AI' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.lemon-ai/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `lemon-acp` toolset definition
- `lemon_cli/main.py` — `lemon acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `lemon-acp` script
