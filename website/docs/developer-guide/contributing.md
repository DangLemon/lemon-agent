...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Lemon AI — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Lemon AI! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Lemon AI core? Start with [Build a Lemon AI Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Lemon AI itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Lemon AI instance (throwaway LEMON_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m lemon_cli.main
scripts/dev-sandbox.sh --persistent python -m lemon_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.lemon-ai/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.lemon-ai` — use `get_lemon_home()` from `lemon_constants` for code paths and `display_lemon_home()` for user-facing messages. See [AGENTS.md](https://github.com/DangLemon/lemon-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Projects built on (or reviewed by) Lemon AI can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask Lemon AI to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/DangLemon/lemon-agent/blob/main/LICENSE).
