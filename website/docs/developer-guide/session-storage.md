...
Lemon AI uses a SQLite database (`~/.lemon-ai/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source files: `lemon_state.py` (facade) plus the `lemon_state_*.py` siblings (schema, fts, search, compression, portability, gateway, ...)


## Architecture Overview

```
~/.lemon-ai/state.db (SQLite, WAL mode)
...
`lemon sessions recover` copies the row-bearing tables above into the
...
Abridged — see `SCHEMA_SQL` in `lemon_state_common.py` (applied by `lemon_state_schema.py`) for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `lemon_state_common.py` for the exact SQL.
...
Multiple lemon processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.lemon-ai/state.db
...
Default path: `~/.lemon-ai/state.db`

This is derived from `lemon_constants.get_lemon_home()` which resolves to
`~/.lemon-ai/` by default, or the value of `LEMON_HOME` environment variable.
