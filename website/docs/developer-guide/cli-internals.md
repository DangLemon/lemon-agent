...
sidebar_position: 15
title: "CLI Internals"
description: "How lemon_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `lemon_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `lemon_cli/AGENTS.md`; user-facing behaviour
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `lemon_cli.update_cmd._lemon_holder_subcommand` (top-level subcommand of any Lemon AI argv). Never
...
Built-in skins (`_BUILTIN_SKINS` in `lemon_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.lemon-ai/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
`display.skin: <name>`; the full YAML template is in the
[Skins & Themes](../user-guide/features/skins.md) user guide.

## Profiles: multi-instance support

Lemon AI supports profiles — fully isolated instances, each with its own `LEMON_HOME` (config, API
keys, memory, sessions, skills, gateway). `_apply_profile_override()` in `lemon_cli/main.py` sets
`LEMON_HOME` before any module imports, so every `get_lemon_home()` reference scopes to the active
profile. Profile operations are HOME-anchored (`_get_profiles_root()` returns
`Path.home() / ".lemon-ai" / "profiles"`, not `get_lemon_home() / "profiles"`) so
`lemon -p coder profile list` sees all profiles regardless of which one is active — intentional.
