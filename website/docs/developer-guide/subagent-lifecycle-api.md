...
Plugins can launch and supervise fresh Lemon AI child sessions without importing
...
active agent turn fails closed with `No active Lemon AI parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Lemon AI can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Lemon AI's existing unsafe-tool block remains enforced.
