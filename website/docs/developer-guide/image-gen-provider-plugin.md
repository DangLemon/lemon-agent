...
description: "How to build an image-generation backend plugin for Lemon AI"
---
...
2. **User** — `~/.lemon-ai/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** — packages declaring a `lemon_agent.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `lemon tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `lemon tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.lemon-ai/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `lemon plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `lemon tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `lemon tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `lemon tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under $LEMON_HOME/cache/images/ via save_b64_image()
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `lemon plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `lemon tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `lemon tools` model picker |
...
Some backends return image URLs (fal, Replicate); others return base64 payloads (OpenAI gpt-image-2). For the base64 case, use `save_b64_image()` — it writes to `$LEMON_HOME/cache/images/<prefix>_<timestamp>_<uuid>.<ext>` and returns the absolute `Path`. Pass that path (as `str`) as `image=` in `success_response()`. Gateway delivery (Telegram photo bubble, Discord attachment) recognizes both URLs and absolute paths.

## User overrides

Drop a user plugin at `~/.lemon-ai/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `lemon plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export LEMON_HOME=/tmp/lemon-imggen-test
mkdir -p $LEMON_HOME/plugins/image_gen/my-backend
# …copy __init__.py + plugin.yaml into that dir…

export MY_BACKEND_API_KEY=your-test-key
lemon plugins enable my-backend

# Pick it as the active provider
echo "image_gen:" >> $LEMON_HOME/config.yaml
echo "  provider: my-backend" >> $LEMON_HOME/config.yaml

# Exercise it
lemon -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `lemon tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
...
- [Build a Lemon AI Plugin](/developer-guide/plugins) — general tools/hooks/slash commands guide
