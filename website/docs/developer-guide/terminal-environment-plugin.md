...
Lemon AI runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.lemon-ai/plugins/`, registering a backend the user selects
...
| `lemon setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `lemon status` / `lemon doctor` | `doctor_checks()` |
...
```python title="~/.lemon-ai/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.lemon-ai"    # where synced cache files land, or None
...
```yaml title="~/.lemon-ai/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
lemon plugins enable acmebox
lemon config set terminal.backend acmebox
```
