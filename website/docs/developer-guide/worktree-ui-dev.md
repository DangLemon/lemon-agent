...
| Variable | Meaning |
|----------|---------|
| `LEMON_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `LEMON_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `LEMON_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Lemon AI itself — they're private to these helpers. The variables Lemon AI *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `lemon --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_lemon_root)" || { echo "htui: not in a Lemon AI checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$LEMON_MAIN_CHECKOUT/.venv/bin/python" -m lemon_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `LEMON_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_lemon_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `LEMON_TUI_DIR` are mutually exclusive
`LEMON_TUI_DIR` points Lemon AI at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `lemon --tui --dev` exits with an error. Run `unset LEMON_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_lemon_root)" || { echo "hgui: not in a Lemon AI checkout" >&2; return 1; }
  deps="${LEMON_GUI_DEPS_CHECKOUT:-$LEMON_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _lemon_link_deps "$desktop" "$deps/apps/desktop"
    _lemon_link_deps "$root" "$deps"
  else
  ...
  trap '_lemon_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    LEMON_DESKTOP_LEMON_ROOT="$root" \
    LEMON_DESKTOP_PYTHON="$LEMON_MAIN_CHECKOUT/.venv/bin/python" \
    LEMON_DESKTOP_IGNORE_EXISTING=1 \
    LEMON_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `LEMON_DESKTOP_LEMON_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `lemon`. |
| `LEMON_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `LEMON_DESKTOP_IGNORE_EXISTING` | Ignores any `lemon` on `PATH` so it can't shadow the worktree. |
| `LEMON_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Lemon AI checkout.
_lemon_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/lemon_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_lemon_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'lemon_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `lemon --tui --dev` and the `LEMON_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/DangLemon/lemon-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `LEMON_*` variable Lemon AI reads
