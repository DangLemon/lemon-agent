...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Lemon AI already has applies. The plugin never sees a
...
lemon-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Lemon AI provider id (e.g. "openrouter")
...
using Lemon AI built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Lemon AI provider
        # (must be one Lemon AI already knows about — same names as
        # `lemon model` and config.yaml model.provider).
        ...
        # plugin sends — Lemon AI does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.lemon-ai/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Lemon AI' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Lemon AI subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/DangLemon/lemon-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/DangLemon/lemon-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/lemon-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/lemon-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
