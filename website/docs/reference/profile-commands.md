...
This page covers all commands related to [Lemon AI profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `lemon profile` without a subcommand shows help.
...
```bash
lemon profile use <name>
```

Sets `<name>` as the active profile. All subsequent `lemon` commands (without `-p`) will use this profile.
...
lemon profile use default
```

## `lemon profile create`

```bash
lemon profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `lemon profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `lemon update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.lemon-ai`), use `lemon skills opt-out` / `lemon skills opt-in`. |
...
lemon profile create work2 --clone-from work

# Clone everything from a specific profile
lemon profile create work2-backup --clone-from work --clone-all
```

## `lemon profile describe`

```bash
lemon profile describe [<name>] [options]
```
...
lemon profile describe researcher

# Set it explicitly
lemon profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
lemon profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
lemon profile describe --all --auto
```

## `lemon profile delete`

```bash
lemon profile delete <name> [options]
```
...
```bash
lemon profile delete mybot
lemon profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.lemon-ai`) cannot be deleted — use `lemon uninstall` to remove everything.
:::

## `lemon profile show`

```bash
lemon profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Lemon AI home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ lemon profile show work
Profile: work
Path:    ~/.lemon-ai/profiles/work
...
```bash
lemon profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Lemon AI installation.
...
```bash
lemon profile alias work
# Creates/updates ~/.local/bin/work

lemon profile alias work --name mywork
# Creates ~/.local/bin/mywork

lemon profile alias work --remove
# Removes the wrapper script
```

## `lemon profile rename`

```bash
lemon profile rename <old-name> <new-name>
```
...
```bash
lemon profile rename mybot assistant
# ~/.lemon-ai/profiles/mybot → ~/.lemon-ai/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `lemon profile export`

```bash
lemon profile export <name> [options]
```
...
```bash
lemon profile export work
# Creates work.tar.gz in the current directory

lemon profile export work -o ./work-2026-03-29.tar.gz
```
...
## `lemon profile import`

```bash
lemon profile import <archive> [options]
```
...
```bash
lemon profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

lemon profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `lemon -p telemetry`). |
...
lemon profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
lemon profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
lemon profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
lemon profile install ./telemetry/
```

### `lemon profile update`

```bash
lemon profile update <name> [--force-config] [--yes]
```
...
```bash
lemon profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Lemon AI version, author, env var requirements, the source URL/path, and
...
`lemon profile list` also shows the distribution name and version in a
`Distribution` column, and `lemon profile show <name>` / `delete <name>`
...
lemon profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
lemon profile install https://github.com/your-org/internal-assistant.git
```
...
`lemon_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Lemon AI version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Lemon AI can clone from.
3. Tell recipients to run `lemon profile install <your-repo-url>`.
...
## `lemon -p` / `lemon --profile`

```bash
lemon -p <name> <command> [options]
lemon --profile <name> <command> [options]
```

Global flag to run any Lemon AI command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
lemon -p work chat -q "Check the server status"
lemon --profile dev gateway start
lemon -p personal skills list
lemon -p work config edit
```

## `lemon completion`

```bash
lemon completion <shell>
```
...
- `lemon profile <TAB>` — subcommands (list, use, create, etc.)
- `lemon profile use <TAB>` — profile names
- `lemon -p <TAB>` — profile names
