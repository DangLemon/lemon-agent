...
2. The unified Lemon AI Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                lemon_meta = metadata.get("lemon", {})
                if isinstance(lemon_meta, dict):
                    tags = lemon_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
