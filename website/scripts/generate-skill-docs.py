...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/DangLemon/lemon-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    lemon_meta = (fm.get("metadata") or {}).get("lemon") or {}
    tags = lemon_meta.get("tags") or []
    related = lemon_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `lemon skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Lemon AI"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Lemon AI ships with a large built-in skill library copied into `~/.lemon-ai/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Lemon AI also syncs bundled skills on `lemon update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.lemon-ai/skills/` tree, it is still shipped with Lemon AI; restore it with `lemon skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with lemon-agent — install via lemon skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with lemon-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "lemon skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "lemon skills install official/blockchain/solana",
        "lemon skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "lemon skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
