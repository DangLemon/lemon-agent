#!/usr/bin/env python3
"""SWE Runner with Lemon AI Trajectory Format

Runs tool-calling agent tasks in Lemon AI-Agent's execution environments (local,
docker, modal) and writes trajectories in Lemon AI format (from/value pairs with
...
def create_environment(env_type: str = "local", image: str = "python:3.11-slim", cwd: str = "/tmp", timeout: int = 60, **kwargs):
    """Create a Lemon AI execution environment (``local`` ignores ``image``/``kwargs``)."""
    if env_type == "local":
...
class MiniSWERunner:
    """Tool-calling agent loop over a Lemon AI execution environment, emitting Lemon AI trajectories."""
    ...
    def _convert_to_lemon_format(self, messages: List[Dict[str, Any]], user_query: str) -> List[Dict[str, Any]]:
        """Convert the OpenAI-style message list to the Lemon AI trajectory format used by batch_runner.py."""
        system_msg = LEMON_SYSTEM_PREFIX + f"<tools>\n{self._format_tools_for_system_message()}\n</tools>\n" + LEMON_SYSTEM_SUFFIX
    ...
    def run_task(self, task: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._cleanup_env()
        return {"conversations": self._convert_to_lemon_format(messages, task), "completed": completed, "api_calls": api_call_count,
...
def main(
    ...
    """
    Run SWE tasks with Lemon AI trajectory format output.
    ...
    """
    print("🚀 Mini-SWE Runner with Lemon AI Trajectory Format")
    print("=" * 60)
