...
const BACKEND = process.env.LEMON_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `lemon dashboard` server injects a one-shot
 * session token into `index.html` (see `lemon_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__LEMON_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function lemonDevToken(): Plugin {
  const TOKEN_RE = /window\.__LEMON_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__LEMON_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "lemon:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[lemon] Could not find session token in ${BACKEND} — ` +
              `is \`lemon dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__LEMON_SESSION_TOKEN__="${match[1]}";` +
              `window.__LEMON_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[lemon] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`lemon dashboard\` or set LEMON_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@lemon-ai/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `lemon dashboard` must serve these; Vite has no
