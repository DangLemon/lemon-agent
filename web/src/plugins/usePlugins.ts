...
import { api, LEMON_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "lemon:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${LEMON_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${LEMON_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?lemon_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-lemon-plugin", manifest.name);
