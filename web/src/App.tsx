          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="lemon-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="lemon-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateLemon = status?.can_update_lemon === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'lemon update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "lemon update";
    return (
      t.status.updateLemonConfirmMessage ??
      `This will run 'lemon update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateLemonConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateLemon) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateLemon,
      runningLabel: t.status.updatingLemon,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Lemon AI gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateLemonConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateLemonConfirmTitle ?? `${t.status.updateLemon}?`}
    />
    </>
  );
}
