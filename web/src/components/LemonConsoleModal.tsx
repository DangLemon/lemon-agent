...
interface LemonConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function LemonConsoleModal({ open, onClose }: LemonConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("lemon> ");
  const inputPromptRef = useRef("lemon> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "lemon> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Lemon AI Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="lemon-console-title"
    >
      <div
        className={cn(
          themedBody,
