/**
 * ChatPage — embeds `lemon --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (lemon_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "lemon.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__LEMON_SESSION_TOKEN__ &&
    !window.__LEMON_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `lemon dashboard`, not directly."
      : null,
  );
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("lemon-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("lemon-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__LEMON_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Lemon AI TUI has enabled xterm mouse-events
    ...
    //      payloads upload to LEMON_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to LEMON_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[lemon-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__LEMON_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets LEMON_HOME pointed at the
    ...
    // forwarding them into Lemon AI. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="lemon-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
