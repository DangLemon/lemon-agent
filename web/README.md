...
Browser-based dashboard for managing Lemon AI configuration, API keys, and monitoring active sessions.
...
cd ../
python -m lemon_cli.main web --no-open
...
`lemon dashboard` on port 9119 serves the **built** bundle from `lemon_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../lemon_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `lemon-agent/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Lemon AI” — mixed case, no Mondwest, no `text-display` |
