# web/ + lemon_cli/web_routers/ — the dashboard (`lemon dashboard` → `/chat`)

Applies on top of the root `AGENTS.md`. Backend routers: `lemon_cli/web_routers/*.py`, one file per
dashboard surface, mounted by `lemon_cli/web_server.py` (+ `web_server_*.py` siblings). Frontend:
`web/src/`. Shared JSON-RPC/WS client: `apps/shared` (`@lemon-ai/shared`), also used by the desktop.

## The dashboard embeds the REAL `lemon --tui` — not a rewrite

`lemon_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `web_server.py`:
...
- The server spawns exactly what `lemon --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL
...
`cmd_dashboard` skips `_build_web_ui` and exports `LEMON_SERVE_HEADLESS=1` so `mount_spa()`
...
- Routers are one-file-per-surface; a new surface is a new `web_routers/<surface>.py`, not a growing
  `web_server.py`.
- Tests: Python in `tests/lemon_cli/` (routers, pty bridge); JS in the `web/` vitest suite. Python
