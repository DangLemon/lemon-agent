...
# 5. Symlinks the 'lemon' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/lemon-uv-install.$$.log")"
...
if is_termux; then
    # Lemon AI currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Lemon AI requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink lemon into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up lemon command..."

LEMON_BIN="$SCRIPT_DIR/venv/bin/lemon"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$LEMON_BIN" "$COMMAND_LINK_DIR/lemon"
echo -e "${GREEN}✓${NC} Symlinked lemon → $COMMAND_LINK_DISPLAY_DIR/lemon"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Lemon AI — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.lemon-ai/skills/
# ============================================================================

LEMON_SKILLS_DIR="${LEMON_HOME:-$HOME/.lemon-ai}/skills"
mkdir -p "$LEMON_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.lemon-ai/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$LEMON_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  lemon status        # Check configuration"
if is_termux; then
    echo "  lemon gateway       # Run gateway in foreground"
else
    echo "  lemon gateway install # Install gateway service (messaging + cron)"
fi
echo "  lemon cron list     # View scheduled jobs"
echo "  lemon doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m lemon_cli.main setup
fi
