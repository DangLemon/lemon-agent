...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include ``lemon_home`` (profile-scoped storage; never hardcode
        ``~/.lemon-ai``) and ``platform``; may include ``agent_context`` ("primary" |
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        """Setup fields for ``lemon memory setup`` ([] if none): ``key``, ``description``,
    ...
    def backup_paths(self) -> List[str]:
        """Absolute paths of provider state OUTSIDE LEMON_HOME for ``lemon backup``/``import``
