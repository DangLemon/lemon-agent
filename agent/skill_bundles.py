...
YAML files in ``<LEMON_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``LEMON_BUNDLES_DIR`` override (tests) or ``<LEMON_HOME>/skill-bundles``."""
    override = os.environ.get("LEMON_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_lemon_home() / "skill-bundles"
...
# File-level CRUD — used by `lemon bundles`.
