...
Every Lemon AI request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=lemon-agent", "client=lemon-client-v<__version__>"]``.
The version is read live from ``lemon_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("lemon_affinity_scope", default=None)
...
def lemon_client_tag() -> str:
    """``client=lemon-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if lemon_cli is unimportable)."""
    try:
        from lemon_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=lemon-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=lemon-agent", lemon_client_tag()]
