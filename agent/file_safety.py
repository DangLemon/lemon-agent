...
def _constants_path(getter_name: str) -> Path:
    """Call ``lemon_constants.<getter_name>()`` (local import avoids cycles); ``~/.lemon-ai`` on any failure."""
    try:
        import lemon_constants

        return getattr(lemon_constants, getter_name)()
    except Exception:
        return Path(os.path.expanduser("~/.lemon-ai"))


def _lemon_home_path() -> Path:
    """Active LEMON_HOME (profile-aware). Tests monkeypatch this name."""
    return _constants_path("get_lemon_home")


def _lemon_root_path() -> Path:
    """Lemon AI root dir (parent of any profile, never per-profile)."""
    return _constants_path("get_default_lemon_root")


def _lemon_dirs() -> list[Path]:
    """Resolved active LEMON_HOME and global root, deduplicated.

    Both are checked so credential stores at <root>/... stay guarded when
    running under a profile (LEMON_HOME = <root>/profiles/<name>).
    """
    return list(dict.fromkeys(_resolve_each((_lemon_home_path(), _lemon_root_path()))))
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    lemon_files = (".env", ".anthropic_oauth.json", os.path.join("cache", "bws_cache.enc.json"))
    paths = [
        *(os.path.join(home, *f) for f in home_files),
        *(str(base / f) for f in lemon_files for base in (_lemon_home_path(), _lemon_root_path())),
...
def get_safe_write_roots() -> set[str]:
    """Resolved LEMON_WRITE_SAFE_ROOT paths (``os.pathsep``-separated list)."""
    roots: set[str] = set()
    for path in filter(None, os.getenv("LEMON_WRITE_SAFE_ROOT", "").split(os.pathsep)):
...
# LEMON_HOME / root subpaths that the agent's generic file tools must not
...
_LEMON_PROTECTED_SUBPATHS = ("state.db", "sessions", "mcp-tokens", "pairing")
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    for base in _lemon_dirs():
        for sub in _LEMON_PROTECTED_SUBPATHS:
            with suppress(Exception):
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside LEMON_WRITE_SAFE_ROOT "
...
# Exact-file credential stores under LEMON_HOME / <root>. The agent never
...
# Directory-prefix read denies under LEMON_HOME / <root>: (subdir, message for
...
_READ_DENIED_DIRS = (
    ("mcp-tokens",
     "is the Lemon AI MCP token directory and cannot be read directly.",
     "is a Lemon AI MCP token file and cannot be read directly."),
    ("browser-profile",
     "is the Lemon AI real-profile browser snapshot directory (copied cookies/logins) and cannot be read directly.",
     "is inside the Lemon AI real-profile browser snapshot (copied cookies/logins) and cannot be read directly."),
)


def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Lemon AI path.

    Blocked: internal skill-hub caches (prompt-injection carriers), credential
    stores under LEMON_HOME and the global root (exact files, plus anything
    ...
    lemon_dirs = _lemon_dirs()
    reason = None
    if any(_is_under(resolved, hd / "skills" / ".hub") for hd in lemon_dirs):
        reason = (
            "is an internal Lemon AI cache file and cannot be read directly to prevent "
            "prompt injection. Use the skills_list or skill_view tools instead."
        )
    elif any(resolved in _resolve_each(hd / name for hd in lemon_dirs) for name in _CREDENTIAL_FILE_NAMES):
        reason = (
            "is a Lemon AI credential store and cannot be read directly. Provider tools "
            "consume these credentials through internal channels." + _DID_SUFFIX
        )
    else:
        for subdir, dir_msg, file_msg in _READ_DENIED_DIRS:
            for blocked_dir in _resolve_each(hd / subdir for hd in lemon_dirs):
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Lemon AI read (see ``get_read_block_error``).
...
def _resolve_active_profile_name() -> str:
    """Active profile name from LEMON_HOME: ``~/.lemon-ai`` -> ``"default"``,
    ``~/.lemon-ai/profiles/X`` -> ``"X"``; ``"default"`` on any resolution failure."""
    try:
        parts = _lemon_home_path().resolve().relative_to(_lemon_root_path().resolve() / "profiles").parts
...
#   <LEMON_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.lemon-ai/...
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Lemon AI state: ``None``
    for non-mirror paths, else ``target_path`` (resolved), ``mirror_root`` (the
    ``…/home/.lemon-ai`` prefix) and ``inner_path`` (what the agent meant on the host)."""
    target = _resolve_target(path)
    parts = target.parts if target is not None else ()
    # Need at least: sandboxes / <backend> / <task> / home / .lemon-ai / <thing>; inner_idx = the .lemon-ai part.
    inner_idx = next(
        (i + 4 for i, part in enumerate(parts)
         if part == "sandboxes" and i + 5 < len(parts) and parts[i + 3] == "home" and parts[i + 4] == ".lemon-ai"),
        None,
    )
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "Writes here land on a copy that the host Lemon AI process never reads — the "
        "authoritative file is likely {inner_path!r} under the real LEMON_HOME.",
...
def classify_container_mirror_target(path: str, mirror_prefix: str | None = None) -> Optional[dict]:
    ...
    the bind mount strips the ``sandboxes/`` prefix (the agent sees plain ``/root/.lemon-ai/…``),
...
def get_container_mirror_warning(path: str, mirror_prefix: str | None = None) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "the container's bind-mounted home — a per-task mirror that the host Lemon AI "
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Lemon AI scope, or is inside
    ...
    try:
        ...
        root_real = _lemon_root_path().resolve()
