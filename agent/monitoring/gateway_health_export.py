...
_OBSERVABLE_METRIC_NAMES = (
    "lemon.gateway.up", "lemon.gateway.state", "lemon.gateway.active_agents", "lemon.gateway.busy",
    "lemon.gateway.drainable", "lemon.gateway.restart_requested", "lemon.gateway.background_work",
    "lemon.gateway.background_delegations", "lemon.platform.up", "lemon.platform.degraded",
    "lemon.cron.scheduler.heartbeat_age_seconds", "lemon.cron.scheduler.last_success_age_seconds",
    "lemon.cron.scheduler.catch_up_occurrences", "lemon.cron.jobs.enabled", "lemon.cron.jobs.running",
    "lemon.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="lemon-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("lemon.gateway.background_work", _read_background_work_count),
            ("lemon.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("lemon.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("lemon.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'lemon-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="lemon-gateway-health-export", daemon=True)
            thread.start()
