...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "lemon.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("lemon.gateway.up", int(bool(gateway_running))),
        metric("lemon.gateway.active_agents", active_agents),
        metric("lemon.gateway.busy", int(bool(busy))),
        metric("lemon.gateway.drainable", int(bool(drainable))),
        metric("lemon.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("lemon.gateway.state", 1, **{"lemon.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"lemon.platform": str(platform), "lemon.platform.state": state}
        metrics.append(metric("lemon.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("lemon.platform.degraded", int(is_degraded), **pattrs, **{"lemon.error_code": error_code}))
        if is_degraded:
