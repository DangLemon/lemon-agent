...
Shared by CLI ``lemon journey``, the TUI ``/journey`` overlay and the desktop.
Deleting a skill *archives* it (``lemon curator restore`` recovers it);
...
def _locate_memory(node_id: str) -> tuple[Path, list[str], int]:
    ...
    path = get_lemon_home() / "memories" / _MEMORY_FILES[source]
    if not path.exists():
...
def _delete_skill(name: str) -> dict[str, Any]:
    ...
    if skill_usage.get_record(name).get("pinned"):
        return {"ok": False, "message": f"'{name}' is pinned — unpin it first (lemon curator unpin {name})"}
    ok, message = skill_usage.archive_skill(name)
    if ok:
        _clear_skill_cache()
    return {"ok": ok, "message": f"archived '{name}' — restore with: lemon curator restore {name}" if ok else message}
