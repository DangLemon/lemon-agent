"""Fold an agent-as-provider's own activity back into Lemon AI' turn state.

Agent providers (ACP CLI shims, the codex app-server) run their own tools, so that
work must never come back as pending ``tool_calls`` (Lemon AI would re-run it) — but
...
``reasoning``. The client hands back ``lemon_projected_messages`` (completed
assistant/tool rows) and ``lemon_provider_tool_iterations`` on the completion
...
def splice_provider_projection(agent: Any, response: Any, messages: list[dict[str, Any]]) -> int:
    ...
    """
    projected = getattr(response, "lemon_projected_messages", None)
    ...
    try:
        iterations = int(getattr(response, "lemon_provider_tool_iterations", 0) or 0)
