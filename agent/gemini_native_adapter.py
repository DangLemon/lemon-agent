...
try:
    import lemon_cli as _lemon_cli

    _LEMON_VERSION = str(_lemon_cli.__version__)
except Exception:
    _LEMON_VERSION = "0.0.0"
_API_CLIENT = f"lemon-agent/{_LEMON_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Lemon AI typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.lemon-ai/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.lemon-ai/.env (get one at https://aistudio.google.com/app/apikey), or run `lemon setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Lemon AI retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No LEMON_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
