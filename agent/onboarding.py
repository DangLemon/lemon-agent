...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Lemon AI finds ``~/.openclaw/``: migrate first, cleanup (which breaks OpenClaw) after."""
    return (
        "A legacy OpenClaw directory was detected at ~/.openclaw/.\n"
        "To port your config, memory, and skills over to Lemon AI, run `lemon claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, run `lemon claw cleanup` "
