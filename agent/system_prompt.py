...
from agent.prompt_builder import (
    ...
    LEMON_AGENT_HELP_GUIDANCE, LEMON_AGENT_HELP_GUIDANCE_NO_SKILLS, KANBAN_GUIDANCE, MEMORY_GUIDANCE,
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    """Append the desktop embedded-terminal clarifier when ``LEMON_DESKTOP_TERMINAL``
    is set (only the desktop's TUI PTY, never the chat backend). Idempotent."""
    if not hint or _TUI_EMBEDDED_PANE_CLARIFIER in hint or not is_truthy_value(os.getenv("LEMON_DESKTOP_TERMINAL")):
        return hint
...
def _active_profile_name(agent: Any, ambient) -> str:
    ...
    failure. Ambient resolution misreports on threads that lost the LEMON_HOME
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        try:
            from lemon_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from lemon_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    A bound LEMON_HOME ContextVar override wins (the gateway multiplexes
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses ``get_default_lemon_root()`` (NOT ``get_lemon_home()``): on a bound
    ...
    try:
        ...
        rel = home.resolve().relative_to((get_default_lemon_root() / "profiles").resolve())
...
def _active_profile_line(agent: Any) -> str:
    """Name the running profile so the agent doesn't conflate ``~/.lemon-ai/skills``
    (default) with ``~/.lemon-ai/profiles/<active>/skills``.  Resolved from the
    ...
    if active_profile == "default":
        ...
        # ROOT (get_lemon_home() on a bound profile session is the PROFILE dir).
        # Without one, keep the ambient (patchable) resolution byte-identical.
        _root_str = str(get_default_lemon_root() if _agent_home_path is not None else get_lemon_home())
        return (
            "Active Lemon AI profile: default. Other profiles (if any) live "
    ...
    profile_home = str(_agent_home_path) if _agent_home_path is not None else str(get_lemon_home())
    ...
    # default profile's data sits at the ROOT (get_default_lemon_root()), which in ambient profile mode is
    # NOT get_lemon_home().
    default_root = get_default_lemon_root()
    return (
        f"Active Lemon AI profile: {active_profile}. This session reads "
...
def _timestamp_line(agent: Any) -> str:
    ...
    from lemon_time import get_timezone as _lemon_tz, now as _lemon_now
    now = _lemon_now()
    _bits = _zone_bits(now, _lemon_tz())
...
def _context_files_part(agent: Any, ctx_len: Optional[int], soul_loaded: bool) -> List[str]:
    ...
    fallback they really are so the guard can reject Lemon AI's bundled AGENTS.md."""
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # lemon-agent skill installed, so the variant is chosen after the skills
    # index is built; this slot holds its position.
    _help_guidance_slot = len(stable_parts)
    stable_parts.append(LEMON_AGENT_HELP_GUIDANCE_NO_SKILLS)
    stable_parts.extend(_guidance_parts(agent))
    skills_prompt = _skills_prompt(agent)
    # Skill-pointer variant requires BOTH skill_view AND the lemon-agent skill
    # in the rendered index (pure string check — inherits the index's stability).
    if "skill_view" in (agent.valid_tool_names or set()) and "- lemon-agent:" in skills_prompt:
        stable_parts[_help_guidance_slot] = LEMON_AGENT_HELP_GUIDANCE
