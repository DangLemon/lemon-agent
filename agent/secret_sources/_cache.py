...
a cache problem must never block Lemon AI startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_lemon_dotenv()``, else ``$LEMON_HOME``/``~/.lemon-ai``."""
    if home_path is None:
        from lemon_constants import get_lemon_home

        home_path = get_lemon_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<lemon_home>/cache/<basename>``::
