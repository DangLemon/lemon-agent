...
``skill_manage`` per the Lemon AI authoring standards; large prose sources get the knowledge-base layout (lean SKILL.md
...
Follow the Lemon AI skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Lemon AI`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Lemon AI.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.lemon.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through Lemon AI tools.
...
Lemon AI-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Lemon AI tools by name in backticks: `terminal`, `read_file`,
