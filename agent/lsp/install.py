...
Installs go to a Lemon AI-owned staging dir, ``<LEMON_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``lemon lsp status`` reports it).
...
# ``bin`` in ``<LEMON_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `lemon lsp status` reports its presence.
...
def lemon_lsp_bin_dir() -> Path:
    """Return the Lemon AI-owned bin staging dir for LSP servers."""
    from lemon_constants import get_lemon_home

    p = get_lemon_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(lemon_lsp_bin_dir() / name):
...
def _link_into_bin(target: Path) -> str:
    ...
    link = lemon_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $LEMON_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that Lemon AI installed.
    npm = find_node_executable("npm")
    if npm is None:
        logger.info("[install] cannot install %s: no usable npm found", pkg)
        return None
    staging = lemon_lsp_bin_dir().parent  # <LEMON_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = lemon_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``lemon lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "lemon_lsp_bin_dir"]
