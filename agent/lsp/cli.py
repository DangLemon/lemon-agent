"""``lemon lsp`` CLI subcommand: status / list / install / install-all / restart / which.

Handlers live here (not in ``lemon_cli/main.py``) so the LSP module ships self-contained.
"""
...
def register_subparser(subparsers: argparse._SubParsersAction) -> None:
    """Wire the ``lemon lsp`` subcommand tree into the main argparse."""
...
def run_lsp_command(args: argparse.Namespace) -> int:
    """Top-level dispatcher for ``lemon lsp <subcommand>``."""
