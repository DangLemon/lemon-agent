...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    ``PSES_BUNDLE_PATH`` env, then ``<LEMON_HOME>/lsp/PowerShellEditorServices``."""
    ...
    candidates = [
        ...
        os.path.join(str(get_lemon_home()), "lsp", "PowerShellEditorServices"),
    ]
...
_PSES_MISSING_MSG = (
    ...
    "lsp.servers.powershell.command to the bundle path or unzip it to <LEMON_HOME>/lsp/PowerShellEditorServices."
)
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    session_dir = lemon_lsp_session_dir()
    inner = (
        ...
        f"-HostName Lemon AI -HostProfileId lemon -HostVersion 1.0.0 -Stdio -LogLevel Normal"
    )
    return SpawnSpec(
...
def lemon_lsp_session_dir() -> str:
    """Return (and create) the dir for PSES session/log scratch files."""
    from lemon_constants import get_lemon_home
    d = os.path.join(str(get_lemon_home()), "lsp", "pses")
