"""Unified removal contract for every credential source Lemon AI reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``lemon auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.lemon-ai/.env; hint when the shell exports it."""
    from lemon_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like lemon_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            f"(not in ~/.lemon-ai/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Lemon AI.",
            f"  The pool entry is now suppressed — Lemon AI will ignore "
            f"{env_var} until you run `lemon auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_lemon_pkce(provider: str, removed) -> RemovalResult:
    """~/.lemon-ai/.anthropic_oauth.json is ours — delete it outright."""
    from lemon_constants import get_lemon_home

    result = RemovalResult()
    oauth_file = get_lemon_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Lemon AI Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `lemon model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `lemon auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `lemon auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `lemon auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="lemon_pkce",
        remove_fn=_remove_lemon_pkce,
        description="~/.lemon-ai/.anthropic_oauth.json",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `lemon auth add qwen-oauth` to re-enable if needed.",
        ),
