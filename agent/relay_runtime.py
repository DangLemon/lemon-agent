"""Profile-scoped NeMo Relay runtimes owned by the Lemon AI agent core."""
...
from lemon_cli.relay_plugin_cutover import (RELAY_PLUGINS_CONFIG_ENV, configured_legacy_relay_env_vars)

logger = logging.getLogger(__name__)

SESSION_SCOPE = "lemon.session"
TURN_SCOPE = "lemon.turn"
LOGICAL_LLM_SCOPE = "lemon.logical_llm_call"
RUNTIME_SCHEMA_KEY = "lemon.relay.schema_version"
RUNTIME_SCHEMA_VERSION = "lemon.relay.runtime.v1"
RUNTIME_INSTANCE_KEY = "lemon.relay.runtime_instance"
RELAY_PLUGINS_EXECUTION_CONSUMER = "lemon.nemo_relay.plugins"
...
def runtime_metadata(runtime_id: str, **extra: Any) -> dict[str, Any]:
    """Return the scope metadata that stamps every Lemon AI-owned Relay scope."""
...
@dataclass
class RelaySession:
    """One isolated Relay scope stack owned by a Lemon AI session."""
    ...
    def run_in_session(self, session: RelaySession, callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        ...
        with self._lock:
            if self._runtime is None:
                raise RuntimeError("Lemon AI Relay operation lease is released")
...
class _ProcessRelayPluginConfiguration:
    ...
    def __init__(self) -> None:
        ...
        self._relay: Any = None  # set while a Lemon AI-owned configuration is active
        ...
        with self._lock:
            if not self._owners:
                ...
                if self._state is _RelayPluginConfigurationState.ACTIVE:
                    logger.info(
                        "Relay plugins are active process-wide and apply to all profiles hosted by this Lemon AI process."
                    )
    ...
    def _activate(self, relay: Any) -> _RelayPluginConfigurationState:
        try:
            ...
            logger.warning("Lemon AI Relay plugin initialization failed: %s", exc, exc_info=True)
    ...
    def _preflight(self, relay: Any) -> _RelayPluginConfigurationState | None:
        ...
        if self._relay is not None and not self._clear_active():
            logger.warning(
                "Lemon AI Relay plugin cleanup is still pending; refusing to replace the process-global configuration"
            )
            return _RelayPluginConfigurationState.FAILED
        try:
            ...
            logger.warning(
                "Lemon AI could not determine whether a process-global Relay plugin configuration is already "
                "active; refusing to replace it", exc_info=True,
            )
            return _RelayPluginConfigurationState.FAILED
        if existing_report is not None:
            logger.warning(
                "A process-global Relay plugin configuration is already active outside Lemon AI native ownership; "
                "leaving it unchanged and disabling Lemon AI-managed Relay middleware for this process"
            )
    ...
    def _initialize(self, relay: Any) -> bool:
        ...
        if dynamic_plugins:
            try:
                ...
                raise RuntimeError("Lemon AI Relay dynamic plugin activation failed") from exc
    ...
    def _clear_active(self) -> bool:
        ...
        for what, step in (
            ...
            try:
                ...
                logger.warning("Lemon AI Relay plugin %s failed", what, exc_info=True)
                return False
...
class RelayRuntime:
    ...
    def managed_execution_enabled(self) -> bool:
        """Return whether a Lemon AI-managed consumer needs the Relay pipeline."""
        with self._execution_consumers_lock:
    ...
    def rotate_session_scope(self, session: RelaySession, *, reason: str) -> None:
        ...
        with session.lock:
            ...
            try:
                self.run_in_session(
                    session, self.relay.scope.pop, old_handle, output={"lemon.session.segment_reason": reason},
                    metadata=runtime_metadata(self.runtime_id), timeout=_SCOPE_OP_TIMEOUT,
                )
            except Exception:
                logger.warning(
                    "Lemon AI Relay segment close failed (session=%s segment=%d); abandoning the old segment span",
                    session.session_id, session.segment - 1, exc_info=True,
                )
            scope_metadata = runtime_metadata(
                self.runtime_id, **{"lemon.session.segment": session.segment, "lemon.session.segment_reason": reason},
            )
            try:
                self._open_session_scope(session, scope_metadata, resolve_parent=False)
            except Exception:
                logger.warning(
                    "Lemon AI Relay segment open failed (session=%s segment=%d); keeping the prior scope handle",
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active Lemon AI Relay session without creating one."""
        with self._sessions_lock:
    ...
    def _session_context(self, session: RelaySession, *, allow_closing: bool) -> contextvars.Context:
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("Lemon AI Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("Lemon AI Relay session context is unavailable")
    ...
    def _begin_operation(self) -> None:
        ...
        with self._sessions_lock:
            if self._closing:
                raise RuntimeError("Lemon AI Relay runtime is shutting down")
    ...
    def apply_tool_request_intercepts(self, *, session_id: str, tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
        """Apply Relay request rewriting before Lemon AI authorizes a tool call."""
    ...
    def _pop_with_drain(
        ...
        for _ in range(drain_limit):
            ...
            if session_root is not None and _same_handle(top, session_root) and handle is not session_root:
                break
            try:
                orphan_output = {"outcome": "cancelled", "lemon.orphan_drain": True}
                pop_relay_scope(self.relay, top, output=orphan_output, metadata=metadata)
                drained += 1
            except Exception:
                logger.warning("Lemon AI Relay orphaned scope drain failed", exc_info=True)
                break
        if drained:
            logger.warning("Lemon AI Relay drained %d orphaned scope(s) before closing %s", drained, handle)
        try:
            pop_relay_scope(self.relay, handle, output=output, metadata=metadata)
            return None
    ...
    def _close_session(self, event: dict[str, Any]) -> None:
        ...
        with self._sessions_lock:
            ...
            self._forget_subagent(session_id)
        if failure:
            logger.warning("Lemon AI Relay session %s closed with errors: %s", session_id, failure)
    ...
    def shutdown(self) -> None:
        ...
        thread = threading.Thread(
            ...
            name=f"lemon-nemo-relay-shutdown-{self.runtime_id[:8]}", daemon=True,
        )
        try:
            thread.start()
        except Exception:
            ...
            logger.warning("Lemon AI Relay deferred shutdown could not start", exc_info=True)
    ...
    def _finish_shutdown(self) -> None:
        try:
            ...
            with self._sessions_lock:
                self._shutdown_started = False
            logger.warning("Lemon AI Relay shutdown failed", exc_info=True)
            return
        with self._sessions_lock:
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical Lemon AI profile."""
    ...
    def for_profile(self, profile_key: str | None = None, *, create: bool = True) -> RelayHost | None:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Lemon AI Relay runtime initialization failed", exc_info=True)
...
class RelayTurnContext:
    """Runtime-only context for one Lemon AI turn or top-level task."""
...
_CURRENT_TURN: contextvars.ContextVar[RelayTurnContext | None] = contextvars.ContextVar(
    "lemon_relay_turn", default=None
)

# >0 while the native pipeline is mid-dispatch of a Lemon AI tool/LLM callback. Nested managed
...
_MANAGED_CALLBACK_DEPTH: contextvars.ContextVar[int] = contextvars.ContextVar(
    "lemon_relay_managed_callback_depth", default=0
)
...
def _warn_on_error(what: str, callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
    """Run fail-open telemetry work: log ``Lemon AI Relay <what> failed`` and return None on error."""
    try:
        return callback(*args, **kwargs)
    except Exception:
        logger.warning("Lemon AI Relay %s failed", what, exc_info=True)
        return None
...
class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for Lemon AI core."""
    ...
    def _prepare_session(self, host: RelayRuntime, context: dict[str, Any]) -> None:
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning("Lemon AI Relay session initializer failed: %s", name, exc_info=True)
    ...
    def _open_conversation_session(self, host: RelayRuntime, context: dict[str, Any]) -> RelaySession | None:
        ...
        metadata = {"lemon.execution_surface": context["platform"] or "unknown"}
    ...
    def begin_turn(self, lease: ConversationLease, *, turn_id: str, task_id: str) -> RelayTurnContext:
        if lease.released:
            raise RuntimeError("Lemon AI Relay conversation lease is released")
        ...
        with self._active_turns_lock:
            if self._active_turns.get(key):
                ...
                logger.warning(
                    "Skipping Relay instrumentation for concurrent Lemon AI turn %s in session %s",
        ...
        if host is not None:
            ...
            turn.handle = _warn_on_error(
                ...
                metadata=runtime_metadata(host.runtime_id, **{"lemon.execution_surface": lease.platform or "unknown"}),
    ...
    def _close_turn_scope(self, host: RelayRuntime, turn: RelayTurnContext, *, outcome: str) -> None:
        ...
        if failure:
            logger.warning("Lemon AI Relay turn finalization failed: %s", failure)
    ...
    def _finish_logical_calls(turn: RelayTurnContext, *, outcome: str) -> None:
        ...
        while logical_calls:
            ...
            logger.warning("Lemon AI Relay logical LLM finalization failed: %s", failure)
            break
...
def resolve_execution_context(session_id: str) -> tuple[RelayRuntime | None, RelaySession | None, Any]:
    ...
    if _MANAGED_CALLBACK_DEPTH.get() > 0 or not relay_instrumentation_enabled():
        ...
        # ``tools.execute`` pipeline is mid-dispatch of a Lemon AI tool). Nested managed execution here is
...
def apply_tool_request_intercepts(*, session_id: str, tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
    """Return Relay-rewritten arguments at Lemon AI's authorization boundary."""
...
def get_runtime(*, create: bool = True, profile_key: str | None = None) -> RelayRuntime | None:
    """Return the Relay host for the active Lemon AI profile."""
...
def current_profile_key() -> str:
    ...
    home = get_lemon_home().expanduser()
...
def _configured_plugin_inputs(relay: Any) -> tuple[dict[str, Any], list[Any]] | None:
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError("Lemon AI [[dynamic_plugins]] records are unsupported; use Relay [[plugins.dynamic]] records")
        ...
        raise _RelayPluginConfigurationLoadError(
            f"Lemon AI Relay plugin configuration could not be loaded from {config_path}; continuing without Relay plugins"
        ) from exc
...
def _resolve_plugin_awaitable(value: Any) -> Any:
    ...
    return _run_on_daemon_thread(lambda: asyncio.run(value), name="lemon-nemo-relay-plugin-lifecycle")
...
def emit_mark(
    ...
    *,
    ...
    """Emit a fail-open Relay mark under a Lemon AI session."""
    runtime = get_runtime(create=False)
    if runtime is None:
        return False
    try:
        ...
        logger.warning("Lemon AI Relay mark failed: %s", name, exc_info=True)
        return False

def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by Lemon AI core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("Lemon AI Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared Lemon AI session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Lemon AI Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("Lemon AI Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared Lemon AI session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Lemon AI Relay runtime is unavailable")
    session = runtime.get_session(session_id)
