...
inferred from the atlas shape so Lemon AI renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (Lemon AI names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy Lemon AI/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical Lemon AI names -> accepted row-name aliases in descending preference.
