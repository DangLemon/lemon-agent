...
``lemon pause`` writes a sentinel at ``$LEMON_HOME/ESTOP``; ``lemon resume``
...
(fail safe, e.g. ``touch ~/.lemon-ai/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.lemon-ai).
from agent.file_safety import _lemon_home_path as _lemon_home, _lemon_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `lemon pause`."""
    return _lemon_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (LEMON_HOME=~/.lemon-ai/profiles/<n>) must still honor an operator's ~/.lemon-ai/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Lemon AI is paused{tag}. New work is on hold; run `lemon resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `lemon resume` (%s)",
