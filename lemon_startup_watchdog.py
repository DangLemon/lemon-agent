...
timeout. Config is env-only (``LEMON_STARTUP_WATCHDOG=0``,
``LEMON_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "LEMON_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "LEMON_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (lemon_cli.main / lemon_cli.gateway /
...
def _process_lemon_home() -> Path:
    """LEMON_HOME for diagnostic files — stdlib-only replica of the lemon_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<LEMON_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_lemon_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``LEMON_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``LEMON_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
