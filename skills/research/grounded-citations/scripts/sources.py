...
Ledger path resolution (first wins):
  --ledger PATH
  $LEMON_CITATION_LEDGER
  $LEMON_HOME/cache/citations/ledger.json
"""
...
from _lemon_home import get_lemon_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("LEMON_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_lemon_home() / "cache" / "citations" / "ledger.json"
