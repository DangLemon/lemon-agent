#!/usr/bin/env bash
# GitHub environment detection helper for Lemon AI skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _lemon_env="${LEMON_HOME:-$HOME/.lemon-ai}/.env"; [ -f "$_lemon_env" ] && grep -q "^GITHUB_TOKEN=" "$_lemon_env" 2>/dev/null; then
