...
Config: `~/.lemon-ai/config.yaml` (settings), `~/.lemon-ai/.env` (API keys) — both under `$LEMON_HOME` when it is set.
...
`_LEMON_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_lemon_home()` for paths,
never hardcode `~/.lemon-ai`. For custom/local-only tools, write a plugin in
`~/.lemon-ai/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `lemon_cli/commands.py`
...
- Tests auto-redirect `LEMON_HOME` to temp dirs — never touch real `~/.lemon-ai/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/lemon_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_lemon_home()` from `lemon_constants` for all paths (profile-safe)
