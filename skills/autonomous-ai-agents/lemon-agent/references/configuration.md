...
`lemon config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `lemon tools` (interactive) or `lemon tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_LEMON_CORE_TOOLS` is the default bundle most platforms inherit).
