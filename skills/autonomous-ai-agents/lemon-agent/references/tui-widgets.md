...
Author widget apps for the Lemon AI TUI (`lemon --tui`): glanceable ambient
...
- The TUI must be in use (`lemon --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.lemon-ai/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.lemon-ai/logs/tui_gateway_crash.log` if a widget never appears.
