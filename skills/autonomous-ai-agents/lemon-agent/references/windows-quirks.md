...
Lemon AI runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`lemon config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Lemon AI's env scrubber dropping
...
Lemon AI-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Lemon AI tool and most Windows APIs. Prefer forward slashes in code
