...
Write plugins for the Lemon AI desktop app: statusbar items, layout panes,
...
system (`~/.lemon-ai/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$LEMON_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$LEMON_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Lemon AI desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$LEMON_HOME/desktop-plugins/` (usually
  `~/.lemon-ai/desktop-plugins/`).

## How to Run

1. Create `$LEMON_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.lemon-ai/...` by default, or `~/.lemon-ai/profiles/<profile>/...` under a
...
The ONLY import surface is `@lemon-ai/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Lemon AI
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.lemon-ai/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@lemon-ai/plugin-sdk`, `react`, and
