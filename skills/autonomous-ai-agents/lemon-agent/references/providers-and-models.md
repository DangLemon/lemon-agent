...
| nous | OAuth device code | `lemon auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `lemon auth add openai-codex` |
| qwen-oauth | OAuth | `lemon auth add qwen-oauth` |
| minimax-oauth | OAuth | `lemon auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`lemon auth`).
Fallback chain when the primary fails: `lemon fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`lemon_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   lemon config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
