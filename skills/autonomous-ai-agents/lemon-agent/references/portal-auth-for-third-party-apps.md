...
## Layer 1 — Is this thing a Lemon AI plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Lemon AI process**. Its LLM calls go through Lemon AI's already-configured provider. | Already uses Portal if user's Lemon AI is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Lemon AI. That LLM call is already authenticated
via Lemon AI's provider config. The plugin's own server auth (e.g.
...
What Lemon AI has that *feels* like OAuth — `lemon login --provider nous`
opening a browser, user signs in, token lands in `~/.lemon-ai/auth.json` — is a
**Lemon AI-specific browser flow**. Under the hood it produces a credential
Lemon AI uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Lemon AI's existing Portal credential out of `~/.lemon-ai/auth.json`.
...
rides on the credential Lemon AI already holds.

Where this could live in Lemon AI:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Lemon AI itself.
...
1. First decide: Lemon AI plugin (runs inside Lemon AI) or separate app? If plugin,
   it already uses Portal via Lemon AI's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Lemon AI agent itself") is fully supported. Lemon AI treats agent
...
pitch in the Tool Gateway announcement is specifically about Lemon AI's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Lemon AI is configured to use Portal as its provider. It is
