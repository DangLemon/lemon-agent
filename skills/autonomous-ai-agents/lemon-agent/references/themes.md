# Themes / Skins — Author a Lemon AI Color Theme

Author a Lemon AI **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`lemon_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.lemon-ai/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Lemon AI itself.
...
- Write access to the Lemon AI home dir — `~/.lemon-ai` by default, or `$LEMON_HOME`
  / the active profile's dir. Skins live in `<lemon-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `lemon config set`).
...
3. `write_file` it to `<lemon-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `lemon config set display.skin default` (or they can `/skin default`).
...
```
lemon skin set <key> <hex>      # e.g. lemon skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `lemon skin set background "#08201f"` changes
only the background; `lemon skin use <name>` / `lemon skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.lemon-ai`** when a profile is active — resolve the real home
  from `$LEMON_HOME` first, falling back to `~/.lemon-ai`.
...
- **Never hand-edit `config.yaml` to activate.** Use `lemon config set
...
- **You apply it, not the user.** `lemon config set display.skin <name>` is
...
- `read_file` the written `<lemon-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `lemon config get display.skin` and confirm it reports `<name>`.
