...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Lemon AI agent runs by POSTing events to a URL.
...
Add to `${LEMON_HOME:-~/.lemon-ai}/.env`:
```bash
...
All management is via the `lemon webhook` CLI command:

### Create a subscription
```bash
lemon webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.lemon-ai/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.lemon-ai/scripts/` (path traversal is blocked).

```bash
lemon webhook subscribe todoist-lemon \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-lemon-label.py" \
...
```bash
lemon webhook remove <name>
```

### Test a subscription
```bash
lemon webhook test <name>
lemon webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
lemon webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
lemon webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
lemon webhook subscribe alerts \
...
- Subscriptions persist to `~/.lemon-ai/webhook_subscriptions.json`

## How It Works

1. `lemon webhook subscribe` writes to `~/.lemon-ai/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status lemon-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.lemon-ai/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `lemon webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `lemon webhook test <name>` to verify the route works.
