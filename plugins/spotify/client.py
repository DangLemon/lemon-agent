"""Thin Spotify Web API helper used by Lemon AI native tools.
...
from lemon_cli.auth import AuthError, resolve_spotify_runtime_credentials
...
def _friendly_spotify_error_message(*, status_code: int, detail: str, path: str, retry_after: Optional[str]) -> str:
    ...
    if status_code == 401:
        return "Spotify authentication failed or expired. Run `lemon auth spotify` again."
    if status_code == 403:
        if is_playback_path:
        ...
        if "scope" in detail.lower() or "permission" in detail.lower():
            return "Spotify rejected the request because the current auth scope is insufficient. Re-run `lemon auth spotify` to refresh permissions."
