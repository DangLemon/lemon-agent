"""Per-plugin persistent storage: ``<lemon home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<lemon home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `lemon plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<lemon home>/plugin-data/<name>/``; resolves ``get_lemon_home()`` on
    ...
    root = get_lemon_home() / "plugin-data" / _validate_name(name)
