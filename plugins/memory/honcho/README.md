...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/lemon>
...
```bash
lemon memory setup honcho   # configure Honcho directly (works on a fresh install)
lemon memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
lemon config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.lemon-ai/.env
```

> `lemon honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `lemon memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `lemon model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$LEMON_HOME/honcho.json` | Profile-local (isolated Lemon AI instances) |
| 2 | `~/.lemon-ai/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Lemon AI profile: `lemon` (default) or `lemon_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `lemon honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Lemon AI session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`lemon-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`lemon`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-lemon-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening lemon in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Lemon AI session ID (timestamp + hex). Every `lemon` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Lemon AI profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`lemon`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Lemon AI profile: `lemon` (default) or `lemon_<profile>` (e.g. `lemon -p coder` -> host key `lemon_coder`). Older `lemon.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `LEMON_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `lemon-agent`) |
...
|---------|-------------|
| `lemon memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `lemon honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `lemon memory setup`) |
| `lemon honcho status` | Show resolved config for active profile |
| `lemon honcho enable` / `disable` | Toggle Honcho for active profile |
| `lemon honcho mode <mode>` | Change recall or observation mode |
| `lemon honcho peer --user <name>` | Update user peer name |
| `lemon honcho peer --ai <name>` | Update AI peer name |
| `lemon honcho tokens --context <N>` | Set context token budget |
| `lemon honcho tokens --dialectic <N>` | Set dialectic max chars |
| `lemon honcho map <name>` | Map current directory to a session name |
| `lemon honcho sync` | Create host blocks for all Lemon AI profiles |
