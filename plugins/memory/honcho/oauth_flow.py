...
core (the code can arrive via the loopback listener here or a ``lemon://``
...
# Loopback redirect registered for the Lemon AI OAuth client. IP-literal so the browser can't resolve the
...
_DEFAULT_CLIENT_ID = "lemon-agent"
...
_CALLBACK_HTML = _CALLBACK_PAGE.format(
    title="Honcho connected", body="Connected to Honcho. You can close this tab and return to Lemon AI."
).encode()
...
def start_loopback_flow_background(
    *, config_path: Path | None = None, host: str | None = None, source: str = "lemon-desktop",
    timeout: float = 300.0,
) -> dict[str, str]:
    ...
    global _flow_thread
    # Resolve under the caller's profile scope NOW — a context-local LEMON_HOME override can't reach the worker.
