...
- OpenViking server running and reachable from Lemon AI

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Lemon AI can identify older servers that expose the legacy status-only health
...
connection values into Lemon AI, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.lemon-ai/.env`; for a named profile use
...
OpenViking's server config is separate from Lemon AI:
...
Lemon AI-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Lemon AI lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Lemon AI sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Lemon AI also sends `User-Agent: openviking-memory-lemon/<version>` on
OpenViking requests. This standard harness identifier contains the Lemon AI
...
does not ask for a peer ID. Without a configured peer, Lemon AI sends neither
...
order remains environment, linked OpenViking config, then Lemon AI YAML. To use
...
on the old implicit `lemon` peer now use user memory for new writes. Without
...
Set `agent: lemon` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `lemon-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Lemon AI
...
Use the same OpenViking profile and credentials as Lemon AI for manual recovery.
...
Lemon AI built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Lemon AI action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Lemon AI
...
file URIs, such as
`viking://user/default/peers/lemon/memories/preferences/mem_abc123.md` (any
