...
```bash
lemon memory setup    # select "mem0"
```
...
Behavioral settings live in `$LEMON_HOME/mem0.json` (set them via `lemon memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.lemon-ai/.env`.
...
| `user_id` | `lemon-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Lemon AI session and call `mem0_search` — it connects to your server.
...
```bash
lemon memory setup mem0 --mode oss \
...
```bash
lemon memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$LEMON_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.lemon-ai/mem0_qdrant"}}
  }
}
```
...
```bash
lemon memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$LEMON_HOME/mem0.json`).
