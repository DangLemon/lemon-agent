...
_DEFAULT_CONTAINER_TAG = "lemon"
_VALID_SEARCH_MODES = ("hybrid", "memories", "documents")
_DEFAULT_BASE_URL = "https://api.supermemory.ai"
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=lemon"
...
def _load_supermemory_config(lemon_home: Optional[str] = None) -> dict:
    """Defaults overlaid with $lemon_home/supermemory.json (None = defaults only), every key normalized."""
    config = {k: (list(d) if isinstance(d, list) else d) for k, (d, _) in _CONFIG_SPEC.items()}
    if lemon_home is not None:
        config.update({k: v for k, v in _read_json_dict(Path(lemon_home) / "supermemory.json").items() if v is not None})
    for key, (_, normalize) in _CONFIG_SPEC.items():
        config[key] = normalize(config[key])
    return config


def _save_supermemory_config(values: dict, lemon_home: str) -> None:
    from utils import atomic_json_write
    config_path = Path(lemon_home) / "supermemory.json"
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(api_key=api_key, base_url=self._base_url, timeout=timeout, max_retries=0,
                                   default_headers={"x-sm-source": "lemon"})

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Lemon AI writes into the "Lemon AI" Space in the Supermemory app so the user
        # can filter / bulk-manage them per source agent (a routing key for the user, not telemetry).
        merged = {"sm_source": "lemon", **(metadata or {})}
...
def _probe_supermemory_connection(api_key: str, lemon_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(lemon_home)
...
class SupermemoryMemoryProvider(MemoryProvider):
    def __init__(self):
        self._api_key = self._session_id = self._lemon_home = ""
    ...
    def get_config_schema(self):
        # Only the API key is prompted during `lemon memory setup`; other options live in supermemory.json / env.
    ...
    def save_config(self, values, lemon_home):
        ...
        _save_supermemory_config(sanitized, lemon_home)

    def get_status_config(self, provider_config: dict) -> dict:
        from lemon_constants import get_lemon_home
        return {"summary": _format_connection_summary(_probe_supermemory_connection(get_secret("SUPERMEMORY_API_KEY", "") or "", str(get_lemon_home())))}

    def post_setup(self, lemon_home: str, config: dict) -> None:
        from lemon_cli.config import save_config
        from lemon_cli.memory_setup import _prompt, _write_env_vars
        ...
        save_config(config)
        if val:
            _write_env_vars({"SUPERMEMORY_API_KEY": val}, lemon_home=lemon_home)
        ...
        status = _probe_supermemory_connection(api_key, lemon_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from lemon_constants import get_lemon_home
        self._lemon_home = kwargs.get("lemon_home") or str(get_lemon_home())
        self._session_id, self._turn_count, self._session_turns = session_id, 0, []
        config = _load_supermemory_config(self._lemon_home)
