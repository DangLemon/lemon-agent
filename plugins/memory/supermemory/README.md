...
- Hosted: API key from [app.supermemory.ai/integrations?connect=lemon](http://app.supermemory.ai/integrations?connect=lemon)
...
Before running `lemon memory setup`, add the local endpoint to
...
Then run `lemon memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$LEMON_HOME/supermemory.json`
...
| `container_tag` | `lemon` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `lemon-{identity}` → `lemon-coder`). |
...
`https://api.supermemory.ai`. Lemon AI resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: lemon`, and document writes stamp
`metadata.sm_source: lemon`. This is a **functional routing key, not telemetry**:
it groups Lemon AI-written memories into a dedicated "Lemon AI" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Lemon AI profile:

```json
{
  "container_tag": "lemon-{identity}"
}
```

For a profile named `coder`, this resolves to `lemon-coder`. The default profile resolves to `lemon-default`. Without `{identity}`, all profiles share the same container.
