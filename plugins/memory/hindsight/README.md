...
Lemon AI spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.lemon-ai/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p lemon ui start
```
...
Config file: `~/.lemon-ai/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `lemon` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `lemon-{profile}` isolates memory per active Lemon AI profile. Empty placeholders collapse cleanly (e.g. `lemon-{user}` with no user becomes `lemon`). |
...
| `retain_source` | — | Opt-in `metadata.source` attached to retained memories (identifies the storing client, e.g. `lemon`). Empty by default — no attribution tag ships unless you set it. |
...
The LLM API key is stored in `~/.lemon-ai/.env` as `HINDSIGHT_LLM_API_KEY`.
