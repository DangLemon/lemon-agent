...
catalog, filter to the ``lemon`` integration, apply a manifest via
...
def fetch_lemon_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``lemon`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "lemon" in (e.get("integrations") or [])]
