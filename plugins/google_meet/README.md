...
Let the lemon agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where lemon runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `lemon meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `lemon meet` CLI |
...
| `cli.py` | `lemon meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$LEMON_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `lemon meet node {run,list,approve,remove,status,ping}` |
...
lemon meet install                                      # pip + Chromium
lemon meet setup                                        # preflight
lemon meet auth                                         # optional
lemon meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
lemon meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.lemon-ai/.env
lemon meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
lemon meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
lemon meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.lemon-ai/.env
lemon meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, lemon will **not** switch your system audio input automatically — the
...
lemon plugins enable google_meet
lemon meet node run --display-name my-mac --host 0.0.0.0 --port 18789
