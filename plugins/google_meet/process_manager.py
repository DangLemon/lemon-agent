...
One active meeting at a time, recorded in ``$LEMON_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_lemon_home()) / "workspace" / "meetings"
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Lemon AI", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "LEMON_MEET_URL": url, "LEMON_MEET_OUT_DIR": str(out),
           "LEMON_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "LEMON_MEET_HEADED"),
        (auth_state, "LEMON_MEET_AUTH_STATE"),
        (duration, "LEMON_MEET_DURATION"),
        (mode, "LEMON_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "LEMON_MEET_REALTIME_MODEL"),
        (realtime_voice, "LEMON_MEET_REALTIME_VOICE"),
        (realtime_instructions, "LEMON_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("LEMON_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["LEMON_MEET_REALTIME_KEY"] = realtime_api_key
