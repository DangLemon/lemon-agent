/**
 * Lemon AI Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.lemon-ai/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__LEMON_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__LEMON_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "lemon-kanban-dot-triage",
    todo: "lemon-kanban-dot-todo",
    ready: "lemon-kanban-dot-ready",
    running: "lemon-kanban-dot-running",
    blocked: "lemon-kanban-dot-blocked",
    review: "lemon-kanban-dot-review",
    done: "lemon-kanban-dot-done",
    archived: "lemon-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-lemon-task";
  ...
  const DOCS_URL = "https://github.com/DangLemon/lemon-agent/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://github.com/DangLemon/lemon-agent/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "lemon.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="lemon-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])lemon-kanban-md-code\1/i.test(attrs)) {
      return ' class="lemon-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "lemon-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "lemon-kanban-md",
  ...
  // standard `drop` event and our `lemon-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("lemon-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("lemon-kanban-column--drop");
          if (target) target.classList.add("lemon-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("lemon-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("lemon-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("lemon-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__LEMON_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "lemon-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "lemon-kanban-attention",
        "lemon-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "lemon-kanban-attention-bar" },
        h("span", { className: "lemon-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "lemon-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "lemon-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "lemon-kanban-attention-row",
                  "lemon-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "lemon-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "lemon-kanban-attention-row-id" }, task.id),
                h("span", { className: "lemon-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "lemon-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "lemon-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "lemon-kanban-diag-action-btn",
      action.suggested ? "lemon-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " lemon-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".lemon-ai-kanban-drawer-comment-row input, .lemon-ai-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "lemon-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("lemon-kanban-diag", sevClass) },
      h("div", { className: "lemon-kanban-diag-header" },
        h("span", { className: "lemon-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "lemon-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "lemon-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "lemon-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "lemon-kanban-diag-data-row" },
                  h("span", { className: "lemon-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "lemon-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "lemon-kanban-diag-data-row" },
                h("span", { className: "lemon-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "lemon-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "lemon-kanban-diag-reassign-row" },
            h("span", { className: "lemon-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "lemon-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "lemon-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "lemon-kanban-diag-msg",
              msg.ok ? "lemon-kanban-diag-msg--ok" : "lemon-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "lemon-kanban-section" },
      h("div", { className: "lemon-kanban-section-head-row" },
        h("span", { className: "lemon-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "lemon-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "lemon-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "lemon-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "lemon-kanban-docs-link",
      title: "Open Lemon AI Kanban docs in a new tab",
      "aria-label": "Lemon AI Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `lemon kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "lemon-kanban-msg-ok" : "lemon-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "lemon-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "lemon-kanban-boardswitcher" },
      h("div", { className: "lemon-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "lemon-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "lemon-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "lemon-kanban-dialog-title" },
        ...
        h("div", { className: "lemon-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Lemon AI profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "lemon-kanban-bulk" },
      h("span", { className: "lemon-kanban-bulk-count" },
      ...
      h("div", { className: "lemon-kanban-bulk-priority",
      ...
      h("div", { className: "lemon-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Lemon AI profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "lemon-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("lemon-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("lemon-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "lemon-kanban-trash",
        dragOver ? "lemon-kanban-trash--drop" : "",
        props.draggingTaskId ? "lemon-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "lemon-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "lemon-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".lemon-ai-kanban-card")) return true;
      if (target.closest && target.closest(".lemon-ai-kanban-column-add")) return true;
      if (target.closest && target.closest(".lemon-ai-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("lemon-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("lemon-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".lemon-ai-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "lemon-kanban-columns",
        isScrollable ? "lemon-kanban-columns--scrollable" : "",
        isPanning ? "lemon-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("lemon-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("lemon-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "lemon-kanban-column",
        dragOver ? "lemon-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "lemon-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "lemon-kanban-col-check",
        ...
        h("span", { className: cn("lemon-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "lemon-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "lemon-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "lemon-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "lemon-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "lemon-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "lemon-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "lemon-kanban-lane" },
                  h("div", { className: "lemon-kanban-lane-head" },
                    h("span", { className: "lemon-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "lemon-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "lemon-kanban-card--stale-red";
    if (age >= tier.amber) return "lemon-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".lemon-ai-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "lemon-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "lemon-kanban-card",
        props.selected ? "lemon-kanban-card--selected" : "",
        props.failed ? "lemon-kanban-card--failed" : "",
        props.draggingSource ? "lemon-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "lemon-kanban-card-content" },
          h("div", { className: "lemon-kanban-card-row" },
            h("label", {
              className: "lemon-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "lemon-kanban-card-check",
            ...
            h("span", { className: "lemon-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or lemon kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "lemon-kanban-warning-badge",
                    "lemon-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "lemon-kanban-needs-assignee",
          ...
          h("div", { className: "lemon-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "lemon-kanban-card-row lemon-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "lemon-kanban-assignee",
                            title: `Assigned to Lemon AI profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "lemon-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "lemon-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "lemon-kanban-ago",
  ...
  // centered modal reusing the lemon-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "lemon-kanban-dialog lemon-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Lemon AI profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Lemon AI profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "lemon-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "lemon-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "lemon-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "lemon-kanban-drawer-close",
        ...
        data ? h("div", { className: "lemon-kanban-drawer-comment-foot" },
          h("div", {
            className: "lemon-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "lemon-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "lemon-kanban-section" },
      h("div", { className: "lemon-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "lemon-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "lemon-kanban-drawer-body" },
      h("div", { className: "lemon-kanban-drawer-title" },
        h("span", { className: cn("lemon-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "lemon-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "lemon-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "lemon-kanban-section" },
            h("div", { className: "lemon-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "lemon-kanban-section" },
            h("div", { className: "lemon-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "lemon-kanban-done-no-result lemon-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "lemon-kanban-section" },
            h("div", { className: "lemon-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "lemon-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "lemon-kanban-section" },
        h("div", { className: "lemon-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "lemon-kanban-comment" },
            h("div", { className: "lemon-kanban-comment-head" },
              h("span", { className: "lemon-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "lemon-kanban-diag-action-btn",
      ...
      h("div", { className: "lemon-kanban-section" },
        h("div", { className: "lemon-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "lemon-kanban-comment" },
                h("div", { className: "lemon-kanban-comment-head" },
                  h("span", { className: "lemon-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "lemon-kanban-comment-ago" },
      ...
      h("div", { className: "lemon-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "lemon-kanban-event",
              isDiag ? "lemon-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "lemon-kanban-event-header" },
                  h("span", { className: "lemon-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "lemon-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "lemon-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "lemon-kanban-event-header-plain" },
                  h("span", { className: "lemon-kanban-event-kind" }, e.kind),
                  h("span", { className: "lemon-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "lemon-kanban-event-phantom-row" },
                  h("span", { className: "lemon-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "lemon-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "lemon-kanban-event-payload" },
    ...
    return h("div", { className: "lemon-kanban-section" },
      h("div", { className: "lemon-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "lemon-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `lemon-kanban-run--${r.outcome || r.status || "ended"}`
          : "lemon-kanban-run--active";
        return h("div", { key: r.id, className: cn("lemon-kanban-run", outcomeClass) },
          h("div", { className: "lemon-kanban-run-head" },
            h("span", { className: "lemon-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "lemon-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "lemon-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "lemon-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "lemon-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "lemon-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "lemon-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "lemon-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "lemon-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "lemon-kanban-pre lemon-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "lemon-kanban-meta-row" },
      h("span", { className: "lemon-kanban-meta-label" }, props.label),
      h("span", { className: "lemon-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "lemon-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "lemon-kanban-meta-row" },
        h("span", { className: "lemon-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "lemon-kanban-meta-value lemon-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "lemon-kanban-meta-row" },
        h("span", { className: "lemon-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "lemon-kanban-meta-value lemon-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "lemon-kanban-meta-row" },
        h("span", { className: "lemon-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "lemon-kanban-meta-value lemon-kanban-editable",
    ...
    return h("div", { className: "lemon-kanban-meta-row" },
      h("span", { className: "lemon-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "lemon-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "lemon-kanban-section" },
      h("div", { className: "lemon-kanban-section-head-row" },
        h("span", { className: "lemon-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "lemon-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "lemon-kanban-section" },
      h("div", { className: "lemon-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "lemon-kanban-deps-row" },
        h("span", { className: "lemon-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "lemon-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "lemon-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "lemon-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "lemon-kanban-dep-chip-x",
      ...
      h("div", { className: "lemon-kanban-deps-row" },
        h("span", { className: "lemon-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "lemon-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "lemon-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "lemon-kanban-section" },
      ...
      h("div", { className: "lemon-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "lemon-kanban-home-sub lemon-kanban-home-sub--on"
              : "lemon-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__LEMON_PLUGINS__ && typeof window.__LEMON_PLUGINS__.register === "function") {
    window.__LEMON_PLUGINS__.register("kanban", KanbanPage);
  }
})();
