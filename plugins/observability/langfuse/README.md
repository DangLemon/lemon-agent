...
This plugin ships bundled with Lemon AI but is **opt-in** — it only loads when
...
lemon tools  # → Langfuse Observability
...
Set these in `~/.lemon-ai/.env` (or via `lemon tools`):
...
lemon chat -q "hello"              # then check Langfuse for a "Lemon AI turn" trace
```

Generation observations include the Lemon AI system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `LEMON_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
LEMON_LANGFUSE_ENV=production       # environment tag
LEMON_LANGFUSE_RELEASE=v1.0.0       # release tag
LEMON_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
LEMON_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
LEMON_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
LEMON_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`LEMON_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
lemon plugins disable observability/langfuse
```
