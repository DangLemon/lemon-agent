...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, LEMON_HOME).
        ...
        try:
            ...
            lemon_home = Path(get_lemon_home()).resolve()
        except Exception:
            lemon_home = Path.home().joinpath(".lemon-ai").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), lemon_home)):
...
def is_connected(config) -> bool:
    """Surface in ``lemon status`` even before the adapter is instantiated."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """Seed PlatformConfig.extra from env-only setups so ``lemon status`` sees them."""
    if not _env_credentials_present():
        return None
    seeded: Dict[str, Any] = {}
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``lemon setup line`` (writes ``~/.lemon-ai/.env``)."""
    print("\nLINE Messaging API setup\n------------------------\n"
          "Create a Messaging API channel at https://developers.line.biz/console/\nthen copy the values below.\n")
    try:
        from lemon_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("lemon_cli.config not available; set LINE_* vars manually in ~/.lemon-ai/.env")
        return
