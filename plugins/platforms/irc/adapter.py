"""IRC Platform Adapter for Lemon AI — stdlib asyncio only, zero external dependencies.

config.yaml ``gateway.platforms.irc.extra`` keys: server, port (6697), nickname (lemon-bot), channel,
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = _env_or_extra(extra, "IRC_NICKNAME", "nickname", "lemon-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Lemon AI")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            with contextlib.suppress(Exception):
                await self._send_raw("QUIT :Lemon AI shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        elif command == "433":  # ERR_NICKNAMEINUSE — retry: lemon_, lemon_1, lemon_2...
...
def interactive_setup() -> None:
    """`lemon gateway setup` flow (lazy lemon_cli imports keep the plugin importable outside the CLI)."""
    ...
    if existing_server:
        ...
        if not prompt_yes_no("Reconfigure IRC?", False):
            return
    info("Connect Lemon AI to an IRC network. Uses Python stdlib — no extra packages needed.",
    ...
    for label, env, what in (("Bot nickname (e.g. lemon-bot)", "IRC_NICKNAME", "Nickname"),
                             ("Channel to join (e.g. #lemon — comma-separate for multiple)", "IRC_CHANNEL", "Channel")):
    ...
    print()
    print_success("IRC configuration saved to ~/.lemon-ai/.env")
    print_info("Restart the gateway for changes to take effect: lemon gateway restart")
...
async def _sa_register(conn: _StandaloneConn, nick_base: str, server_password: str) -> Optional[Dict[str, Any]]:
    ...
    await conn.raw(f"NICK {standalone_nick}")
    await conn.raw(f"USER {standalone_nick} 0 * :Lemon AI (cron)")
...
async def _standalone_send(pconfig, chat_id: str, message: str, *, thread_id: Optional[str] = None,
    ...
    nick_base = _env_or_extra(extra, "IRC_NICKNAME", "nickname", "lemon-bot").rstrip("_0123456789-")[:24] or "lemon-bot"
