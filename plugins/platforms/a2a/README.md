# A2A — Agent-to-Agent protocol for Lemon AI
...
peer (another Lemon AI, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
lemon gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Lemon AI serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.lemon-ai/a2a_audit.jsonl`.
- Conversations persist to `~/.lemon-ai/a2a_conversations/` — they survive context
