...
class HomeAssistantAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        payload = {"title": "Lemon AI", "message": content[:self.MAX_MESSAGE_LENGTH]}
...
# lemon_cli/gateway.py, and the _send_email dispatch in tools/send_message_tool.py. EMAIL_*
...
def _is_connected(config) -> bool:
    """Connected when ``HASS_TOKEN`` is set; read via ``lemon_cli.gateway.get_env_value`` at call
