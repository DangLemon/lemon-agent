...
This plugin connects Lemon AI to iMessage (and other Spectrum
...
TypeScript-only, Lemon AI runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Lemon AI channel,
...
Run `lemon photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.lemon-ai/.env` (the same place every other
...
Management metadata lives in `~/.lemon-ai/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `lemon photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Lemon AI
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Lemon AI coalesces
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage read
  after forwarding it to Lemon AI, so the sender sees `Read` without waiting for
  a model/tool turn. Inbound receipts for Lemon AI-sent messages are consumed as
  presence telemetry and never create an agent turn. Set
  `PHOTON_READ_RECEIPTS=false` to keep messages at `Delivered`.
- **Native polls are supported.** Lemon AI posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `lemon send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<lemon-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `lemon photon status`, a DM and a group roundtrip,
