...
// `message.content.text` before Lemon AI can see it. We rewrite the two inbound
...
const MARKER = "Lemon AI patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Lemon AI to patch; keep the legacy v8 path below for older pinned installs.
    if (
