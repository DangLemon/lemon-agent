// Lemon AI — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Lemon AI): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Lemon AI -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Lemon-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `lemon photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Lemon AI can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "lemon-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Lemon AI'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Lemon AI allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Lemon AI, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-lemon-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Lemon AI supplied them so a known-good
