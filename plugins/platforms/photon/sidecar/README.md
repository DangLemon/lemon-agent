...
Small Node helper that bridges Lemon AI to Photon's Spectrum SDK
(`spectrum-ts`).  Lemon AI is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Lemon-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Lemon AI can receive inbound messages
...
The Lemon AI plugin's `lemon photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Lemon AI is Python, so replies go through this sidecar
