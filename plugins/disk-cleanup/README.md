...
Auto-tracks and cleans up ephemeral files created during Lemon AI
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$LEMON_HOME` and `/tmp/lemon-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `LEMON_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `LEMON_HOME` or `/tmp/lemon-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$LEMON_HOME/disk-cleanup/` is itself excluded
- `$LEMON_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
