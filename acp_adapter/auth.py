"""ACP auth helpers — detect and advertise Lemon AI authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "lemon-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Lemon AI runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Lemon AI.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Lemon AI credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Lemon AI using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Lemon AI provider", type="terminal", args=["--setup"],
        description=("Open Lemon AI' interactive model/provider setup in a terminal. "
                     "Use this when Lemon AI has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Lemon AI can resolve any runtime provider credentials."""
