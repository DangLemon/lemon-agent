...
Each probe runs in a FRESH subprocess with an isolated LEMON_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "LEMON_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "LEMON_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("LEMON_SESSION", "LEMON_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "lemon_cli.main", "gateway.run", "tools.registry", "lemon_state", "tui_gateway.server", "lemon_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "lemon-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `lemon --version`, `lemon --help`, `lemon doctor --help`, `lemon config get model` (no network)
CLI = {"version": ["lemon_cli/main.py", "--version"], "help": ["lemon_cli/main.py", "--help"], "config_get": ["lemon_cli/main.py", "config", "get", "model"], "tools_list": ["lemon_cli/main.py", "tools", "--help"], "skills_help": ["lemon_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_lemon_default_ms", lambda: resolve_toolset("lemon-default"), 200)
...
try:
    import lemon_state, tempfile, uuid
    from pathlib import Path
    db=lemon_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(lemon_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
