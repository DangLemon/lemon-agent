...
ctx keys: workspace, lemon_home, events (desktop emit records),
...
def _grep_home(ctx, needle):
    """Binary-safe grep of the entire LEMON_HOME tree (sqlite included)."""
    nb = needle.encode()
    for root, _dirs, files in os.walk(ctx["lemon_home"]):
        for fn in files:
            try:
