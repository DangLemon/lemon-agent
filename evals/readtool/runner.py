"""Run the read-tool eval through the REAL Lemon AI AIAgent.

For each task: fresh temp LEMON_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    lemon_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".lemon-ai"
    ...
    try:
        ...
        shutil.rmtree(lemon_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.lemon-ai/.env; set +a  — then relaunch."
        )
