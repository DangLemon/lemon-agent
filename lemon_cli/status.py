...
from lemon_cli.auth import AuthError, resolve_provider
from lemon_cli.colors import Colors, color
from lemon_cli.config import get_env_path, get_env_value, get_lemon_home, load_config
from lemon_cli.models import provider_label
from lemon_cli.runtime_provider import resolve_requested_provider
from lemon_cli.vercel_auth import describe_vercel_auth
from lemon_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers, _render_nous_gateway)
from lemon_constants import OPENROUTER_MODELS_URL
...
def _estop_status_line():
    """One-line pause banner for `lemon status`, or None when not paused."""
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `lemon resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'lemon-agent[vercel]')")
        ...
        # (fail-soft — never break `lemon status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        from lemon_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
...
def _render_cron(ctx):
    ...
    jobs_file = get_lemon_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from lemon_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_lemon_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
    ...
    try:
        from lemon_cli.active_sessions import (
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'lemon doctor' for detailed diagnostics", "  Run 'lemon setup' to configure"),
            Colors.DIM)
    print()


# Print order of `lemon status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
...
def show_status(args):
    """Show status of all Lemon AI components."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nous_portal_account_info': ('lemon_cli.nous_account', 'get_nous_portal_account_info'),
