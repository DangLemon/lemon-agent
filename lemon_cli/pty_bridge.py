"""PTY bridge for `lemon dashboard` chat tab.
...
``lemon_cli.web_server``. POSIX-only: depends on ``fcntl``, ``termios`` and ``ptyprocess`` (native
...
# ui-tui/packages/lemon-ink/src/ink/termio/host.ts — keep the two in sync.
PTY_HOST_ENV = "LEMON_PTY_HOST"
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError("Pseudo-terminals are unavailable on this platform. "
                                          "Lemon AI supports Windows only via WSL.")
