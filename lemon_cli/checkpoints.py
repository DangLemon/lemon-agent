"""`lemon checkpoints` CLI subcommand.
...
from lemon_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: lemon checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``lemon checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `lemon checkpoints` → status
...
_PLUGIN_COMPAT_LAZY = {
    'cmd_list': ('lemon_cli.plugins_cmd', 'cmd_list'),
}
