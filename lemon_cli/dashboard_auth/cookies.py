...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``lemon_session_at``
(access token; Max-Age = token TTL), ``lemon_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``lemon_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``lemon_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``lemon_sso_attempt`` (auto-SSO loop guard, 60 s).
...
SESSION_AT_COOKIE = "lemon_session_at"
SESSION_RT_COOKIE = "lemon_session_rt"
SESSION_PROVIDER_COOKIE = "lemon_session_provider"
PKCE_COOKIE = "lemon_session_pkce"
SSO_ATTEMPT_COOKIE = "lemon_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    """``Path=/lemon`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    Nous Portal issues a 24h rotating refresh token (lemon #37247); a provider that omits it returns
