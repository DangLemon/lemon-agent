# lemon_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `LemonCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`lemon_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`LemonCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.lemon-ai/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`lemon_cli/curses_ui.py`; example
`lemon_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`lemon_cli/commands.py`)
...
## Config system (`lemon_cli/config.py`)
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`lemon tools/setup`, most subcommands, `lemon_cli/config.py`, merges `DEFAULT_CONFIG`); raw
...
## Skin engine (`lemon_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.lemon-ai/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`lemon update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `lemon update --plan`): read-only inventory — install kind, all
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `lemon-gateway*` unit /
  `ai.lemon.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.lemon-ai/logs/update_receipts/`
...
`_apply_profile_override()` in `lemon_cli/main.py` sets `LEMON_HOME` before any module import, so
every `get_lemon_home()` scopes to the active profile (rules in root). Profiles are independent
