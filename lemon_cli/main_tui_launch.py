...
Split out of ``lemon_cli/main.py``. Names that still live in main (``PROJECT_ROOT``, ...)
...
logger = logging.getLogger("lemon_cli.main")
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    from lemon_cli.main import _resolve_last_session
    target = (
    ...
    print(f"\nResume this session with:\n  lemon --tui --resume {target}")
    if title:
        print(f'  lemon --tui -c "{title}"')
    print(f"\nSession:        {target}")
    if title:
        print(f"Title:          {title}")
...
def _tui_selected_workspace_keys(tui_dir: Path, ws_root: Path) -> set:
    ...
    from lemon_cli.main import _is_termux_startup_environment
    try:
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @lemon-ai/ink is missing or node_modules is behind package-lock.json.
    ...
    if not (ws_root / "node_modules" / "@lemon" / "ink" / "package.json").is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_DIRS = ("src", "packages/lemon-ink/src")
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/lemon-ink/package.json",
    "packages/lemon-ink/index.js",
    "packages/lemon-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    ``LEMON_TUI_FORCE_BUILD=1`` forces a rebuild."""
    force = (os.environ.get("LEMON_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    the resolved node dir to PATH. ``LEMON_SKIP_NODE_BOOTSTRAP=1`` disables auto-install."""
    from lemon_cli.main import PROJECT_ROOT
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("LEMON_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "LEMON_HOME": lemon_home},
    ...
    extras += [Path(lemon_home) / "node" / "bin", Path.home() / ".local" / "bin"]
    for extra in extras:
        s = str(extra)
...
def _find_bundled_tui(lemon_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if lemon_cli_dir is None:
        lemon_cli_dir = Path(__file__).parent
    bundled = lemon_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    tracked files after ``lemon update``); True when the directory exists afterwards.

    On Windows an antivirus / NTFS filter driver can leave tracked ``ui-tui/`` files deleted in the working
    tree after ``lemon update`` (HEAD stays intact; the files just vanish — see issue #49145). Those files
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("LEMON_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Lemon AI checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `lemon update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Lemon AI checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `lemon --tui`\n"
        "If the checkout is still inconsistent, run `lemon update --force`.",
        file=sys.stderr)
    sys.exit(1)
...
def _tui_node_bin(bin: str) -> str:
    """Resolve ``node``/``npm`` for the TUI launch, or exit with a hint. ``LEMON_NODE`` wins for node;
    ``find_node_executable()`` sees the managed ``$LEMON_HOME/node`` tree a bare which() misses."""
    if bin == "node":
        env_node = os.environ.get("LEMON_NODE")
        if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
            return env_node
    from lemon_constants import find_node_executable
    path = find_node_executable(bin)
    if not path and bin == "node":
        with contextlib.suppress(Exception):
            from lemon_cli.dep_ensure import ensure_dependency
...
def _install_tui_dependencies(tui_dir: Path, *, termux_startup: bool) -> None:
    ...
    def _run_tui_install() -> subprocess.CompletedProcess:
        from lemon_constants import with_lemon_node_path
        ...
        return subprocess.run(
            ...
            env=_npm_lifecycle_env(with_lemon_node_path()))
    ...
    if result.returncode != 0:
        ...
        from lemon_cli.npm_engine import maybe_repair_npm_engine
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (LEMON_TUI_DIR prebuilt or esbuild)."""
    from lemon_cli.main import _is_termux_startup_environment
    _ensure_tui_node()

    # Footgun: --dev against a prebuilt bundle that has no source/node_modules.
    ext_dir = os.environ.get("LEMON_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with LEMON_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset LEMON_TUI_DIR (e.g. `unset LEMON_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr)
        sys.exit(1)
    ...
    # lemon_cli/tui_dist/entry.js but never ui-tui/ (git checkouts only).
    # 1. A prebuilt install (Docker image, Nix build, or prior `npm run build`) ships
    #   lemon_cli/tui_dist/entry.js but never ships ui-tui/ at all (that directory only exists in a git
    ...
    if tui_dev:
        # --dev runs src/entry.tsx directly, but @lemon-ai/ink resolves through
        # packages/lemon-ink/dist/entry-exports.js; a stale dist after a pull
        # leaves newer hooks/components missing at runtime. Prebuild it here.
        npm = _tui_node_bin("npm")
        _run_tui_npm_build(npm, tui_dir / "packages" / "lemon-ink", "TUI dev prebuild failed.")
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from lemon_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    from lemon_cli.main import PROJECT_ROOT
    src_root = str(env.get("LEMON_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["LEMON_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("LEMON_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["LEMON_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("LEMON_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["LEMON_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["LEMON_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    import tempfile
    # TUI child is a lemon process: propagate the profile-home contract via
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="lemon-tui-active-session-", suffix=".json")
    os.close(active_session_fd)
    env["LEMON_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")

    wt_info = None
    if worktree:
        wt_info = _setup_tui_worktree()
        env["LEMON_CWD"] = wt_info["path"]
    ...
    for key, value in (
        ("LEMON_MODEL", model), ("LEMON_INFERENCE_MODEL", model),
        ("LEMON_TUI_PROVIDER", provider), ("LEMON_INFERENCE_PROVIDER", provider),
        ("LEMON_TUI_TOOLSETS", ",".join(_normalize_tui_toolsets(toolsets))),
        ("LEMON_TUI_SKILLS", skills_value),
        ("LEMON_TUI_QUERY", query), ("LEMON_TUI_IMAGE", image),
        ("LEMON_TUI_CHECKPOINTS", "1" if checkpoints else None),
        ("LEMON_TUI_PASS_SESSION_ID", "1" if pass_session_id else None),
        ("LEMON_TUI_MAX_TURNS", str(max_turns) if max_turns is not None else None),
        ("LEMON_TUI_TOOL_PROGRESS", "verbose" if verbose else "off" if quiet else None),
        ("LEMON_ACCEPT_HOOKS", "1" if accept_hooks else None)):
        if value:
            env[key] = value
    ...
    # LEMON_TUI_RESUME is an internal hand-off to the Ink app. We start from a
    # full os.environ snapshot, so a stale exported value would make a plain
    # `lemon --tui` try to resume a non-existent session; only forward the id
    # argparse resolved for this invocation.
    env.pop("LEMON_TUI_RESUME", None)
    if resume_session_id:
        env["LEMON_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `lemon update`;
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``LEMON_KANBAN_BOARD`` so in-process tools and shelled-out
    ``lemon kanban`` calls agree even if a concurrent ``boards switch`` flips the file mid-turn.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls (``lemon kanban …``) resolve
    the board on different paths: the env-pin if set, otherwise the global ``<root>/kanban/current`` file. A
    concurrent ``lemon kanban boards switch`` from another session can flip the file mid-turn, so the same
    ...
    """
    if os.environ.get("LEMON_KANBAN_BOARD"):
        return
    with contextlib.suppress(Exception):
        from lemon_cli.kanban_db import get_current_board
        os.environ["LEMON_KANBAN_BOARD"] = get_current_board()


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.lemon-ai/skills/`` with the bundled library (idempotent, milliseconds when synced).
...
def _resolve_use_tui(args) -> bool:
    ...
    ``LEMON_TUI=1`` → TUI; ``display.interface`` config; default classic.

    The TTY gate is load-bearing: ambient preferences must never hijack a piped
    ``lemon chat -q`` (kanban workers, cron) — the Ink no-TTY bail-out exits 0 and
