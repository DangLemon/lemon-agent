...
Pure leaf: imports nothing from ``lemon_cli.auth`` so the per-provider modules
...
# httpx is imported lazily (~30ms) because lemon_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("lemon_cli.auth.httpx.Client")
...
DEFAULT_NOUS_CLIENT_ID = "lemon-cli"
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from lemon_cli import __version__ as _LEMON_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _LEMON_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"lemon-cli/{_LEMON_CLI_VERSION}"
