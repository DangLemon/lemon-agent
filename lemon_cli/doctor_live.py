"""``lemon doctor --live`` — opt-in bounded real-call tool-backend probes.

Opt-in only: these probes make real (cheap, metadata/read-only) network calls and may spend a
trivial amount of quota. They run ONLY when the user passes ``lemon doctor --live``.
"""
...
from lemon_cli.doctor import _section, check_info
from lemon_cli.doctor_report import check_fail, check_ok, check_warn
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from lemon_cli.doctor import LEMON_HOME, PROJECT_ROOT
        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (LEMON_HOME / "node" / "bin", LEMON_HOME / "node", LEMON_HOME / "node_modules" / ".bin"):
    ...
    # probes above. Mirror the rung lemon_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    from lemon_cli.mcp_config import _probe_single_server
