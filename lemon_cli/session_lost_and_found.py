...
# Lemon AI session ids are timestamps (20260812_135332_ab12cd): the strongest sentinel for schema-less rows.
...
# 3.50.7 and 3.44.6 — the same version gate lemon_state applies to the
...
# caught exactly this shell converting a live Lemon AI state.db into two
# generations. A salvage shell must therefore be version-gated, not just
# capability-gated, before it is pointed at (a copy of) a Lemon AI database.
#
# The predicate lives in lemon_cli.sqlite_runtime (stdlib-only, shared with
# the installer/update gates) so the embedded runtime and the salvage shell
# can never disagree about which versions are safe.
from lemon_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _wal_reset_vulnerable  # noqa: E502

_WAL_RESET_VULNERABLE_GUIDANCE = (
    "salvage against a Lemon AI database with the WAL-reset bug "
...
def _cli_supports_recover(binary: str) -> bool:
    ...
    scratch_dir = tempfile.mkdtemp(prefix="lemon-recover-probe-")
