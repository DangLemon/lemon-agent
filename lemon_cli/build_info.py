"""Baked-in build metadata for Lemon AI.

Source installs report their git revision live via ``git rev-parse`` (``lemon_cli/dump.py``,
``lemon_cli/banner.py``). Inside the published Docker image ``.dockerignore`` excludes ``.git``,
so those callsites fall back to the ``.lemon-ai_build_sha`` file written by the Dockerfile.
"""
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".lemon-ai_build_sha"
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    ``.lemon-ai_build_sha`` for Docker images, else unknown. Cached per process — code identity
...
def get_build_sha(short: int = 8) -> Optional[str]:
    """Baked-in build SHA (Dockerfile ``LEMON_GIT_SHA`` build-arg, full 40-char hash on one
