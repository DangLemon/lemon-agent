"""Bounded product contract for the first Lemon AI shared-metrics slice."""
...
SCHEMA_KEY = "lemon.metrics.schema_version"
SCHEMA_VERSION = "lemon.metrics.event.v2"
MODEL_CALL_SCOPE = "lemon.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "lemon.task_run"
TOOL_CALL_SCOPE = "lemon.tool_call"
CLIENT_ACTIVE_MARK = "lemon.client.active"
TOOL_APPROVAL_MARK = "lemon.tool_approval"
SKILL_LIFECYCLE_MARK = "lemon.skill.lifecycle"
SKILL_LOAD_MARK = "lemon.skill.load"
SUBSCRIBER_NAME = "lemon.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "lemon.client.active"
LEGACY_MODEL_CALL_METRIC = "lemon.model_call.count"
MODEL_ROUTE_METRIC = "lemon.model_route.count"
TASK_STARTED_METRIC = "lemon.task_run.started"
TASK_FINISHED_METRIC = "lemon.task_run.finished"
TOOL_CALL_METRIC = "lemon.tool_call.count"
TOOL_APPROVAL_METRIC = "lemon.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "lemon.skill.lifecycle.count"
SKILL_LOAD_METRIC = "lemon.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "lemon_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Lemon AI installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    lemon_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(lemon_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "lemon_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("lemon_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Lemon AI logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "lemon.call_role"
    )
    call_role = (metadata or {}).get("lemon.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from lemon_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Lemon AI terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "lemon-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Lemon AI registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Lemon AI tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Lemon AI."""
