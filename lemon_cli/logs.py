"""``lemon logs`` — view and filter Lemon AI log files.

``lemon logs [name] [-n N] [-f] [--level L] [--session S] [--component C] [--since 1h]``;
``lemon logs list`` shows the available files.
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_lemon_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Lemon AI runs — try 'lemon chat' first)")
        sys.exit(1)
    ...
    print(f"--- {display_lemon_home()}/logs/{filename}{filter_desc} ({mode}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_lemon_home()}/logs/")
        return

    print(f"Log files in {display_lemon_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'lemon chat' to generate logs)")
