...
def detect_service_manager() -> ServiceManagerKind:
    ...
    backend-agnostic code (profile hooks, the s6 dispatch in ``lemon gateway``).
    """
    # Deferred so importing this module (Protocol type, validate_profile_name) doesn't drag in
    # the whole gateway dependency graph.
    from lemon_cli.gateway import is_macos, is_windows, supports_systemd_services
    ...
    # made the s6 dispatch inert on Fly, so `lemon gateway start` spawned a foreground gateway
...
def _s6_running() -> bool:
    ...
    Must work for the unprivileged lemon user too: ``/proc/1/exe`` is unreadable for other UIDs
    ...
    resolved name is the literal ``"exe"`` and detection always fails. Since every Lemon AI runtime call
    inside the container drops to lemon via ``s6-setuidgid``, that silent failure made the entire
...
# ---------------------------------------------------------------------------
# Host backends: thin facades over ``lemon_cli.gateway`` (systemd/launchd) and
# ``lemon_cli.gateway_windows``. The protocol's ``name`` parameter is unused here — host backends
# operate on the currently active profile (``lemon -p <profile>``); the shape exists for s6 where
# each profile maps to a distinct service directory.
# ---------------------------------------------------------------------------


class _HostServiceManager:
    """``start``/``stop``/``restart`` resolve to ``<_fn_prefix><op>`` on ``lemon_cli.<_backend>``
    ...
    def _backend_module(self):
        import importlib
        import lemon_cli
        importlib.import_module(f"lemon_cli.{self._backend}")
        return getattr(lemon_cli, self._backend)
...
class SystemdServiceManager(_HostServiceManager):
    """Wraps the ``systemd_*`` functions in lemon_cli.gateway (host call sites still use those
...
class LaunchdServiceManager(_HostServiceManager):
    """Wraps the ``launchd_*`` functions in lemon_cli.gateway."""
...
class WindowsServiceManager(_HostServiceManager):
    """Wraps ``lemon_cli.gateway_windows`` (Scheduled Task / Startup-folder fallback).
...
# `lemon profile create` inside the container. Static services (main-lemon, dashboard) live in
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    the caller's LEMON_HOME blindly: derive the shared profile root and map the suffix to the root
    default profile or ``<root>/profiles/<profile>``.
    """
    profile = _profile_from_service(name)
    validate_profile_name(profile)
    lemon_home = Path(os.environ.get("LEMON_HOME", "/opt/data"))
    root = lemon_home.parent.parent if lemon_home.parent.name == "profiles" else lemon_home
...
# UID/GID of the in-image ``lemon`` user; hardcoded to match what ``stage2-hook.sh`` enforces
# (tests/docker/test_uid_remap.py). s6-supervise starts as root and drops via ``s6-setuidgid``.
_LEMON_UID = 10000
_LEMON_GID = 10000


def _chown_lemon(path: Path) -> None:
    try:
        os.chown(path, _LEMON_UID, _LEMON_GID)
    except PermissionError:
        # Already running as lemon → the dir is lemon-owned by default; swallowing keeps root
        # and unprivileged callers on one code path.
        pass


def _seed_supervise_skeleton(svc_dir: Path) -> None:
    """Pre-create lemon-owned ``supervise/`` and top-level ``event/`` inside a service directory.

    s6-supervise (root) creates ``event/``/``supervise/`` 0700 and the control FIFO 0600, so the
    lemon user gets EACCES on every ``s6-svc``/``s6-svstat``. s6 treats EEXIST as success and skips
...
class GatewayNotRegisteredError(S6Error):
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`lemon profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(profile: str, extra_env: dict[str, str]) -> str:
        ...
        Sources LEMON_HOME via with-contenv (run time, not baked in), resets ``HOME`` before the
        privilege drop so root's HOME does not leak, activates the venv, drops to lemon.
        ``profile == "default"`` emits NO ``-p`` flag: it is the sentinel for the root LEMON_HOME
        ...
        allocator: because each supervised profile gateway loads its own ``LEMON_HOME``, two profiles that
        ...
        deterministic SHA-256 allocator in ``lemon_cli.profiles._allocate_gateway_port``). PR #30136 review
        ...
        lines.append("export LEMON_S6_SUPERVISED_CHILD=1")
        # Generalized supervisor marker — same meaning for the profile-redirect guard in
        # lemon_cli.main._apply_profile_override; kept alongside the s6 one for back-compat.
        lines.append("export LEMON_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its LEMON_HOME. Without it
        # a gateway started OUTSIDE s6 (stray ``lemon gateway run``, an agent action, the Open
        ...
        if profile == "default":
            gateway_cmd = "lemon gateway run --replace"
        else:
            gateway_cmd = f"lemon -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID → s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid lemon {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            f': "${{LEMON_HOME:=/opt/data}}"\n'
            f'log_dir="$LEMON_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock AS LEMON when starting as root. Never
            # chown/unlink lemon-writable volume paths from this restartable root-context script:
            # an unprivileged user can race a pathname op through a symlink swap (CWE-59/CWE-367).
            # Parent logs/gateways is seeded lemon-owned at stage2 boot (test_log_dir_seed.py).
            # See #45258.
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid lemon mkdir -p "$log_dir"\n'
            f'  s6-setuidgid lemon rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid lemon s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        try:
            ...
            # Seed lemon-owned supervise/ BEFORE publishing so the lemon-user s6-svc/s6-svstat/
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        root-owned but the parent ``supervise/`` is lemon-owned (see ``_seed_supervise_skeleton``)
