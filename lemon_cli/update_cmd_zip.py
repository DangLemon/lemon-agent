"""ZIP-download fallback for ``lemon update`` (Windows with broken git): two-phase stage/commit swap, dirty-tree guard.

Split out of ``update_cmd.py``; every name is re-imported there so ``lemon_cli.update_cmd.<name>`` keeps
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = ".lemon-ai-update-staging", ".lemon-ai-update-old"
...
_STASH_HINT = "  Stash or commit your changes, then rerun `lemon update`."


def _zip_reinstall_url_for_configured_repository() -> str:
    from lemon_cli.update_cmd import _configured_update_repository, _is_default_update_repository
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    Thin alias over the two-phase helpers; retained for the ``lemon_cli.main`` re-export surface.
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    staging = f"{dst}.lemon-ai-update-staging"
    backup = f"{dst}.lemon-ai-update-old"
...
def _zip_overlay_block_reason(root: Path, *, ignore_staging_artifacts: bool = False) -> Optional[str]:
    ...
    is for the pre-swap re-check: phase 1 leaves our own ``*.lemon-ai-update-staging`` siblings that git
...
def _extracted_root(tmp_dir: str, branch: str) -> str:
    """GitHub ZIPs extract to ``lemon-agent-<branch>/``; fall back to the first non-``__MACOSX`` dir."""
    extracted = os.path.join(tmp_dir, f"lemon-agent-{branch}")
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str) -> None:
    ...
    need = sum(
        ...
        # root holds 20 first-party modules (run_agent.py, cli.py, lemon_constants.py, ...). Check up front
...
def _stage_entries(extracted: str, entries: list[str], project_root: str) -> list[tuple[str, str]]:
    ...
    try:
        for item in entries:
            ...
            # desktop app, win-unpacked/ Lemon AI.exe) exists only in the LIVE tree. Graft the live release
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    tmp_dir = tempfile.mkdtemp(prefix="lemon-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"lemon-agent-{branch}.zip")
        ...
        print(
            "  Re-run `lemon update` to retry; if the agent won't start, "
...
def _zip_source_archive_url(branch: str) -> str:
    from lemon_cli.update_cmd import _configured_update_repository
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `lemon update "
            f"--branch {branch}`, or update against main with `lemon update`."
        )
        _m().sys.exit(1)
    ...
    if not import_ok:
        ...
        print("  Re-run `lemon update` to complete it.")
        _m().sys.exit(1)
