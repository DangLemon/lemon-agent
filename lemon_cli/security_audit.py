"""On-demand supply-chain audit for Lemon AI installs.
...
def _discover_plugins(lemon_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.lemon-ai/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, lemon_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = lemon_home or Path(get_lemon_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `lemon security audit`."""
    home = Path(get_lemon_home())
    ...
    components = _discover_components(lemon_home=home, **skips)
    ...
    try:
        findings = run_audit(lemon_home=home, components=components)
