...
def active_session_limit_message(
    ...
    return (
        f"Lemon AI is at the active session limit ({active_count}/{max_sessions})."
...
def _registry_home(registry_home: str | Path | None = None) -> Path:
    return Path(registry_home) if registry_home is not None else Path(get_lemon_home())
...
class ActiveSessionLease:
    ...
    enabled: bool = True
    released: bool = False
    # Pinned at acquisition: a lease taken under the root LEMON_HOME must release
    ...
    with _FileLock(lock_path):
        ...
        if loaded is None:
            return None, ActiveSessionRefusal(
                "Lemon AI could not read the active-session registry at "
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile LEMON_HOME override.
    # See #85431.
