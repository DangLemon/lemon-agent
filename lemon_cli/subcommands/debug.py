...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Lemon AI. Use 'lemon debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    lemon debug share              Upload debug report (asks for confirmation)
    lemon debug share --yes        Skip confirmation (for scripts/CI)
    lemon debug share --lines 500  Include more log lines
    lemon debug share --expire 30  Keep paste for 30 days
    lemon debug share --local      Print report locally (no upload)
    lemon debug share --no-redact  Disable upload-time secret redaction
    lemon debug share --nous       Upload to Nous-internal storage (private)
    lemon debug delete <url>       Delete a previously uploaded paste
""")
    ...
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'lemon debug share'")
