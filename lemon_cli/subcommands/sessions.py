"""``lemon sessions`` subcommand parser."""
...
def build_sessions_parser(subparsers, *, cmd_sessions: Callable) -> None:
    ...
    sessions_export.add_argument("output", nargs="?",
        help="Output path. JSONL: file path (use - for stdout, required). "
            "md/qmd: output directory (default: <lemon home>/session-exports)")
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import", help="Import a Claude Code or Codex CLI session into Lemon AI",
        description="Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Lemon AI session store "
            "so it can be resumed with 'lemon --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in lemon_cli/sessions_cmd.py; the parser is threaded
