...
def build_claw_parser(subparsers, *, cmd_claw: Callable) -> None:
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Lemon AI")
    claw_subparsers = claw_parser.add_subparsers(dest="claw_action")

    # claw migrate
    claw_migrate = claw_subparsers.add_parser(
        "migrate", help="Migrate from OpenClaw to Lemon AI",
    ...
    claw_migrate.add_argument(
        ...
        help="Skip the pre-migration zip snapshot of ~/.lemon-ai/ (by default a "
        "single restore-point archive is written to ~/.lemon-ai/backups/ "
        "before apply; restorable with 'lemon import').")
    claw_migrate.add_argument(
