...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter Lemon AI log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    lemon logs                    Show last 50 lines of agent.log
    lemon logs -f                 Follow agent.log in real time
    lemon logs errors             Show last 50 lines of errors.log
    lemon logs gateway -n 100     Show last 100 lines of gateway.log
    lemon logs gui -f             Follow gui.log in real time
    lemon logs desktop -f         Follow desktop.log (Electron app boot/backend)
    lemon logs --level WARNING    Only show WARNING and above
    lemon logs --session abc123   Filter by session ID
    lemon logs --component tools  Only show tool-related lines
    lemon logs --since 1h         Lines from the last hour
    lemon logs --since 30m -f     Follow, starting from 30 min ago
    lemon logs list               List available log files with sizes
""")
