...
def _strip_lemon_path_lines(content: str) -> str:
    """Drop the ``# Lemon AI`` marker (+ its PATH line) and any lemon PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Lemon AI' in line or '# lemon-agent' in line:
            skip_next = True
            continue
        if skip_next and ('lemon' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'lemon' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Lemon AI PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_lemon_path_lines(content)
...
def remove_wrapper_script():
    """Remove the lemon wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a lemon_cli / lemon-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('lemon_cli' in content or 'lemon-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("lemon", "lemon-acp", "lemon-agent"))
...
def remove_node_symlinks(lemon_home: Path) -> list:
    ...
    node_dir = (lemon_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from lemon_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_systemd_gateway() -> bool:
    ...
    from lemon_cli.gateway import _systemctl_cmd, get_service_name, get_systemd_unit_path
...
def _remove_launchd_gateway() -> bool:
    ...
    from lemon_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from lemon_cli import gateway_windows as gw
...
# LEMON_HOME / LEMON_GIT_BASH_PATH / Lemon aliases (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\Lemon AI\git\{cmd,bin,usr\bin}, ...\lemon\node), PortableGit + Node copies
...
def _lemon_path_markers(lemon_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Lemon AI-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(lemon_home).rstrip("\\/")
    subs = ("lemon-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(lemon_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Lemon AI-owned entries from User-scope PATH in the registry (see ``_lemon_path_markers``)."""
    markers = tuple(m.lower() for m in _lemon_path_markers(lemon_home, include_managed_bin=include_managed_bin))
...
def _windows_uninstall_env_var_names(winreg, key, lemon_home: "Path | None" = None) -> tuple[str, ...]:
    names: list[str] = []
    registered_lemon_home = _query_user_env_value(winreg, key, "LEMON_HOME")
    active_home = str(lemon_home) if lemon_home is not None else registered_lemon_home
    registered_git_bash = _query_user_env_value(winreg, key, "LEMON_GIT_BASH_PATH")
    registered_hermes_home = _query_user_env_value(winreg, key, "HERMES_HOME")
    registered_hermes_git_bash = _query_user_env_value(winreg, key, "HERMES_GIT_BASH_PATH")
    owns_lemon_aliases = lemon_home is None or _same_windows_path(
        active_home, registered_lemon_home
    )

    if owns_lemon_aliases:
        names.extend(["LEMON_HOME", "LEMON_INSTALL_RUNTIME_DIR_NAME"])
    if owns_lemon_aliases or _windows_path_is_within(registered_git_bash, active_home):
        names.append("LEMON_GIT_BASH_PATH")
    if _same_windows_path(active_home, registered_hermes_home):
        names.append("HERMES_HOME")
    if _same_windows_path(active_home, registered_hermes_home) or _windows_path_is_within(
        registered_hermes_git_bash, active_home
    ):
        names.append("HERMES_GIT_BASH_PATH")
    return tuple(dict.fromkeys(names))


def remove_lemon_env_vars_windows(lemon_home: "Path | None" = None) -> list[str]:
    """Delete active-runtime User-scope env vars written by install.ps1."""
    def edit(winreg, key, removed):
        for name in _windows_uninstall_env_var_names(winreg, key, lemon_home):
            try:
...
def remove_portable_tooling_windows(lemon_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``lemon_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (lemon_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``lemon`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from lemon_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from lemon_constants import get_default_lemon_root
        bin_dir = get_default_lemon_root() / "bin"
    except Exception as e:
...
def _is_default_lemon_home(lemon_home: Path) -> bool:
    """Return True when ``lemon_home`` points at the default (non-profile) root."""
    try:
        from lemon_constants import get_default_lemon_root
        return lemon_home.resolve() == get_default_lemon_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``lemon -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current LEMON_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m lemon_cli.main` (the `lemon` wrapper may be half-gone).
    lemon_invocation = [sys.executable, "-m", "lemon_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                lemon_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's LEMON_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``lemon uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from lemon_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    lemon_home = get_lemon_home()
    summary = gui_install_summary(lemon_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ⚕ Lemon AI Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Lemon AI Chat GUI installation was found.")
        print(f"  Checked: {lemon_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Lemon AI agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(lemon_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Lemon AI agent at {lemon_home / 'lemon-agent'}")
        print(f"  • Your config, sessions, and secrets under {lemon_home}")
        print()
    ...
    print()
    uninstall_gui(lemon_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Lemon AI agent is still installed. Run 'lemon' to use the CLI,")
    print("or 'lemon uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.lemon-ai/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, lemon_home=lemon_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie LEMON_HOMEs and systemd units behind.
    is_default_profile = _is_default_lemon_home(lemon_home)
    ...
    print()
    _print_box("│            ⚕ Lemon AI Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {lemon_home / 'config.yaml'}")
    print(f"  Secrets: {lemon_home / '.env'}")
    print(f"  Data:    {lemon_home / 'cron/'}, {lemon_home / 'sessions/'}, {lemon_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, LEMON_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Lemon AI data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Lemon AI code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, lemon_home=lemon_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, lemon_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Lemon AI PATH entries from shell configs / Windows User PATH")
    print("  • Lemon AI wrapper scripts and Lemon AI-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Lemon AI config/data: {lemon_home}")
    else:
        print(f"  • Lemon AI config/data: {lemon_home}")
        profiles = _discover_named_profiles() if _is_default_lemon_home(lemon_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$LEMON_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: lemon_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; lemon\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_lemon_home(lemon_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(lemon_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Lemon AI-owned PATH entries in User environment"),
        (windows, "Removing Lemon AI runtime User env vars...",
         lambda: remove_lemon_env_vars_windows(Path(os.path.expandvars(str(lemon_home)))),
         "Removed User env var: {}", "No Lemon AI-set User env vars to remove"),
        (True, "Removing lemon command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows lemon launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows lemon launchers found"),
        (True, "Removing Lemon AI-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(lemon_home), "Removed {}", "No Lemon AI-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE LEMON_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from lemon_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(lemon_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(lemon_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.lemon-ai/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(lemon_home)
    else:
        log_info(f"Keeping configuration and data in {lemon_home}")
    ...
    print()
    print("Thank you for using Lemon AI! ⚕")
    print()
...
def _configured_update_repository_for_reinstall() -> str:
    explicit = (
        _safe_update_repository(os.environ.get("LEMON_UPDATE_REPOSITORY"))
        or _safe_update_repository(os.environ.get("LEMON_INSTALL_REPOSITORY"))
    )
    if explicit:
        return explicit
...
def main(argv=None) -> int:
    """``python -m lemon_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``lemon_constants`` + ``lemon_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m lemon_cli.uninstall")
    parser.add_argument(
