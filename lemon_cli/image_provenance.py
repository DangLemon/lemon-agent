"""Image-authored deployment provenance for immutable Lemon AI runtimes.

The image bakes ``/etc/lemon/image-provenance.json`` outside ``$LEMON_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/lemon/image-provenance.json")
