"""cua-driver installer, lock hygiene and pip-install helper for `lemon tools` /
...
# Quiet ``lemon update`` refreshes stay bounded even when upstream waits on Read-Host / a consent
...
_UPGRADE_CMD = "lemon computer-use install --upgrade"
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("LEMON_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Lemon AI telemetry policy applied; falls back to the current
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Lemon AI' runtime contract (30s cache keyed on the
...
def _pip_install(args: List[str], *, timeout: int = 300, capture_output: bool = True):
    ...
    # Managed uv first: $LEMON_HOME/bin is never on PATH, so a bare which() misses the uv Lemon AI
...
def _confirmed_update_check(driver_cmd: str, require_confirmed_update: bool) -> tuple:
    ...
    None: `lemon update` then keeps the installed version — an indeterminate check must never
...
def install_cua_driver(upgrade: bool = False, require_confirmed_update: bool = False,
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if not upgrade:  # silent under `lemon update`, which calls this for every user
            _print_warning(
    ...
    override = os.environ.get("LEMON_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        return _fail(f"    LEMON_CUA_DRIVER_CMD does not resolve to an executable: {override}",
    ...
    # A driver failing Lemon AI' runtime contract (version floor, missing manifest verbs) is repaired
    # regardless of mode. Lemon AI' minimum requirement IS the confirmation an upgrade is needed, so
    ...
    if repair_existing:
        _print_warning(f"    Found cua-driver {contract.get('version') or 'unknown version'}, but "
                       "Lemon AI cannot use its current runtime contract: "
                       f"{contract.get('reason') or 'required runtime features are missing'}.")
        if override:
            return _fail("    Update the binary selected by LEMON_CUA_DRIVER_CMD, or unset the "
...
def _print_cua_platform_notes(is_windows: bool, is_linux: bool, *, fresh_install: bool) -> None:
    ...
    if is_windows:
        ...
        if fresh_install:
            _print_info("    Both must allow the terminal / Lemon AI process.")
...
def _installer_popen_kwargs(is_windows: bool, verbose: bool, env: dict) -> dict:
    ...
    holding the install lock and wedge every later run. Non-verbose (`lemon update` refresh):
...
def _record_installer_output(out: str, returncode: int) -> None:
    ...
    During `lemon update`, sys.stdout is the mirroring _UpdateOutputStream whose `_log` handle is
    ~/.lemon-ai/logs/update.log — write straight to it so the full output is kept (success AND
    failure)."""
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True,
    ...
    # Unattended refreshes (installer_timeout set by `lemon update`) preflight and may skip.
    # Unattended refreshes (installer_timeout set by `lemon update`) fail FAST on the two conditions that
