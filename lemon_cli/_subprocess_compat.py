...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_LEMON_GATEWAY_BREAKAWAY"
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Lemon AI's plumbing calls. ``GIT_ASKPASS``/``SSH_ASKPASS`` are deliberately left alone: a
    *working* askpass helper or ssh-agent should still succeed non-interactively. Pair with
    ...
    Lemon AI shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_lemon(text: str) -> bool:
    r"""True when *text* names Lemon AI at a path-segment / token boundary.

    A bare ``"lemon" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\slemona\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("lemon", ".lemon-ai"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_lemon(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Lemon AI code."""
    try:
        import psutil
        ...
        return _text_names_lemon(f"{command} {executable}")
    except Exception:
        return False


def pid_is_lemon(pid: int, *, expected_start_time: int | None = None) -> bool:
    """Whether it is safe to use ``taskkill`` for *pid*.

    The PID must be valid, currently exist, and identify a Lemon AI process. When the caller captured
    ...
    if expected_start_time is not None and current_start_time != expected_start_time:
        return False
    try:
        return _process_command_is_lemon(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # be recycled while we hold it. The fail-closed ``pid_is_lemon`` guard is for BARE pids.
        try:
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``lemon update`` on slow-WMI
