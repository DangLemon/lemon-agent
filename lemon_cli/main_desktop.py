"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (lemon gui/desktop).
...
from lemon_cli.main_tui_launch import _npm_lifecycle_env
...
def _select_checkout_desktop_identity(desktop_dir: Path) -> None:
    ...
    Lemon AI identity.  An explicit new or legacy selector always wins, including
    ...
    from lemon_cli.desktop_identity import is_valid_internal_harness_path

    if is_valid_internal_harness_path(canonical):
        os.environ["LEMON_DESKTOP_HARNESS_CONFIG"] = str(canonical)
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $LEMON_HOME."""
    from lemon_constants import get_lemon_home

    return get_lemon_home() / "desktop-build-stamp.json"
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        ...
        name_groups = [("Lemon AI", "lemon-ai", "lemon"), ("Hermes", "hermes")]
        candidates = []
        for names in name_groups:
            candidates = [
                release_dir / d / n
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture Lemon AI.exe to the launcher. Prefer
        ...
        # wrong-architecture Lemon AI.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# download, missing dep, disk full — leaves the user with NO app, and ``lemon update`` used to report
...
# (Desktop → lemon-setup --update → `lemon update` → `lemon desktop --build-only` → relaunch) rebuilds
# Lemon AI.exe on the end user's machine and used to verify only that the file EXISTS before declaring
...
# Lemon AI.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    emulated process arch) and rejected a correctly-built ARM64 ``Lemon AI.exe``. Binding
...
def _windows_native_machine() -> str:
    ...
    chain runs an x64 lemon-setup.exe (and thus x64 Python) on Windows-on-ARM devices, where
...
def _ensure_desktop_exe_launchable(
    ...
    if restored is not None:
        ...
        print(
            "    Your existing version was kept and still works. Run `lemon desktop`"
        )
        print(
    ...
    print("    Run `lemon desktop --force-build` to rebuild, or re-run the Lemon AI")
...
def _redownload_electron_dist(
    ...
    if not installer.is_file():
        return False
    from lemon_constants import find_node_executable, with_lemon_node_path
    ...
    dl_env = with_lemon_node_path(env)
    if mirror:
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Lemon AI is damaged" and loses every grant. Clear quarantine xattrs, then sign
    ...
    if exe is None:
        return True
    # exe = .../Lemon AI.app/Contents/MacOS/Lemon AI  ->  app bundle = .../Lemon AI.app
    app = exe.parents[2]
...
def _macos_create_signing_identity(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="lemon-tcc-"))
    try:
...
def _desktop_macos_setup_tcc_identity(identity: str = "Lemon AI Local Signing") -> bool:
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Lemon AI (Linux only, best-effort).

    ``Exec`` and ``Icon`` are absolute so the entry works outside a login shell.
    ``lemon uninstall --gui`` removes it.
    """
    from lemon_cli.main import PROJECT_ROOT

    try:
        from lemon_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from lemon_cli.main import PROJECT_ROOT
    from lemon_cli.main_web_build import _run_npm_install_deterministic
    ...
    nixos_env = with_lemon_node_path(_nixos_build_env())
...
def _run_desktop_pack_with_recovery(
    ...
    if (
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose Lemon AI.exe is
...
def _build_desktop_app(
    ...
    if build_result.returncode != 0:
        ...
        print("    ELECTRON_MIRROR=<mirror-base-url> lemon desktop --force-build")
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    # with_lemon_node_path() copies os.environ when called with no arg.
    env = with_lemon_node_path()
    for attr, key in (
        ("fake_boot", "LEMON_DESKTOP_BOOT_FAKE"),
        ("ignore_existing", "LEMON_DESKTOP_IGNORE_EXISTING"),
    ):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "lemon_root", None):
        env["LEMON_DESKTOP_LEMON_ROOT"] = str(
            Path(args.lemon_root).expanduser().resolve()
        )
    cwd = getattr(args, "cwd", None)
    env["LEMON_DESKTOP_CWD"] = (
    ...
    if config_disable_gpu != "auto" and "LEMON_DESKTOP_DISABLE_GPU" not in os.environ:
        env["LEMON_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if sys.platform == "linux" and "LEMON_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
            config_password_store
        ...
        if password_store:
            env["LEMON_DESKTOP_PASSWORD_STORE"] = password_store
...
def cmd_gui(args: argparse.Namespace):
    ...
    from lemon_cli.main import PROJECT_ROOT
    from lemon_cli.main_install_repair import _resolve_node_runtime_npm
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Lemon AI Local Signing"
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  lemon gui")
            sys.exit(1)
