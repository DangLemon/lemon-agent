"""Lemon AI-managed uv and Python runtime repair.

The Python backing the install is shared by every Lemon AI profile because the checkout's ``venv``
is shared. Runtime repair therefore uses an install-scoped store under
``<checkout>/.lemon-ai-runtime/python``. A vulnerable interpreter is never reinstalled in place.
"""
...
def managed_uv_path() -> Path:
    """Path of Lemon AI' own uv binary (``$LEMON_HOME/bin/uv[.exe]``); may not exist yet."""
    return get_lemon_home() / "bin" / ("uv.exe" if platform.system() == "Windows" else "uv")
...
def managed_python_env(
    ...
    """Return a sanitized environment for Lemon AI-private uv Python commands."""
    target = (
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print("    Sessions stay protected meanwhile: Lemon AI keeps databases "
              "out of WAL mode on this SQLite build. The next `lemon update` "
              "will retry.")
        return
...
def _uv_self_update_stamp() -> Path:
    from lemon_constants import get_lemon_home
    return get_lemon_home() / "cache" / ".uv_self_update_stamp"


def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    """True when ``uv self update`` ran recently enough to skip.

    uv releases roughly weekly while many users run ``lemon update`` daily; a blocking network
...
def _reload_lemon_constants():
    """Re-execute ``lemon_constants`` from disk (the imported one may predate venv_python_path)."""
    import lemon_constants
    return importlib.reload(lemon_constants)
...
def _attempt_install_generation(
    ...
    try:
        ...
        return reject("uv resolved Python outside the Lemon AI generation: %s", python)
...
def _install_safe_python_generation(
    ...
    # supported minor (e.g. 3.11 → 3.12) so the user isn't stuck on every `lemon update`. The
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("lemon_cli.main")
    ...
    if holders:
        ...
        return True, f"other Lemon AI processes still hold the venv (PID {pids})"
    return False, ""
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    (``lemon update`` itself runs from the venv python), which is correct for the dependency-sync
    ...
    updater's own ``python.exe`` (or a waiting ``lemon.exe`` launcher ancestor) can never be renamed from
    ...
    # Belt-and-braces: the venv\Scripts\lemon.exe launcher stays mapped while it waits for this
...
def _repair_windows_preflight(
    ...
    if self_locked:
        ...
        for line in (
            ...
            # See #93032.
            "    Retrying `lemon update` from inside this venv cannot help: "
            ...
            f"      cd {root}",
            "      <system Python> -m lemon_cli.main update",
            "    Sessions stay protected meanwhile: Lemon AI keeps databases "
...
def _install_uv(target: Path) -> None:
    ...
    into ``$LEMON_HOME/bin/`` instead of ``~/.local/bin/``.
    """
