...
from lemon_cli.providers import (
...
# Re-exported: callers/tests patch lemon_cli.model_switch.<name>.
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, str):
        ...
        # Pre-fix Lemon AI wrote sentinel keys inside the user-facing ``models`` mapping.
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Lemon AI.
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    ``discover_models: false``. A catalog Lemon AI itself persisted (``discovered``) is never a pin."""
    if discovered:
        return False
    if isinstance(value, str):
...
_LEMON_MODEL_WARNING = (
    ...
    "for use with Lemon AI. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.).")

# Match only the real Nous Research Hermes 3 / 4 chat families. Product branding must not
# rewrite provider-owned model IDs such as ``NousResearch/Hermes-3-Llama-3.1-70B``.
_NOUS_LEMON_NON_AGENTIC_RE = re.compile(r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)", re.IGNORECASE)
...
def is_nous_lemon_non_agentic(model_name: str) -> bool:
    """True if *model_name* is a real Nous Hermes 3/4 chat model."""
    return bool(model_name and _NOUS_LEMON_NON_AGENTIC_RE.search(model_name))


def _check_lemon_model_warning(model_name: str) -> str:
    """Warning string if *model_name* is a Nous Hermes 3/4 chat model, else ``""``."""
    return _LEMON_MODEL_WARNING if is_nous_lemon_non_agentic(model_name) else ""
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    try:
        from lemon_constants import get_config_path
...
def resolve_startup_model_route(
    ...
    ``LemonCLI`` is constructed before the interactive ``/model`` pipeline runs; resolving here
    ...
    if current_provider:
        try:
            from lemon_cli.providers import is_routing_aggregator, normalize_provider as _norm_prov
    ...
    try:
        from lemon_cli.models import normalize_provider
...
def _ollama_configured_base() -> tuple[dict, str]:
    from lemon_cli.models import _get_provider_config_dict
...
def _unknown_provider_message(explicit_provider: str) -> str:
    msg = (
        f"Unknown provider '{explicit_provider}'. Check 'lemon model' for available "
    ...
    try:  # Surface common config issues that cause provider resolution failures
        ...
        if issues:
            msg += "\n\nRun 'lemon doctor' — config issues detected:" + "".join(f"\n  • {ci.message}" for ci in issues[:3])
    except Exception:
        pass
    return msg
...
def _route_from_model_input(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from lemon_cli.models import detect_provider_for_model
    raw_input, current_provider = st.raw_input, st.current_provider
    try:
        from lemon_cli.config import load_config
        from lemon_cli.moa_config import exact_moa_preset_name, normalize_moa_config
...
def _creds_for_switched_provider(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if st.explicit_provider and st.user_providers:
        from lemon_cli.providers import resolve_user_provider
...
def _creds_for_current_provider(st: _Switch) -> None:
    ...
    from lemon_cli.models_local import _get_ollama_request_headers, _same_ollama_native_root
...
def _copilot_api_mode(provider: str, model: str, api_key: str) -> str:
    from lemon_cli.models import copilot_model_api_mode
...
def _opencode_api_mode(provider: str, model: str, api_key: str) -> str:
    ...
    # #16878.
    from lemon_cli.models import opencode_model_api_mode
...
def _build_switch_result(st: _Switch) -> ModelSwitchResult:
    ...
    from lemon_cli.models import normalize_opencode_base_url, opencode_provider_family
    ...
    warnings = [w for w in (st.validation.get("message"), _check_lemon_model_warning(st.new_model)) if w]
    ...
    try:
        from lemon_cli.runtime_provider import _get_named_custom_provider, _custom_provider_request_overrides
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'list_picker_providers': ('lemon_cli.model_switch_providers', 'list_picker_providers'),
    'prewarm_picker_cache_async': ('lemon_cli.model_switch_providers', 'prewarm_picker_cache_async'),
}
