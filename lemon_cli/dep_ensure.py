...
from lemon_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import lemon_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $LEMON_HOME/node is not on PATH, so
    ...
    "browser": lambda: (agent_browser_runnable(shutil.which("agent-browser")) or _has_system_browser()
                        or _has_lemon_agent_browser() or _has_npx_agent_browser()),
...
def _has_lemon_agent_browser() -> bool:
    from lemon_constants import get_lemon_home  # late import: tests patch lemon_constants.get_lemon_home
    home = get_lemon_home()
    if _IS_WINDOWS:  # npm -g --prefix puts .cmd shims directly in the prefix dir
        return (home / "node" / "agent-browser.cmd").is_file()
    # install.sh installs into $LEMON_HOME/node/bin/ via npm -g --prefix; legacy git clones used node_modules/.bin/.
...
def ensure_dependency(dep: str, interactive: bool = True) -> bool:
    ...
    if shell == "powershell":
        ...
        if not ps_bin:
            ...
            return False
        cmd = [ps_bin, "-ExecutionPolicy", "Bypass", "-File", str(script), "-Ensure", dep, "-LemonHome", str(get_lemon_home())]
    else:
        cmd = ["bash", str(script), "--ensure", dep]
    run_env = {**lemon_subprocess_env(inherit_credentials=False), "IS_INTERACTIVE": "false"}
