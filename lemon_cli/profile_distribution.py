"""Profile distributions — shareable, packaged Lemon AI profiles via git.
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "lemon_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".lemon-ai_history",
    # User data
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            lemon_requires=_str(data, "lemon_requires"), author=_str(data, "author"),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        optional = (
            ("description", self.description), ("lemon_requires", self.lemon_requires),
...
def check_lemon_requires(spec: str, current_version: str) -> None:
    ...
    if not _VERSION_OPS[op](_parse_semver(current_version), _parse_semver(target)):
        raise DistributionError(f"This distribution requires Lemon AI {op}{target}, but you have {current_version}.")
    ...
    *workdir* (``.git`` removed); a local directory is used in place."""
    ...
    if _looks_like_git_url(src_str):
        ...
        missing = (
            ...
            "This repository is not a Lemon AI profile distribution."
        )
...
def plan_install(source: str, workdir: Path, override_name: Optional[str] = None) -> InstallPlan:
    ...
    from lemon_cli.profiles import _canon_valid, get_profile_dir
    ...
    if manifest is None:
        raise DistributionError(
            f"No {MANIFEST_FILENAME} found at the distribution root — this source is not a Lemon AI distribution."
        )
    check_lemon_requires(manifest.lemon_requires, lemon_version)  # fail fast
    canon = _canon_valid(override_name or manifest.name)
    if canon == "default":
        raise DistributionError(
            "Cannot install a distribution as 'default' — that is the built-in "
            "root profile (~/.lemon-ai).  Pass --name <name> to install under a new profile."
        )
    manifest.name = canon
...
def _bootstrap_user_dirs(target: Path) -> None:
    ...
    from lemon_cli.profiles import _PROFILE_DIRS
...
def install_distribution(
    ...
    from lemon_cli.profiles import check_alias_collision, create_wrapper_script
    with tempfile.TemporaryDirectory(prefix="lemon_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `lemon profile update` to upgrade in place, or pass --force to overwrite."
            )
...
def _existing_profile(profile_name: str) -> Tuple[str, Path]:
    ...
    from lemon_cli.profiles import _existing_profile_dir
...
def update_distribution(profile_name: str, force_config: bool = False) -> InstallPlan:
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`lemon profile install <source> --name {canon} --force`."
        )
    with tempfile.TemporaryDirectory(prefix="lemon_dist_update_") as tmp:
