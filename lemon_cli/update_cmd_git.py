"""Git plumbing for ``lemon update``: fork/upstream sync, trampoline-git detection, lockfile/EOL churn cleanup, orphan rescue refs, parked-branch assessment, fetch-failure classification.
...
_BAR = "=" * 68
UPDATE_REPOSITORY_ENV = "LEMON_UPDATE_REPOSITORY"
INSTALL_REPOSITORY_ENV = "LEMON_INSTALL_REPOSITORY"
DEFAULT_UPDATE_REPOSITORY = "DangLemon/lemon-agent"
INTERNAL_UPDATE_REPOSITORY = "DangLemon/lemon-agent"
INTERNAL_UPDATE_ENV_VARS = ("LEMON_DESKTOP_INTERNAL", "HERMES_DESKTOP_INTERNAL", "LEMON_DESKTOP_INTERNAL_PACKAGE")
...
def _git_stdout(git_cmd, args, cwd, **kw) -> Optional[str]:
    ...
    from lemon_cli.update_cmd import _git_run
...
def _prune_orphan_rescue_refs(
    ...
    """Expire old orphan rescue refs (``refs/lemon-update-backups/orphan-<branch>-<ts>-<sha>``).
    ...
    with suppress(OSError):
        prefix = f"refs/lemon-update-backups/orphan-{branch}-"
...
def _print_parked_branch_skip_warning(git_cmd: list[str], cwd: Path, current_branch: str, target_branch: str, reason: str) -> None:
    ...
    print(
        ...
        f"    git -C {cwd} status\n"
        f"    git -C {cwd} checkout {target_branch} && lemon update\n"
...
def _configured_update_repository() -> str:
    """Configured update source repository, defaulting to public Lemon AI.

    Desktop passes ``LEMON_UPDATE_REPOSITORY`` at runtime. The installers also
    expose ``LEMON_INSTALL_REPOSITORY`` and Lemon internal build flags, so direct
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_lemon_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    with suppress(Exception):
        from lemon_constants import get_lemon_home
        (get_lemon_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
...
def _offer_upstream_remote(git_cmd: list[str], cwd: Path, *, assume_yes: bool, input_fn) -> bool:
    ...
    from lemon_cli.update_cmd import _add_upstream_remote, _mark_skip_upstream_prompt
    print(
        "\nℹ Your fork is not tracking the official Lemon AI repository.\n"
        "  This means you may miss updates from DangLemon/lemon-agent.\n"
    )
    ...
    if not _add_upstream_remote(git_cmd, cwd):
        ...
        return False
    print("  ✓ Added upstream: https://github.com/DangLemon/lemon-agent.git")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    See #97052.
    """
    from lemon_cli.update_cmd import _count_commits_between, _has_upstream_remote, _no_prompt_git_kwargs, _should_skip_upstream_prompt
...
def _portable_git_candidates() -> list:
    ...
    profile-scoped LEMON_HOME), then profile home as a fallback for custom layouts.

    The Lemon AI-managed PortableGit tree lives under the SHARED root (``<root>/git/...``), not the
    profile-scoped LEMON_HOME (``<root>/profiles/<name>``), so a profile-scoped ``lemon update`` must look
    there (monerostar review, #87876).
    """
    from lemon_cli.update_cmd import get_default_lemon_root, get_lemon_home
    candidates = []
    with suppress(Exception):
        candidates += [root / "git" / "mingw64" / "libexec" / "git-core" / "git.exe" for root in (get_default_lemon_root(), Path(get_lemon_home()))]
    return candidates
...
def _ensure_non_trampoline_git(git_cmd: list) -> list:
    ...
    from lemon_cli.update_cmd import _locate_real_git
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    on update; install.ps1 pins ``false`` but older checkouts never got it and only ``lemon update`` can
    ...
    reuses its build-pinned ``install.ps1`` forever — so ``lemon update``, which ships with the checkout
