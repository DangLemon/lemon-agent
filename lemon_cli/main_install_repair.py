...
# Install-scoped breadcrumbs live next to the venv (not under $LEMON_HOME)
...
def _stdout_to_stderr():
    ...
    a protocol stream (``lemon acp`` speaks JSON-RPC on stdout) must never get install noise."""
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``lemon update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    from lemon_cli.main import PROJECT_ROOT
    print(
        "⚠ A previous `lemon update` was interrupted mid-install — "
        "finishing dependency installation now...")

    # Windows: a ``lemon.exe`` launch has the launcher as an ancestor; the quarantined full
    ...
    self_locked = _windows_running_lemon_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from lemon.exe; applying package-only first aid, "
    ...
    try:
        from lemon_cli import _install_repair as _ir
        ...
        manual = (
            "  Lemon AI is still running from the launcher that needs "
            "replacing. Close other Lemon AI windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\lemon.exe`` holds itself open (no ``FILE_SHARE_DELETE``) for the whole
    ...
    with the project venv's own shims so a foreign ``lemon.exe`` never matches.
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _lemon_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_running_lemon_launcher_locked() -> bool:
    """True when a venv ``lemon*.exe`` shim is this process or an ancestor (best-effort)."""
    return _windows_shim_in_process_chain() is not None


# Set on the re-exec'd child so it can never spawn another one.
_UPDATE_REEXEC_ENV = "LEMON_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    ``.update_exit_code``. The child re-runs ``lemon update`` so the sync and its tail happen
    ...
    user's own console.
    ``venv\\Scripts\\lemon.exe`` is a launcher that runs the interpreter with the shim as its script and
    ...
    cmd = [str(python_exe), "-m", "lemon_cli.main", *sys.argv[1:]]
...
def _lemon_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    names = set(_load_console_script_names()) or {"lemon", "lemon-agent", "lemon-acp"}
...
def _quarantine_running_lemon_exe(
    ...
    """Pre-empt the Windows file lock on the running ``lemon.exe``.
    ...
    transient; Lemon AI backend child: until closed) — retry with backoff, then warn
    ...
    for shim in _lemon_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Lemon AI, exit other `lemon` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `lemon update`.")
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Lemon AI versions queued for our shims: ``MOVEFILE_DELAY_UNTIL_REBOOT``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            if value_type != winreg.REG_MULTI_SZ or not isinstance(entries, list):
                return 0
            kept, removed = _filter_pending_shim_renames(entries, _lemon_exe_shims(scripts_dir))
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_lemon_exe`` if uv didn't write replacements. Safety-
    critical: a failed quarantine only aborts an update; a failed restore leaves no ``lemon``
...
class ShimQuarantineError(RuntimeError):
    """A live ``lemon*.exe`` shim could not be renamed aside. Raised by
...
def _run_quarantined_install(
    ...
    """Run an editable install, quarantining the running ``lemon.exe`` first.

    Every editable install rewrites the entry-point shims; on Windows the live ``lemon.exe``
    can be neither deleted nor overwritten, so without quarantine ``lemon`` drops off PATH.
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_lemon_exe(scripts_dir, failed_out=failed)
...
# rewrites no entry points, which would otherwise leave the shims quarantined aside and `lemon` missing
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``lemon.exe.old.*`` from updates.

    Called early on every invocation. Two cases an unconditional ``unlink()`` gets wrong:
    (1) orphan rescue — ``lemon.exe`` missing while ``lemon.exe.old.*`` exists means the .old
...
def _run_package_only_install(cmd: list[str], *, env: dict[str, str] | None = None) -> None:
    ...
    ``lemon.exe``, and the quarantine path would rename shims uv then never recreates.
...
def _repair_venv_via_import_probes(
    ...
    ``lemon.exe``. Never raises. Returns ``"healthy"``, ``"repaired"``, ``"failed"``
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `lemon update`:")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    attempt quarantines the live ``lemon*.exe`` shims first. When ``env`` carries a
    ...
    if pin_python:
        ...
        # dir, not PROJECT_ROOT/venv; quarantining the wrong dir leaves lemon.exe locked.
...
def _verify_console_scripts_installed(
    ...
    On Windows ``uv pip install -e .`` can register ``lemon.exe`` in the wheel RECORD while the
    file never lands (live shim locked, launcher write skipped), so ``lemon`` drops off PATH
    after a "successful" install. Missing shims get ``--reinstall -e .`` under quarantine.

    The symptom is ``lemon-agent.exe`` and ``lemon-acp.exe`` present but ``lemon.exe`` missing, so
    ``lemon`` drops off PATH even though the install reported success (issue #52931).
    """
    if not _is_windows():
        return
    scripts_dir = _venv_scripts_dir()
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            "  ⚠ Entry point repair failed; try `lemon update --force` after "
            "closing other lemon processes.")):
        return
    _report_still_missing(
        _missing(), "Workaround: python -m lemon_cli.main <command>",
...
def _verify_core_dependencies_installed(
    ...
    # ``lemon update``; the install prefix/env encode which environment we targeted.
    venv_python = _resolve_install_target_python(install_cmd_prefix, env)
    if venv_python is None:
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg="  ⚠ Repair install failed; check `lemon update` output above."):
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            ...
            "Run `lemon update --force` after closing other lemon processes.")):
        return
    _report_still_missing(
        _missing_deps(), "Run `lemon update --force` after closing other lemon processes.",
