"""Pydantic request/response models for the Lemon AI dashboard web server."""
...
class ImportRequest(BaseModel):
    archive: str
    # --force: the spawned `lemon import` has stdin=DEVNULL, so its "Continue? [y/N]" prompt would
...
class ProfileCreate(BaseModel):
    ...
    # Installed async via `lemon -p <name> skills install` (skills_hub.SKILLS_DIR is import-time-bound,
    # so LEMON_HOME can't redirect it); PIDs go back for the UI to poll.
    hub_skills: List[str] = []
...
class ProfileExport(BaseModel):
    ...
    output: str = ""  # archive path; empty → a staging path under LEMON_HOME
