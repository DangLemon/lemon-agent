"""Lemon AI Plugin System — discovers, loads, and manages plugins.

Sources, later overriding earlier on key collision: bundled ``<repo>/plugins/<name>/`` (``memory/``
and ``context_engine/`` have their own discovery), user ``~/.lemon-ai/plugins/<name>/``, project
``./.lemon-ai/plugins/<name>/`` (opt-in via ``LEMON_ENABLE_PROJECT_PLUGINS``), and pip packages in
the ``lemon_agent.plugins`` entry-point group. A directory plugin needs a ``plugin.yaml`` manifest
...
from lemon_constants import get_lemon_home, lemon_home_key
from registration_lifecycle import replacement_coordinator
from utils import env_var_enabled
from lemon_cli.config import load_config_readonly
from lemon_cli.middleware import VALID_MIDDLEWARE
from lemon_cli.plugin_capabilities import plugin_capability_granted
from lemon_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV, legacy_relay_plugin_keys
# Sibling modules' names are re-exported here (origin) so plugins and tests keep one import path.
from lemon_cli.plugins_manifest import (  # noqa: F401 — re-exported
...
from lemon_cli.plugins_discovery import (  # noqa: F401 — re-exported
...
from lemon_cli.plugins_loader import (
...
from lemon_cli.plugins_dispatch import (  # noqa: F401 — re-exported
    DEFAULT_SYSTEM_PROMPT_SECTION_MAX_CHARS, LEMON_EVENT_NAMESPACE, MAX_SYSTEM_PROMPT_SECTION_CHARS,
...
from lemon_cli.plugins_ledger import PluginLedgerMixin, PluginRegistration
from lemon_cli.plugins_state import (
...
def get_bundled_plugins_dir() -> Path:
    """Bundled ``plugins/`` dir: ``LEMON_BUNDLED_PLUGINS`` (Nix wrapper / packaged installs, read-only
    store paths) first, else the in-repo path."""
    env_override = os.getenv("LEMON_BUNDLED_PLUGINS")
    if env_override:
...
# ``LEMON_PLUGINS_DEBUG=1`` tees verbose discovery logs to stderr in addition to agent.log. Read
# once at import; tests flip it mid-process via ``_install_plugin_debug_handler(force=True)``.
_PLUGINS_DEBUG = env_var_enabled("LEMON_PLUGINS_DEBUG")
_DEBUG_HANDLER_INSTALLED = False


def _install_plugin_debug_handler(force: bool = False) -> None:
    """When LEMON_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG (once per process)."""
    ...
    logger.debug("LEMON_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled")
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task observers (lemon_cli.kanban_db), fired AFTER the DB commit so a slow plugin never
    ...
    # | "idle", result: lemon_cli.kanban_db.DispatchResult (spawned, reclaimed, promoted,
...
class PluginContext:
    ...
    def platform_actions(self):
        ...
        from lemon_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Active profile name (``"default"``, the ``~/.lemon-ai/profiles/<name>`` id, or ``"custom"``),
        derived from ``LEMON_HOME`` — not ``_cli_ref``, which is None outside the interactive CLI —
    ...
    def register_approval_transport(self, name: str, present_fn: Callable) -> None:
        ...
        from lemon_cli.approval_transport import RegisteredApprovalTransport
        ...
        entry = RegisteredApprovalTransport(
            ...
            profile_home=str(get_lemon_home().resolve()),
        )
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Lemon AI core) are trusted by default — an override there is a
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (``lemon <name> ...``). *setup_fn* receives the argparse
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        """Register a :class:`lemon_cli.dashboard_auth.DashboardAuthProvider` for the dashboard
        auth gate (non-loopback bind without ``--insecure``). Wrong type / duplicate name warn and
        are ignored, never raised."""
        from lemon_cli.dashboard_auth import DashboardAuthProvider
        from lemon_cli.dashboard_auth.registry import register_global_provider, unregister_global_provider
    ...
    def register_auxiliary_task(
        ...
        from lemon_cli.main_provider_setup import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        the subscriber count scheduled. Any ``':'`` in the name (``lemon:x`` is reserved for core,
        ...
        if ":" in event:
            logger.warning("Plugin '%s' tried to emit namespaced/reserved event '%s' — a plugin may only emit "
                           ...
                           plugin_key, event, plugin_key, LEMON_EVENT_NAMESPACE)
            raise ValueError(f"Plugin '{plugin_key}' may not emit '{event}': emit only the bare event name; "
                             f"the namespace is forced to '{plugin_key}:' and the '{LEMON_EVENT_NAMESPACE}:' "
    ...
    def register_skill(
        ...
        Not in ``~/.lemon-ai/skills/`` nor ``<available_skills>`` — explicit loads only. Raises
...
_SCOPED_PROVIDER_REGISTRARS: Tuple[Tuple[str, str, str, str, str, str, Dict[str, Any]], ...] = (
    ...
    ("register_secret_source", "secret_source", "agent.secret_sources.registry",
     ...
     "Register a :class:`agent.secret_sources.base.SecretSource`, run by ``load_lemon_dotenv()`` "
     "(after ``~/.lemon-ai/.env``, before credentials are read) when ``secrets.<name>`` is enabled. The "
...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        self.scope_key = scope_key or lemon_home_key()
        ...
        # is keyed per (lemon_home, plugin_id) and every inverse is identity-conditional — one
        ...
        # and contributed tool names (so `lemon plugins list` still attributes them).
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if env_var_enabled("LEMON_SAFE_MODE"):
                logger.info("LEMON_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_lemon_dotenv() ran at import, before plugin secret sources existed: re-pull.
                # Plugin secret sources register during discover; the initial load_lemon_dotenv() already
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning("Removed Lemon AI plugin %s is still listed in plugins.enabled; "
    ...
    def _refresh_plugin_compat_report(self, manifests: List[PluginManifest]) -> None:
        """Refresh LEMON_HOME/.plugin-compat-report.json from this discovery pass (lemon_cli.plugin_compat).
        ...
        """
        try:
            from lemon_cli.plugin_compat import compat_report
    ...
    def register_approval_transport(self, name: str, present_fn: Callable, *, plugin_id: str) -> None:
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            name=clean, present=present_fn, plugin_id=plugin_id, profile_home=str(get_lemon_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None or registered.profile_home != str(get_lemon_home().resolve()):
            return None
        return registered
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        if _env_enabled("LEMON_SAFE_MODE"):
            return False
        ...
        for lookup_key, manifest in {manifest_key(m): m for m in self._collect_directory_manifests()}.items():
            ...
            try:  # lazy: this is a startup probe, keep agent_plugins unimported unless needed
                from lemon_cli.agent_plugins import _discover_mcp
                if _discover_mcp(Path(manifest.path), get_lemon_home() / "plugin-data"
...
# Resolved Lemon AI home -> PluginManager. A process can switch profiles via
# ``set_lemon_home_override()``; a single slot would leak one profile's plugin/context-engine state
...
def _plugin_home_key() -> Path:
    """Resolved active Lemon AI home — the key for per-profile plugin managers (plugins capture the
    home at registration, so a process serving several profiles cannot share one manager)."""
    try:
        return get_lemon_home().expanduser().resolve()
    except Exception:
        return get_lemon_home().expanduser()
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Lemon AI profile/home (cached per resolved home; a
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=lemon_home_key(current_home))
...
def _reset_plugin_managers_for_tests() -> None:
    ...
    try:
        from lemon_cli.dashboard_auth.registry import clear_providers
        clear_providers()
    except Exception:
...
def _plugin_toolset_keys_cache_path() -> Path:
    return get_lemon_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from lemon_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    threading.Thread(target=_runner, name="lemon-plugin-command-await", daemon=True).start()
...
def get_plugin_toolsets() -> List[tuple]:
    """Plugin toolsets as ``(key, label, description)`` tuples for the ``lemon tools`` TUI."""
    manager = get_plugin_manager()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``lemon:<event>``) to subscriber callbacks in
...
_PLUGIN_COMPAT_LAZY = {
    'CAPABILITY_REGISTRY': ('lemon_cli.plugin_capabilities', 'CAPABILITY_REGISTRY'),
    'ENTRY_POINT_CAPABILITIES_GROUP': ('lemon_cli.plugins_discovery', 'ENTRY_POINT_CAPABILITIES_GROUP'),
    'LEGACY_RELAY_PLUGIN_KEYS': ('lemon_cli.relay_plugin_cutover', 'LEGACY_RELAY_PLUGIN_KEYS'),
    'MAX_SYSTEM_PROMPT_SECTIONS': ('lemon_cli.plugins_dispatch', 'MAX_SYSTEM_PROMPT_SECTIONS'),
    'OBSERVER_SCHEMA_VERSION': ('lemon_cli.middleware', 'OBSERVER_SCHEMA_VERSION'),
    'VALID_CAPABILITY_IDS': ('lemon_cli.plugin_capabilities', 'VALID_CAPABILITY_IDS'),
    'cfg_get': ('lemon_cli.config', 'cfg_get'),
    'fast_safe_load': ('utils', 'fast_safe_load'),
    'format_system_prompt_section': ('lemon_cli.plugins_dispatch', 'format_system_prompt_section'),
    'reset_lemon_home_override': ('lemon_constants', 'reset_lemon_home_override'),
    'set_lemon_home_override': ('lemon_constants', 'set_lemon_home_override'),
}
