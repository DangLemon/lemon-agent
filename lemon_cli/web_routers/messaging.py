...
from lemon_cli.config import OPTIONAL_ENV_VARS, get_env_path, redact_key
from lemon_cli.web_deps import LateState, late
from lemon_cli.web_server_gateway import _restart_gateway_after
from lemon_cli.web_server_messaging import (
...
from lemon_cli.web_models import (
...
_profile_scope = late("_profile_scope", "lemon_cli.web_server_profiles")
_resolve_profile_dir = late("_resolve_profile_dir", "lemon_cli.web_server_profiles")
_restart_gateway_after_whatsapp_onboarding = late("_restart_gateway_after_whatsapp_onboarding", "lemon_cli.web_server_messaging")
_telegram_onboarding_request_sync = late("_telegram_onboarding_request_sync", "lemon_cli.web_server_messaging")
_whatsapp_session_path = late("_whatsapp_session_path", "lemon_cli.web_server_messaging")
_write_platform_enabled = late("_write_platform_enabled", "lemon_cli.web_server_messaging")
load_env = late("load_env", "lemon_cli.config")
load_config = late("load_config", "lemon_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
remove_env_value = late("remove_env_value", "lemon_cli.config")
save_env_value = late("save_env_value", "lemon_cli.config")
_gateway_subcommand = late("_gateway_subcommand", "lemon_cli.web_server_gateway")
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    for key, description, prompt, extra in (
        ...
        ("WEIXIN_ACCOUNT_ID", "iLink Bot account ID obtained through QR login in lemon gateway setup", "iLink Bot account ID", {}),
        ("WEIXIN_TOKEN", "iLink Bot token obtained through QR login in lemon gateway setup", "iLink Bot token", {"password": True}),
...
def _messaging_platform_payload(
    ...
    # and do NOT follow the LEMON_HOME contextvar override, so without it messaging
...
def _platform_payloads(scoped_dir: Optional[Path], entries) -> list[dict[str, Any]]:
    ...
    LEMON_HOME contextvar; the gateway status readers do not, hence the explicit path)."""
...
def _ensure_whatsapp_bridge_dependencies(bridge_dir: Path) -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env=with_lemon_node_path(), creationflags=windows_hide_flags(),
        )
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Lemon AI").strip() or "Lemon AI"
...
async def apply_telegram_onboarding(pairing_id: str, body: TelegramOnboardingApply, profile: Optional[str] = None):
    ...
    # saved token waiting on a manual restart click reads as "Lemon AI is broken" from the
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    def _run():
        ...
        # credentials/state, not the root install's. load_env() honors the LEMON_HOME contextvar override;
        ...
        with _profile_scope(profile) as scoped_dir:
            return {
                ...
                "gateway_start_command": " ".join(["lemon", *_gateway_subcommand(profile, "start")]),
...
def _multiplex_port_binding_conflict(platform_id: str, requested_profile: Optional[str]) -> Optional[str]:
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (unrecognized LEMON_HOME) is outside
