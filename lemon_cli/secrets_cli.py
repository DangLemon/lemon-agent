"""CLI handlers for ``lemon secrets bitwarden ...``."""
...
# module makes the ``lemon update`` self-lock preflight defer. This module is registered
# parse-time from ``lemon_cli.main``, so the backend import stays lazy (nothing touches ``bw``
...
from lemon_cli._secrets_common import (
...
from lemon_cli.config import get_env_path, load_config, save_config, save_env_value
...
def __getattr__(name: str):
    ...
    Existing callers (and upstream tests) monkeypatch attributes on ``lemon_cli.secrets_cli.bw`` directly.
    ...
    so ``import lemon_cli.secrets_cli`` stays crypto-free while ``secrets_cli.bw.find_bws`` still resolves.
    """
    if name == "bw":
        return _load_bw()
...
# ── Argparse wiring — called from lemon_cli.main ──
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print("[green]✓ Bitwarden Secrets Manager is enabled.[/green]  "
                  "Secrets will be pulled at the start of every Lemon AI process.")
    console.print("  Status:  [cyan]lemon secrets bitwarden status[/cyan]\n"
                  "  Refresh: [cyan]lemon secrets bitwarden sync[/cyan]\n"
                  "  Disable: [cyan]lemon secrets bitwarden disable[/cyan]")
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]lemon secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token:
        console.print(f"\n  [yellow]Enabled but {token_env} is not set — Lemon AI will skip BSM "
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    def verify(token: str) -> bool:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `lemon secrets bitwarden setup` to pick a different project.[/yellow]")
        return True
    ...
    return rotate_token(
        ...
        disabled_note=None if bw_cfg.get("enabled") else (
            ...
            "run `lemon secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not args.apply:
        console.print("\n  This was a dry-run — secrets are picked up automatically on the "
                      "next [cyan]lemon[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
...
_PROJECT_LIST_HINTS = (
    (("invalid_client", "400 bad request"),
     ...
     "[cyan]lemon secrets bitwarden setup[/cyan] and pick EU or "
