...
Scope is deliberately narrow: Lemon AI only upgrades an npm inside its **own** managed Node tree
(``$LEMON_HOME/node``), installing in place with ``--prefix`` so ``bin/npm`` keeps resolving to
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Lemon AI-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: ``~/.local/bin/npm`` → ``$LEMON_HOME/node/bin/npm`` →
...
def upgrade_managed_npm(npm: str, npm_range: str, *, prefix: Path, quiet: bool = False) -> bool:
    ...
    into ``$LEMON_HOME/node/etc/npmrc`` (so global installs land on PATH), and without the override
    the "upgrade" would install a second npm elsewhere while the managed one stayed stale.
    """
    if not quiet:
        print(f"→ Upgrading Lemon AI-managed npm to satisfy {npm_range}…", flush=True)
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Lemon AI-managed Node.js tree is in use by a "
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="lemon-npm-upgrade-") as tmp:
...
def _probe_version(npm: str) -> str | None:
    try:
        result = subprocess.run([npm, "--version"], timeout=30, env=with_lemon_node_path(), **_RUN_KW)
    except (OSError, subprocess.SubprocessError):
        return None
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Lemon AI could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision (or reuse) the managed tree under ``$LEMON_HOME/node`` and return a satisfying npm.
    ...
    if not quiet:
        print(
            "→ Provisioning a Lemon AI-managed Node.js runtime "
            "(the resolved npm belongs to your system and is left alone)…",
            flush=True,
        )
    managed_npm = bootstrap_lemon_managed_node()
    if not managed_npm:
        if not quiet:
...
def maybe_repair_npm_engine(npm: str | None, output: str, *, quiet: bool = False) -> str | None:
    ...
    if prefix is not None:
        # Lemon AI owns this npm — upgrade in place. Only an npm-range failure is fixable this way.
