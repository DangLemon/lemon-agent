"""Configuration management for Lemon AI: config.yaml / .env loading, saving,
validation, migration, and the ``lemon config`` command."""
...
from lemon_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from lemon_cli.secret_prompt import masked_secret_prompt
# Re-export from lemon_constants — canonical definition lives there.
from lemon_constants import get_lemon_home, get_process_lemon_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    ...
    "refuse-write": (
        ...
        "Fix the YAML (lemon config edit) and retry.")}
...
def _warn_config_parse_failure(
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  lemon config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# interpreter init (PYTHON*, NODE_*: Lemon AI restarts through them), PATH (fix tool lookup
...
# and Lemon AI runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``LEMON_*`` overall is NOT blocked — many integration credentials use that prefix
# (LEMON_LANGFUSE_PUBLIC_KEY, LEMON_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Lemon AI runtime location
    "LEMON_HOME", "LEMON_PROFILE", "LEMON_CONFIG", "LEMON_ENV",
    "LEMON_CONFIG_PATH", "LEMON_ENV_PATH",
    ...
    "LEMON_COPILOT_ACP_COMMAND", "LEMON_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "LEMON_YOLO_MODE", "LEMON_ACCEPT_HOOKS", "LEMON_REDACT_SECRETS",
    "LEMON_INTERACTIVE", "LEMON_EXEC_ASK", "LEMON_GATEWAY_SESSION",
    "LEMON_CRON_SESSION", "LEMON_SINGLE_QUERY_SESSION",
    "LEMON_SESSION_KEY", "LEMON_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Lemon AI runtime location and security policy (LEMON_HOME, LEMON_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.lemon-ai/.env "
            "directly.")
...
def config_write_transaction(config_path: Optional[Path] = None) -> ContextManager[None]:
    ...
    the two locks in opposite orders. The guarantee covers cooperating Lemon AI
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean LEMON_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "LEMON_TOOL_PROGRESS_MODE",
    ...
    "LEMON_LANGFUSE_ENV", "LEMON_LANGFUSE_RELEASE", "LEMON_LANGFUSE_SAMPLE_RATE",
    "LEMON_LANGFUSE_MAX_CHARS", "LEMON_LANGFUSE_CAPTURE", "LEMON_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "LEMON_ACP_AUTH_METHOD", "LEMON_ACP_AUTO_APPROVE", "LEMON_COPILOT_ACP_COMMAND",
    "LEMON_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# LEMON_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
def get_managed_system() -> Optional[str]:
    ...
    Signals: LEMON_MANAGED env var (systemd service) or a ``.managed`` marker file in
    LEMON_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("LEMON_MANAGED", "").strip().lower() or None
    managed_marker = get_lemon_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Lemon AI is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update Lemon AI through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Lemon AI was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$LEMON_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because LEMON_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``lemon update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``danglemon/lemon-agent`` image bakes a ``docker`` stamp into
    ``/opt/lemon`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of lemon_cli/), NOT of $LEMON_HOME,
    ...
    method = _install_method_stamp(get_lemon_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``lemon_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull danglemon/lemon-agent:latest",
    "apt": "pkg upgrade lemon-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "lemon update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (lemon_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Lemon AI.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``lemon update`` doesn't apply inside the Docker container.

Lemon AI runs as a published image (danglemon/lemon-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull danglemon/lemon-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate lemon-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm danglemon/lemon-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/danglemon/lemon-agent/tags
  • Your config and session history live under ``$LEMON_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``lemon update`` inside Docker."""
    return _DOCKER_UPDATE_MESSAGE


def format_managed_message(action: str = "modify this Lemon AI installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Lemon AI installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Lemon AI.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from LEMON_HOME/.container-mode.
    ...
    already inside the container, or when LEMON_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("LEMON_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_lemon_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "container_name": info.get("container_name", "lemon-agent"),
        "exec_user": info.get("exec_user", "lemon"),
        "lemon_bin": info.get("lemon_bin", "/data/current-package/bin/lemon")}


# ---- Config paths / LEMON_HOME skeleton ----
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / LEMON_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("LEMON_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_lemon_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read LEMON_UID / LEMON_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns LEMON_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Lemon AI commonly set these to map the in-container user to a host user so
    ...
    return _env_int("LEMON_UID"), _env_int("LEMON_GID")


def _chown_to_lemon_uid(path) -> None:
    """Chown ``path`` to ``LEMON_UID:LEMON_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_lemon_home` on Docker deployments. See #34107.
    """
    uid, gid = _resolve_lemon_uid_gid()
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply LEMON_UID/GID ownership. No-op when managed.
    LEMON_HOME_MODE (e.g. 0701) overrides the mode so a web server can traverse LEMON_HOME to
    a served subdirectory without directory listings.

    Also applies ``LEMON_UID``/``LEMON_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    try:
        mode = int(os.environ.get("LEMON_HOME_MODE", "").strip() or "700", 8)
    except ValueError:
        mode = 0o700
    try:
        os.chmod(path, mode)
        ...
        pass
    _chown_to_lemon_uid(path)


def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or LEMON_CONTAINER / LEMON_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("LEMON_CONTAINER") or os.environ.get("LEMON_SKIP_CHMOD")
...
_LEMON_HOME_ENSURED: set = set()
_LEMON_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_lemon_home():
    """Ensure the ~/.lemon-ai directory skeleton exists with secure permissions.
    ...
    from lemon_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _LEMON_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
        ...
        try:
            if not home.is_dir():
                raise RuntimeError(f"LEMON_HOME {home} does not exist.")
        ...
        _secure_dir(home)
        for subdir in _LEMON_HOME_SUBDIRS:
            d = home / subdir
            d.mkdir(parents=True, exist_ok=True)
            _secure_dir(d)
        _ensure_default_soul_md(home)

    _LEMON_HOME_ENSURED.add(key)


# ---- Config loading/saving ----

from lemon_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from lemon_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use lemon_cli.config.<name>)
...
# Back-compat re-exports — :mod:`lemon_cli.personality` owns personality/overlay semantics.
from lemon_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``lemon config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `lemon config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    return (
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `lemon update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (lemon_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # lemon_cli/tools_config.py toolset-save flow
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'lemon tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'lemon setup' to create a valid config")]
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — Lemon AI won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'lemon doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from lemon_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ lemon config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from lemon_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from lemon_cli.toolset_validation import validate_platform_toolsets
        from lemon_cli.toolset_scope import toolset_allowed_for_platform
...
def _offer_list(heading: str, items: List[str], question: str) -> bool:
    ...
    if not _ask_yes_no(question):
        print("  Set later with: lemon config set <key> <value>")
        return False
    print()
    return True
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.lemon-ai/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_lemon_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``lemon config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``lemon status``/``dump``) read ``name`` and
...
# the LEMON_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Lemon AI host
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # load_lemon_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_lemon_dotenv() pins unexpanded literals
...
#   openai-codex (OAuth — lemon auth) — OpenAI Codex
#   nous         (OAuth — lemon auth) — Nous Portal
...
def save_config(
    ...
    """Save configuration to ~/.lemon-ai/config.yaml.
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Lemon AI writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
def load_env() -> Dict[str, str]:
    """Load ~/.lemon-ai/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def sanitize_env_file() -> int:
    """Rewrite ~/.lemon-ai/.env with normalized line formatting; returns the number of changed lines."""
    env_path = get_env_path()
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.lemon-ai/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``LEMON_CUSTOM_`` prefix keeps the
    ...
    return f"LEMON_CUSTOM_{slug}_API_KEY" if slug else "LEMON_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.lemon-ai/.env and os.environ; True if it was found and removed."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from lemon_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.lemon-ai/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Lemon AI (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.lemon-ai/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Lemon AI-managed credential preferring ``~/.lemon-ai/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale LEMON_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("LEMON_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale LEMON_MAX_ITERATIONS={env_ghost} "
                    f"(run 'lemon doctor --fix' to remove)", Colors.YELLOW))
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from lemon_cli.personality import active_personality_name
...
def show_config():
    ...
    print(color("│              ⚕ Lemon AI Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from lemon_cli.auth import get_anthropic_key
    ...
    print(color("  lemon config edit     # Edit config file", Colors.DIM))
    print(color("  lemon config set <key> <value>", Colors.DIM))
    print(color("  lemon setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``LEMON_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("LEMON_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "model/provider. Inspect with `lemon cron list`, then pin the intended values with "
        "`lemon cron edit <job_id> --provider <provider> --model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``lemon config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``lemon config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    lemon config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    lemon config set --force {key} {value!r}"]
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_lemon_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Lemon AI may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `lemon config
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from lemon_cli.credential_lifecycle import remove_provider_env_credential
...
_USAGE_GET = ("Usage: lemon config get <key> [--json]", [
    "lemon config get model", "lemon config get terminal.backend",
    "lemon config get skills.config --json"], None)
_USAGE_SET = ("Usage: lemon config set [--force] <key> <value>", [
    "lemon config set model anthropic/claude-sonnet-4", "lemon config set terminal.backend docker",
    "lemon config set OPENROUTER_API_KEY sk-or-..."], [
    "", "  --force: skip the unknown-key notice for unrecognized keys,",
    "           and allow a scalar to replace a whole mapping section"])
_USAGE_UNSET = ("Usage: lemon config unset <key>", [
    "lemon config unset model", "lemon config unset terminal.backend",
    "lemon config unset OPENROUTER_API_KEY"], None)
...
def _cmd_config_check(args):
    ...
    if missing_config:
        ...
        print("    Run 'lemon config migrate' to add them")
...
_CONFIG_USAGE = """Available commands:
  lemon config           Show current configuration
  lemon config edit      Open config in editor
  lemon config get <key>          Print a resolved config value
  lemon config set <key> <value>   Set a config value
  lemon config unset <key>        Remove a config value
  lemon config check     Check for missing/outdated config
  lemon config migrate   Update config with new options
  lemon config path      Show config file path
  lemon config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``lemon config`` UI without the core knowing they exist.
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``lemon_cli/`` — i.e. the git checkout for source
    installs, ``/opt/lemon`` inside the published image. It is a property of
    the running interpreter, NOT of ``$LEMON_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$LEMON_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/lemon`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('lemon_cli.route_identity', 'normalize_route_base_url'),
}
