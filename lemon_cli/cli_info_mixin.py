...
Mixin split out of ``cli.py``; bound onto ``LemonCLI`` via the MRO. cli.py-internal symbols are
...
from lemon_constants import is_termux as _is_termux_environment
from rich.markup import escape as _escape
from utils import base_url_hostname

from lemon_cli.cli_modal_mixin import _gated_confirm
...
class CLIInfoMixin:
    ...
    def show_banner(self):
        ...
        from lemon_cli.banner import build_welcome_banner
        self.console.clear()
        ctx_len = None
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Lemon AI needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        from lemon_cli.model_switch import is_nous_lemon_non_agentic
        if is_nous_lemon_non_agentic(getattr(self, "model", "") or ""):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Hermes 3 & 4 models are NOT agentic and are not "
                "designed for use with Lemon AI.[/]")
            self._console_print(
        ...
        # `lemon skills trust`. Never raises.
        try:
            ...
            if _proj_dirs:
                ...
                if _untrusted is not None:
                    ...
                    self._console_print(
                        ...
                        f"loaded — run `lemon skills trust` to enable them.[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from lemon_cli.models import model_supports_fast_mode
        except Exception:
            return False
    ...
    def show_help(self, arg: str = ""):
        ...
        from lemon_cli.commands import COMMANDS_BY_CATEGORY, HELP_SESSION_SUBGROUPS
        ...
        try:
            from lemon_cli.skin_engine import get_active_help_header
    ...
    def handle_bang_shell(self, text: str) -> bool:
        ...
        from lemon_cli.bang_shell import (
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_lemon_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_lemon_home()}/config.yaml")
            print()

    def _print_random_tip(self) -> None:
        """Best-effort discovery tip (startup + /clear); never raises."""
        try:
            from lemon_cli.tips import get_random_tip
            _tip = get_random_tip()
            try:
    ...
    def _toggle_verbose(self):
        ...
        if getattr(self, "_focus_view_enabled", False):
            ...
            try:
                from lemon_cli.focus_view import FOCUS_CONFIG_KEY
    ...
    def _usage_reset(self, force: bool = False):
        ...
        if str(self._agent_or_self("provider") or "").strip().lower() != "openai-codex":
            ...
            print("  Switch with `/model` or `lemon auth` first.")
            return
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from lemon_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.lemon-ai/skills/ and queue a note for the next user turn.
