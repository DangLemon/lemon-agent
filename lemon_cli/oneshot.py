...
Toolsets = explicit --toolsets, else the user's "cli" toolsets from `lemon tools`. Rules /
memory / AGENTS.md / preloaded skills = same as a normal chat turn. Approvals are auto-bypassed
(LEMON_YOLO_MODE=1). Model/provider mirror `lemon chat`: both optional; only --model → auto-detect
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"lemon -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in _ALL_TOOLSETS for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "lemon -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"lemon -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "lemon -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )
    if not valid:
        return None, "lemon -z: --toolsets did not contain any valid toolsets.\n"
    return valid, None
...
def run_oneshot(
    ...
    Model/provider fall back to ``LEMON_INFERENCE_MODEL`` and config.yaml. ``usage_file`` gets a
    ...
    env_model_early = os.getenv("LEMON_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "lemon -z: --provider requires --model (or LEMON_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"lemon -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        if result.get("failed") or result.get("partial"):
            return 2
        real_stderr.write("lemon -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
    return 0
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB — oneshot bypasses ``LemonCLI._init_agent()``, so it must wire the
...
def _resolve_model_and_provider(cfg: dict, model: Optional[str], provider: Optional[str]) -> _ModelChoice:
    ...
    env_model = os.getenv("LEMON_INFERENCE_MODEL", "").strip()
    ...
    try:
        from lemon_cli import model_switch as _ms
    ...
    if direct is None:
        ...
        current_provider = cfg_provider or os.getenv("LEMON_INFERENCE_PROVIDER", "").strip().lower() or "auto"
...
def _run_agent(
    ...
    try:
        agent = AIAgent(
            ...
            # LEMON_INTERACTIVE (never set), hook approval via LEMON_ACCEPT_HOOKS=1, dangerous
            # commands via LEMON_YOLO_MODE=1, skill secret capture degrades gracefully.
