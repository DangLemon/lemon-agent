"""Cross-process mutual exclusion for in-flight Lemon AI updates.
...
MARKER_NAME = ".lemon-ai-update-in-progress"

# Set by an orchestrating updater (Tauri `lemon-setup --update`) to its own pid before
# spawning `lemon update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "LEMON_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Lemon AI is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Lemon AI home (never the context-local profile override): the Rust
    updater resolves ``$LEMON_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_lemon_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``lemon update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Lemon AI update is already running (PID {holder.pid}, "
