...
def _anthropic_oauth_login(args) -> dict:
    ...
    creds = anthropic_mod.run_lemon_oauth_login_pure()
    if not creds:
...
def _qwen_oauth_login(args) -> dict:
    from lemon_cli.auth_qwen import _mark_qwen_oauth_active
...
class _OAuthAddSpec:
    """Per-provider parameters for the generic ``lemon auth add <provider> --type oauth`` path."""
...
_OAUTH_ADD_SPECS: dict[str, _OAuthAddSpec] = {
    "anthropic": _OAuthAddSpec(
        ...
        source=f"{SOURCE_MANUAL}:lemon_pkce",
...
def _add_nous_oauth_credential(args, provider: str) -> None:
    """``lemon auth add nous --type oauth``: shared-credential import, else device-code login."""
    ...
    # Codex-style auto-import: a shared Nous credential at <lemon-root>/shared/nous_auth.json
    # (written by any previous login) makes `lemon --profile <name> auth add nous --type oauth`
...
def auth_add_command(args) -> None:
    ...
    if spec is None:
        raise SystemExit(f"`lemon auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Every credential source Lemon AI reads from (env vars, external OAuth files, auth.json blocks,
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `lemon auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `lemon auth` is called bare."""
