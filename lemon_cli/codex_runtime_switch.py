...
_ARG_SYNONYMS = {
    ...
    "off": "auto", "default": "auto", "disable": "auto", "lemon": "auto"}

_LEMON_TOOLS_CALLBACK_NOTE = (
    "Lemon AI tool callback registered: codex can now use "
    "web_search, web_extract, browser_*, vision_analyze, "
    "image_generate, skill_view, skills_list, text_to_speech, "
    "kanban_* (worker + orchestrator) via MCP.",
    "  (delegate_task, memory, session_search, todo run "
    "only on the default Lemon AI runtime — they need the "
    "agent loop context.)")
...
def _migration_lines(config: dict) -> list[str]:
    ...
    lines: list[str] = []
    try:
        from lemon_cli.codex_runtime_plugin_migration import migrate
        mig_report = migrate(config)
        # The lemon-tools callback is internal plumbing — surfaced separately below.
        user_servers = [s for s in mig_report.migrated if s != "lemon-tools"]
        if user_servers:
        ...
        if "lemon-tools" in mig_report.migrated:
            lines.extend(_LEMON_TOOLS_CALLBACK_NOTE)
...
def apply(
    ...
    # correct but the world state (managed block in ~/.codex/config.toml, lemon-tools MCP
    ...
    if new_value == "codex_app_server":
        ...
        # Migrate Lemon AI' MCP servers + Codex's curated plugins into ~/.codex/config.toml so the
        # spawned codex subprocess sees the same tool surface AND can call back into Lemon AI.
        ...
        msg_lines.append(
            ...
            "Lemon AI tools available via MCP callback).")
        msg_lines.append(
            "Effective on next session — current cached agent keeps "
            "the prior runtime to preserve prompt cache.")
    else:
        msg_lines.append("OpenAI/Codex turns will use the default Lemon AI runtime.")
