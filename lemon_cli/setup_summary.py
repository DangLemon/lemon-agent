...
resolved through the module object so test patches on ``lemon_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from lemon_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'lemon setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'lemon setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'lemon tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("lemon setup", "          Re-run the full wizard"), ("lemon setup model", "    Change model/provider"),
    ("lemon setup terminal", " Change terminal backend"), ("lemon setup gateway", "  Configure messaging"),
    ("lemon setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("lemon config", "         View current settings"), ("lemon config edit", "    Open config in your editor"),
    ("lemon config set <key> <value>", ""))
_READY_ROWS = (
    ("lemon", "              Start chatting"), ("lemon gateway", "      Start messaging gateway"),
    ("lemon doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'lemon setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `lemon tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'lemon setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via lemon auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, lemon_home):
    ...
    try:
        from lemon_cli.auth import resolve_provider
        resolve_provider()
    except Exception:
        print()
        _setup.print_warning("No inference provider is configured — Lemon AI cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    lemon model            (pick any provider/model)",
              "    lemon setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'lemon setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {lemon_home}/cron/, sessions/, logs/")):
...
import lemon_cli.setup as _setup  # noqa: E402  (bottom: lemon_cli.setup imports this module)
