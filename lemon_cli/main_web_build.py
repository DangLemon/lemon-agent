...
# to the checkout (NOT in LEMON_HOME): __pycache__ is per-checkout state shared
# by every profile.
...
def _record_bytecode_fingerprint() -> None:
    ...
    from lemon_cli.main import PROJECT_ROOT, _read_git_revision_fingerprint
    try:
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    Update-time clears can't close the stale-bytecode class: ``lemon update`` runs
    ...
    pull inside ``lemon update``, a manual ``git pull``, a ZIP update, a file-sync restore) while
    ...
    """
    from lemon_cli.main import PROJECT_ROOT, _clear_bytecode_cache, _read_git_revision_fingerprint
    try:
        fingerprint = _read_git_revision_fingerprint(PROJECT_ROOT)
        if not fingerprint:
...
def _web_dist_dir(web_dir: Path) -> Path:
    """Vite outputs to ``lemon_cli/web_dist/`` (vite.config.ts outDir), NOT ``web/dist/``."""
    return _web_project_root(web_dir) / "lemon_cli" / "web_dist"
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    Content hash, NOT mtime: ``git checkout`` / ``lemon update`` rewrite source
...
def _web_ui_stamp_path() -> Path:
    """Path of the web UI build stamp under $LEMON_HOME."""
    from lemon_constants import get_lemon_home
    return get_lemon_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    half-state with the ``lemon`` launcher present but ``lemon_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    """``PYTHON=`` env for node-gyp on NixOS (bare PATH lookup fails outside nix-shell): the lemon
...
def _run_npm_install_deterministic(
    ...
    run_env["PATH"] = with_lemon_node_path(run_env)["PATH"]
...
def _web_npm_install_context(web_dir: Path) -> tuple[Path, tuple[str, ...]]:
    ...
    closure as ``lemon update``'s ``_update_node_dependencies()`` (ui-tui + web +
    ...
    # When running from the workspace root, this must name the SAME closure as `lemon update`'s
...
def _report_web_build_failure(step: str, result: subprocess.CompletedProcess, *, fatal: bool) -> bool:
    ...
    _console_print(f"  {'✗' if fatal else '⚠'} Web UI {step} failed" + ("" if fatal else " (lemon web will not be available)"))
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    soft warning (used by ``lemon web``). Returns True when the build succeeded
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_lemon_node_path())
