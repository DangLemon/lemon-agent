...
tests/lemon_cli/test_commands_execute.py).
"""
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    home_display = str(ctx.options.get("home_display") or "").strip() or display_lemon_home()
    # Presentation-only display name (profile.yaml); `data.profile` stays the canonical id.
    label = profile_name
    try:
        from lemon_cli.profiles import format_profile_label, get_profile_dir, read_profile_meta
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: lemon bundles create <name> --skill <s1> --skill <s2>\n"
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from lemon_cli.commands import gateway_help_lines
