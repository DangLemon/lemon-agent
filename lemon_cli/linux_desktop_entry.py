"""Install and remove the Linux desktop entry (``lemon.desktop``).
...
DESKTOP_ENTRY_NAME = "lemon.desktop"
...
def _icon_theme_name() -> str:
    return "lemon-ai" if _internal_desktop_build() else "lemon"
...
def _can_import_lemon_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``lemon_cli.main`` unaided.
...
def _running_interpreter_fallback() -> str:
    """The RUNNING interpreter — it has ``lemon_cli`` importable by definition."""
    return os.path.abspath(sys.executable)


def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``lemon desktop``.

    Prefer the real ``lemon`` launcher; fall back to ``<python> -m lemon_cli.main desktop``.
    """
    from lemon_cli.relaunch import resolve_lemon_bin

    bin_path = _resolve_lemon_bin_for_desktop_entry(
        resolve_lemon_bin, checkout_root=project_root
    )
    interpreter = _running_interpreter()
    if not _can_import_lemon_cli(Path(interpreter)):
        ...
        # The candidate interpreter cannot actually import lemon_cli.main (checked in isolated mode from a
    ...
    argv = [interpreter, "-m", "lemon_cli.main", "desktop"]
    if bin_path:
        resolved = Path(bin_path).resolve()
        # A Python launcher whose shebang points OUTSIDE the venv (e.g. the repo's `lemon` script
...
def _inside_checkout(candidate: str, checkout_root: Path, original_argv0: str) -> bool:
    ...
    try:
        ...
        return False
    # Anything shipped in the tree (e.g. the repo `lemon` script) is checkout-internal. Compare
    ...
    for root in {checkout_root, resolved_root}:
        if root is not None and (path == root or root in path.parents):
            return True
    # The `python -m lemon_cli.main` relaunch context surfaces the invoking interpreter as
...
def _resolve_lemon_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`lemon_cli.relaunch.resolve_lemon_bin` with one rule: an ``argv[0]`` inside this
    ...
    if resolve_fn is None:
        from lemon_cli.relaunch import resolve_lemon_bin as resolve_fn
    ...
    # An external primary (another install's /opt/.../bin/lemon, a venv console script) wins
    ...
    # /opt/.../bin/lemon from another install method, or a venv console script). It must be evaluated
    ...
    # argv[0] was checkout-internal AND PATH had no `lemon` — common in stripped systemd user
...
def _known_wrapper_candidates():
    ...
    (``/usr/local/bin``), and user (``~/.local/bin``). The wrapper is always named ``lemon``.
    """
    candidates = []
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "lemon")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/lemon"))
    candidates.append(Path.home() / ".local" / "bin" / "lemon")
    return candidates
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    """Install the app icon into the user's hicolor tree so ``Icon=lemon`` resolves without an
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Lemon AI desktop entry and return its path.
