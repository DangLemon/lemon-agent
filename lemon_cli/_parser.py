"""Top-level argparse construction for the lemon CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``lemon_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``LEMON_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/lemon_cli/
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``lemon --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``lemon_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    lemon                        Start interactive chat
    lemon chat -q "Hello"        Single query mode
    lemon --tui                  Launch the modern TUI (or set display.interface: tui)
    lemon --cli                  Force the classic REPL (overrides display.interface: tui)
    lemon -c                     Resume the most recent session
    lemon -c "my project"        Resume a session by name (latest in lineage)
    lemon --resume <session_id>  Resume a specific session by ID
    lemon --resume latest        Resume the most recent session (same as -c)
    lemon --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    lemon setup                  Run setup wizard
    lemon logout                 Clear stored authentication
    lemon auth add <provider>    Add a pooled credential
    lemon auth list              List pooled credentials
    lemon auth remove <p> <t>    Remove pooled credential by index, id, or label
    lemon auth reset <provider>  Clear exhaustion status for a provider
    lemon model                  Select default model
    lemon fallback [list]        Show fallback provider chain
    lemon fallback add           Add a fallback provider (same picker as `lemon model`)
    lemon fallback remove        Remove a fallback provider from the chain
    lemon config                 View configuration
    lemon config edit            Edit config in $EDITOR
    lemon config set model gpt-4 Set a config value
    lemon gateway                Run messaging gateway
    lemon -s lemon-agent-dev,github-auth
    lemon -w                     Start in isolated git worktree
    lemon gateway install        Install gateway background service
    lemon sessions list          List past sessions
    lemon sessions browse        Interactive session picker
    lemon sessions rename ID T   Rename/title a session
    lemon logs                   View agent.log (last 50 lines)
    lemon logs -f                Follow agent.log in real time
    lemon logs errors            View errors.log
    lemon logs --since 1h        Lines from the last hour
    lemon debug share             Upload debug report for support
    lemon console                Open the safe Lemon AI command console
    lemon update                 Update to latest version
    lemon dashboard              Start web UI dashboard (port 9119)
    lemon dashboard --stop       Stop running dashboard processes
    lemon dashboard --status     List running dashboard processes
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via LEMON_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `lemon setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to LEMON_ACCEPT_HOOKS=1 or "
    ...
    inherited(parser, "--ignore-user-config", action="store_true", default=False,
              help="Ignore ~/.lemon-ai/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)")
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``lemon -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/lemon_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Lemon AI")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also LEMON_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--ignore-user-config", action="store_true", default=SUPPRESS,
              help="Ignore ~/.lemon-ai/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.")
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Lemon AI itself.")
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="lemon", description="Lemon AI - AI assistant with tool-calling capabilities",
