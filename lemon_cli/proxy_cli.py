...
from lemon_cli.config import load_config, load_env, save_config


def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    """Attach the egress subcommand tree to a parent parser."""
    # dest='egress_command' keeps this tree disjoint from the inbound OAuth ``lemon proxy``
    ...
    commands = [
        ...
        ("setup", "Interactive wizard: install + CA + mint tokens + write config", cmd_setup, [
            ...
            ("--no-restart", dict(dest="restart", action="store_false", help=(
                ...
                "`lemon egress restart` yourself for changes to take effect."))),
        ]),
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        "  Start:   [cyan]lemon egress start[/cyan]\n"
        "  Restart: [cyan]lemon egress restart[/cyan]  (after any re-setup)\n"
        "  Reload:  [cyan]lemon egress reload[/cyan]   (apply ruleset edits "
        "in-place, no restart)\n"
        "  Status:  [cyan]lemon egress status[/cyan]\n"
        "  Stop:    [cyan]lemon egress stop[/cyan]\n"
        "  Disable: [cyan]lemon egress disable[/cyan]"
    )
    return 0
...
def _setup_mint_tokens(console: Console, args: argparse.Namespace):
    ...
    if args.from_bitwarden:
        ...
        # Operators commonly keep provider keys only in ~/.lemon-ai/.env (loaded when the agent
        # runs, NOT exported into an interactive shell); backfill so discovery sees them.
        loaded = _load_env_file_into_environ()
        if loaded:
            console.print(f"  [dim]Loaded {loaded} provider key name(s) from ~/.lemon-ai/.env for discovery.[/dim]")
    ...
    if rotate and existing:
        ...
        if sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Lemon AI sandbox.  They will start 401-ing against upstreams until restarted."
            )
...
def _setup_restart_daemon(console: Console, args: argparse.Namespace, proxy_cfg: dict) -> None:
    ...
    if do_restart:
        try:
            ...
            console.print("  Run [cyan]lemon egress start[/cyan] manually before launching new Docker sandboxes.")
        else:
        ...
        console.print(
            ...
            "changed.  Run [cyan]lemon egress restart[/cyan] (or "
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print("[yellow]proxy.enabled is false — run `lemon egress setup` first.[/yellow]")
        return 1
    ...
    if credential_source == "bitwarden" and not refresh_bw:
        if bool(proxy_cfg.get("allow_env_fallback", False)):
            ...
            return _refuse(
                ...
                "back to env credentials with `lemon egress setup "
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            return _refuse(
                ...
                "`lemon egress setup --no-bitwarden` to switch back to env-based credentials.",
            )
        if not (bw_cfg or {}).get("project_id"):
            return _refuse(
                console,
                "credential_source=bitwarden but secrets.bitwarden.project_id is empty.",
                "Run `lemon secrets bitwarden setup` to configure the "
                "project, or switch back via `lemon egress setup --no-bitwarden`.",
            )
    try:
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    New upstream SECRETS still need ``lemon egress restart``: the daemon reads credentials from
    ...
    console.print(
        ...
        "still need `lemon egress restart` — the daemon reads real "
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", "Next: run `lemon egress setup` to mint tokens and write proxy.yaml."])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", "Next: run `lemon egress start` before launching Docker sandboxes."])
    return "\n".join(lines)
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            "  iron-proxy is still running — stop it with [cyan]lemon egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print("[yellow](no config generated — run `lemon egress setup`)[/yellow]")
        return 1
...
def _bitwarden_env_names(console: Console) -> Optional[List[str]]:
    ...
    if not bw_cfg.get("enabled"):
        ...
        console.print("  Run `lemon secrets bitwarden setup` first, or omit --from-bitwarden.")
        return None
    try:
...
def _load_env_file_into_environ() -> int:
    """Backfill known provider keys from ``~/.lemon-ai/.env`` into ``os.environ``; returns the count.
