"""Host-platform checks for lemon doctor: interpreter, SQLite, certificates, macOS TCC, gateway supervision, command install.
Split out of ``lemon_cli/doctor.py``, which re-exports every name so ``lemon_cli.doctor.<name>`` keeps resolving (and monkeypatching)."""
...
from lemon_cli.doctor_report import (
    Finding,
    _fail_and_issue,
    _section,
    check_bool,
...
from lemon_constants import is_termux as _is_termux
...
def _desktop_bundle_name() -> str:
    return "Lemon AI.app"


def _desktop_app_identity(app: Path) -> tuple[str, str]:
    if app.name == "Lemon AI.app":
        return "Lemon AI", "com.lemondigital.lemonai"
    return "Hermes", "com.nousresearch.hermes"
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    from lemon_cli.doctor import PROJECT_ROOT
    from lemon_cli.config import detect_install_method
    ...
    action = (
        ...
        else {  # nix: prose guidance, not a shell command
            "docker": f"run `{cmd}`, then recreate all Lemon AI containers",
            "apt": f"run `{cmd}`",
        }.get(method, "run `lemon update`")
    )
    return f"({action}; fixed versions: 3.51.3+ / 3.50.7 / 3.44.6 — see https://sqlite.org/wal.html#walresetbug)"


def _lemon_database_paths(lemon_home: Path) -> list[tuple[str, Path]]:
    """(display name, path) pairs for Lemon AI-managed SQLite databases: backup.py's per-profile store list + per-board kanban.db."""
    from lemon_cli.backup import _QUICK_STATE_FILES

    entries = [
        (name, lemon_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    for board_db in sorted((lemon_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(lemon_home)), board_db))
    return entries
...
def _unreadable_reason(db_path: Path) -> str:
    ...
    return (
        ...
        else f"permission denied: {db_path}"
    )
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    process's POSIX advisory locks (see ``lemon_cli.sqlite_safe_read``), and the dashboard console runs
    ``run_doctor`` in-process with live ``SessionDB`` connections — the helper refuses then (unreadable).
    """
    from lemon_cli.sqlite_safe_read import (
        has_live_connection,
    ...
    if header is None:
        return None, "database is open in this process" if has_live_connection(
            db_path
        ) else _unreadable_reason(db_path)
    if len(header) == 0:
    ...
    return (
        (mode, None)
        if mode
        else (None, f"unrecognized file-format version {header[18]}")
    )
...
def _report_database_journal_modes(
    lemon_home: Path | None = None, version_info: tuple[int, ...] | None = None
) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from lemon_cli.doctor import LEMON_HOME
    from lemon_state_wal import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    try:
        databases = _lemon_database_paths(
            lemon_home if lemon_home is not None else LEMON_HOME
        )
    except Exception as exc:
        check_warn(f"Could not list Lemon AI databases: {exc}")
        return
    exposed = []
    for name, path in databases:
        ...
        if error is not None:
            ...
            check_info(
                f"{name}: rollback journal mode ({size}{', not exposed' if vulnerable else ''})"
            )
    if exposed:
...
def _check_version_consistency(issues: list[str]) -> None:
    """Detect pyproject.toml vs lemon_cli.__version__ drift (a conflict resolution can revert one but not the
    other). Silent no-op for installed wheels (no pyproject)."""
    try:
        from lemon_cli import __version__ as init_version
    except Exception:
        return
    ...
    _fail_and_issue(
        ...
        f"(pyproject.toml {pyproject_version} != lemon_cli/__init__.py {init_version})",
        "Re-sync version files (e.g. run 'lemon update', or set lemon_cli/__init__.py __version__ to match pyproject.toml)",
        issues,
    )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    try:
        from lemon_cli.service_manager import S6ServiceManager, detect_service_manager
    except Exception:
        return
    ...
    for static in (
        "main-lemon",
        "dashboard",
    ):  # s6-rc symlinks under /run/service/, same s6-svstat probe
        up = mgr.is_running(static)
        (check_ok if up else check_info)(
            f"{static}: up"
            if up
            else f"{static}: down (expected if not enabled via env)"
        )
    profiles = mgr.list_profile_gateways()
    if not profiles:
        return check_info(
            "No per-profile gateways registered yet — create one with `lemon profile create <name>`"
        )
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        issues.append(
            f"Repair the CA bundle: run `lemon doctor --fix`, or `{pip_cmd}`"
        )
        return
    ...
    try:
        ...
        failure = (
            ("certifi reinstall failed", (result.stderr or result.stdout or "")[-500:])
    ...
    if failure:
        return _fail_and_issue(
            *failure, f"Reinstall certifi manually: {pip_cmd}", issues
        )
    # Drop cached certifi modules so where() resolves the fresh install without a restart.
    import importlib

    for mod_name in [
        m for m in sys.modules if m == "certifi" or m.startswith("certifi.")
    ]:
    ...
    try:
        ...
        _fail_and_issue(
            "SSL CA certificate bundle still broken after reinstall",
            str(e),
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        ...
        return check_warn(
            "Gateway service linger", f"(could not import gateway helpers: {e})"
        )
    if (
        not is_linux()
        or detect_service_manager() == "s6"
        or not get_systemd_unit_path().exists()
    ):
        return
...
_TCC_CDHASH_DETAIL = (
    ...
    "all permission grants. Run `lemon update` to get the stable identifier-pinned signing identity, "
    "then re-grant permissions once."
)
_TCC_STABLE_DETAIL = {
    True: "(certificate-anchored DR; grants survive rebuilds)",
    False: "(identifier-pinned DR; grants survive rebuilds — for the strongest anchor, see `lemon desktop --setup-tcc-identity`)",
}
...
def check_macos_tcc_grants() -> None:
    ...
    if "cdhash" in dr.lower():
        return check_warn(
            "macOS TCC grants will reset after every update", _TCC_CDHASH_DETAIL
        )
    # --setup-tcc-identity or notarized build (certificate-anchored) is the strongest anchor.
    check_ok(
        "macOS TCC signing identity is stable",
        _TCC_STABLE_DETAIL["certificate" in dr.lower()],
    )
    product_name, bundle_id = _desktop_app_identity(app)
    check_info(
        "If macOS still re-prompts for permissions (toggle shows ON): the stored grant is stale — run "
        f"`tccutil reset ScreenCapture {bundle_id}` (repeat per affected service), toggle it ON in "
        f"System Settings, then fully quit & relaunch {product_name} once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    candidate_groups = (
        ("mac*/Lemon AI.app", "Lemon AI"),
        ("mac*/Hermes.app", "Hermes"),
    )
    for pattern, executable_name in candidate_groups:
        candidates = [
            p
            for p in release_dir.glob(pattern)
            if p.is_dir()
            for executable in (p / "Contents" / "MacOS" / executable_name,)
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    with warn_on_error("macOS TCC anchor check failed"):
        from lemon_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    try:
        ...
        product_possessive = f"{product_name}'" if product_name.endswith("s") else f"{product_name}'s"
        check_info(
            f"One switch silences all macOS folder prompts: grant your terminal app Full Disk Access and {product_name} "
            ...
            f"    then enable your terminal (and {_desktop_bundle_name()} if you use Desktop), and restart them once. "
            f"With {product_possessive} stable signing identities the grant survives every update."
        )
    except OSError:
...
def _check_security_advisories(should_fix: bool, f: Finding) -> None:
    ...
    from lemon_cli.security_advisories import (
        detect_compromised,
        filter_unacked,
    ...
    for hit in fresh_hits:
        ...
        _fail_and_issue(
            ...
            f"and rotate credentials, then run `lemon doctor --ack {hit.advisory.id}`.",
            f.manual_issues,
        )
    ...
    for h in all_hits:
        if h.advisory.id in acked_ids:
            check_warn(
                f"{h.package}=={h.installed_version} still installed (advisory {h.advisory.id} acknowledged)"
            )
...
def _check_python_environment(should_fix: bool, f: Finding) -> None:
    ...
    elif check_bool(v >= (3, 10), label, (label, "(3.10+ recommended)")) and v < (
        3,
        11,
    ):
        check_warn(
    ...
    with warn_on_error("SQLite version probe failed: {e}", ""):
        import sqlite3
        from lemon_state_wal import is_sqlite_wal_reset_vulnerable, sqlite_source_id

        src = sqlite_source_id()
        # Warn-only: Lemon AI already refuses WAL on fresh DBs and runtime repair is best-effort.
        check_bool(
            not is_sqlite_wal_reset_vulnerable(),
            f"SQLite {sqlite3.sqlite_version}",
            (
                f"SQLite {sqlite3.sqlite_version} (WAL-reset bug)",
                _sqlite_upgrade_hint(),
            ),
        )
        if src:
            check_info(
                f"SQLite source id: {(src[:48] + '…') if len(src) > 48 else src}"
            )
...
_PACKAGES = (
    ...
    ("yaml", "PyYAML", False),
    ("httpx", "HTTPX", False),
    ("croniter", "Croniter (cron expressions)", True),
    ("telegram", "python-telegram-bot", True),
    ("discord", "discord.py", True),
)
...
def _check_command_installation(should_fix: bool, f: Finding) -> None:
    ...
    venv_bin = next(
        (
            c
            for c in (PROJECT_ROOT / n / "bin" / "lemon" for n in ("venv", ".venv"))
            if c.exists()
        ),
        None,
    )
    if venv_bin is None:
        check_warn(
            ...
            "(lemon not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')",
        )
    ...
    link_dir, display = (
        (Path(prefix) / "bin", "$PREFIX/bin")
        if termux
        else (Path.home() / ".local" / "bin", "~/.local/bin")
    )
    link = link_dir / "lemon"
    if link.is_symlink():
        target, expected = link.resolve(), venv_bin.resolve()
        if target == expected:
            return check_ok(f"{display}/lemon → correct target")
        check_warn(
            f"{display}/lemon points to wrong target",
            f"(→ {target}, expected → {expected})",
        )
        if not should_fix:
            return f.issues.append(
                f"Broken symlink at {display}/lemon — run 'lemon doctor --fix'"
            )
        link.unlink()
        verb = "Fixed"
    elif link.exists():  # regular file (wrapper script), not a symlink
        return check_ok(f"{display}/lemon exists (non-symlink)")
    else:
        check_fail(
            f"{display}/lemon not found",
            "(lemon command may not work outside the venv)",
        )
        if not should_fix:
            return f.issues.append(
                f"Missing {display}/lemon symlink — run 'lemon doctor --fix'"
            )
        link_dir.mkdir(parents=True, exist_ok=True)
        verb = "Created"
    link.symlink_to(venv_bin)
    check_ok(f"{verb} symlink: {display}/lemon → {venv_bin}")
    f.fixed += 1
    if verb == "Created" and str(link_dir) not in os.environ.get("PATH", "").split(
        os.pathsep
    ):
        check_warn(
            f"{display} is not on your PATH",
