"""Post-``lemon update`` dependency sync: venv preflight, editable reinstall, lazy refresh,
npm/Desktop rebuilds, self-lock deferral. Names are re-imported by ``update_cmd`` (so
``lemon_cli.update_cmd.<name>`` resolves/monkeypatches); origin helpers are imported lazily."""
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    -e .`` always rewrites console-script shims (Windows: ``lemon.exe`` quarantine, ``os error 32``
...
_UPDATE_CRITICAL_MODULES = "lemon_cli.main", "run_agent", "model_tools", "toolsets"
...
def _critical_module_import_failures(
    ...
    """
    from lemon_cli.update_cmd import _UPDATE_CRITICAL_MODULES, _m
    from lemon_constants import FIRST_PARTY_MODULE_ROOTS
    import secrets
    marker = f"__LEMON_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        ...
        # only our own packages count. Roots come from lemon_constants so the user hint can't drift.
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('lemon_') or %r:\n"
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``lemon tools``."""
    try:
        from lemon_cli import tools_config
        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Lemon AI Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    """Restore allowlisted ``lemon tools`` dependencies (from a pre-rebuild probe) into a rebuilt
    ...
    try:
        ...
        logger.debug("Lemon AI Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Lemon AI Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `lemon update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `lemon update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``plugin.yaml`` (not Lemon AI extras / ``LAZY_DEPS``), so the core reinstall can strip them;
    ...
    try:
        from lemon_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    from lemon_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    from lemon_cli.update_cmd import _m
    from lemon_cli.managed_uv import resolve_uv
    existing = resolve_uv()
    if existing:
        return existing
...
def _npm_lockfile_changed(lemon_root: Path) -> bool:
    from lemon_cli.update_cmd import _m
    ...
    if (web_dir / "package.json").is_file() and not _web_build_toolchain_ready(
        *_web_toolchain_roots(web_dir)):
        return True
    try:
        cache_file = _npm_lock_cache_file(lemon_root)
...
def _npm_lock_cache_file(lemon_root: Path) -> Path:
...
def _record_npm_lockfile_hash(lemon_root: Path) -> None:
    digest = _npm_manifests_digest()
    if digest is None:
        return
    try:
        _npm_lock_cache_file(lemon_root).write_text(digest, encoding="utf-8")
    except OSError:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    """Repair Node deps on the ``commit_count == 0`` path: a failed npm install says "re-run lemon
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `lemon update`.")
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from lemon_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `lemon update`.")
            has_workspace = any(
    ...
    shared_lemon_root = get_default_lemon_root()
    ...
    # Runs before the lockfile-unchanged early return below since that's the common `lemon update` case.
    # See #43564.
    ...
    if not _m()._npm_lockfile_changed(shared_lemon_root):
    ...
    nixos_env = with_lemon_node_path(_m()._nixos_build_env())
    ...
    # The chatty npm-deprecation noise during `lemon update` comes from the *desktop* build, not this step;
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_lemon_root)
    ...
    print("    deps). Fix npm and re-run `lemon update`.")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (_m().PROJECT_ROOT / ".lemon-ai-bootstrap-complete", _m()._update_marker_path())
...
# canonical case: ``lemon_cli.main`` used to import it at startup while resolving external secret sources;
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    a mapped ``.pyd`` -> exit 2, next launch's marker recovery finishes; the ``lemon.exe`` shim
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `lemon` launch will complete it")
...
def _rebuild_desktop_after_update(
    ...
    desktop_build_cmd = [sys.executable, "-m", "lemon_cli.main", "desktop", "--build-only"]
    # Capture the loud build output into update.log; retry once on failure (still-settling
    # rebuild window), then surface the tail. Put Lemon AI-managed Node on PATH: the desktop
    # updater chain loses shell PATH customizations, so a bare-PATH child hits `node: not found`.
    from lemon_constants import with_lemon_node_path
    build_env = with_lemon_node_path()
    for _attempt in range(2):
        ...
        if build_result.returncode == 0:
            break
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `lemon desktop` to retry)")
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv touched by ``sudo pip``/``sudo lemon`` dies mid-update with ``venv/bin/lemon`` already
    ...
    See #83529.
    A later normal ``lemon update`` then dies mid-mutation inside ``uv pip install -e .`` ("Permission
    denied (os error 13)") with ``venv/bin/lemon`` already deleted — the CLI is bricked. Same philosophy as
    the contended-venv gate (#87331): a venv we cannot safely mutate is never mutated at all.
    """
    from lemon_cli.update_cmd import _path_uid
    try:
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Lemon AI broken.")
    print("  This usually happens after running lemon or pip with sudo. Offending paths:")
...
def _dependency_reinstall_url_for_configured_repository() -> str:
    from lemon_cli.update_cmd import _configured_update_repository, _is_default_update_repository, _m
    if _is_default_update_repository():
        return "https://github.com/DangLemon/lemon-agent"
...
def _sync_python_dependencies_after_pull(
    ...
    if _m()._refresh_active_lazy_features(install_prefix, env=lazy_env, features=active_lazy_features):
        ...
        print(
            "  ⚠ Lazy-refresh recovery incomplete — run `lemon` again "
    ...
    if not import_ok:
        ...
        print("    Run `lemon update` again — if it persists, reinstall:")
