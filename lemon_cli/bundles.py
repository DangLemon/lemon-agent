"""Implementation of the ``lemon bundles`` CLI subcommand."""
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  lemon bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``lemon bundles`` argparse tree (called from lemon_cli/main.py, which owns the
    top-level subparser)."""
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `lemon bundles list`")
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``lemon bundles <subcommand>`` to the right handler."""
