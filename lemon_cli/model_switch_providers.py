...
Split out of ``lemon_cli/model_switch.py``; every moved name is re-imported there so
``lemon_cli.model_switch.<name>`` keeps resolving (and monkeypatching) as before."""
...
from lemon_cli.providers import custom_provider_aliases, custom_provider_slug, get_label
from utils import base_url_host_matches

# Log-record parity with the origin module.
logger = logging.getLogger("lemon_cli.model_switch")
...
def _fetch_picker_live_models(
    ...
    from lemon_cli.models import _get_ollama_native_headers, cached_fetch_api_models
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    from lemon_cli.model_switch import list_authenticated_providers
    ...
    def _warm() -> None:
        try:
            from lemon_cli.inventory import load_picker_context
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from lemon_cli.models import update_provider_cache_entry
...
def _iter_builtin_candidates(models_dev_data: dict, excluded: set, seen: set):
    """Yield ``(lemon_id, mdev_id, pconfig, env_vars)`` for section-1 rows.
    ...
    from lemon_cli.auth import PROVIDER_REGISTRY, is_runtime_provider_routable
    from lemon_cli.models import _AGGREGATOR_PROVIDERS
    from lemon_cli.providers import ALIASES
    for lemon_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        alias_target = ALIASES.get(lemon_id)
        if alias_target and alias_target != lemon_id and alias_target in _AGGREGATOR_PROVIDERS:
            continue
        try:
            from providers import get_provider_profile
            prof = get_provider_profile(lemon_id)
            if prof is not None and prof.name != lemon_id:
                continue
        except Exception:
            pass
        if lemon_id.lower() in seen:
            continue
        if lemon_id.lower() in excluded or mdev_id.lower() in excluded:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(lemon_id)
        if (pconfig and pconfig.auth_type != "api_key") or not is_runtime_provider_routable(lemon_id):
            continue
        env_vars = list(pconfig.api_key_env_vars) if pconfig and pconfig.api_key_env_vars else pdata.get("env", [])
        if isinstance(env_vars, list):
            yield lemon_id, mdev_id, pconfig, env_vars


def _auth_store_has_provider(*keys: str) -> bool:
    """True when ``auth.json`` has a ``providers`` entry under any of *keys*."""
    try:
        from lemon_cli.auth import _load_auth_store
...
def _raw_pool_usable(lemon_id: str) -> bool:
    """Section-1 pool check: only consult the pool when auth.json lists a raw entry."""
    try:
        from lemon_cli.auth import _load_auth_store
        store = _load_auth_store()
        if store and store.get("credential_pool", {}).get(lemon_id):
            return _credential_pool_is_usable(lemon_id, raw_pool_present=True)
    except Exception:
        pass
    return False
...
def _overlay_has_env_creds(pid: str, lemon_slug: str, overlay, read_env) -> bool:
...
def _live_or_curated_ids(slug: str, curated: dict, *fallback_keys: str, merge_models_dev: bool = True) -> list:
    """``cached_provider_model_ids`` (the SAME disk-cached list ``lemon model`` builds), falling
    back to the curated list (merged with models.dev for preferred providers) when live is empty."""
    from lemon_cli.models import _MODELS_DEV_PREFERRED, _merge_with_models_dev, cached_provider_model_ids
...
def _nous_picker_model_ids(curated: dict, force_fresh_nous_tier: bool) -> list:
    ...
    try:
        from lemon_cli.models_pricing import get_pricing_for_provider
        from lemon_cli.models import (
            check_nous_free_tier,
            union_with_portal_free_recommendations,
            union_with_portal_paid_recommendations,
        )
        from lemon_cli.auth import get_provider_auth_state
...
def _absorb_entry_models(grp: dict, entry: dict, active_model: Any) -> None:
    ...
    from lemon_cli.model_switch import _declared_model_ids, _entry_models_discovered, _models_config_is_allowlist
...
def _display_prefix(name: str) -> str:
    """Text before the per-model separator Lemon AI's own writer uses ("—" / " - ")."""
...
def _collect_authed_provider_slugs(
    ...
    (heavier detection)."""
    from lemon_cli.model_switch import _scoped_key_env
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from lemon_cli.auth import PROVIDER_REGISTRY
    from lemon_cli.providers import LEMON_OVERLAYS
    from lemon_cli.models import CANONICAL_PROVIDERS
    ...
    for lemon_id, _mdev_id, _pconfig, env_vars in _iter_builtin_candidates(models_dev_data, excluded_set, seen):
        if _any_env(env_vars, _scoped_key_env) or _raw_pool_usable(lemon_id):
            _emit(lemon_id, lemon_id)

    mdev_to_lemon = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in LEMON_OVERLAYS.items():
        lemon_slug = mdev_to_lemon.get(pid, pid)
        if _skip(seen, excluded_set, pid, lemon_slug) or overlay.auth_type == "aws_sdk":
            continue
        if (
            _overlay_has_env_creds(pid, lemon_slug, overlay, _scoped_key_env)
            or _auth_store_has_provider(pid, lemon_slug) or _pool_usable(lemon_slug)):
            _emit(lemon_slug, pid, lemon_slug)
...
class _PickerBuild:
    ...
    models.dev, 2 Lemon AI-only overlays, 2b canonical providers missed by 1/2, 3 ``providers:``
    entries + 3b the bare active custom endpoint, 4 ``custom_providers:`` entries. Row-builder
    imports of ``lemon_cli.auth/models`` stay lazy so tests can patch those modules."""
...
def _lap_builtin_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    ...
    from lemon_cli.model_switch import _declared_model_ids
    from agent.models_dev import get_provider_info
    for lemon_id, mdev_id, pconfig, env_vars in _iter_builtin_candidates(data, b.excluded, b.seen_slugs):
        if not (_any_env(env_vars) or _raw_pool_usable(lemon_id)):
            continue
        model_ids = _live_or_curated_ids(lemon_id, b.curated)
        ...
        configured = user_providers.get(lemon_id) if isinstance(user_providers, dict) else None
        ...
        b.add_builtin_row(
            lemon_id, display_name, b.current_provider in (lemon_id, mdev_id), model_ids, "built-in")


def _overlay_has_creds(b: _PickerBuild, pid: str, lemon_slug: str, overlay) -> bool:
    ...
    if overlay.auth_type == "aws_sdk":
        has_creds = _has_aws_sdk_creds_for_listing(lemon_slug, b.current_provider)
    else:
        has_creds = _overlay_has_env_creds(pid, lemon_slug, overlay, os.environ.get)
    ...
    if not has_creds and overlay.auth_type == "external_process":
        try:
            ...
            _ext_status = get_auth_status(lemon_slug) or {}
    ...
    has_creds = has_creds or _auth_store_has_provider(pid, lemon_slug)
    if not has_creds:
        # Full auto-seeding pool check catches external stores (Codex CLI ~/.codex/auth.json)
        # not yet in auth.json.
        try:
            if _credential_pool_is_usable(lemon_slug):
                has_creds = True
            elif b.for_picker:
                ...
                try:
                    from agent.credential_pool import load_pool
                    has_creds = load_pool(lemon_slug).has_credentials()
                except Exception:
                    pass
        except Exception as exc:
            logger.debug("Credential pool check failed for %s: %s", lemon_slug, exc)
    if not has_creds and lemon_slug == "anthropic":
        ...
        try:
            from agent.anthropic_credentials import read_claude_code_credentials, read_lemon_oauth_credentials
            lemon_creds = read_lemon_oauth_credentials()
            cc_creds = read_claude_code_credentials()
            if (lemon_creds and lemon_creds.get("accessToken")) or (cc_creds and cc_creds.get("accessToken")):
                has_creds = True
        except Exception as exc:
...
def _lap_overlay_rows(b: _PickerBuild, data: dict) -> None:
    """Section 2: Lemon AI-only providers (nous, openai-codex, copilot, opencode-go, ...)."""
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from lemon_cli.providers import LEMON_OVERLAYS

    # LEMON_OVERLAYS keys may be models.dev IDs ("github-copilot") while config.yaml uses
    # Lemon AI IDs ("copilot").
    mdev_to_lemon = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in LEMON_OVERLAYS.items():
        lemon_slug = mdev_to_lemon.get(pid, pid)
        if _skip(b.seen_slugs, b.excluded, pid, lemon_slug):
            continue
        if not _overlay_has_creds(b, pid, lemon_slug, overlay):
            continue
        if lemon_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            from lemon_cli.models import cached_provider_model_ids
            model_ids = cached_provider_model_ids(lemon_slug)
        elif overlay.auth_type == "aws_sdk":
            model_ids = _aws_live_or_curated_ids(lemon_slug, b.curated, lemon_slug, pid)
        elif lemon_slug == "nous":
            model_ids = _nous_picker_model_ids(b.curated, b.force_fresh_nous_tier)
        else:
            model_ids = _live_or_curated_ids(lemon_slug, b.curated, lemon_slug, pid)
        b.add_builtin_row(
            lemon_slug, get_label(lemon_slug), b.current_provider in (lemon_slug, pid), model_ids, "lemon")
...
def _lap_user_provider_rows(b: _PickerBuild, user_providers: dict) -> None:
    ...
    from lemon_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from lemon_cli.config import coerce_provider_id, is_provider_enabled
...
def _lap_custom_provider_rows(b: _PickerBuild, custom_providers: list) -> None:
    ...
    their own rows."""
    from lemon_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from lemon_cli.config import coerce_provider_id
...
def _build_curated_lists(current_provider: str, current_base_url: str, current_model: str) -> dict[str, list[str]]:
    """Curated model lists keyed by lemon provider id, plus the dynamic ones (nous manifest,
    Ollama Cloud, LM Studio live probe)."""
    from lemon_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS, get_curated_nous_model_ids
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # Remote manifest so new Portal models surface without a release; in-repo snapshot fallback.
    curated["nous"] = get_curated_nous_model_ids()
    if "ollama-cloud" not in curated:
        from lemon_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (os.environ.get("LM_API_KEY") or os.environ.get("LM_BASE_URL") or is_current_lmstudio):
        from lemon_cli.models_local import fetch_lmstudio_models
...
def list_authenticated_providers(
    ...
    ("built-in", "lemon", "canonical", "user-config", "model-config").
    ...
    from lemon_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # as (lemon_id / mdev_id / canonical slug).
    b = _PickerBuild(
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    try:
        from lemon_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    OpenRouter's list is replaced with :func:`lemon_cli.models.fetch_openrouter_models` (curated
    ...
    from lemon_cli.model_switch import list_authenticated_providers
    from lemon_cli.models import fetch_openrouter_models
