"""Slash command registry for the Lemon AI CLI and gateway.
...
class CommandDef:
    ...
    # Key in ``lemon_cli.slash_exec.EXECUTORS`` (a string, not a callable: keeps this module
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Lemon AI config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Lemon AI works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .lemon-ai/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey Lemon AI' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]", subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Lemon AI is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.lemon-ai/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Lemon AI to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Lemon AI version", "Info", aliases=("v",),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``lemon skills list`` and the agent's ``/skill-name`` dispatch but
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'discord_skill_commands_by_category': ('lemon_cli.commands_platforms', 'discord_skill_commands_by_category'),
    'slack_app_manifest': ('lemon_cli.commands_platforms', 'slack_app_manifest'),
    'slack_native_slashes': ('lemon_cli.commands_platforms', 'slack_native_slashes'),
    'slack_subcommand_map': ('lemon_cli.commands_platforms', 'slack_subcommand_map'),
    'telegram_bot_commands': ('lemon_cli.commands_platforms', 'telegram_bot_commands'),
    'telegram_menu_commands': ('lemon_cli.commands_platforms', 'telegram_menu_commands'),
    'telegram_menu_max_commands': ('lemon_cli.commands_platforms', 'telegram_menu_max_commands'),
}
