...
Split out of ``lemon_cli.models``. OpenRouter and
...
``lemon_cli.models`` — tests reset them there — and are read/written by attribute name.
...
# that must never block on HTTP — so without a disk copy, `lemon -p`, a cron job, or a freshly
...
def _reasoning_caps_disk_path() -> Path:
    ...
    return get_lemon_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._LEMON_USER_AGENT})
...
class _CapsSource:
    """One catalog's cache slots on ``lemon_cli.models`` plus how to name its URL.
