"""Shell completion script generation for lemon CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_lemon_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Lemon AI bash completion
# Add to ~/.bashrc:
#   eval "$(lemon completion bash)"

_lemon_profiles() {{
    local profiles_dir="$HOME/.lemon-ai/profiles"
...
_lemon_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_lemon_profiles)" -- "$cur"))
        return
    fi
...
complete -F _lemon_completion lemon
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef lemon
# Lemon AI zsh completion
# Add to ~/.zshrc:
...
_lemon_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.lemon-ai/profiles" ]]; then
        profiles+=($HOME/.lemon-ai/profiles/*(N/:t))
    fi
...
_lemon() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_lemon_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'lemon command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Lemon AI fish completion",
        "# Add to your config:",
        "#   lemon completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __lemon_profiles",
        "    echo default",
        "    if test -d $HOME/.lemon-ai/profiles",
        "        for d in $HOME/.lemon-ai/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c lemon -f -s p -l profile"
        " -d 'Profile name' -xa '(__lemon_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c lemon -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__lemon_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
