#!/usr/bin/env python3
"""Lemon AI CLI - Main entry point.

Usage:
    lemon                     # Interactive chat (default)
    lemon chat / gateway / setup / status / cron / doctor / update / ...
    lemon --version           # Show version and update status
    lemon <cmd> --help        # Per-command help
"""

# lemon_bootstrap must be the very first import — it sets up UTF-8 stdio on
# Windows (no-op on POSIX). Guarded: after a ``git pull`` / interrupted
# ``lemon update`` the editable install's ``.pth`` may not list it yet; crashing
# here would block ``lemon update``.
try:
    import lemon_bootstrap  # noqa: F401
...
from lemon_cli._subprocess_compat import suppress_platform_ver_console
...
# Inline path math so ``python lemon_cli/main.py`` (script mode: sys.path[0]
# is lemon_cli/, not the repo root) can import lemon_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from lemon_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below. A prior
# ``lemon update`` may have left a recovery marker with a core package wiped;
# the lemon_cli.config/env_loader imports further down would then crash before
...
# PID" class it exists for. ``lemon_startup_watchdog`` is stdlib-only so it
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from lemon_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from lemon_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Cosmetic: show 'lemon' instead of 'python3.xx' in ps/top/htop.

    Order: opt-in ``setproctitle`` dep; ctypes ``prctl(PR_SET_NAME)`` (Linux,
    15-char limit); ``pthread_setname_np`` (macOS — lldb/top only, not ``ps
    aux``); no-op on Windows (the .exe is already ``lemon.exe``). Never fatal.
    """
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("lemon")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"lemon", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"lemon")
    except Exception:
        pass
...
# lemon_cli.config is importable. Cached so early callers don't re-parse YAML.
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("LEMON_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".lemon-ai", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    Precedence: ``--cli`` wins, then ``--tui``/``LEMON_TUI=1``, then a
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("LEMON_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `resetTerminalModes()` is the later cousin. ``LEMON_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("LEMON_TUI_NO_EARLY_DISABLE") == "1":
        return
...
# ``lemon --version`` is answered before config/logging imports.
...
from lemon_cli.subcommands.cron import build_cron_parser
from lemon_cli.subcommands.sync import build_sync_parser
from lemon_cli.subcommands.gateway import build_gateway_parser
from lemon_cli.subcommands.profile import build_profile_parser
from lemon_cli.subcommands.model import build_model_parser
from lemon_cli.subcommands.setup import build_setup_parser

from lemon_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
from lemon_cli.subcommands.slack import build_slack_parser
from lemon_cli.subcommands.login import build_login_parser
from lemon_cli.subcommands.logout import build_logout_parser
from lemon_cli.subcommands.auth import build_auth_parser
from lemon_cli.subcommands.status import build_status_parser
from lemon_cli.subcommands.pause import build_pause_parser
from lemon_cli.subcommands.webhook import build_webhook_parser
from lemon_cli.subcommands.hooks import build_hooks_parser
from lemon_cli.subcommands.doctor import build_doctor_parser
from lemon_cli.subcommands.verify import build_verify_parser
from lemon_cli.subcommands.security import build_security_parser
from lemon_cli.subcommands.approvals import build_approvals_parser
from lemon_cli.subcommands.dump import build_dump_parser
from lemon_cli.subcommands.debug import build_debug_parser
from lemon_cli.subcommands.backup import build_backup_parser
from lemon_cli.subcommands.import_cmd import build_import_cmd_parser
from lemon_cli.subcommands.import_agent import build_import_agent_parser
from lemon_cli.subcommands.config import build_config_parser
from lemon_cli.subcommands.skin import build_skin_parser
from lemon_cli.subcommands.console import build_console_parser
from lemon_cli.subcommands.update import build_update_parser
from lemon_cli.subcommands.uninstall import build_uninstall_parser
from lemon_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
from lemon_cli.subcommands.gui import build_gui_parser
from lemon_cli.subcommands.logs import build_logs_parser
from lemon_cli.subcommands.prompt_size import build_prompt_size_parser
from lemon_cli.subcommands.memory import build_memory_parser
from lemon_cli.subcommands.acp import build_acp_parser
from lemon_cli.subcommands.tools import build_tools_parser
from lemon_cli.subcommands.insights import build_insights_parser
from lemon_cli.subcommands.monitoring import build_monitoring_parser
from lemon_cli.subcommands.skills import build_skills_parser
from lemon_cli.subcommands.pairing import build_pairing_parser
from lemon_cli.subcommands.plugins import build_plugins_parser
from lemon_cli.subcommands.mcp import build_mcp_parser
from lemon_cli.subcommands.claw import build_claw_parser
from lemon_cli.subcommands.moa import build_moa_parser
from lemon_cli.subcommands.fallback import build_fallback_parser
from lemon_cli.subcommands.worktree import build_worktree_parser
from lemon_cli.subcommands.browser import build_browser_parser
from lemon_cli.subcommands.secrets import build_secrets_parser
from lemon_cli.subcommands.egress import build_egress_parser
from lemon_cli.subcommands.migrate import build_migrate_parser
from lemon_cli.subcommands.checkpoints import build_checkpoints_parser
from lemon_cli.subcommands.bundles import build_bundles_parser
from lemon_cli.subcommands.curator import build_curator_parser
from lemon_cli.subcommands.pets import build_pets_parser
from lemon_cli.subcommands.journey import build_journey_parser
from lemon_cli.subcommands.computer_use import build_computer_use_parser
from lemon_cli.subcommands.sessions import build_sessions_parser
from lemon_cli.subcommands.completion import build_completion_parser
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: 'lemon {command_name}' requires an interactive terminal.\n"
...
# Profile override — MUST happen before any lemon module import: many modules
# cache LEMON_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# LEMON_HOME set, and the flag stripped so argparse never sees it. Falls back
# to ~/.lemon-ai/active_profile for the sticky default.
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors lemon_cli.profiles._PROFILE_ID_RE


def _inside_mcp_add_args(argv: list, index: int) -> bool:
    """True once argv reaches `lemon mcp add ... --args <command argv>`.
    ...
    ``--profile``), not to Lemon AI' own profile selector.
    """
    try:
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    Historically the flag worked even after the subcommand (`lemon chat -p
    ...
    """
    from lemon_cli._parser import top_level_value_flag_sets
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    """Resolve `sudo lemon -p <name>` against the invoking user's home.
    ...
    try:
        ...
        candidate = Path(pwd.getpwnam(sudo_user).pw_dir) / ".lemon-ai" / "profiles" / name
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    ``-p <name>`` or pin LEMON_HOME to the profile dir; a bare invocation
    means "the root LEMON_HOME profile". If a supervised default-profile
    child read active_profile, switching the active profile (dashboard,
    ``lemon profile use``) would silently redirect the default gateway into
    ...
    LEMON_SUPERVISED_CHILD (systemd unit / launchd plist / Windows task),
    LEMON_S6_SUPERVISED_CHILD (legacy s6 container), INVOCATION_ID (systemd
    ...
    self-hosted CI runners), LEMON_GATEWAY_EXTERNAL_SUPERVISOR (explicit
    opt-in). XPC_SERVICE_NAME is deliberately NOT consulted: interactive macOS
    terminals set it too.
    """
    if os.environ.get("LEMON_SUPERVISED_CHILD") or os.environ.get("LEMON_S6_SUPERVISED_CHILD"):
        return True
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set LEMON_HOME before imports."""
    argv = sys.argv[1:]
    profile_name, consume, profile_index = _scan_profile_flag(argv)

    # LEMON_HOME already set with no explicit flag: trust it only when it
    # points at a specific profile dir ("profiles" as immediate parent). If it
    # points at the lemon root (systemd hardcodes LEMON_HOME=/root/.lemon-ai)
    # we must still read active_profile — the user may have run
    # `lemon profile use` and the gateway should honour it (#22502).
    lemon_home_env = os.environ.get("LEMON_HOME", "")
    if profile_name is None and lemon_home_env and Path(lemon_home_env).parent.name == "profiles":
        return
    ...
    if profile_name is None and not _under_gateway_supervisor(argv):
        try:
            ...
            active_path = get_default_lemon_root() / "active_profile"
    ...
    if profile_name is None:
        return
    try:
        from lemon_cli.profiles import resolve_profile_env

        lemon_home = resolve_profile_env(profile_name)
    except FileNotFoundError as exc:
        lemon_home = _resolve_sudo_user_profile_env(profile_name)
        if not lemon_home:
        ...
        # A bug in profiles.py must NEVER prevent lemon from starting
        print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
        return
    os.environ["LEMON_HOME"] = lemon_home
...
# Windows launcher self-heal — the ``lemon`` command is a COPY of the venv
# console script staged into the managed bin dir (outside the checkout, since
# ``lemon update``'s autostash once swept ``<checkout>\bin`` copies off disk;
...
# app's ``python -m lemon_cli.main`` spawn. Gates fail toward inaction. Sits
# AFTER the profile override on purpose — no lemon module may import before
...
# ``<checkout>\bin``, where ``lemon update``'s autostash (``git stash push --include-untracked``) swept
# them off disk; with the desktop updater's ``--keep-stash`` nothing restored them and ``lemon`` stopped
...
if sys.platform == "win32":
    try:
        from lemon_cli import _install_repair as _install_repair_mod

        _install_repair_mod.ensure_windows_bin_launchers(_bootstrap_root)
    except Exception:
        pass

# Load .env from ~/.lemon-ai/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from lemon_cli.config import get_lemon_home
from lemon_cli.env_loader import load_lemon_dotenv
...
# Bridge security.redact_secrets → LEMON_REDACT_SECRETS BEFORE lemon_logging
...
try:
    ...
    # lemon_logging and later raw reads: 3-4 config.yaml parses become one.
    from lemon_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_lemon_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "LEMON_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["LEMON_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from lemon_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from lemon_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if lemon_constants not importable yet

import logging
import threading
from datetime import datetime

from lemon_cli import __version__, __release_date__

from lemon_cli.model_setup_flows import (
...
from lemon_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from lemon_cli.main_platform_setup import (
    cmd_slack,
    cmd_sync,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from lemon_cli.main_dashboard import (
...
from lemon_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from lemon_cli.main_provider_setup import (
...
from lemon_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from lemon_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from lemon_cli.main_desktop import (
    cmd_gui,
)
from lemon_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _desktop_build_needed,
    _desktop_dist_exists,
    _desktop_macos_relaunchable_fixup,
    _desktop_packaged_executable,
)
from lemon_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from lemon_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from lemon_cli.main_tui_launch import (
    _launch_tui,
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_lemon_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("LEMON_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("LEMON_TERMUX_PREFETCH_UPDATES") == "1"


def _dotenv_has_provider_key(env_file: Path, provider_env_vars: set) -> bool:
    """True if ~/.lemon-ai/.env assigns a non-empty value to any provider key."""
...
def _auth_store_logged_in(auth_file: Path, registry, strict_profile_scope: bool) -> bool:
    ...
    from lemon_cli.auth import get_auth_status
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    ...
    """
    from lemon_cli.config import DEFAULT_CONFIG, get_env_path, get_lemon_home, load_config
    from lemon_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    _has_lemon_config = _model_name and _model_name != DEFAULT_CONFIG.get("model", "")
    ...
    if _auth_store_logged_in(get_lemon_home() / "auth.json", PROVIDER_REGISTRY, strict_profile_scope):
        return True
    ...
    # Claude Code OAuth credentials count only once Lemon AI is explicitly
    # configured — having Claude Code installed isn't consent to use its tokens.
    if _has_lemon_config and not strict_profile_scope:
        try:
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from lemon_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_db():
    """Yield a ``SessionDB`` (lazy import, so test patches on ``lemon_state``
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``lemon -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, lemon_bin
        cli_args: the original CLI arguments (everything after 'lemon')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    lemon_bin = container_info["lemon_bin"]
    ...
    if _probe_container(inspect_cmd, backend).returncode != 0:
        ...
        if not sudo_path:
            print(
                ...
                f"The container may be running under root. Try: sudo lemon {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if _probe_container(cmd_prefix[:2] + inspect_cmd, backend, via_sudo=True).returncode != 0:
            print(
                ...
                f"  }}];\n"
                f"\n"
                f"Or run: sudo lemon {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'lemon sessions list' to see available sessions, or "
            ...
            try:
                from lemon_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from lemon_cli.foreign_sessions import import_foreign_session, pick_foreign_session
    ...
    print(f"  (later: lemon --resume {_imported_id})")
...
def _resolve_chat_session_args(args, use_tui: bool) -> None:
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'lemon sessions list' to see available sessions.")
            sys.exit(1)
...
def _warn_retired_xai_models() -> None:
    ...
    try:
        ...
        from lemon_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'lemon doctor' for details.\033[0m\n\n")
    except Exception:
        pass
...
def _start_chat_background_prefetch() -> None:
    ...
    The ONE exception is an unseeded ~/.lemon-ai/skills: there the banner
    ...
    if _termux_should_prefetch_update_check():
        try:
            from lemon_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from lemon_cli.config import get_lemon_home
            skills_dir = Path(get_lemon_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import lemon_cli.banner as _banner_mod
...
def _first_run_setup_guard(args) -> None:
    """No provider configured: offer `lemon setup` (TTY) or exit 1 with guidance."""
    print()
    print(
        "It looks like Lemon AI isn't configured yet -- no API keys or providers found."
    )
    print()
    print("  Run:  lemon setup")
    print()

    from lemon_cli.setup import (
    ...
    if reply in {"", "y", "yes"}:
        ...
        return
    print()
    print("You can run 'lemon setup' at any time to configure.")
    sys.exit(1)
...
def cmd_chat(args):
    ...
    if getattr(args, "ignore_rules", False):
        os.environ["LEMON_IGNORE_RULES"] = "1"
    # --source: tag session source for filtering (e.g. 'tool' for integrations)
    if getattr(args, "source", None):
        os.environ["LEMON_SESSION_SOURCE"] = args.source
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older lemon_cli.config) crash
        # here — e.g. missing resolve_turn_limit / split_model_config_default
        # (#96900). The agent-setup mixin prints this hint too late: LemonCLI
        # construction already failed. Fast-chat launch also goes through
        # cmd_chat, so this one catch covers `lemon` / `lemon chat`.
        from lemon_constants import emit_partial_update_hint
...
def cmd_gateway(args):
    ...
    from lemon_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from lemon_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    """A ``lemon <cmd>`` handler that hands ``args`` to ``<module>.<attr>``.
...
cmd_setup = _forward_command("cmd_setup", "lemon_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "lemon_cli.auth", "login_command", doc='Authenticate Lemon AI CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "lemon_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "lemon_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "lemon_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "lemon_cli.cron", "cron_command", doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "lemon_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "lemon_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "lemon_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "lemon_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "lemon_cli.doctor", "run_doctor", doc='Check configuration and dependencies.')
cmd_dump = _forward_command("cmd_dump", "lemon_cli.dump", "run_dump", doc='Dump setup summary for support/debugging.')
cmd_debug = _forward_command("cmd_debug", "lemon_cli.debug", "run_debug", doc='Debug tools (share report, etc.).')
cmd_skin = _forward_command("cmd_skin", "lemon_cli.skin_cmd", "skin_command", doc='Skin management (list / use / set).')
cmd_import = _forward_command("cmd_import", "lemon_cli.backup", "run_import", doc='Restore a Lemon AI backup from a zip file.')
cmd_dashboard_register = _forward_command("cmd_dashboard_register", "lemon_cli.dashboard_register", "cmd_dashboard_register", doc='Register a self-hosted dashboard OAuth client with Nous Portal.')
cmd_gateway_enroll = _forward_command("cmd_gateway_enroll", "lemon_cli.gateway_enroll", "cmd_gateway_enroll", doc='Enroll a self-hosted gateway with a relay connector.')
cmd_prompt_size = _forward_command("cmd_prompt_size", "lemon_cli.prompt_size", "cmd_prompt_size", doc='Show a byte/char breakdown of the system prompt + tool schemas.')
cmd_pairing = _forward_command("cmd_pairing", "lemon_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "lemon_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "lemon_cli.mcp_config", "mcp_command")
cmd_claw = _forward_command("cmd_claw", "lemon_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "lemon_cli.agent_import", "import_agent_command")
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from lemon_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from lemon_cli.setup import run_setup_action_with_navigation
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from lemon_cli.auth import AuthError, format_auth_error, resolve_provider
    from lemon_cli.config import get_compatible_custom_providers, get_env_value
    from lemon_cli.providers import custom_provider_aliases, resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            print(
                f"Warning: Unknown provider '{effective_provider}'. Check 'lemon model' for "
                "available providers, or run 'lemon doctor' to diagnose config "
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``lemon model``) and the setup wizard
    ...
    effective_provider = config_provider or os.getenv("LEMON_INFERENCE_PROVIDER") or "auto"
    ...
    from lemon_cli.models import _PROVIDER_LABELS
    ...
    # lemon_cli.main keep intercepting.
    ...
    # "custom") leaves a stale OPENAI_BASE_URL in ~/.lemon-ai/.env that poisons
...
# ``lemon_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on lemon_cli.main; importing update_cmd eagerly would cost every ``lemon``
...
_FROZEN_UPDATER_SURFACE: dict[str, tuple[str, ...]] = {
    "lemon_cli.update_cmd": (
        ...
        "_print_parked_branch_skip_warning", "_purge_stale_lemon_modules",
    ...
    "lemon_cli.dashboard_procs": (
        "_detect_concurrent_lemon_instances", "_kill_stale_dashboard_processes",
    ),
}
...
def cmd_verify(args):
    ...
    from lemon_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `lemon security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from lemon_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `lemon approvals <subcmd>`."""
    from lemon_cli.approvals_suggest import approvals_command
...
def cmd_config(args):
    ...
    from lemon_cli.config import config_command
...
def cmd_backup(args):
    """Back up Lemon AI home directory to a zip file."""
    from lemon_cli import backup

    (backup.run_quick_backup if getattr(args, "quick", False) else backup.run_backup)(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    # Shared with the `lemon --version` pre-import fast path.
...
def cmd_uninstall(args):
    """Uninstall Lemon AI (or just the Chat GUI with --gui).
    ...
    if getattr(args, "gui_summary", False):
        from lemon_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from lemon_cli.uninstall import run_gui_uninstall
    ...
    from lemon_cli.uninstall import run_uninstall
...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    try:
        ...
        from lemon_cli.update_receipt import finalize_pending_update_receipt
...
def _update_preflight_handled(args) -> bool:
    ...
    from lemon_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Lemon AI")
        return True
    ...
    if getattr(args, "plan", False):
        # Read-only plan phase (#91277 Phase 2): inventory every running Lemon AI runtime across profiles, its
        # supervisor, and its running code version — without mutating anything. Safe on a live fleet.
        from lemon_cli.update_inventory import (
    ...
    from lemon_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        from lemon_cli.update_cmd import _cmd_update_check
...
def cmd_update(args):
    """Update Lemon AI: hangup protection + update lock around ``_cmd_update_impl``."""
    ...
    from lemon_cli.update_lock import (
    ...
    from lemon_cli.update_cmd import _cmd_update_impl
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    ``lemon -c Pokemon Agent Dev`` → ``['-c', 'Pokemon Agent Dev']``; tokens
...
from lemon_cli.profile_cmd import cmd_profile
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        if not _find_stale_dashboard_pids():
            print("No lemon dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `lemon update`;
        # it prints outcomes itself. Exit 1 only if every pid was unkillable.
        from lemon_cli.dashboard_procs import _kill_stale_dashboard_processes
...
def _dashboard_validate_serve_args(args, headless_backend, token_file):
    ...
    # `lemon serve` is headless/non-interactive: fail closed on a corrupt
    ...
    if token_file and not headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with lemon serve")
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    Desktop Electron spawns its backend with LEMON_DESKTOP=1 plus
    LEMON_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    LEMON_SERVE_HEADLESS=1). A shell inheriting those then running
    `lemon dashboard` would serve the desktop renderer ("Desktop IPC bridge
    ...
    (LEMON_DESKTOP=1) keeps its dist. Headless `serve` re-sets
    LEMON_SERVE_HEADLESS itself.
    """
    if os.environ.get("LEMON_DESKTOP") != "1":
        if _is_electron_packaged_web_dist(os.environ.get("LEMON_WEB_DIST", "")):
            os.environ.pop("LEMON_WEB_DIST", None)
    if not headless_backend:
        os.environ.pop("LEMON_SERVE_HEADLESS", None)
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    # the same logs directory as every other Lemon AI surface.
    try:
        from lemon_logging import setup_logging as _setup_logging_gui
    ...
    try:
        ...
        # #61115, #65696.
        from lemon_cli.config import apply_terminal_config_to_env
    ...
    mcp_discovery_after_bind = headless_backend and os.environ.get("LEMON_DESKTOP") == "1"
    if not mcp_discovery_after_bind:
        try:
            from lemon_cli.mcp_startup import start_background_mcp_discovery
...
def cmd_dashboard(args):
    ...
    from lemon_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from lemon_cli import completion
...
def cmd_logs(args):
    """View and filter Lemon AI log files."""
    from lemon_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Lemon AI command console."""
    from lemon_cli.console_engine import run_console_repl
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    """
    first = _first_positional_argv()  # None = bare ``lemon`` → chat
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    on import; ``discover_plugins()`` alone leaves ``lemon photon`` failing
    ...
    not import it, so the CLI registration never happens and ``lemon photon`` fails with argparse ``invalid
    choice`` (issue #54678).
    """
    if not command_name:
        return
...
def _is_tui_chat_launch(args) -> bool:
    if getattr(args, "tui", False) or os.environ.get("LEMON_TUI") == "1":
        return True
    # The chat path decides TUI-vs-classic via _resolve_use_tui (--cli/--tui
    # flags, TTY gate, LEMON_TUI env, display.interface config). Bare
    # `lemon`/`lemon chat` with a TUI display config was previously missed
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo chokepoint: LEMON_YOLO_MODE must be set before any discovery
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from lemon_cli.plugins import start_background_plugin_discovery
    ...
    try:
        from lemon_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from lemon_cli.mcp_startup import get_mcp_server_filter
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["LEMON_SAFE_MODE"] = "1"
    os.environ["LEMON_IGNORE_USER_CONFIG"] = "1"
    os.environ["LEMON_IGNORE_RULES"] = "1"
...
def _light_chat_parser():
    ...
    from lemon_cli._parser import build_top_level_parser
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("LEMON_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
    ...
    try:
        from lemon_cli.config import get_container_exec_info
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("LEMON_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("LEMON_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["LEMON_DEFER_AGENT_STARTUP"] = "1"
            os.environ["LEMON_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["LEMON_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `lemon --tui` is the hot path on phones and the TUI immediately execs
...
def _advertise_agent_env() -> None:
    ...
    (``lemon-agent``) — matching is exact. ``LEMON_AGENT`` is the
    Lemon AI-specific marker. setdefault: never clobber an outer harness.
    ...
    harness that spawned them. Lemon AI running inside another agent's terminal).
    """
    os.environ.setdefault("AI_AGENT", "lemon-agent")
    os.environ.setdefault("LEMON_AGENT", "true")
...
def _register_plugin_cli_commands(subparsers) -> None:
    ...
    try:
        ...
        from lemon_cli.plugins import discover_plugins, get_plugin_manager
...
def _cmd_sessions_lazy(args, **kwargs):
    """``lemon sessions`` handler; sessions_cmd imports only when the subcommand runs."""
    from lemon_cli.sessions_cmd import cmd_sessions

    return cmd_sessions(args, **kwargs)


def _build_cli_parser():
    """Build the full ``lemon`` argparse tree -> ``(parser, subparsers)``.

    Registration ORDER is the ``lemon --help`` order; keep it stable. Groups
    live in ``lemon_cli/subcommands/<group>.py`` with handlers injected so
    ...
    # OUTBOUND egress firewall; ``lemon proxy`` (gateway group) is the INBOUND one.
    ...
    from lemon_cli.send_cmd import register_send_subparser
    ...
    from lemon_cli.subcommands.peer import build_peer_parser
    build_peer_parser(subparsers)

    from lemon_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    from lemon_cli.kanban import build_parser as _build_kanban_parser
    _build_kanban_parser(subparsers).set_defaults(func=cmd_kanban)

    from lemon_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # "desktop" is canonical (Lemon AI-Setup.exe tells users to run it, so it
...
def _parse_cli_args(parser, subparsers, argv):
    ...
    routing; if that fails (``lemon -c model`` — 'model' is the session name)
...
def main():
    """Main entry point for lemon CLI."""
    _set_process_title()
    _advertise_agent_env()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from lemon_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``lemon.exe.old.*`` quarantine files from previous Windows
    # updates (see ``_quarantine_running_lemon_exe``). No-op elsewhere.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass
    ...
    # Self-heal a venv left half-built by an interrupted ``lemon update``, and
    ...
    # substring match is deliberately loose: over-matching (``lemon skills
    # install update``) only defers recovery one launch; under-matching
    # (``lemon -p work update``) would race. Never raises.
    # See #95294.
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from lemon_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('lemon_cli.cli_output', 'line_input'),
}
