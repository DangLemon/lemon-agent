"""Helpers for loading Lemon AI .env files consistently across entrypoints."""
...
# Once-per-process guards: load_lemon_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `lemon
...
# LEMON_HOME paths already pulled external secrets for: load_lemon_dotenv() runs at import time from
...
# Behavioral routing keys a parent Lemon AI process injects into child env that silently redirect a profile
...
def get_secret_source_values(lemon_home: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``lemon_home``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(lemon_home).resolve()), {}))


def hydrate_profile_secret_sources(lemon_home: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(lemon_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_lemon_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `lemon setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from lemon_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_lemon_dotenv(
    *,
    lemon_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
    load_external_secrets: bool = True,
) -> list[Path]:
    """Load Lemon AI env files: ``~/.lemon-ai/.env`` overrides stale shell exports; project ``.env`` is a dev
    fallback that only fills gaps when the user env exists (and overrides shell vars when it does not)."""
    home_path = Path(lemon_home or os.getenv("LEMON_HOME", Path.home() / ".lemon-ai"))
    ...
    if is_multiplex_active() and get_lemon_home_override() is not None:
        ...
        if load_external_secrets:
            from lemon_cli import _early_recovery
    ...
    # Windows (#73381, #86735). 2. A fresh ``lemon update`` retry just completed a deferred dependency
    ...
    # ~/.lemon-ai/.env win on every reload and flip the backend mid-session in long-lived processes.
    ...
    # with override=True — so a stale TERMINAL_ENV=docker left in ~/.lemon-ai/.env (e.g. written by an older
    # `lemon setup` before the user switched terminal.backend in config.yaml) silently wins again on every
    # reload. Startup launchers bridge config→env once, but long-lived processes (gateway per-turn reload,
    # cron standalone runs) call load_lemon_dotenv() repeatedly and used to flip the effective backend back
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_lemon_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE Lemon AI reads credentials; failures never block startup. Precedence/conflicts/provenance
    ...
    # load_lemon_dotenv() calls per startup don't re-fetch / re-print — error retries within one process
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and lemon_logging reuse one parse instead of 3-4.
    if home_path == _process_lemon_home():
        try:
...
def _process_lemon_home() -> Path:
    """The LEMON_HOME the shared config cache is keyed to."""
    try:
