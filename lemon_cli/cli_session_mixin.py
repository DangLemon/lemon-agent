...
Mixin bound onto ``LemonCLI`` via the MRO. cli.py-internal symbols are imported LAZILY
...
def _reset_model_to_config_default(cli, silent: bool) -> None:
    ...
    if not _config_model or _config_model == getattr(cli, "model", None):
        return
    try:
        from lemon_cli.model_switch import switch_model as _switch_model
...
class CLISessionMixin:
    ...
    def _show_session_status(self):
        ...
        lines = [
            "Lemon AI CLI Status", "", f"Session ID: {self.session_id}", f"Path: {display_lemon_home()}",
        ]
        if title:
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from lemon_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from lemon_cli.timefmt import relative_time as _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            _cli_visible_print(f"\n  [Lemon AI #{visible_index}]{_ts_suffix(msg)}")
            ...
            if self._session_db:
                with contextlib.suppress(Exception):
                    ...
                    self._session_db.create_session(
                        ...
                        source=os.environ.get("LEMON_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self, cmd: str = "/save"):
        ...
        """
        from cli import datetime
        from lemon_cli.session_export import (
        ...
        if filename:
            ...
            path = saved_dir / f"lemon_conversation_{timestamp}.{fmt}"
        ...
        try:
            ...
            # tools/subprocesses on this thread resolve LEMON_SESSION_ID to the child id after an
            # out-of-place rotation (idempotent when no rotation happened).
            if self.session_id:
                print(f"       Resume the live session with: lemon --resume {self.session_id}")
        except Exception as e:
    ...
    def _write_terminal_breadcrumb(self) -> None:
        """Record this terminal's live session for bare ``lemon -c``. Called whenever
        ...
        with contextlib.suppress(Exception):
            from lemon_cli.terminal_breadcrumbs import write_breadcrumb
    ...
    def _toggle_yolo(self):
        ...
        ``LEMON_YOLO_MODE``: that env var is frozen into ``tools.approval._YOLO_MODE_FROZEN``
        ...
        if _YOLO_MODE_FROZEN:
            _cprint(
                ...
                " for this process (started with --yolo / LEMON_YOLO_MODE)."
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware: summarize everything except the most recent
        ...
        from lemon_cli.partial_compress import (
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        if not msg_count:
            try:
                from lemon_cli.skin_engine import get_active_goodbye
        ...
        # the hint ("default"/"custom" use the standard LEMON_HOME).
        try:
        ...
        print(f"  lemon --resume {self.session_id}{profile_flag}")
        if session_title:
            print(f"  lemon -c \"{session_title}\"{profile_flag}")
        print()
