...
The provider list shown by ``lemon model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`lemon_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `lemon model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``lemon model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("lemon_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("lemon_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    LEMON_OVERLAYS = _safe_import("lemon_cli.providers", "LEMON_OVERLAYS", {})
    try:
