...
Split out of ``lemon_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from lemon_cli.kanban_db import Task
...
# ``LEMON_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Lemon AI profile. Expected steady-state
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``lemon kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``lemon_cli.profiles.profile_exists``, or ``None`` when it cannot be
    imported (local import avoids a cycle; callers fall back to trusting the
    assignee)."""
    try:
        from lemon_cli.profiles import profile_exists
    except Exception:
        return None
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Lemon AI profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Lemon AI ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``LEMON_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``lemon -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``lemon kanban tail`` shows why the task looks stuck.
...
def _module_lemon_argv() -> list[str]:
    """Interpreter-bound Lemon AI CLI invocation (``lemon_cli.main`` is the
    console-script target — there is no top-level ``lemon`` package)."""
    return [sys.executable, "-m", "lemon_cli.main"]


def _absolute_lemon_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Lemon AI shim."""
...
def _lemon_path_argv(path: str) -> list[str]:
    """argv for a resolved Lemon AI executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_lemon_argv()
    return [_absolute_lemon_path(path)]


def _resolve_lemon_argv() -> list[str]:
    """Resolve the ``lemon`` invocation as argv for ``Popen``: ``$LEMON_BIN``
    (path-like -> absolute; bare names keep PATH semantics, never a
    same-directory file), then ``which("lemon")`` (Windows: safe PATH search,
    batch shims fall back to the module form), then ``sys.executable -m
    lemon_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_lemon_bin``; local because
    ``lemon_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("LEMON_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _lemon_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _lemon_path_argv(resolved_env_bin)
        return _module_lemon_argv()

    lemon_bin = _safe_which_no_cwd("lemon") if _kb._IS_WINDOWS else shutil.which("lemon")
    if lemon_bin:
        return _lemon_path_argv(lemon_bin)
    return _module_lemon_argv()
...
def _resolve_worker_cli_toolsets(lemon_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``LEMON_KANBAN_TASK`` is set.
    """
    if not lemon_home:
        return None
    try:
        ...
        token = set_lemon_home_override(lemon_home)
        try:
            cfg = load_config()
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for LEMON_HOME=%r (%s)",
            lemon_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, lemon_home: Optional[str]) -> list[str]:
    """Build the ``lemon -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_lemon_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped LEMON_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(lemon_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `lemon kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``lemon -p <profile> chat -q ...`` subprocess.
    ...
    ``LEMON_KANBAN_DB`` / ``LEMON_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from lemon_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    # Inject LEMON_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_lemon_home() falls back to the DEFAULT
    # profile root because `lemon -p` applies its override before
    # lemon_constants is imported.
    try:
        env["LEMON_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # No profile dir (isolated test fixtures) — the CLI resolves it from
        # LEMON_PROFILE (set below) instead.
        pass
    if task.tenant:
        env["LEMON_TENANT"] = task.tenant
    env["LEMON_KANBAN_TASK"] = task.id
    env["LEMON_KANBAN_WORKSPACE"] = workspace
    ...
    env["LEMON_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["LEMON_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["LEMON_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["LEMON_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["LEMON_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["LEMON_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `lemon -p` rewrites LEMON_HOME (symlink / Docker layouts).
    env["LEMON_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["LEMON_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["LEMON_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["LEMON_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads LEMON_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["LEMON_PROFILE"] = profile_arg
    # `--cli` is the highest-precedence TUI override; dropping LEMON_TUI covers
    # older lemon builds on PATH that predate the flag's precedence.
    env.pop("LEMON_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("LEMON_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`lemon` executable not found on PATH. "
            "Install Lemon AI or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``lemon kanban dispatch`` — the standalone
...
from lemon_cli import kanban_db as _kb  # noqa: E402
from lemon_cli import kanban_db_connect as _kbc  # noqa: E402
from lemon_cli import kanban_db_workspace as _kbw  # noqa: E402
