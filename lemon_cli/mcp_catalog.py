...
Entries are added only by merging a PR into lemon-agent; presence in ``optional-mcps/`` = Nous
...
from lemon_constants import get_lemon_home, get_optional_mcps_dir
from lemon_cli._subprocess_compat import noninteractive_git_env
from lemon_cli.colors import Colors, color
from lemon_cli.config import load_config, save_config, get_env_value, save_env_value
from lemon_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # go through auth.env so they are prompted for and land in ~/.lemon-ai/.env.
...
def _parse_auth(path: Path, raw: Any, name: str, http: bool) -> AuthSpec:
    ...
    if http and a_type == "api_key":
        ...
        from lemon_cli.mcp_config import _env_key_for_server
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Lemon AI understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    skipped too but surfaced via :func:`catalog_diagnostics` so UIs can say "update Lemon AI".
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    ``kind`` is ``future_manifest`` (newer than this Lemon AI) or ``invalid`` (malformed)."""
...
def _install_root() -> Path:
    ...
    root = get_lemon_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.lemon-ai/mcp-installs/<name>`` and run bootstrap. Returns the dir."""
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    """Prompt for each env spec; secrets and non-secrets alike go to ~/.lemon-ai/.env."""
...
def _build_server_config(entry: CatalogEntry, install_dir: Optional[Path]) -> dict:
    """Translate a manifest into the ``mcp_servers.<name>`` block format used by lemon_cli/mcp_config.py."""
    cfg: dict = {}
    t = entry.transport
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from lemon_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    if not server_cfg:
        return None
    try:
        from lemon_cli.mcp_config import _probe_single_server  # lazy: keep this module cheap
...
def _apply_tool_selection(
    ...
    *,
    ...
    else apply ``default_enabled``, else no filter; point the user at ``lemon mcp configure``.
    """
    print()
    name = entry.name
    configure_hint = f"`lemon mcp configure {name}`"
    ...
    if len(chosen_indices) == len(probed):
        ...
        # re-run `lemon mcp configure <name>` and unselect a tool (switches to include-mode).
        _write_tools_filter(name, "include", None)
        _say(
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        # Provider-mediated OAuth relies on the existing `lemon auth <provider>` flow; surface
        ...
        _say(
            ...
            f"`lemon auth {entry.auth.provider}` if you have not "
    ...
    from lemon_cli.mcp_config import _save_mcp_server
    ...
    _say(
        ...
        f"Start a new Lemon AI session to load its tools."
    )
