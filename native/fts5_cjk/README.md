...
storage discipline as the other indexes). On a populated database, run
    ...
    lemon sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.lemon-ai/config.yaml` to disable. Override
the .so location with `LEMON_FTS5_CJK_SO`.
