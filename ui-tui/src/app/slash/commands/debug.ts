...
import { terminalBackgroundHex } from '@lemon-ai/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $LEMON_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see LEMON_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['LEMON_TUI_BACKGROUND', process.env.LEMON_TUI_BACKGROUND ?? '(unset)'],
            ['LEMON_TUI_THEME', process.env.LEMON_TUI_THEME ?? '(unset)'],
