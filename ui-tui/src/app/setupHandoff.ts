import type { RunExternalProcess } from '@lemon-ai/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`lemon ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching lemon: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`lemon ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
