...
// Deferred @lemon-ai/ink import: loading `@lemon-ai/ink` at module top-level
...
async function _ensureEvictInkCaches(): Promise<(level: 'all' | 'half') => unknown> {
  ...
  _evictInkCachesPromise ??= import('@lemon-ai/ink')
    .then(mod => {
...
export function startMemoryMonitor({
  ...
  const cooldownRaw = process.env.LEMON_AUTO_HEAPDUMP_COOLDOWN_MS?.trim()
  ...
  const tick = async () => {
    ...
    // Deferred import keeps `@lemon-ai/ink` off the cold-start critical path;
