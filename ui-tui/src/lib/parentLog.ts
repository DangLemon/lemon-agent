...
const logDir = join(process.env.LEMON_HOME?.trim() || join(homedir(), '.lemon-ai'), 'logs')
...
// a real ~/.lemon-ai (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('lemon-tui: parent lifecycle log unavailable\n')
    }
  }
}
