 ...
 * `lemon-boot-background` / `lemon-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports LEMON_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.lemon-ai matches get_lemon_home()'s default.
const bootFilePath = () => join(process.env.LEMON_HOME ?? join(homedir(), '.lemon-ai'), 'tui-theme-boot.json')

// Never touch the user's real ~/.lemon-ai from test runs (the TS suite has no
// LEMON_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into LEMON_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.LEMON_TUI_THEME || env.LEMON_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.LEMON_TUI_BACKGROUND
  ) {
    env.LEMON_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.LEMON_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.LEMON_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into LEMON_TUI_THEME.
