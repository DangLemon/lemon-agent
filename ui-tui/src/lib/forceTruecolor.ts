...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.LEMON_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Lemon AI on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via LEMON_TUI_TRUECOLOR=1.
