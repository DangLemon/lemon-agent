import type { InputEvent, Key } from '@lemon-ai/ink'
import * as Ink from '@lemon-ai/ink'
 ...
 * but that accepts `key.meta` on macOS — and lemon-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.LEMON_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which lemon-ink decodes as meta+'d'; without this branch it
