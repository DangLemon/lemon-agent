import type { SkinBranding, SkinColors } from '@lemon-ai/shared/skin'
...
// Mirrors @lemon-ai/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @lemon-ai/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Lemon AI navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Lemon AI look was never hand-authored: for years the
 ...
 *  answer when known (cached in LEMON_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.LEMON_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit LEMON_TUI_THEME / COLORFGBG signals above still win,
...
// `LEMON_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `LEMON_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `LEMON_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `LEMON_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Lemon AI palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.LEMON_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.LEMON_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.LEMON_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into LEMON_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into LEMON_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Lemon AI navy
  ...
  // the derived value already carries the identity seeds (e.g. Lemon AI navy).
  const activeRow =
