...
import { Box, Text } from '@lemon-ai/ink'
...
/**
 * User widget apps — Lemon AI authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$LEMON_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.lemon-ai/plugins/`: files under LEMON_HOME execute
...
const widgetsDir = () => join(process.env.LEMON_HOME?.trim() || join(homedir(), '.lemon-ai'), 'tui-widgets')
 ...
 *  every change, so a widget Lemon AI writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Lemon AI) ever writes must hot-load too — a 10s poll here read
