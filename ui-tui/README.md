...
React + Ink terminal UI for Lemon AI. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `LEMON_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `lemon setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.lemon-ai/.lemon-ai_history` or under `LEMON_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `lemon setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/lemon-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external lemon setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.lemon-ai/tui-parent.log
    ...
    types/
      lemon-ink.d.ts            type declarations for @lemon-ai/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent LemonCLI subprocess for slash commands
```
