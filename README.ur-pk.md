...
```bash
lemon              # انٹرایکٹو CLI — بات چیت شروع کریں
lemon model        # اپنا LLM پرووائیڈر اور ماڈل منتخب کریں
lemon tools        # کنفیگر کریں کہ کون سے ٹولز ایکٹو ہیں
lemon config set   # انفرادی کنفگ (config) ویلیوز سیٹ کریں
lemon gateway      # میسجنگ گیٹ وے شروع کریں (ٹیلی گرام، ڈسکارڈ، وغیرہ)
lemon setup        # مکمل سیٹ اپ وزرڈ چلائیں (یہ سب کچھ ایک ساتھ کنفیگر کر دے گا)
lemon claw migrate # OpenClaw سے مائیگریٹ کریں (اگر آپ OpenClaw سے آ رہے ہیں)
lemon update       # لیٹسٹ ورژن پر اپ ڈیٹ کریں
lemon doctor       # کسی بھی مسئلے کی تشخیص کریں
```
...
ہرمیس کے دو بنیادی انٹر فیس ہیں: آپ ٹرمینل UI کو `lemon` کے ساتھ شروع کریں، یا گیٹ وے چلا کر اس کے ساتھ ٹیلی گرام، ڈسکارڈ، سلیک، واٹس ایپ، سگنل، یا ای میل کے ذریعے بات کریں۔ جب آپ کسی بات چیت میں ہوتے ہیں، تو بہت سی سلیش (slash) کمانڈز دونوں انٹرفیسز میں ایک جیسی ہوتی ہیں۔

<div dir="ltr">

| کارروائی (Action)                         | سی ایل آئی (CLI)                              | میسجنگ پلیٹ فارمز (Messaging platforms)                                          |
| --------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------- |
| بات چیت شروع کریں                       | `lemon`                                      | `lemon gateway setup` اور `lemon gateway start` چلائیں، پھر بوٹ کو میسج بھیجیں |
...
**پہلی بار سیٹ اپ کے دوران:** سیٹ اپ وزرڈ (`lemon setup`) خود بخود `~/.openclaw` کو پہچان لیتا ہے اور کنفیگریشن شروع ہونے سے پہلے مائیگریٹ (migrate) کرنے کا آپشن دیتا ہے۔
...
```bash
lemon claw migrate              # انٹرایکٹو مائیگریشن (مکمل پری سیٹ)
lemon claw migrate --dry-run    # جائزہ لیں کہ کیا کیا مائیگریٹ ہوگا
lemon claw migrate --preset user-data   # حساس معلومات (secrets) کے بغیر مائیگریٹ کریں
lemon claw migrate --overwrite  # موجودہ متصادم فائلوں کو اوور رائٹ کریں
```
...
- **مہارتیں (Skills)** — صارف کی بنائی گئی مہارتیں → `~/.lemon-ai/skills/openclaw-imports/`
...
تمام آپشنز دیکھنے کے لیے `lemon claw migrate --help` استعمال کریں، یا انٹرایکٹو ایجنٹ کی مدد سے مائیگریٹ کرنے کے لیے `openclaw-migration` سکل کا استعمال کریں (جس میں ڈرائی رن (dry-run) پریویوز شامل ہیں)۔
...
cd lemon-agent
./setup-lemon.sh     # uv کو انسٹال کرتا ہے، venv بناتا ہے، .[all] کو انسٹال کرتا ہے، اور ~/.local/bin/lemon کا سیم لنک (symlink) بناتا ہے
./lemon              # خود بخود venv کی شناخت کرتا ہے، پہلے `source` کرنے کی ضرورت نہیں
```
...
- 🐛 [مسائل (Issues)](https://github.com/DangLemon/lemon-agent/issues)
...
- 🔌 [LemonClaw](https://github.com/AaronWong1999/lemonclaw) — کمیونٹی وی چیٹ (WeChat) برج: ہرمیس ایجنٹ اور OpenClaw کو ایک ہی وی چیٹ اکاؤنٹ پر چلائیں۔
